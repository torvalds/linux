...
static int compression_decompress_bio(struct list_head *ws,
  ...
  switch (cb->compress_type) {
    case BTRFS_COMPRESS_ZLIB:
      return zlib_decompress_bio(ws, cb);
    case BTRFS_COMPRESS_LZO:
      return lzo_decompress_bio(ws, cb);
    case BTRFS_COMPRESS_ZSTD:
      return zstd_decompress_bio(ws, cb);
    case BTRFS_COMPRESS_NONE:
    default:
      /*
...
static int compression_decompress(int type, struct list_head *ws,
  ...
  switch (type) {
    case BTRFS_COMPRESS_ZLIB:
      return zlib_decompress(ws, data_in, dest_page,
          dest_pgoff, srclen, destlen);
    case BTRFS_COMPRESS_LZO:
      return lzo_decompress(ws, data_in, dest_page,
          dest_pgoff, srclen, destlen);
    case BTRFS_COMPRESS_ZSTD:
      return zstd_decompress(ws, data_in, dest_page,
          dest_pgoff, srclen, destlen);
    case BTRFS_COMPRESS_NONE:
    default:
      /*
...
static unsigned long btrfs_compr_pool_count(struct shrinker *sh,
...
static unsigned long btrfs_compr_pool_scan(struct shrinker *sh,
    struct shrink_control *sc) {
...
static struct list_head *get_workspace(int type, int level) {
  switch (type) {
    case BTRFS_COMPRESS_NONE:
      return btrfs_get_workspace(type, level);
    case BTRFS_COMPRESS_ZLIB:
...
static void put_workspace(int type, struct list_head *ws) {
  switch (type) {
    case BTRFS_COMPRESS_NONE:
      return btrfs_put_workspace(type, ws);
    case BTRFS_COMPRESS_ZLIB:
      return btrfs_put_workspace(type, ws);
    case BTRFS_COMPRESS_LZO:
      return btrfs_put_workspace(type, ws);
