...
again:
  ...
  /* Allocate and reserve the slot, from now it can return a NULL from
   * xa_load(). */
 ...
 * that fit in a leaf. The delayed items (dir index keys) are sorted by their
 *key
 ...
 * removed (by btrfs_delete_delayed_dir_index()) before we attempted to flush
 *them
 * into the subvolume tree. Dir index keys also have their offsets coming from a
 * monotonically increasing counter, so we can't get new keys with an offset
 *that
