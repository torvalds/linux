 ...
 *
 * Returns: A non-NULL block group pointer if we can do a NOCOW write, the
 *caller
 ...
 *
 * This is meant to be called after a previous call to
 *btrfs_inc_nocow_writers(),
 * and on the block group returned by that call. Typically this is called after
 * creating an ordered extent for a NOCOW write, to prevent races with scrub and
 * relocation.
 *
 * After this call, the caller should not use the block group anymore. It it
 *wants
 ...
 * error code on error.
 */
static int sample_block_group_extent_item(
    struct btrfs_caching_control *caching_ctl,
    struct btrfs_block_group *block_group,
 ...
 *
 * If we are caching in a block group from disk, then there are three major
 *cases
 * to consider:
 ...
 */
static int load_block_group_size_class(
    struct btrfs_caching_control *caching_ctl,
    struct btrfs_block_group *block_group) {
  struct btrfs_fs_info *fs_info = block_group->fs_info;
...
static inline void btrfs_free_excluded_extents(
...
static bool should_reclaim_block_group(struct btrfs_block_group *bg,
    u64 bytes_freed) {
...
int btrfs_add_reserved_bytes(struct btrfs_block_group *cache,
  ...
  if (btrfs_block_group_should_use_size_class(cache)) {
    ...
    ret = btrfs_use_block_group_size_class(cache, size_class,
...
static struct btrfs_block_group *do_chunk_alloc(
 ...
 * most of the time, chunk allocation may still be triggered for several
 *reasons:
 *
 ...
 *    operation comes in and, temporarily, removes the last free space entry
 *from
 ...
 *
 * Allocation of system chunks does not happen through this function. A task
 *that
 * needs to update the chunk btree (the only btree that uses system chunks),
 *must
 ...
 * holding fs_info->chunk_mutex. This is important to guarantee that while
 *COWing
 * an extent buffer from the chunks btree we never trigger allocation of a new
 * system chunk, which would result in a deadlock (trying to lock twice an
 * extent buffer of the chunk btree, first time before triggering the chunk
 * allocation and the second time during chunk allocation while attempting to
 * update the chunks btree). The system chunk array is also updated while
 *holding
 * that mutex. The same logic applies to removing chunks - we must reserve
 *system
