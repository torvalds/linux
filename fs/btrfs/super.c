...
static void btrfs_put_super(struct super_block *sb) {
  ...
  btrfs_info(fs_info, "last unmount of filesystem %pU",
...
static const struct fs_parameter_spec btrfs_fs_parameters[] = {
  ...
  fsparam_enum("space_cache", Opt_space_cache_version,
...
bool btrfs_check_options(struct btrfs_fs_info *info, unsigned long *mount_opt,
  ...
  bool ret = true;
  if (!(flags & SB_RDONLY)
      && (check_ro_option(info, *mount_opt, BTRFS_MOUNT_NOLOGREPLAY,
          "nologreplay")
      || check_ro_option(info, *mount_opt, BTRFS_MOUNT_IGNOREBADROOTS,
      "ignorebadroots")
      || check_ro_option(info, *mount_opt, BTRFS_MOUNT_IGNOREDATACSUMS,
      "ignoredatacsums"))) {
    ret = false;
  }
...
static inline void btrfs_remount_cleanup(struct btrfs_fs_info *fs_info,
   ...
   */
  if (btrfs_raw_test_opt(old_opts, AUTO_DEFRAG)
      && (!btrfs_raw_test_opt(fs_info->mount_opt,
          AUTO_DEFRAG) || sb_rdonly(fs_info->sb))) {
...
static void btrfs_emit_options(struct btrfs_fs_info *info,
  ...
  btrfs_info_if_set(info, old, SSD_SPREAD,
      "using spread ssd allocation scheme");
  btrfs_info_if_set(info, old, NOBARRIER, "turning off barriers");
  btrfs_info_if_set(info, old, NOTREELOG, "disabling tree log");
  btrfs_info_if_set(info, old, NOLOGREPLAY,
  ...
  btrfs_info_if_set(info, old, USEBACKUPROOT,
