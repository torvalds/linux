...
static struct extent_inode_elem *unode_aux_to_inode_list(
 ...
 */
static bool lookup_backref_shared_cache(
    struct btrfs_backref_share_check_ctx *ctx,
    struct btrfs_root *root,
 ...
 * snapshot field changing while updating or checking the cache.
 */
static void store_backref_shared_cache(
    struct btrfs_backref_share_check_ctx *ctx,
    struct btrfs_root *root,
    u64 bytenr, int level, bool is_shared) {
  const struct btrfs_fs_info *fs_info = root->fs_info;
...
int btrfs_is_data_extent_shared(struct btrfs_inode *inode, u64 bytenr,
    u64 extent_gen,
    struct btrfs_backref_share_check_ctx *ctx) {
  struct btrfs_backref_walk_ctx walk_ctx = {
    0
  };
