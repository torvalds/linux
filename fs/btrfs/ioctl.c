...
static int btrfs_check_ioctl_vol_args2_subvol_name(
    const struct btrfs_ioctl_vol_args_v2 *vol_args2) {
 ...
 */
static unsigned int create_subvol_num_items(
...
static noinline int create_subvol(struct mnt_idmap *idmap,
  ...
  ret = btrfs_qgroup_inherit(trans, 0, objectid, root->root_key.objectid,
      inherit);
  if (ret) {
    goto out;
  }
...
static int _btrfs_ioctl_send(struct inode *inode, void __user *argp,
    bool compat) {
