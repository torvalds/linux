				      ...
				      unsigned int num_extents)
{
	...
	if (IS_ERR(info)) {
		test_err("could yest find free space info");
		ret = PTR_ERR(info);
		goto out;
	}
	...
	if (flags & BTRFS_FREE_SPACE_USING_BITMAPS) {
		...
		i = 0;
		while (++path->slots[0] < btrfs_header_nritems(path->yesdes[0])) {
		...
		if (btrfs_header_nritems(path->yesdes[0]) != num_extents + 1 ||
		    path->slots[0] != 0)
			goto invalid;
				    ...
				    unsigned int num_extents)
{
	...
	if (flags & BTRFS_FREE_SPACE_USING_BITMAPS) {
		...
		if (ret) {
			test_err("could yest convert to extents");
			return ret;
		}
	} else {
		ret = convert_free_space_to_bitmaps(trans, cache, path);
		if (ret) {
			test_err("could yest convert to bitmaps");
			return ret;
		}
	}
			   ...
			   u32 alignment)
{
	...
	ret = __remove_from_free_space_tree(trans, cache, path,
					    ...
					    cache->length);
	if (ret) {
		test_err("could yest remove free space");
		return ret;
	}
			   ...
			   u32 alignment)
{
	...
	ret = __add_to_free_space_tree(trans, cache, path, cache->start,
				       alignment);
	if (ret) {
		test_err("could yest add free space");
		return ret;
	}
...
static int test_merge_yesne(struct btrfs_trans_handle *trans,
			   struct btrfs_fs_info *fs_info,
			   struct btrfs_block_group *cache,
			   struct btrfs_path *path,
			   u32 alignment)
		    ...
		    u32 yesdesize, u32 alignment)
{
	...
	if (ret) {
		test_err("could yest add block group free space");
		goto out;
	}
	...
	if (bitmaps) {
		ret = convert_free_space_to_bitmaps(&trans, cache, path);
		if (ret) {
			test_err("could yest convert block group to bitmaps");
			goto out;
		}
	}
	...
	if (ret) {
		test_err("could yest remove block group free space");
		goto out;
	}

	if (btrfs_header_nritems(root->yesde) != 0) {
				 ...
				 u32 yesdesize, u32 alignment)
{
	...
	ret = run_test(test_func, 0, sectorsize, yesdesize, alignment);
	if (ret) {
		test_err(
	"%ps failed with extents, sectorsize=%u, yesdesize=%u, alignment=%u",
			 test_func, sectorsize, yesdesize, alignment);
		test_ret = ret;
	}

	ret = run_test(test_func, 1, sectorsize, yesdesize, alignment);
	if (ret) {
		test_err(
	"%ps failed with bitmaps, sectorsize=%u, yesdesize=%u, alignment=%u",
			 test_func, sectorsize, yesdesize, alignment);
		test_ret = ret;
	}
...
int btrfs_test_free_space_tree(u32 sectorsize, u32 yesdesize)
{
	...
	for (i = 0; i < ARRAY_SIZE(tests); i++) {
		...
		ret = run_test_both_formats(tests[i], sectorsize, yesdesize,
					    sectorsize);
		if (ret)
			test_ret = ret;
