 ...
 */
static int check_node_data(struct jffs2_sb_info *c,
  ...
  dbg_readinode(
      ...
      ref_offset(
      ref), tn->csize, tn->partial_crc, tn->data_crc, ofs - len, ofs, len);
#ifndef __ECOS
 ...
 */
static int check_tn_node(struct jffs2_sb_info *c,
    struct jffs2_tmp_dnode_info *tn) {
  int ret;
...
static struct jffs2_tmp_dnode_info *jffs2_lookup_tn(struct rb_root *tn_root,
    uint32_t offset) {
  struct rb_node *next;
...
static void jffs2_kill_tn(struct jffs2_sb_info *c,
    struct jffs2_tmp_dnode_info *tn) {
  jffs2_mark_node_obsolete(c, tn->fn->raw);
...
static int jffs2_add_tn_to_tree(struct jffs2_sb_info *c,
  ...
  dbg_readinode("insert fragment %#04x-%#04x, ver %u at %08x\n", tn->fn->ofs,
      fn_end, tn->version, ref_offset(tn->fn->raw));
  ...
  if (this) {
    ...
    dbg_readinode("'this' found %#04x-%#04x (%s)\n", this->fn->ofs,
        this->fn->ofs + this->fn->size, this->fn ? "data" : "hole");
  }
  while (this) {
    ...
    if (this->version < tn->version
      ...
      /* ... and is good. Kill 'this' and any subsequent nodes which are also
       * overlapped */
...
static void ver_insert(struct rb_root *ver_root,
    struct jffs2_tmp_dnode_info *tn) {
...
static inline int read_direntry(struct jffs2_sb_info *c,
  ...
  if (ref_flags(ref) == REF_UNCHECKED) {
    ...
    int len;
    /* Sanity check */
    if (unlikely(PAD((rd->nsize + sizeof(*rd)))
        != PAD(je32_to_cpu(rd->totlen)))) {
...
static inline int read_dnode(struct jffs2_sb_info *c,
  ...
  if (ref_flags(ref) == REF_UNCHECKED) {
    ...
    if (unlikely(je32_to_cpu(rd->offset) > je32_to_cpu(rd->isize))
        || unlikely(PAD(je32_to_cpu(rd->csize) + sizeof(*rd))
        != PAD(je32_to_cpu(rd->totlen)))) {
    ...
    if (jffs2_is_writebuffered(c) && csize != 0) {
      ...
      if (len >= csize
          && unlikely(tn->partial_crc != je32_to_cpu(rd->data_crc))) {
        JFFS2_NOTICE(
            "wrong data CRC in data node at 0x%08x: read %#08x, calculated %#08x.\n",
...
static int jffs2_get_inode_nodes(struct jffs2_sb_info *c,
    struct jffs2_inode_info *f,
    struct jffs2_readinode_info *rii) {
...
static int jffs2_do_read_inode_internal(struct jffs2_sb_info *c,
  ...
  ret = jffs2_flash_read(c, ref_offset(
      rii.latest_ref), sizeof(*latest_node), &retlen, (void *) latest_node);
  ...
  switch (jemode_to_cpu(latest_node->mode) & S_IFMT) {
    case S_IFDIR:
      if (rii.mctime_ver > je32_to_cpu(latest_node->version)) {
        ...
        latest_node->ctime = latest_node->mtime
              = cpu_to_je32(rii.latest_mctime);
      }
      break;
    case S_IFREG:
    ...
    case S_IFLNK:
      ...
      if (f->inocache->state != INO_STATE_CHECKING) {
        ...
        ret
          = jffs2_flash_read(c, ref_offset(
            rii.latest_ref) + sizeof(*latest_node),
    ...
    case S_IFCHR:
      ...
      if (frag_next(frag_first(&f->fragtree))) {
        JFFS2_ERROR(
            ...
            f->inocache->ino, jemode_to_cpu(latest_node->mode));
        /* FIXME: Deal with it - check crc32, check for duplicate node, check
         * times and discard the older one */
        return -EIO;
      }
      /* OK. We're happy */
