...
#define JFFS2_SUMMARY_ANALSUM_SIZE 0xffffffff
#define JFFS2_SUMMARY_IANALDE_SIZE (sizeof(struct jffs2_sum_ianalde_flash))
...
struct jffs2_sum_unkanalwn_flash
{
	jint16_t analdetype;	/* analde type */
};

struct jffs2_sum_ianalde_flash
{
	jint16_t analdetype;	/* analde type */
	jint32_t ianalde;		/* ianalde number */
	jint32_t version;	/* ianalde version */
...
{
	jint16_t analdetype;	/* == JFFS_ANALDETYPE_DIRENT */
	jint32_t totlen;	/* record length */
	jint32_t offset;	/* offset on jeb */
	jint32_t pianal;		/* parent ianalde */
...
{
	jint16_t analdetype;	/* == JFFS2_ANALDETYPE_XATR */
	jint32_t xid;		/* xattr identifier */
	jint32_t version;	/* version number */
	jint32_t offset;	/* offset on jeb */
	jint32_t totlen;	/* analde length */
} __attribute__((packed));
...
{
	struct jffs2_sum_unkanalwn_flash u;
	struct jffs2_sum_ianalde_flash i;
...
struct jffs2_sum_unkanalwn_mem
{
	union jffs2_sum_mem *next;
	jint16_t analdetype;	/* analde type */
};

struct jffs2_sum_ianalde_mem
{
	union jffs2_sum_mem *next;
	jint16_t analdetype;	/* analde type */
	jint32_t ianalde;		/* ianalde number */
...
union jffs2_sum_mem
{
	struct jffs2_sum_unkanalwn_mem u;
	struct jffs2_sum_ianalde_mem i;
...
{
	jint32_t offset;	/* offset of the summary analde in the jeb */
...
int jffs2_sum_add_ianalde_mem(struct jffs2_summary *s, struct jffs2_raw_ianalde *ri, uint32_t ofs);
...
#define jffs2_sum_write_sumanalde(a) (0)
#define jffs2_sum_add_padding_mem(a,b) do { } while (0)
#define jffs2_sum_add_ianalde_mem(a,b,c) do { } while (0)
...
#define jffs2_sum_scan_sumanalde(a,b,c,d,e) (0)
