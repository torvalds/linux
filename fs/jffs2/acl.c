...
static int __jffs2_set_acl(struct inode *inode, int xprefix,
    struct posix_acl *acl) {
  char *value = NULL;
  size_t size = 0;
  int rc;
...
int jffs2_init_acl_pre(struct inode *dir_i, struct inode *inode,
    umode_t *i_mode) {
...
int jffs2_init_acl_post(struct inode *inode) {
  ...
  if (inode->i_default_acl) {
    rc
      = __jffs2_set_acl(inode, JFFS2_XPREFIX_ACL_DEFAULT, inode->i_default_acl);
    if (rc) {
      return rc;
    }
  }
