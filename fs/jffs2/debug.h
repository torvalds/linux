...
#ifndef CONFIG_JFFS2_FS_DE
#define CONFIG_JFFS2_FS_DE 0
#endif

#if CONFIG_JFFS2_FS_DE > 0
/* Enable "paranoia" checks and dumps */
#define JFFS2_DBG_PARANOIA_CHECKS
#define JFFS2_DBG_DUMPS

/*
 * By defining/undefining the below macros one may select deging messages
...
/*
 * Dx() are mainly used for deging messages, they must go away and be
...
do {						\
	if (CONFIG_JFFS2_FS_DE >= level)	\
		pr_de(fmt, ##__VA_ARGS__);	\
} while (0)

/* The prefixes of JFFS2 messages */
#define JFFS2_DBG		KERN_DE
...
#define JFFS2_DE(fmt, ...)						\
	printk(KERN_DE "[JFFS2 DBG] (%d) %s: " fmt,			\
	       task_pid_nr(current), __func__, ##__VA_ARGS__)

/*
 * We split our deging messages on several parts, depending on the JFFS2
 * subsystem the message belongs to.
 */
/* Read inode deging messages */
#ifdef JFFS2_DBG_READINODE_MESSAGES
#define dbg_readinode(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
#define dbg_readinode(fmt, ...)
#endif
#ifdef JFFS2_DBG_READINODE2_MESSAGES
#define dbg_readinode2(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
#define dbg_readinode2(fmt, ...)
#endif

/* Fragtree build deging messages */
#ifdef JFFS2_DBG_FRAGTREE_MESSAGES
#define dbg_fragtree(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
#define dbg_fragtree(fmt, ...)
#endif
#ifdef JFFS2_DBG_FRAGTREE2_MESSAGES
#define dbg_fragtree2(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
#define dbg_fragtree2(fmt, ...)
#endif

/* Directory entry list manilulation deging messages */
#ifdef JFFS2_DBG_DENTLIST_MESSAGES
#define dbg_dentlist(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
...
#define dbg_noderef(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
...
#define dbg_inocache(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
#define dbg_inocache(fmt, ...)
#endif

/* Summary deging messages */
#ifdef JFFS2_DBG_SUMMARY_MESSAGES
#define dbg_summary(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
...
#define dbg_fsbuild(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
...
#define dbg_memalloc(fmt, ...)	JFFS2_DE(fmt, ##__VA_ARGS__)
#else
...
#define dbg_xattr(fmt, ...)  JFFS2_DE(fmt, ##__VA_ARGS__)
#else
