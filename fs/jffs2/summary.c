...
static int jffs2_sum_add_mem(struct jffs2_summary *s,
...
/* The following 3 functions are called from scan.c to collect summary info for
 * not closed jeb */
...
int jffs2_sum_add_inode_mem(struct jffs2_summary *s, struct jffs2_raw_inode *ri,
    uint32_t ofs) {
  struct jffs2_sum_inode_mem *temp = kmalloc(sizeof(struct jffs2_sum_inode_mem),
      GFP_KERNEL);
  if (!temp) {
    return -ENOMEM;
  }
  ...
  temp->offset = cpu_to_je32(ofs); /* relative offset from the beginning of the
                                    * jeb */
...
int jffs2_sum_add_kvec(struct jffs2_sb_info *c, const struct kvec *invecs,
  ...
  switch (je16_to_cpu(node->u.nodetype)) {
    ...
    case JFFS2_NODETYPE_DIRENT:
      {
        struct jffs2_sum_dirent_mem *temp
          = kmalloc(sizeof(struct jffs2_sum_dirent_mem) + node->d.nsize,
            GFP_KERNEL);
        if (!temp) {
          goto no_mem;
        }
...
static int jffs2_sum_process_sum_data(struct jffs2_sb_info *c,
    struct jffs2_eraseblock *jeb,
      ...
      case JFFS2_NODETYPE_DIRENT:
        {
          ...
          fd->raw = sum_link_node_ref(c, jeb, je32_to_cpu(
              spd->offset) | REF_UNCHECKED,
      ...
      case JFFS2_NODETYPE_XATTR:
        {
          ...
          if (xd->version > je32_to_cpu(spx->version)) {
            ...
            struct jffs2_raw_node_ref *raw
              = sum_link_node_ref(c, jeb, je32_to_cpu(
                spx->offset) | REF_UNCHECKED,
...
int jffs2_sum_scan_sumnode(struct jffs2_sb_info *c,
  ...
  if (je32_to_cpu(summary->cln_mkr)) {
    ...
    if (je32_to_cpu(summary->cln_mkr) != c->cleanmarker_size) {
      ...
      if ((ret
            = jffs2_scan_dirty_space(c, jeb,
              PAD(je32_to_cpu(summary->cln_mkr))))) {
        return ret;
      }
...
static int jffs2_sum_write_data(struct jffs2_sb_info *c,
    struct jffs2_eraseblock *jeb,
  ...
  isum.hdr_crc
    = cpu_to_je32(crc32(0, &isum, sizeof(struct jffs2_unknown_node) - 4));
