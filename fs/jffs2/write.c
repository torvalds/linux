...
retry:
  ...
  if (ret || (retlen != sizeof(*ri) + datalen)) {
    ...
    if (retlen) {
       ...
       * this node */
      jffs2_add_physical_node_ref(c, flash_ofs | REF_OBSOLETE,
          PAD(sizeof(*ri) + datalen), NULL);
    } else {
      pr_notice(
  ...
  if ((je32_to_cpu(ri->dsize) >= PAGE_SIZE)
      ...
      && (je32_to_cpu(ri->dsize) + je32_to_cpu(ri->offset)
      == je32_to_cpu(ri->isize)))) {
    flash_ofs |= REF_PRISTINE;
  } else {
    flash_ofs |= REF_NORMAL;
  }
  fn->raw = jffs2_add_physical_node_ref(c, flash_ofs, PAD(
      sizeof(*ri) + datalen), f->inocache);
  if (IS_ERR(fn->raw)) {
...
/* The OS-specific code fills in the metadata in the jffs2_raw_inode for us, so
 * that
...
int jffs2_do_unlink(struct jffs2_sb_info *c, struct jffs2_inode_info *dir_f,
  ...
  if (!jffs2_can_mark_obsolete(c)) {
    /* We can't mark stuff obsolete on the medium. We need to write a deletion
     * dirent */
    rd = jffs2_alloc_raw_dirent();
    if (!rd) {
      return -ENOMEM;
