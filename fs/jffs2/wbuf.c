...
#define PAGE_DIV(x) (((unsigned long) (x) \
  / (unsigned long) (c->wbuf_pagesize)) \
  * (unsigned long) (c->wbuf_pagesize))
...
static inline void jffs2_refile_wbuf_blocks(struct jffs2_sb_info *c) {
  ...
  list_for_each_safe(this, next, &c->erasable_pending_wbuf_list) {
    struct jffs2_eraseblock *jeb = list_entry(this, struct jffs2_eraseblock,
        list);
    jffs2_dbg(1,
...
static void jffs2_block_refile(struct jffs2_sb_info *c,
    struct jffs2_eraseblock *jeb, int allow_empty) {
...
static struct jffs2_raw_node_ref **jffs2_incore_replace_raw(
...
read_failed:
    ...
    if (ret || retlen != towrite || jffs2_verify_write(c, rewrite_buf, ofs)) {
      ...
      if (retlen) {
        jffs2_add_physical_node_ref(c, ofs | REF_OBSOLETE,
            ref_totlen(c, jeb, first_raw), NULL);
      }
      return;
    }
  ...
  /* Now sort out the jffs2_raw_node_refs, moving them from the old to the next
   * block */
 ...
 * 0: Do not pad. Probably pointless - we only ever use this when we can't pad
 * anyway.
...
static int __jffs2_flush_wbuf(struct jffs2_sb_info *c, int pad) {
  ...
  if (jffs2_prealloc_raw_node_refs(c, wbuf_jeb,
      c->nextblock->allocated_refs + 1)) {
    return -ENOMEM;
  }
...
wfail:
  ...
  if (pad) {
    ...
    spin_lock(&c->erase_completion_lock);
    jffs2_link_node_ref(c, wbuf_jeb, (c->wbuf_ofs + c->wbuf_len) | REF_OBSOLETE,
        waste, NULL);
