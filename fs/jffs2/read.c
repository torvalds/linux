...
int jffs2_read_dnode(struct jffs2_sb_info *c, struct jffs2_inode_info *f,
  ...
  ret = jffs2_flash_read(c, ref_offset(
      fd->raw), sizeof(*ri), &readlen, (char *) ri);
  if (ret) {
   ...
   * provided, check CRC.
   * Reading whole node and it's compressed - read into comprbuf, check CRC and
   * decompress to buffer provided
   * Reading partial node and it's uncompressed - read into readbuf, check CRC,
   * and copy
   * Reading partial node and it's compressed - read into readbuf, check
   * checksum, decompress to decomprbuf and copy
   */
  ...
  if (ri->compr != JFFS2_COMPR_NONE) {
    ...
    ret = jffs2_decompress(c, f, ri->compr | (ri->usercompr << 8), readbuf,
        decomprbuf, je32_to_cpu(ri->csize),
