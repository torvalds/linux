...
int jffs2_do_setattr(struct inode *inode, struct iattr *iattr) {
  ...
  ri->isize
    = cpu_to_je32((ivalid & ATTR_SIZE) ? iattr->ia_size : inode->i_size);
  ri->atime
    = cpu_to_je32(I_SEC((ivalid
      & ATTR_ATIME) ? iattr->ia_atime : inode_get_atime(inode)));
  ri->mtime
    = cpu_to_je32(I_SEC((ivalid
      & ATTR_MTIME) ? iattr->ia_mtime : inode_get_mtime(inode)));
  ri->ctime
    = cpu_to_je32(I_SEC((ivalid
      & ATTR_CTIME) ? iattr->ia_ctime : inode_get_ctime(inode)));
...
void jffs2_dirty_inode(struct inode *inode, int flags) {
  ...
  jffs2_dbg(1, "%s(): calling setattr() for ino #%lu\n",
      __func__, inode->i_ino);
  iattr.ia_valid = ATTR_MODE | ATTR_UID | ATTR_GID | ATTR_ATIME | ATTR_MTIME
      | ATTR_CTIME;
...
int jffs2_do_fill_super(struct super_block *sb, struct fs_context *fc) {
  ...
  c->inocache_list
    = kcalloc(c->inocache_hashsize, sizeof(struct jffs2_inode_cache *),
      GFP_KERNEL);
  if (!c->inocache_list) {
    ret = -ENOMEM;
    goto out_wbuf;
