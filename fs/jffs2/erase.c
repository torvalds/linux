...
static void jffs2_erase_block(struct jffs2_sb_info *c,
  ...
  if (!instr) {
    pr_warn(
        "kzalloc for struct erase_info in jffs2_erase_block failed. Refiling block for later\n");
...
int jffs2_erase_pending_blocks(struct jffs2_sb_info *c, int count) {
  ...
  while (!list_empty(&c->erase_complete_list)
    ...
    if (!list_empty(&c->erase_complete_list)) {
      jeb = list_entry(c->erase_complete_list.next, struct jffs2_eraseblock,
          list);
...
static void jffs2_erase_failed(struct jffs2_sb_info *c,
  ...
  if (jffs2_cleanmarker_oob(c)
      && (bad_offset != (uint32_t) MTD_FAIL_ADDR_UNKNOWN)) {
...
static void jffs2_mark_erased_block(struct jffs2_sb_info *c,
  ...
  if (jffs2_cleanmarker_oob(c) || c->cleanmarker_size == 0) {
    ...
    marker.hdr_crc
      = cpu_to_je32(crc32(0, &marker, sizeof(struct jffs2_unknown_node) - 4));
  ...
  if (c->cleanmarker_size && !jffs2_cleanmarker_oob(c)) {
    jffs2_link_node_ref(c, jeb, jeb->offset | REF_NORMAL, c->cleanmarker_size,
        NULL);
  }
