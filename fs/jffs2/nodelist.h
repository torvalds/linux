...
struct jffs2_raw_node_ref {
  struct jffs2_raw_node_ref *next_in_ino; /* Points to the next raw_node_ref
                                           ...
                                           * last, it points to the inode_cache,
                                           * xattr_ref or xattr_datum instead.
                                           * The common part of those structures
                                           * has NULL in the first word. See
                                           * jffs2_raw_ref_to_ic() below */
...
static inline struct jffs2_raw_node_ref *ref_next(
...
static inline struct jffs2_inode_cache *jffs2_raw_ref_to_ic(
...
#define REF_NORMAL  3 /* Possibly overlapped. Read the page and write again on
                       * GC */
...
#define dirent_node_state(rd) ((je32_to_cpu((rd)->ino) ? REF_PRISTINE : \
  REF_NORMAL))
...
struct jffs2_inode_cache {
  ...
  struct jffs2_full_dirent *scan_dents; /* Used during scan to hold
                                         * temporary lists of dirents, and later
                                         * must be set to
                                         * NULL to mark the end of the
                                         * raw_node_ref->next_in_ino
                                         * chain. */
...
#define write_ofs(c) ((c)->nextblock->offset + (c)->sector_size \
...
static inline int jffs2_blocks_use_vmalloc(struct jffs2_sb_info *c) {
  return ((c->flash_size / c->sector_size) * sizeof(struct jffs2_eraseblock))
    > (128 * 1024);
}
...
#define ALLOC_NORETRY 3 /* For jffs2_write_dnode: On failure, return -EAGAIN
                         * instead of retrying */
...
#define frag_next(frag) rb_entry(rb_next( \
    &(frag)->rb), struct jffs2_node_frag, rb)
#define frag_prev(frag) rb_entry(rb_prev( \
    &(frag)->rb), struct jffs2_node_frag, rb)
#define frag_parent(frag) rb_entry(rb_parent( \
    &(frag)->rb), struct jffs2_node_frag, rb)
...
#define tn_prev(tn) rb_entry(rb_prev(&(tn)->rb), struct jffs2_tmp_dnode_info, \
    rb)
#define tn_parent(tn) rb_entry(rb_parent( \
    &(tn)->rb), struct jffs2_tmp_dnode_info, rb)
...
int jffs2_add_full_dnode_to_inode(struct jffs2_sb_info *c,
    ...
    struct jffs2_full_dnode *fn);
uint32_t jffs2_truncate_fragtree(struct jffs2_sb_info *c, struct rb_root *list,
    uint32_t size);
...
int jffs2_scan_dirty_space(struct jffs2_sb_info *c,
    struct jffs2_eraseblock *jeb, uint32_t size);
