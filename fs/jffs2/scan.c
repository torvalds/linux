...
/* These helper functions _must_ increase ofs and also do the dirty/used space
 * accounting.
 * Returning an error will abort the mount - bad checksums etc. should just mark
 *the space
 * as dirty.
 */
  ...
  for (i = 0; i < c->nr_blocks; i++) {
    ...
    switch (ret) {
      ...
      case BLK_STATE_ALLDIRTY:
         ...
         * For now we just put it on the erasing list. We'll start the erases
         *later*/
        jffs2_dbg(1,
...
static int jffs2_scan_xref_node(struct jffs2_sb_info *c,
  ...
  c->xref_temp = ref;
  jffs2_link_node_ref(c, jeb, ofs | REF_PRISTINE, PAD(je32_to_cpu(
      rr->totlen)), (void *) ref);
  if (jffs2_sum_active()) {
    ...
    switch (je16_to_cpu(node->nodetype)) {
      case JFFS2_NODETYPE_INODE:
        if (buf_ofs + buf_len < ofs + sizeof(struct jffs2_raw_inode)) {
          buf_len
            = min_t(uint32_t, buf_size, jeb->offset + c->sector_size - ofs);
          jffs2_dbg(1,
      ...
      case JFFS2_NODETYPE_CLEANMARKER:
        ...
        if (je32_to_cpu(node->totlen) != c->cleanmarker_size) {
          ...
          jffs2_link_node_ref(c, jeb, ofs | REF_NORMAL, c->cleanmarker_size,
              NULL);
          ...
          case JFFS2_FEATURE_RWCOMPAT_COPY:
            {
              ...
              jffs2_link_node_ref(c, jeb, ofs | REF_PRISTINE,
                  PAD(je32_to_cpu(node->totlen)), NULL);
...
static int jffs2_scan_inode_node(struct jffs2_sb_info *c,
   ...
   * attribute
   * this node; we can do all the CRC checking etc. later. There's a tradeoff
   * here --
   * we used to scan the flash once only, reading everything we want from it
   * into
   * memory, then building all our in-core data structures and freeing the extra
   * information. Now we allow the first part of the mount to complete a lot
   * quicker,
   * but we have to go _back_ to the flash in order to finish the CRC checking,
   * etc.
   * Which means that the _full_ amount of time to get to proper write mode with
   * GC
  ...
  /* Wheee. It worked */
  jffs2_link_node_ref(c, jeb, ofs | REF_UNCHECKED, PAD(je32_to_cpu(
      ri->totlen)), ic);
...
static int jffs2_scan_dirent_node(struct jffs2_sb_info *c,
  ...
  if (crc != je32_to_cpu(rd->node_crc)) {
    ...
    /* We believe totlen because the CRC on the node _header_ was OK, just the
     * node itself failed. */
