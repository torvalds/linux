 ...
 *   is used to calcurate xdatum hashkey. The reminder of hashkey into
 *XATTRINDEX_HASHSIZE is
 *   the index of the xattr name/value pair cache (c->xattrindex).
 * is_xattr_datum_unchecked(c, xd)
 *   returns 1, if xdatum contains any unchecked raw nodes. if all raw nodes are
 *not
 *   unchecked, it returns 0.
 * unload_xattr_datum(c, xd)
 *   is used to release xattr name/value pair and detach from c->xattrindex.
 * reclaim_xattr_datum(c)
 *   is used to reclaim xattr name/value pairs on the xattr name/value pair
 *cache when
 *   memory usage by cache is over c->xdatum_mem_threshold. Currently, this
 *threshold
 *   is hard coded as 32KiB.
 * do_verify_xattr_datum(c, xd)
 *   is used to load the xdatum informations without name/value pair from the
 *medium.
 *   It's necessary once, because those informations are not collected during
 *mounting
 *   process when EBS is enabled.
 *   0 will be returned, if success. An negative return value means recoverable
 *error, and
 *   positive return value means unrecoverable error. Thus, caller must remove
 *this xdatum
 ...
 *   is used to be as a wrapper of do_verify_xattr_datum() and
 *do_load_xattr_datum().
 *   If xd need to call do_verify_xattr_datum() at first, it's called before
 *calling
 *   do_load_xattr_datum(). The meanings of return value is same as
 ...
 *   is used to delete a xdatum. When nobody refers this xdatum,
 *JFFS2_XFLAGS_DEAD
 ...
 * -------------------------------------------------- */
static uint32_t xattr_datum_hashkey(int xprefix, const char *xname,
    const char *xvalue, int xsize) {
  int name_len = strlen(xname);
  return crc32(xprefix, xname, name_len) ^ crc32(xprefix, xvalue, xsize);
}

static int is_xattr_datum_unchecked(struct jffs2_sb_info *c,
...
static void unload_xattr_datum(struct jffs2_sb_info *c,
    struct jffs2_xattr_datum *xd) {
...
static int do_verify_xattr_datum(struct jffs2_sb_info *c,
    struct jffs2_xattr_datum *xd) {
  /* must be called under down_write(xattr_sem) */
...
complete:
  for (raw = xd->node; raw != (void *) xd; raw = raw->next_in_ino) {
    ...
    if (ref_flags(raw) == REF_UNCHECKED) {
      c->unchecked_size -= totlen;
      c->used_size += totlen;
      jeb->unchecked_size -= totlen;
      jeb->used_size += totlen;
    }
    raw->flash_offset = ref_offset(raw)
        | ((xd->node == raw) ? REF_PRISTINE : REF_NORMAL);
  }
  spin_unlock(&c->erase_completion_lock);
...
static int do_load_xattr_datum(struct jffs2_sb_info *c,
    struct jffs2_xattr_datum *xd) {
  /* must be called under down_write(xattr_sem) */
  char *data;
  size_t readlen;
...
retry:
  ...
  ret = jffs2_flash_read(c, ref_offset(
      xd->node) + sizeof(struct jffs2_raw_xattr),
      length, &readlen, data);
  ...
  xd->hashkey = xattr_datum_hashkey(xd->xprefix, xd->xname, xd->xvalue,
      xd->value_len);
...
static int load_xattr_datum(struct jffs2_sb_info *c,
    struct jffs2_xattr_datum *xd) {
  /* must be called under down_write(xattr_sem);
...
static int save_xattr_datum(struct jffs2_sb_info *c,
    struct jffs2_xattr_datum *xd) {
  /* must be called under down_write(xattr_sem) */
  struct jffs2_raw_xattr rx;
  struct kvec vecs[2];
  ...
  rx.hdr_crc
    = cpu_to_je32(crc32(0, &rx, sizeof(struct jffs2_unknown_node) - 4));
  ...
  if (rc || totlen != length) {
    ...
    rc = rc ? rc : -EIO;
    if (length) {
      jffs2_add_physical_node_ref(c, phys_ofs | REF_OBSOLETE, PAD(totlen),
          NULL);
    }
    return rc;
  }
  /* success */
  jffs2_add_physical_node_ref(c, phys_ofs | REF_PRISTINE, PAD(
      totlen), (void *) xd);
  dbg_xattr(
...
static void unrefer_xattr_datum(struct jffs2_sb_info *c,
    struct jffs2_xattr_datum *xd) {
  /* must be called under down_write(xattr_sem) */
 ...
 * -------------------------------------------------- */
static int verify_xattr_ref(struct jffs2_sb_info *c,
...
complete:
  for (raw = ref->node; raw != (void *) ref; raw = raw->next_in_ino) {
    ...
    raw->flash_offset = ref_offset(raw)
        | ((ref->node == raw) ? REF_PRISTINE : REF_NORMAL);
  }
  spin_unlock(&c->erase_completion_lock);
...
static int save_xattr_ref(struct jffs2_sb_info *c,
    struct jffs2_xattr_ref *ref) {
  /* must be called under down_write(xattr_sem) */
  struct jffs2_raw_xref rr;
  size_t length;
  ...
  rr.hdr_crc
    = cpu_to_je32(crc32(0, &rr, sizeof(struct jffs2_unknown_node) - 4));
  ...
  if (ret || sizeof(rr) != length) {
    ...
    ret = ret ? ret : -EIO;
    if (length) {
      jffs2_add_physical_node_ref(c, phys_ofs | REF_OBSOLETE, PAD(
          sizeof(rr)), NULL);
    }
    return ret;
  }
  /* success */
  ref->xseqno = xseqno;
  jffs2_add_physical_node_ref(c, phys_ofs | REF_PRISTINE, PAD(sizeof(rr)),
      (void *) ref);
...
static void delete_xattr_ref(struct jffs2_sb_info *c,
    struct jffs2_xattr_ref *ref) {
  /* must be called under down_write(xattr_sem) */
  struct jffs2_xattr_datum *xd;
  xd = ref->xd;
...
static int check_xattr_ref_inode(struct jffs2_sb_info *c,
    struct jffs2_inode_cache *ic) {
...
retry:
  rc = 0;
  for (ref = ic->xref, pref = &ic->xref; ref;
      pref = &ref->next, ref = ref->next) {
    if (!ref->xd->xname) {
    ...
    for (cmp = ref->next, pcmp = &ref->next; cmp;
        pcmp = &cmp->next, cmp = cmp->next) {
      if (!cmp->xd->xname) {
...
static struct jffs2_xattr_datum *jffs2_find_xattr_datum(struct jffs2_sb_info *c,
    uint32_t xid) {
  struct jffs2_xattr_datum *xd;
...
void jffs2_release_xattr_ref(struct jffs2_sb_info *c,
  ...
  for (tmp = c->xref_dead_list, ptmp = &c->xref_dead_list; tmp;
      ptmp = &tmp->next, tmp = tmp->next) {
    if (ref == tmp) {
      *ptmp = tmp->next;
      break;
    }
