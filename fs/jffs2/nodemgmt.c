 ...
 *  active at any time. The semaphore is later released by
 *jffs2_commit_allocation()
 *
...
int jffs2_reserve_space(struct jffs2_sb_info *c, uint32_t minsize,
  ...
  while (ret == -EAGAIN) {
    while (c->nr_free_blocks + c->nr_erasing_blocks < blocksneeded) {
       ...
       * If one block is actually erased, it is not longer counted as
       *dirty_space
       * but it is counted in c->nr_erasing_blocks, so we add it and subtract it
       * with c->nr_erasing_blocks * c->sector_size again.
       * Blocks on erasable_list are counted as dirty_size, but not in
       *c->nr_erasing_blocks
       * This helps us to force gc and pick eventually a clean block to spread
       *the load.
       * We add unchecked_size here, as we hopefully will find some space to
       *use.
       ...
       * don't know, if unchecked size contains obsoleted nodes, which could
       *give us some
       * more usable space. This will affect the sum only once, as gc first
       *finishes checking
       * of nodes.
       + Return -ENOSPC, if the maximum possibly available space is less or
       +equal than
       * blocksneeded * sector_size.
       * This blocks endless gc looping on a filesystem, which is nearly full,
       *even if
       * the check above passes.
       */
      avail = c->free_size + c->dirty_size + c->erasing_size
          + c->unchecked_size;
...
static void jffs2_close_nextblock(struct jffs2_sb_info *c,
...
restart:
  ...
  if (jffs2_sum_active() && (sumsize != JFFS2_SUMMARY_NOSUM_SIZE)) {
    ...
    if (jeb) {
      reserved_size = PAD(
          sumsize + c->summary->sum_size + JFFS2_SUMMARY_FRAME_SIZE);
...
struct jffs2_raw_node_ref *jffs2_add_physical_node_ref(struct jffs2_sb_info *c,
  ...
  if ((c->nextblock || ((ofs & 3) != REF_OBSOLETE))
      && (jeb != c->nextblock
      || (ofs & ~3) != jeb->offset + (c->sector_size - jeb->free_size))) {
...
void jffs2_mark_node_obsolete(struct jffs2_sb_info *c,
  ...
  if ((jeb->dirty_size || ISDIRTY(jeb->wasted_size + freed_len))
      && jeb != c->nextblock) {
  ...
  if (jeb == c->nextblock) {
    ...
    jffs2_dbg(2, "Not moving gcblock 0x%08x to dirty_list\n",
        jeb->offset);
  } else if (ISDIRTY(jeb->dirty_size)
      && !ISDIRTY(jeb->dirty_size - addedsize)) {
    jffs2_dbg(1,
  ...
  /* The erase_free_sem is locked, and has been since before we marked the node
   * obsolete
   * and potentially put its eraseblock onto the erase_pending_list. Thus, we
   * know that
   * the block hasn't _already_ been erased, and that 'ref' itself hasn't been
   * freed yet
...
int jffs2_thread_should_wake(struct jffs2_sb_info *c) {
  ...
  if (c->unchecked_size) {
    jffs2_dbg(1,
        "jffs2_thread_should_wake(): unchecked_size %d, check_ino #%d\n",
   ...
   * Blocks on erasable_list are counted as dirty_size, but not in
   *c->nr_erasing_blocks
   * This helps us to force gc and pick eventually a clean block to spread the
   *load.
   */
  dirty = c->dirty_size + c->erasing_size - c->nr_erasing_blocks
      * c->sector_size;
