...
static int alloc_dinode(struct gfs2_inode *ip, u32 flags, unsigned *dblocks) {
  ...
  error = gfs2_trans_begin(sdp,
      (*dblocks * RES_RG_BIT) + RES_STATFS + RES_QUOTA, 0);
  if (error) {
    goto out_ipreserv;
  }
...
static void gfs2_init_dir(struct buffer_head *dibh,
  ...
  gfs2_qstr2dirent(&gfs2_qdotdot,
      dibh->b_size - GFS2_DIRENT_SIZE(1) - sizeof(struct gfs2_dinode), dent);
...
static int gfs2_create_inode(struct inode *dir, struct dentry *dentry,
  ...
  error = gfs2_glock_get(sdp, ip->i_no_addr, &gfs2_inode_glops, CREATE,
      &ip->i_gl);
  if (error) {
    goto fail_free_inode;
  }
...
static int gfs2_symlink(struct mnt_idmap *idmap, struct inode *dir,
  ...
  return gfs2_create_inode(dir, dentry, NULL, S_IFLNK | S_IRWXUGO, 0, symname,
      size, 0);
}
...
static int gfs2_mkdir(struct mnt_idmap *idmap, struct inode *dir,
  ...
  return gfs2_create_inode(dir, dentry, NULL, S_IFDIR | mode, 0, NULL, dsize,
      0);
}
