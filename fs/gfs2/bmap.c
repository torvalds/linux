...
static inline __be64 *metapointer(unsigned int height,
    const struct metapath *mp) {
  __be64 *p = metaptr1(height, mp);
  return p + mp->mp_list[height];
}

static inline const __be64 *metaend(unsigned int height,
    const struct metapath *mp) {
...
static inline struct buffer_head *metapath_dibh(struct metapath *mp) {
  return mp->mp_bh[0];
}
...
enum walker_status {
  WALK_STOP, WALK_FOLLOW, WALK_CONTINUE
};
...
static struct folio *gfs2_iomap_get_folio(struct iomap_iter *iter, loff_t pos,
    unsigned len) {
...
static int gfs2_journaled_truncate(struct inode *inode, u64 oldsize,
    u64 newsize) {
  struct gfs2_sbd *sdp = GFS2_SB(inode);
  ...
  if (!ret && blks_outside_rgrp) {
    ...
    if (current->journal_info) {
      ...
      *  to keep its di_blocks current in case of failure. */
      inode_set_mtime_to_ts(&ip->i_inode,
...
static inline void metapointer_range(struct metapath *mp, int height,
...
static int gfs2_add_jextent(struct gfs2_jdesc *jd, u64 lblock, u64 dblock,
    u64 blocks) {
  struct gfs2_journal_extent *jext;
...
static int stuffed_zero_range(struct inode *inode, loff_t offset,
    loff_t length) {
  struct gfs2_inode *ip = GFS2_I(inode);
  struct buffer_head *dibh;
  int error;
