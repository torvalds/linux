...
static int punch_hole(struct gfs2_iyesde *ip, u64 offset, u64 length);

/**
 * gfs2_unstuffer_page - unstuff a stuffed iyesde into a block cached by a page
 * @ip: the iyesde
 * @dibh: the diyesde buffer
...
static int gfs2_unstuffer_page(struct gfs2_iyesde *ip, struct buffer_head *dibh,
			       u64 block, struct page *page)
{
	...
	if (!page_has_buffers(page))
		create_empty_buffers(page, BIT(iyesde->i_blkbits),
				     BIT(BH_Uptodate));
...
/**
 * gfs2_unstuff_diyesde - Unstuff a diyesde when the data has grown too big
 * @ip: The GFS2 iyesde to unstuff
 * @page: The (optional) page. This is looked up if the @page is NULL
 *
 * This routine unstuffs a diyesde and returns it to a "yesrmal" state such
 * that the height can be grown in the traditional way.
 *
 * Returns: erryes
 */

int gfs2_unstuff_diyesde(struct gfs2_iyesde *ip, struct page *page)
{
	...
	if (i_size_read(&ip->i_iyesde)) {
		...
		if (error)
			goto out_brelse;
		if (isdir) {
			gfs2_trans_remove_revoke(GFS2_SB(&ip->i_iyesde), block, 1);
			error = gfs2_dir_get_new_buffer(ip, block, &bh);
			if (error)
				goto out_brelse;
			gfs2_buffer_copy_tail(bh, sizeof(struct gfs2_meta_header),
					      dibh, sizeof(struct gfs2_diyesde));
			brelse(bh);
		} else {
	...
	if (i_size_read(&ip->i_iyesde)) {
		*(__be64 *)(di + 1) = cpu_to_be64(block);
		...
		di->di_blocks = cpu_to_be64(gfs2_get_iyesde_blocks(&ip->i_iyesde));
	}
 ...
 *   through the metadata of iyesde "ip" to get to block "block".
 *
 *   Example:
 ...
 *   0 in the diyesde.  We would then load the indirect block pointed to by
 ...
 * @height: The metadata height (0 = diyesde)
 * @mp: The metapath
 */
static inline __be64 *metaptr1(unsigned int height, const struct metapath *mp)
{
	...
	if (height == 0)
		return ((__be64 *)(bh->b_data + sizeof(struct gfs2_diyesde)));
...
static int __fillup_metapath(struct gfs2_iyesde *ip, struct metapath *mp,
 ...
 * @mp: The metapath
 *
 * Assumes that the iyesde's buffer has already been looked up and
 * hooked onto mp->mp_bh[0] and that the metapath has been initialised
 * by find_metapath().
 *
 * If this function encounters part of the tree which has yest been
...
static int lookup_metapath(struct gfs2_iyesde *ip, struct metapath *mp)
{
...
static int fillup_metapath(struct gfs2_iyesde *ip, struct metapath *mp, int h)
{
	unsigned int x = 0;
	int ret;
...
static int gfs2_walk_metadata(struct iyesde *iyesde, struct metapath *mp,
 ...
 *
 * Returns: erryes on error
 */
static int gfs2_hole_size(struct iyesde *iyesde, sector_t lblock, u64 len,
			  struct metapath *mp, struct iomap *iomap)
{
	...
	ret = gfs2_walk_metadata(iyesde, &clone, len, gfs2_hole_walker);
	if (ret < 0)
		goto out;

	if (ret == 1)
		hole_size = metapath_to_block(GFS2_SB(iyesde), &clone) - lblock;
	else
		hole_size = len;
	iomap->length = hole_size << iyesde->i_blkbits;
	ret = 0;
 ...
 * eyesugh contiguous free blocks are available, there will only be one
 * allocation request per call) and uses the state machine to initialise
 * the blocks in order.
 *
 * Right yesw, this function will allocate at most one indirect block
...
static int gfs2_iomap_alloc(struct iyesde *iyesde, struct iomap *iomap,
			    struct metapath *mp)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	...
	size_t dblks = iomap->length >> iyesde->i_blkbits;
	...
	do {
		...
		case ALLOC_DATA:
			...
			iomap->addr = bn << iyesde->i_blkbits;
	...
	iomap->length = (u64)dblks << iyesde->i_blkbits;
	ip->i_height = mp->mp_fheight;
	gfs2_add_iyesde_blocks(&ip->i_iyesde, alloced);
 ...
 */
static u64 gfs2_alloc_size(struct iyesde *iyesde, struct metapath *mp, u64 size)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	 ...
	 * first time needs to be large eyesugh to get the reservation and
...
/**
 * gfs2_iomap_get - Map blocks from an iyesde to disk blocks
 * @iyesde: The iyesde
 ...
 * Returns: erryes
 */
static int gfs2_iomap_get(struct iyesde *iyesde, loff_t pos, loff_t length,
			  ...
			  struct metapath *mp)
{
	...
	ret = gfs2_meta_iyesde_buffer(ip, &dibh);
	if (ret)
		goto unlock;
	mp->mp_bh[0] = dibh;
	...
	if (gfs2_is_stuffed(ip)) {
		...
		iomap->addr = (ip->i_yes_addr << iyesde->i_blkbits) +
			      sizeof(struct gfs2_diyesde);
		iomap->type = IOMAP_INLINE;
		iomap->inline_data = dibh->b_data + sizeof(struct gfs2_diyesde);
		goto out;
	}

unstuff:
	lblock = pos >> iyesde->i_blkbits;
	iomap->offset = lblock << iyesde->i_blkbits;
	lblock_stop = (pos + length - 1) >> iyesde->i_blkbits;
	len = lblock_stop - lblock + 1;
	iomap->length = len << iyesde->i_blkbits;
	...
	iomap->addr = be64_to_cpu(*ptr) << iyesde->i_blkbits;
	iomap->length = len << iyesde->i_blkbits;
	iomap->type = IOMAP_MAPPED;
...
do_alloc:
	...
	if (flags & IOMAP_REPORT) {
		if (pos >= size)
			...
			ret = gfs2_hole_size(iyesde, lblock, len, mp, iomap);
		else
		...
		len = gfs2_alloc_size(iyesde, mp, len);
		alloc_size = len << iyesde->i_blkbits;
 ...
 * @iyesde: the iyesde of the file we're mapping
 * @lblock: the block relative to the start of the file
 * @dblock: the returned dblock, if yes error
 *
 ...
 */
int gfs2_lblk_to_dblk(struct iyesde *iyesde, u32 lblock, u64 *dblock)
{
	struct iomap iomap = { };
	struct metapath mp = { .mp_aheight = 1, };
	loff_t pos = (loff_t)lblock << iyesde->i_blkbits;
	int ret;

	ret = gfs2_iomap_get(iyesde, pos, i_blocksize(iyesde), 0, &iomap, &mp);
	release_metapath(&mp);
	if (ret == 0)
		*dblock = iomap.addr >> iyesde->i_blkbits;

	return ret;
}

static int gfs2_write_lock(struct iyesde *iyesde)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	int error;
...
static void gfs2_write_unlock(struct iyesde *iyesde)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
...
static int gfs2_iomap_page_prepare(struct iyesde *iyesde, loff_t pos,
				   unsigned len, struct iomap *iomap)
{
	unsigned int blockmask = i_blocksize(iyesde) - 1;
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	unsigned int blocks;

	blocks = ((pos & blockmask) + len + blockmask) >> iyesde->i_blkbits;
	return gfs2_trans_begin(sdp, RES_DINODE + blocks, 0);
}

static void gfs2_iomap_page_done(struct iyesde *iyesde, loff_t pos,
...
static int gfs2_iomap_begin_write(struct iyesde *iyesde, loff_t pos,
				  ...
				  struct metapath *mp)
{
	...
	if (unstuff || iomap->type == IOMAP_HOLE) {
		...
		if (unstuff) {
			ret = gfs2_unstuff_diyesde(ip, NULL);
			if (ret)
				goto out_trans_end;
			release_metapath(mp);
			ret = gfs2_iomap_get(iyesde, iomap->offset,
		...
		if (iomap->type == IOMAP_HOLE) {
			ret = gfs2_iomap_alloc(iyesde, iomap, mp);
			if (ret) {
				gfs2_trans_end(sdp);
...
static int gfs2_iomap_begin(struct iyesde *iyesde, loff_t pos, loff_t length,
			    unsigned flags, struct iomap *iomap,
			    struct iomap *srcmap)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	...
	if (gfs2_iomap_need_write_lock(flags)) {
		ret = gfs2_write_lock(iyesde);
		if (ret)
			goto out;
	}

	ret = gfs2_iomap_get(iyesde, pos, length, flags, iomap, &mp);
	if (ret)
		goto out_unlock;
	...
	ret = gfs2_iomap_begin_write(iyesde, pos, length, flags, iomap, &mp);

out_unlock:
	if (ret && gfs2_iomap_need_write_lock(flags))
		gfs2_write_unlock(iyesde);
	release_metapath(&mp);
out:
...
static int gfs2_iomap_end(struct iyesde *iyesde, loff_t pos, loff_t length,
			  ssize_t written, unsigned flags, struct iomap *iomap)
{
	...
	if (iyesde == sdp->sd_rindex)
		adjust_fs_space(iyesde);

	gfs2_inplace_release(ip);

	if (length != written && (iomap->flags & IOMAP_F_NEW)) {
		/* Deallocate blocks that were just allocated. */
		loff_t blockmask = i_blocksize(iyesde) - 1;
		loff_t end = (pos + length) & ~blockmask;

		pos = (pos + written + blockmask) & ~blockmask;
		if (pos < end) {
			truncate_pagecache_range(iyesde, pos, end - 1);
...
/**
 * gfs2_block_map - Map one or more blocks of an iyesde to a disk block
 * @iyesde: The iyesde
 ...
 * when @lblock is yest mapped.  Sets buffer_mapped(bh_map) and
...
int gfs2_block_map(struct iyesde *iyesde, sector_t lblock,
		   struct buffer_head *bh_map, int create)
{
	...
	if (create) {
		ret = gfs2_iomap_get(iyesde, pos, length, IOMAP_WRITE, &iomap, &mp);
		if (!ret && iomap.type == IOMAP_HOLE)
			ret = gfs2_iomap_alloc(iyesde, &iomap, &mp);
		release_metapath(&mp);
	} else {
		ret = gfs2_iomap_get(iyesde, pos, length, 0, &iomap, &mp);
		release_metapath(&mp);
	}
	if (ret)
		goto out;
	...
	if (iomap.addr != IOMAP_NULL_ADDR)
		map_bh(bh_map, iyesde->i_sb, iomap.addr >> iyesde->i_blkbits);
...
/*
 * Deprecated: do yest use in new code
 */
int gfs2_extent_map(struct iyesde *iyesde, u64 lblock, int *new, u64 *dblock, unsigned *extlen)
{
	...
	bh.b_size = BIT(iyesde->i_blkbits + (create ? 0 : 5));
	ret = gfs2_block_map(iyesde, lblock, &bh, create);
	*extlen = bh.b_size >> iyesde->i_blkbits;
	*dblock = bh.b_blocknr;
	if (buffer_new(&bh))
		*new = 1;
	else
...
static int gfs2_block_zero_range(struct iyesde *iyesde, loff_t from,
				 unsigned int length)
{
	return iomap_zero_range(iyesde, from, length, NULL, &gfs2_iomap_ops);
}
 ...
 * @iyesde: The iyesde being truncated
...
static int gfs2_journaled_truncate(struct iyesde *iyesde, u64 oldsize, u64 newsize)
{
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	...
	while (oldsize != newsize) {
		...
		truncate_pagecache(iyesde, oldsize - chunk);
		oldsize -= chunk;
...
static int trunc_start(struct iyesde *iyesde, u64 newsize)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	struct buffer_head *dibh = NULL;
	int journaled = gfs2_is_jdata(ip);
	u64 oldsize = iyesde->i_size;
	int error;
	...
	if (gfs2_is_stuffed(ip)) {
		gfs2_buffer_clear_tail(dibh, sizeof(struct gfs2_diyesde) + newsize);
	} else {
		unsigned int blocksize = i_blocksize(iyesde);
		unsigned int offs = newsize & (blocksize - 1);
		if (offs) {
			error = gfs2_block_zero_range(iyesde, newsize,
						      blocksize - offs);
			if (error)
				goto out;
		}
	...
	if (journaled)
		error = gfs2_journaled_truncate(iyesde, oldsize, newsize);
	else
...
int gfs2_iomap_get_alloc(struct iyesde *iyesde, loff_t pos, loff_t length,
			 struct iomap *iomap)
{
	struct metapath mp = { .mp_aheight = 1, };
	int ret;
 ...
 * At every transaction boundary, we rewrite the iyesde into the journal.
 * That way the bitmaps are kept consistent with the iyesde and we can recover
 * if we're interrupted by power-outages.
 *
 * Returns: 0, or return code if an error occurred.
 *          *btotal has the total number of blocks freed
 */
static int sweep_bh_for_rgrps(struct gfs2_iyesde *ip, struct gfs2_holder *rd_gh,
			      ...
			      bool meta, u32 *btotal)
{
	...
	s64 blen; /* needs to be s64 or gfs2_add_iyesde_blocks breaks */
	int ret = 0;
...
more_rgrps:
	...
	for (p = start; p < end; p++) {
		...
		/* The size of our transactions will be unkyeswn until we
		   actually process all the metadata blocks that relate to
		   the rgrp. So we estimate. We kyesw it can't be more than
		   the diyesde's i_blocks and we don't want to exceed the
		...
		if (current->journal_info == NULL) {
			...
			jblocks_rqsted = rgd->rd_length + RES_DINODE +
				RES_INDIRECT;
			isize_blks = gfs2_get_iyesde_blocks(&ip->i_iyesde);
		...
		if (tr->tr_num_buf_new + RES_STATFS +
			...
			/* This next part is tricky. If the buffer was added
			   ...
			   If the buffer was yest added to the transaction
		...
		*p = 0;
		...
		if (bstart) {
			...
			(*btotal) += blen;
			gfs2_add_iyesde_blocks(&ip->i_iyesde, -blen);
		}
		bstart = bn;
		blen = 1;
	}
...
out_unlock:
	if (!ret && blks_outside_rgrp) { /* If buffer still has yesn-zero blocks
		...
		if (current->journal_info) {
			...
			/* Every transaction boundary, we rewrite the diyesde
			   to keep its di_blocks current in case of failure. */
			ip->i_iyesde.i_mtime = ip->i_iyesde.i_ctime =
				current_time(&ip->i_iyesde);
			gfs2_trans_add_meta(ip->i_gl, dibh);
			gfs2_diyesde_out(ip, dibh->b_data);
			brelse(dibh);
			up_write(&ip->i_rw_mutex);
...
/**
 * find_yesnnull_ptr - find a yesn-null pointer given a metapath and height
 * @mp: starting metapath
 * @h: desired height to search
 *
 * Assumes the metapath is valid (with buffers) out to height h.
 * Returns: true if a yesn-null pointer was found in the metapath buffer
 *          false if all remaining pointers are NULL in the buffer
 */
static bool find_yesnnull_ptr(struct gfs2_sbd *sdp, struct metapath *mp,
			     unsigned int h,
			     __u16 *end_list, unsigned int end_aligned)
{
	...
	while (ptr < end) {
		if (*ptr) { /* if we have a yesn-null pointer */
 ...
 * by power failure, etc.  The diyesde is rewritten in every transaction to
 * guarantee integrity.
 */
static int punch_hole(struct gfs2_iyesde *ip, u64 offset, u64 length)
{
	...
	if (offset >= maxsize) {
		 ...
		 * there are yes blocks do deallocate.
		 */
		return 0;
	}
	...
	while (state != DEALLOC_DONE) {
		...
		case DEALLOC_MP_FULL:
			...
			if (gfs2_assert_withdraw(sdp,
				...
				fs_emerg(sdp, "iyesde %llu, block:%llu, i_h:%u,"
					 "s_h:%u, mp_h:%u\n",
			 ...
			 * point is the first metapath we must yest deallocate!
			 */
			...
			/* If we hit an error or just swept diyesde buffer,
			   just exit. */
			if (ret || !mp_h) {
				state = DEALLOC_DONE;
				break;
		...
		case DEALLOC_MP_LOWER:
			/* We're done with the current buffer, so release it,
			   unless it's the diyesde buffer. Then back up to the
			   previous pointer. */
			if (mp_h) {
			...
			if (walk_done(sdp, &mp, mp_h, end_list, end_aligned))
				break;
			/* Here we've found a part of the metapath that is yest
			 * allocated. We need to search at that height for the
			 * next yesn-null pointer. */
			if (find_yesnnull_ptr(sdp, &mp, mp_h, end_list, end_aligned)) {
				state = DEALLOC_FILL_MP;
				mp_h++;
			}
			/* No more yesn-null pointers at this height. Back up
		...
		case DEALLOC_FILL_MP:
			...
			/* If we find a yesn-null block pointer, crawl a bit
	...
	if (btotal) {
		...
		gfs2_quota_change(ip, -(s64)btotal, ip->i_iyesde.i_uid,
				  ip->i_iyesde.i_gid);
		ip->i_iyesde.i_mtime = ip->i_iyesde.i_ctime = current_time(&ip->i_iyesde);
		gfs2_trans_add_meta(ip->i_gl, dibh);
		gfs2_diyesde_out(ip, dibh->b_data);
...
static int trunc_end(struct gfs2_iyesde *ip)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_iyesde);
	struct buffer_head *dibh;
	int error;
 ...
 *
 * Called with an exclusive lock on @iyesde. The @size must
 * be equal to or smaller than the current iyesde size.
 *
 * Returns: erryes
 */

static int do_shrink(struct iyesde *iyesde, u64 newsize)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	int error;

	error = trunc_start(iyesde, newsize);
	if (error < 0)
		return error;
	if (gfs2_is_stuffed(ip))
		return 0;
...
void gfs2_trim_blocks(struct iyesde *iyesde)
{
	int ret;

	ret = do_shrink(iyesde, iyesde->i_size);
	WARN_ON(ret != 0);
}

/**
 * do_grow - Touch and update iyesde size
 * @iyesde: The iyesde
 * @size: The new size
 *
 * This function updates the timestamps on the iyesde and
 * may also increase the size of the iyesde. This function
 * must yest be called with @size any smaller than the current
 * iyesde size.
 *
 * Although it is yest strictly required to unstuff files here,
 * earlier versions of GFS2 have a bug in the stuffed file reading
 * code which will result in a buffer overrun if the size is larger
 * than the max stuffed file size. In order to prevent this from
 * occurring, such files are unstuffed, but in other cases we can
 * just update the iyesde size directly.
 *
 * Returns: 0 on success, or -ve on error
 */

static int do_grow(struct iyesde *iyesde, u64 size)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
 ...
 * is called holding i_rwsem and an exclusive glock on the iyesde
 * in question.
 *
 * Returns: erryes
 */

int gfs2_setattr_size(struct iyesde *iyesde, u64 newsize)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	int ret;
	...
	if (newsize >= iyesde->i_size) {
		ret = do_grow(iyesde, newsize);
		goto out;
	}

	ret = do_shrink(iyesde, newsize);
out:
	gfs2_rsqa_delete(ip, NULL);
	return ret;
}

int gfs2_truncatei_resume(struct gfs2_iyesde *ip)
{
	int error;
	error = punch_hole(ip, i_size_read(&ip->i_iyesde), 0);
	if (!error)
		error = trunc_end(ip);
	return error;
}

int gfs2_file_dealloc(struct gfs2_iyesde *ip)
{
 ...
 * those journals may yest be sequential.  Less likely is the case where
...
int gfs2_map_journal_extents(struct gfs2_sbd *sdp, struct gfs2_jdesc *jd)
{
	...
	start = ktime_get();
	lblock_stop = i_size_read(jd->jd_iyesde) >> shift;
	...
	do {
		...
		bh.b_blocknr = 0;
		bh.b_size = size;
		rc = gfs2_block_map(jd->jd_iyesde, lblock, &bh, 0);
		...
		if (rc)
			goto fail;
		size -= bh.b_size;
		lblock += (bh.b_size >> ip->i_iyesde.i_blkbits);
	} while(size > 0);
...
fail:
	fs_warn(sdp, "error %d mapping journal %u at offset %llu (extent %u)\n",
		rc, jd->jd_jid,
		(unsigned long long)(i_size_read(jd->jd_iyesde) - size),
		jd->nr_extents);
...
int gfs2_write_alloc_required(struct gfs2_iyesde *ip, u64 offset,
			      unsigned int len)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_iyesde);
	struct buffer_head bh;
	...
	end_of_file = (i_size_read(&ip->i_iyesde) + sdp->sd_sb.sb_bsize - 1) >> shift;
	lblock = offset >> shift;
	...
	do {
		bh.b_state = 0;
		bh.b_size = size;
		gfs2_block_map(&ip->i_iyesde, lblock, &bh, 0);
		if (!buffer_mapped(&bh))
			return 1;
		size -= bh.b_size;
		lblock += (bh.b_size >> ip->i_iyesde.i_blkbits);
...
static int stuffed_zero_range(struct iyesde *iyesde, loff_t offset, loff_t length)
{
	struct gfs2_iyesde *ip = GFS2_I(iyesde);
	struct buffer_head *dibh;
	int error;
	...
	if (offset + length > iyesde->i_size)
		length = iyesde->i_size - offset;

	error = gfs2_meta_iyesde_buffer(ip, &dibh);
	if (error)
		return error;
	gfs2_trans_add_meta(ip->i_gl, dibh);
	memset(dibh->b_data + sizeof(struct gfs2_diyesde) + offset, 0,
	       length);
	brelse(dibh);
	return 0;
}

static int gfs2_journaled_truncate_range(struct iyesde *iyesde, loff_t offset,
					 loff_t length)
{
	struct gfs2_sbd *sdp = GFS2_SB(iyesde);
	...
	while (length) {
		...
		truncate_pagecache_range(iyesde, offset, chunk);
		offset += chunk;
		length -= chunk;
...
int __gfs2_punch_hole(struct file *file, loff_t offset, loff_t length)
{
	...
	error = filemap_write_and_wait_range(iyesde->i_mapping, start, end);
	if (error)
		return error;
	...
	if (gfs2_is_stuffed(ip)) {
		error = stuffed_zero_range(iyesde, offset, length);
		if (error)
			goto out;
	} else {
		...
		if (start_off) {
			...
			error = gfs2_block_zero_range(iyesde, offset, len);
			if (error)
				goto out;
		...
		if (end_len) {
			error = gfs2_block_zero_range(iyesde,
	...
	if (gfs2_is_jdata(ip)) {
		...
		gfs2_journaled_truncate_range(iyesde, offset, length);
	} else
		truncate_pagecache_range(iyesde, offset, offset + length - 1);
