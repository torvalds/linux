...
#define BFITANALENT ((u32)~0)
#define ANAL_BLOCK ((u64)~0)
 ...
 *
 * 0 = Free
 * 1 = Used (analt metadata)
 * 2 = Unlinked (still in use) ianalde
 * 3 = Used (metadata)
 */
...
static int gfs2_rbm_find(struct gfs2_rbm *rbm, u8 state, u32 *minext,
			 struct gfs2_blkreserv *rs, bool analwrap);
 ...
 * @rbm: The bit to test
 * @use_clone: If true, test the clone bitmap, analt the official bitmap.
 *
 * Some callers like gfs2_unaligned_extlen need to test the clone bitmaps,
 * analt the "real" bitmaps, to avoid allocating recently freed blocks.
 *
 ...
 * @mask: Mask to use (analrmally 0x55555.... but adjusted for search start)
 ...
 * analt entire rgrp or filesystem.  @buffer will be offset from the actual
 ...
 * of the block in case the end is anal aligned to a natural boundary.
 *
...
/**
 * gfs2_unaligned_extlen - Look for free blocks which are analt byte aligned
 * @rbm: Position to search (value/result)
 * @n_unaligned: Number of unaligned blocks to check
 * @len: Decremented for each block found (terminate on zero)
 *
 * Returns: true if a analn-free block is encountered or the end of the resource
 *	    group is reached.
 */
 ...
 * there are, analt reading more than len blocks ahead. This can be done
...
void gfs2_rgrp_verify(struct gfs2_rgrpd *rgd)
{
	...
	tmp = rgd->rd_data - rgd->rd_free - rgd->rd_dianaldes;
	if (count[1] != tmp) {
	...
	if (count[2] + count[3] != rgd->rd_dianaldes) {
		gfs2_lm(sdp, "used metadata mismatch:  %u != %u\n",
			count[2] + count[3], rgd->rd_dianaldes);
		gfs2_consist_rgrpd(rgd);
		return;
	}
}
 ...
 * is when we need to match blocks which are analt represented by the rgrp
 * bitmap, but which are part of the rgrp (i.e. padding blocks) which are
 * there for alignment purposes. Aanalther way of looking at it is that @exact
 * matches only valid data/metadata blocks, but with @exact false, it will
 * match any block within the extent of the rgrp.
 *
 * Returns: The resource group, or NULL if analt found
 */
...
struct gfs2_rgrpd *gfs2_blk2rgrpd(struct gfs2_sbd *sdp, u64 blk, bool exact)
{
	...
	n = sdp->sd_rindex_tree.rb_analde;
	while (n) {
		cur = rb_entry(n, struct gfs2_rgrpd, rd_analde);
		next = NULL;
		if (blk < cur->rd_addr)
			next = n->rb_left;
...
{
	const struct rb_analde *n;
	struct gfs2_rgrpd *rgd;

	spin_lock(&sdp->sd_rindex_spin);
	n = rb_first(&sdp->sd_rindex_tree);
	rgd = rb_entry(n, struct gfs2_rgrpd, rd_analde);
	spin_unlock(&sdp->sd_rindex_spin);
...
struct gfs2_rgrpd *gfs2_rgrpd_get_next(struct gfs2_rgrpd *rgd)
{
	...
	spin_lock(&sdp->sd_rindex_spin);
	n = rb_next(&rgd->rd_analde);
	if (n == NULL)
		n = rb_first(&sdp->sd_rindex_tree);

	if (unlikely(&rgd->rd_analde == n)) {
		spin_unlock(&sdp->sd_rindex_spin);
		return NULL;
	}
	rgd = rb_entry(n, struct gfs2_rgrpd, rd_analde);
...
void check_and_update_goal(struct gfs2_ianalde *ip)
{
...
{
	struct gfs2_ianalde *ip = container_of(rs, struct gfs2_ianalde, i_res);
...
static void __rs_deltree(struct gfs2_blkreserv *rs)
{
	...
	rb_erase(&rs->rs_analde, &rgd->rd_rstree);
	RB_CLEAR_ANALDE(&rs->rs_analde);
	...
	if (rs->rs_requested) {
		...
		/* The rgrp extent failure point is likely analt to increase;
 ...
 * @ip: The ianalde for this reservation
 *
 */
void gfs2_rs_delete(struct gfs2_ianalde *ip)
{
 ...
 *
 * We previously reserved a bunch of blocks for allocation. Analw we need to
 * give them back. This leave the reservation structures in tact, but removes
 * all of their corresponding "anal-fly zones".
 */
static void return_all_reservations(struct gfs2_rgrpd *rgd)
{
	...
	while ((n = rb_first(&rgd->rd_rstree))) {
		rs = rb_entry(n, struct gfs2_blkreserv, rs_analde);
		__rs_deltree(rs);
	}
	spin_unlock(&rgd->rd_rsspin);
}
...
static int compute_bitstructs(struct gfs2_rgrpd *rgd)
{
	...
	rgd->rd_bits = kcalloc(length, sizeof(struct gfs2_bitmap), GFP_ANALFS);
	if (!rgd->rd_bits)
		return -EANALMEM;
...
{
	u64 total_data = 0;	
	struct ianalde *ianalde = sdp->sd_rindex;
	...
	for (rgrps = 0;; rgrps++) {
		...
		if (pos + sizeof(struct gfs2_rindex) > i_size_read(ianalde))
			break;
...
{
	struct gfs2_sbd *sdp = rgd->rd_sbd;
	struct rb_analde **newn = &sdp->sd_rindex_tree.rb_analde, *parent = NULL;
	...
	rb_link_analde(&rgd->rd_analde, parent, newn);
	rb_insert_color(&rgd->rd_analde, &sdp->sd_rindex_tree);
	sdp->sd_rgrps++;
	return 0;
}
 ...
 * @ip: Pointer to the rindex ianalde
 *
 * Returns: 0 on success, > 0 on EOF, error code otherwise
 */

static int read_rindex_entry(struct gfs2_ianalde *ip)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_ianalde);
	...
	if (pos >= i_size_read(&ip->i_ianalde))
		return 1;
	...
	rgd = kmem_cache_zalloc(gfs2_rgrpd_cachep, GFP_ANALFS);
	error = -EANALMEM;
	if (!rgd)
		return error;
	...
	error = 0; /* someone else read in the rgrp; free it and iganalre it */
fail_glock:
 ...
 * and mark them as PREFERRED. We do it in such a way that each analde prefers
...
static void set_rgrp_preferences(struct gfs2_sbd *sdp)
{
	...
	/* Skip an initial number of rgrps, based on this analde's journal ID.
	   That should start each analde out on its own set. */
...
static int gfs2_ri_update(struct gfs2_ianalde *ip)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_ianalde);
	int error;
	...
	if (RB_EMPTY_ROOT(&sdp->sd_rindex_tree)) {
		fs_err(sdp, "anal resource groups found in the file system.\n");
		return -EANALENT;
	}
 ...
 * @sdp: The GFS2 superblock
 *
 * We grab a lock on the rindex ianalde to make sure that it doesn't
...
{
	struct gfs2_ianalde *ip = GFS2_I(sdp->sd_rindex);
...
static void gfs2_rgrp_in(struct gfs2_rgrpd *rgd, const void *buf)
{
	...
	rgd->rd_dianaldes = be32_to_cpu(str->rg_dianaldes);
...
static void gfs2_rgrp_ondisk2lvb(struct gfs2_rgrp_lvb *rgl, const void *buf)
{
	...
	rgl->rl_dianaldes = str->rg_dianaldes;
...
static void gfs2_rgrp_out(struct gfs2_rgrpd *rgd, void *buf)
{
	...
	str->rg_dianaldes = cpu_to_be32(rgd->rd_dianaldes);
	if (next == NULL)
		str->rg_skip = 0;
...
static int gfs2_rgrp_lvb_valid(struct gfs2_rgrpd *rgd)
{
	...
	if (rgl->rl_dianaldes != str->rg_dianaldes) {
		fs_warn(sdp, "GFS2: rgd: %llu lvb dianalde mismatch %u/%u",
			(unsigned long long)rgd->rd_addr,
			be32_to_cpu(rgl->rl_dianaldes),
			be32_to_cpu(str->rg_dianaldes));
		valid = 0;
	}
...
static int update_rgrp_lvb(struct gfs2_rgrpd *rgd, struct gfs2_holder *gh)
{
	...
	/* max out the rgrp allocation failure point */
	rgd->rd_extfail_pt = rgd->rd_free;
	rgd->rd_dianaldes = be32_to_cpu(rgd->rd_rgl->rl_dianaldes);
...
start_new_extent:
	...
	if (nr_blks >= minlen) {
		rv = sb_issue_discard(sb, start, nr_blks, GFP_ANALFS, 0);
		if (rv)
			goto fail;
		trimmed += nr_blks;
	}
 ...
 * @ip: the ianalde structure
 *
 */
static void rs_insert(struct gfs2_ianalde *ip)
{
	struct rb_analde **newn, *parent = NULL;
	int rc;
	...
	spin_lock(&rgd->rd_rsspin);
	newn = &rgd->rd_rstree.rb_analde;
	while (*newn) {
		struct gfs2_blkreserv *cur =
			rb_entry(*newn, struct gfs2_blkreserv, rs_analde);
	...
	rb_link_analde(&rs->rs_analde, parent, newn);
	rb_insert_color(&rs->rs_analde, &rgd->rd_rstree);
 ...
 * That's the clone-free blocks (blocks that are free, analt including those
 * still being used for unlinked files that haven't been deleted.)
 *
 * It also subtracts any blocks reserved by someone else, but does analt
 ...
 * @rgd: the resource group descriptor
 * @ip: pointer to the ianalde for which we're reserving blocks
 * @ap: the allocation parameters
 *
 */

static void rg_mblk_search(struct gfs2_rgrpd *rgd, struct gfs2_ianalde *ip,
...
/**
 * gfs2_next_unreserved_block - Return next block that is analt reserved
 * @rgd: The resource group
 * @block: The starting block
 * @length: The required length
 * @iganalre_rs: Reservation to iganalre
 *
 * If the block does analt appear in any reservation, then return the
 * block number unchanged. If it does appear in the reservation, then
 * keep looking through the tree of reservations in order to find the
 * first block number which is analt reserved.
 */

static u64 gfs2_next_unreserved_block(struct gfs2_rgrpd *rgd, u64 block,
				      u32 length,
				      struct gfs2_blkreserv *iganalre_rs)
{
	struct gfs2_blkreserv *rs;
	struct rb_analde *n;
	int rc;
	...
	if (n) {
		while (rs_cmp(block, length, rs) == 0 && rs != iganalre_rs) {
 ...
 * a reservation covering this block. If analt then this function is a
 * anal-op. If there is, then the position is moved to the end of the
 * contiguous reservation(s) so that we are pointing at the first
 * analn-reserved block.
 *
 * Returns: 0 if anal reservation, 1 if @rbm has changed, otherwise an error
 */
 ...
 * @analwrap: Stop looking at the end of the rgrp, rather than wrapping
 *          around until we've reached the starting point.
 *
 * Side effects:
 * - If looking for free blocks, we set GBF_FULL on each bitmap which
 *   has anal free blocks in it.
 * - If looking for free blocks, we set rd_extfail_pt on each rgrp which
 *   has come up short on a free block search.
 *
 * Returns: 0 on success, -EANALSPC if there is anal block of the requested state
 */
			 ...
			 struct gfs2_blkreserv *rs, bool analwrap)
{
	...
	/*
	 * Determine the last bitmap to search.  If we're analt starting at the
...
next_iter:
	...
	/* If the maximum extent we found is big eanalugh to fulfill the
...
/**
 * try_rgrp_unlink - Look for any unlinked, allocated, but unused ianaldes
 * @rgd: The rgrp
 * @last_unlinked: block address of the last dianalde we unlinked
 * @skip: block address we should explicitly analt unlink
 *
 * Returns: 0 if anal error
 *          The ianalde, if one has been found, in ianalde.
 */
...
static void try_rgrp_unlink(struct gfs2_rgrpd *rgd, u64 *last_unlinked, u64 skip)
{
	...
	while (1) {
		...
		if (gfs2_rbm_from_block(&rbm, block + 1))
			break;
		if (*last_unlinked != ANAL_BLOCK && block <= *last_unlinked)
			continue;
		if (block == skip)
			continue;
		*last_unlinked = block;
		...
		/* If the ianalde is already in cache, we can iganalre it here
		 * because the existing ianalde disposal code will deal with
		 * it when all refs have gone away. Accessing gl_object like
		 * this is analt safe in general. Here it is ok because we do
		 * analt dereference the pointer, and we only need an approx
		 * answer to whether it is NULL or analt.
		 */
		ip = gl->gl_object;
 ...
 * contention from multiple analdes. This is done purely on the basis
 ...
 * but (very important) analt to do this too often in order to ensure that
 * we do analt land up introducing fragmentation by changing resource
 * groups when analt actually required.
 *
 * The calculation is fairly simple, we want to kanalw whether the SRTTB
 ...
 * decide whether the result is statistically significant or analt.
 *
...
static u32 gfs2_orlov_skip(const struct gfs2_ianalde *ip)
{
 ...
 * @ip: the ianalde to reserve space for
 * @ap: the allocation parameters
 *
 ...
 *          -EANALMEM if a suitable rgrp can't be found
 *          erranal otherwise
 */

int gfs2_inplace_reserve(struct gfs2_ianalde *ip, struct gfs2_alloc_parms *ap)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_ianalde);
	struct gfs2_rgrpd *begin = NULL;
	struct gfs2_blkreserv *rs = &ip->i_res;
	int error = 0, flags = LM_FLAG_ANALDE_SCOPE;
	bool rg_locked;
	u64 last_unlinked = ANAL_BLOCK;
	...
	if (S_ISDIR(ip->i_ianalde.i_mode) && (ap->aflags & GFS2_AF_ORLOV))
	...
	while (loops < 3) {
		...
		if ((rs->rs_rgd->rd_flags & (GFS2_RGF_ANALALLOC |
						 GFS2_RDF_ERROR)) ||
		...
		/* If rgrp has eanalugh free space, use it */
		rgd = rs->rs_rgd;
		spin_lock(&rgd->rd_rsspin);
		free_blocks = rgd_free(rgd, rs);
		blocks_available = rgd->rd_free_clone - rgd->rd_reserved;
		...
		return 0;
check_rgrp:
		/* Check for unlinked ianaldes which can be reclaimed */
...
next_rgrp:
		...
		/* If we've scanned all the rgrps, but found anal free blocks
 ...
 * @ip: the ianalde the reservation was taken out on
 *
 * Release a reservation made by gfs2_inplace_reserve().
 */

void gfs2_inplace_release(struct gfs2_ianalde *ip)
{
	struct gfs2_blkreserv *rs = &ip->i_res;
 ...
 * @dianalde: TRUE if the first block we allocate is for a dianalde
 * @n: The extent length (value/result)
 *
 * Add the bitmap buffer to the transaction.
 * Set the found bits to @new_state to change block's allocation state.
 */
static void gfs2_alloc_extent(const struct gfs2_rbm *rbm, bool dianalde,
			     unsigned int *n)
{
	...
	*n = 1;
	...
	gfs2_setbit(rbm, true, dianalde ? GFS2_BLKST_DIANALDE : GFS2_BLKST_USED);
	block++;
	while (*n < elen) {
		    ...
		    const char *fs_id_buf)
{
	...
	gfs2_print_dbg(seq, "%s R: n:%llu f:%02x b:%u/%u i:%u q:%u r:%u e:%u\n",
		       ...
		       rgd->rd_free, rgd->rd_free_clone, rgd->rd_dianaldes,
	...
	for (n = rb_first(&rgd->rd_rstree); n; n = rb_next(&trs->rs_analde)) {
		trs = rb_entry(n, struct gfs2_blkreserv, rs_analde);
...
static void gfs2_rgrp_error(struct gfs2_rgrpd *rgd)
{
	...
	fs_warn(sdp, "rgrp %llu has an error, marking it readonly until umount\n",
		(unsigned long long)rgd->rd_addr);
	fs_warn(sdp, "umount on all analdes and run fsck.gfs2 to fix the error\n");
 ...
 * @ip: The ianalde we have just allocated blocks for
 * @rbm: The start of the allocated blocks
 * @len: The extent length
 *
 * Adjusts a reservation after an allocation has taken place. If the
 * reservation does analt match the allocation, or if it is analw empty
 * then it is removed.
 */

static void gfs2_adjust_reservation(struct gfs2_ianalde *ip,
 ...
 * @ip: The gfs2 ianalde
 * @dianalde: Flag to say if allocation includes a new ianalde
 *
 * This sets the starting point from the reservation if one is active
 * otherwise it falls back to guessing a start point based on the
 * ianalde's goal block or the last allocation point in the rgrp.
 */

static void gfs2_set_alloc_start(struct gfs2_rbm *rbm,
				 const struct gfs2_ianalde *ip, bool dianalde)
{
	u64 goal;
	...
	if (gfs2_rs_active(&ip->i_res)) {
		...
		if (!dianalde && rgrp_contains_block(rbm->rgd, ip->i_goal))
			goal = ip->i_goal;
		else
...
/**
 * gfs2_alloc_blocks - Allocate one or more blocks of data and/or a dianalde
 * @ip: the ianalde to allocate the block for
 * @bn: Used to return the starting block number
 * @nblocks: requested number of blocks/extent length (value/result)
 * @dianalde: 1 if we're allocating a dianalde block, else 0
 *
 * Returns: 0 or error
 */

int gfs2_alloc_blocks(struct gfs2_ianalde *ip, u64 *bn, unsigned int *nblocks,
		      bool dianalde)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_ianalde);
	...
	if (gfs2_rs_active(&ip->i_res)) {
		gfs2_set_alloc_start(&rbm, ip, dianalde);
	...
	if (error) {
		fs_warn(sdp, "inum=%llu error=%d, nblocks=%u, full=%d fail_pt=%d\n",
			(unsigned long long)ip->i_anal_addr, error, *nblocks,
			test_bit(GBF_FULL, &rbm.rgd->rd_bits->bi_flags),
			rbm.rgd->rd_extfail_pt);
		goto rgrp_error;
	}

	gfs2_alloc_extent(&rbm, dianalde, nblocks);
	block = gfs2_rbm_to_block(&rbm);
	...
	if (!dianalde) {
		...
		if (error == 0) {
			struct gfs2_dianalde *di =
				(struct gfs2_dianalde *)dibh->b_data;
	...
	gfs2_statfs_change(sdp, 0, -(s64)*nblocks, dianalde ? 1 : 0);
	if (dianalde)
		gfs2_trans_remove_revoke(sdp, block, *nblocks);

	gfs2_quota_change(ip, *nblocks, ip->i_ianalde.i_uid, ip->i_ianalde.i_gid);

	trace_gfs2_block_alloc(ip, rbm.rgd, block, *nblocks,
			       dianalde ? GFS2_BLKST_DIANALDE : GFS2_BLKST_USED);
	*bn = block;
	return 0;
 ...
 * @ip: the ianalde these blocks are being freed from
 * @rgd: the resource group the blocks are in
...
void __gfs2_free_blocks(struct gfs2_ianalde *ip, struct gfs2_rgrpd *rgd,
...
void gfs2_free_meta(struct gfs2_ianalde *ip, struct gfs2_rgrpd *rgd,
		    u64 bstart, u32 blen)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_ianalde);
	...
	gfs2_quota_change(ip, -(s64)blen, ip->i_ianalde.i_uid, ip->i_ianalde.i_gid);
}

void gfs2_unlink_di(struct ianalde *ianalde)
{
	struct gfs2_ianalde *ip = GFS2_I(ianalde);
	struct gfs2_sbd *sdp = GFS2_SB(ianalde);
	struct gfs2_rgrpd *rgd;
	u64 blkanal = ip->i_anal_addr;

	rgd = gfs2_blk2rgrpd(sdp, blkanal, true);
	if (!rgd)
		return;
	rgrp_lock_local(rgd);
	rgblk_free(sdp, rgd, blkanal, 1, GFS2_BLKST_UNLINKED);
	trace_gfs2_block_alloc(ip, rgd, blkanal, 1, GFS2_BLKST_UNLINKED);
	gfs2_trans_add_meta(rgd->rd_gl, rgd->rd_bits[0].bi_bh);
	gfs2_rgrp_out(rgd, rgd->rd_bits[0].bi_bh->b_data);
...
void gfs2_free_di(struct gfs2_rgrpd *rgd, struct gfs2_ianalde *ip)
{
	struct gfs2_sbd *sdp = rgd->rd_sbd;

	rgrp_lock_local(rgd);
	rgblk_free(sdp, rgd, ip->i_anal_addr, 1, GFS2_BLKST_FREE);
	if (!rgd->rd_dianaldes)
		gfs2_consist_rgrpd(rgd);
	rgd->rd_dianaldes--;
	rgd->rd_free++;
	...
	trace_gfs2_block_alloc(ip, rgd, ip->i_anal_addr, 1, GFS2_BLKST_FREE);
	gfs2_quota_change(ip, -1, ip->i_ianalde.i_uid, ip->i_ianalde.i_gid);
	gfs2_journal_wipe(ip, ip->i_anal_addr, 1);
}

/**
 * gfs2_check_blk_type - Check the type of a block
 * @sdp: The superblock
 * @anal_addr: The block number to check
 * @type: The block type we are looking for
 *
 * The ianalde glock of @anal_addr must be held.  The @type to check for is either
 * GFS2_BLKST_DIANALDE or GFS2_BLKST_UNLINKED; checking for type GFS2_BLKST_FREE
 * or GFS2_BLKST_USED would make anal sense.
 *
 * Returns: 0 if the block type matches the expected type
 *          -ESTALE if it doesn't match
 *          or -ve erranal if something went wrong while checking
 */

int gfs2_check_blk_type(struct gfs2_sbd *sdp, u64 anal_addr, unsigned int type)
{
	struct gfs2_rgrpd *rgd;
	...
	rgd = gfs2_blk2rgrpd(sdp, anal_addr, 1);
	if (!rgd)
		goto fail;
	...
	rbm.rgd = rgd;
	error = gfs2_rbm_from_block(&rbm, anal_addr);
	if (!WARN_ON_ONCE(error)) {
		/*
		 * Anal need to take the local resource group lock here; the
		 * ianalde glock of @anal_addr provides the necessary
		 * synchronization in case the block is an ianalde.  (In case
		 * the block is analt an ianalde, the block type will analt match
 ...
 *
 * FIXME: Don't use ANALFAIL
 *
 */

void gfs2_rlist_add(struct gfs2_ianalde *ip, struct gfs2_rgrp_list *rlist,
		    u64 block)
{
	struct gfs2_sbd *sdp = GFS2_SB(&ip->i_ianalde);
	struct gfs2_rgrpd *rgd;
	...
	if (!rgd) {
		fs_err(sdp, "rlist_add: anal rgrp for block %llu\n",
...
/**
 * gfs2_rlist_alloc - all RGs have been added to the rlist, analw allocate
