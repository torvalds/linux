...
static int ea_check_size(struct gfs2_sbd *sdp, unsigned int nsize,
    size_t dsize) {
  unsigned int size;
...
static int ea_set_simple_noalloc(struct gfs2_inode *ip, struct buffer_head *bh,
  ...
  int error;
  error
    = gfs2_trans_begin(GFS2_SB(&ip->i_inode), RES_DINODE + 2 * RES_EATTR, 0);
  if (error) {
    return error;
  }
  gfs2_trans_add_meta(ip->i_gl, bh);
...
static int ea_remove_stuffed(struct gfs2_inode *ip,
    struct gfs2_ea_location *el) {
  struct gfs2_ea_header *ea = el->el_ea;
  struct gfs2_ea_header *prev = el->el_prev;
  int error;
...
static int gfs2_xattr_remove(struct gfs2_inode *ip, int type,
    const char *name) {
  struct gfs2_ea_location el;
  int error;
  if (!ip->i_eattr) {
...
static bool gfs2_xattr_trusted_list(struct dentry *dentry) {
