...
static ssize_t withdraw_store(struct gfs2_sbd *sdp, const char *buf,
    size_t len) {
  int error, val;
  if (!capable(CAP_SYS_ADMIN)) {
    return -EPERM;
...
static ssize_t demote_rq_store(struct gfs2_sbd *sdp, const char *buf,
    size_t len) {
...
static ssize_t lkfirst_store(struct gfs2_sbd *sdp, const char *buf,
    size_t len) {
  unsigned first;
  int rv;
...
static ssize_t recover_store(struct gfs2_sbd *sdp, const char *buf,
    size_t len) {
  unsigned jid;
  int rv;
  rv = sscanf(buf, "%u", &jid);
...
#define GDLM_ATTR(_name, _mode, _show, _store) \
  static struct gfs2_attr gdlm_attr_ ## _name = __ATTR(_name, _mode, _show, \
    _store)
...
#define TUNE_ATTR(name, check_zero)                                           \
  static ssize_t name ## _store(struct gfs2_sbd *sdp, const char *buf, \
    size_t len) \
  {                                                                             \
...
static int gfs2_uevent(const struct kobject *kobj,
