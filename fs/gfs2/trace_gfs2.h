...
TRACE_EVENT(gfs2_ail_flush,
    ...
    TP_PROTO(const struct gfs2_sbd *sdp, const struct writeback_control *wbc,
    int start),
...
TRACE_EVENT(gfs2_iomap_end,
    ...
    TP_printk(
    "%u,%u bmap %llu iomap end %llu/%lu to %llu ty:%d flags:%08x rc:%d",
    MAJOR(__entry->dev), MINOR(__entry->dev),
    (unsigned long long) __entry->inum,
...
TRACE_EVENT(gfs2_rs,
    ...
    TP_printk(
    "%u,%u bmap %llu resrv %llu rg:%llu rf:%u rq:%u rr:%u %s q:%u r:%u",
    MAJOR(__entry->dev), MINOR(__entry->dev),
    (unsigned long long) __entry->inum,
    (unsigned long long) __entry->start,
    (unsigned long long) __entry->rd_addr,
