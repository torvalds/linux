...
static void gfs2_sb_in(struct gfs2_sbd *sdp, const void *buf) {
  ...
  sb->sb_master_dir.no_formal_ino
    = be64_to_cpu(str->sb_master_dir.no_formal_ino);
...
static int init_sb(struct gfs2_sbd *sdp, int silent) {
  ...
  sb->s_root
    = dget(sdp->sd_args.ar_meta ? sdp->sd_master_dir : sdp->sd_root_dir);
out:
...
/**
 * init_statfs - look up and initialize master and local (per node) statfs
 *inodes
 * @sdp: The GFS2 superblock
 *
