...
static void gfs2_qd_dealloc(struct rcu_head *rcu) {
  struct gfs2_quota_data *qd
    = container_of(rcu, struct gfs2_quota_data, qd_rcu);
...
static struct gfs2_quota_data *qd_alloc(unsigned hash, struct gfs2_sbd *sdp,
    struct kqid qid) {
  struct gfs2_quota_data *qd;
  int error;
...
static int gfs2_adjust_quota(struct gfs2_sbd *sdp, loff_t loc,
  ...
  if (fdq) {
    if (fdq->d_fieldmask & QC_SPC_SOFT) {
      q.qu_warn
        = cpu_to_be64(fdq->d_spc_softlimit >> sdp->sd_sb.sb_bsize_shift);
      qd->qd_qb.qb_warn = q.qu_warn;
    }
    if (fdq->d_fieldmask & QC_SPC_HARD) {
      q.qu_limit
        = cpu_to_be64(fdq->d_spc_hardlimit >> sdp->sd_sb.sb_bsize_shift);
...
int gfs2_quota_init(struct gfs2_sbd *sdp) {
  ...
  for (x = 0; x < blocks; x++) {
    ...
    qc
      = (const struct gfs2_quota_change *) (bh->b_data
...
static int gfs2_quota_get_state(struct super_block *sb,
...
static int gfs2_set_dqblk(struct super_block *sb, struct kqid qid,
  ...
  offset = qd2offset(qd);
  alloc_required
    = gfs2_write_alloc_required(ip, offset, sizeof(struct gfs2_quota));
  if (gfs2_is_stuffed(ip)) {
    alloc_required = 1;
  }
  if (alloc_required) {
