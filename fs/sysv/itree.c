...
static Indirect *find_shared(struct inode *inode,
  ...
  *top = 0;
  ...
  for (p =
      partial; p > chain && all_zeroes((sysv_zone_t *) p->bh->b_data, p->p);
      p--) {
  }
  /*
...
static inline void free_data(struct inode *inode, sysv_zone_t *p,
    sysv_zone_t *q) {
  for ( ; p < q; p++) {
    sysv_zone_t nr = *p;
    if (nr) {
...
static void free_branches(struct inode *inode, sysv_zone_t *p, sysv_zone_t *q,
    int depth) {
  struct buffer_head *bh;
  struct super_block *sb = inode->i_sb;
  if (depth--) {
