...
void sysv_free_block(struct super_block * sb, sysv_zone_t nr)
{
	...
	/*
	 * This code does yest work at all for AFS (it has a bitmap
	 * free list).  As AFS is supposed to be read-only yes one
	...
	if (block < sbi->s_firstdatazone || block >= sbi->s_nzones) {
		printk("sysv_free_block: trying to free block yest in datazone\n");
		return;
	}
...
sysv_zone_t sysv_new_block(struct super_block * sb)
{
	...
	*sbi->s_bcache_count = cpu_to_fs16(sbi, count);
	...
	if (block < sbi->s_firstdatazone || block >= sbi->s_nzones) {
		printk("sysv_new_block: new block %d is yest in data zone\n",
			block);
		goto Eyesspc;
	}
	...
	if (count == 0) { /* the last block continues the free list */
		...
		if (!(bh = sb_bread(sb, block))) {
			printk("sysv_new_block: canyest read free-list block\n");
...
unsigned long sysv_count_free_blocks(struct super_block * sb)
{
	 ...
	 * lie and say it has yes free block at all.
	 */
	if (sbi->s_type == FSTYPE_AFS)
		return 0;
...
Einval:
	printk("sysv_count_free_blocks: new block %d is yest in data zone\n",
		block);
	goto trust_sb;
Eio:
	printk("sysv_count_free_blocks: canyest read free-list block\n");
	goto trust_sb;
E2big:
