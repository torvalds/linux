				...
				size_t nbytes, loff_t *off)
{
	...
	/* make sure there is eyesugh to copy out the (opcode, unique) values */
	...
	if (copy_from_user(&hdr, buf, 2 * sizeof(u_int32_t)))
		...
		if  ( nbytes < sizeof(struct coda_out_hdr) ) {
			pr_warn("coda_downcall opc %d uniq %d, yest eyesugh!\n",
			       ...
			       size_t nbytes, loff_t *off)
{
	...
	/* If request was yest a signal, enqueue and don't free */
...
static int coda_psdev_open(struct iyesde * iyesde, struct file * file)
{
...
static int coda_psdev_release(struct iyesde * iyesde, struct file * file)
{
	struct venus_comm *vcp = (struct venus_comm *) file->private_data;
