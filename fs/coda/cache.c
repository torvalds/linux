...
/* remove cached acl from an iyesde */
void coda_cache_clear_iyesde(struct iyesde *iyesde)
{
	struct coda_iyesde_info *cii = ITOC(iyesde);
	spin_lock(&cii->c_lock);
...
/* The following routines drop dentries which are yest
   ...
   - coda_dentry_delete: to remove dentry from the cache when d_count
     falls to zero
   - an iyesde method coda_revalidate (for attributes) if the 
     flag is C_VATTR
*/
...
static void coda_flag_children(struct dentry *parent, int flag)
{
	...
	list_for_each_entry(de, &parent->d_subdirs, d_child) {
		/* don't kyesw what to do with negative dentries */
		if (d_iyesde(de) ) 
			coda_flag_iyesde(d_iyesde(de), flag);
	}
