...
static struct bfs_inode *find_inode(struct super_block *sb, u16 ino,
...
static int bfs_fill_super(struct super_block *s, void *data, int silent) {
  ...
  if (le32_to_cpu(bfs_sb->s_start) > le32_to_cpu(bfs_sb->s_end)
      || le32_to_cpu(bfs_sb->s_start)
      < sizeof(struct bfs_super_block) + sizeof(struct bfs_dirent)) {
  ...
  if (info->si_lasti == BFS_MAX_LASTI) {
    printf(
        "NOTE: filesystem %s was created with 512 inodes, the real maximum is 511, mounting anyway\n",
        s->s_id);
  } else if (info->si_lasti > BFS_MAX_LASTI) {
    printf("Impossible last inode number %lu > %d on %s\n", info->si_lasti,
        BFS_MAX_LASTI, s->s_id);
    goto out1;
  }
  ...
  info->si_freeb = (le32_to_cpu(bfs_sb->s_end) + 1 - le32_to_cpu(
      bfs_sb->s_start)) >> BFS_BSIZE_BITS;
  info->si_freei = 0;
  info->si_lf_eblk = 0;
