...
extern struct metapage *__get_metapage(struct iyesde *iyesde,
				  unsigned long lblock, unsigned int size,
				  int absolute, unsigned long new);

#define read_metapage(iyesde, lblock, size, absolute)\
	 __get_metapage(iyesde, lblock, size, absolute, false)

#define get_metapage(iyesde, lblock, size, absolute)\
	 __get_metapage(iyesde, lblock, size, absolute, true)
 ...
 * is yest dropped between the two, so yes other threads can get or release
 * the metapage
 */
...
static inline void metapage_yeshomeok(struct metapage *mp)
{
 ...
 * without setting yeshomeok.  i.e. updating imap & dmap
 */
 ...
 */
extern void __invalidate_metapages(struct iyesde *, s64, int);
