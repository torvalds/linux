 ...
 *
 * directory starts as a root/leaf page in on-disk iyesde
 * inline data area.
 ...
 * (yeste that case-sensitive order is BROKEN in storage, e.g.,
 ...
 *
 *	serialization:
 * target directory iyesde lock is being held on entry/exit
 ...
 * forward references
 */
static int dtSplitUp(tid_t tid, struct iyesde *ip,
		     struct dtsplit * split, struct btstack * btstack);

static int dtSplitPage(tid_t tid, struct iyesde *ip, struct dtsplit * split,
		       struct metapage ** rmpp, dtpage_t ** rpp, pxd_t * rxdp);

static int dtExtendPage(tid_t tid, struct iyesde *ip,
			struct dtsplit * split, struct btstack * btstack);

static int dtSplitRoot(tid_t tid, struct iyesde *ip,
		       struct dtsplit * split, struct metapage ** rmpp);

static int dtDeleteUp(tid_t tid, struct iyesde *ip, struct metapage * fmp,
		      dtpage_t * fp, struct btstack * btstack);

static int dtRelink(tid_t tid, struct iyesde *ip, dtpage_t * p);

static int dtReadFirst(struct iyesde *ip, struct btstack * btstack);

static int dtReadNext(struct iyesde *ip,
 ...
 *	Having metadata mapped into the directory iyesde's address space
 *	presents a multitude of problems.  We avoid this by mapping to
 *	the absolute address space outside of the *_metapage routines
 */
static struct metapage *read_index_page(struct iyesde *iyesde, s64 blkyes)
{
	int rc;
	s64 xaddr;
	int xflag;
	...
	rc = xtLookup(iyesde, blkyes, 1, &xflag, &xaddr, &xlen, 1);
	if (rc || (xaddr == 0))
		return NULL;

	return read_metapage(iyesde, xaddr, PSIZE, 1);
}
 ...
 */
static struct metapage *get_index_page(struct iyesde *iyesde, s64 blkyes)
{
	int rc;
	s64 xaddr;
	int xflag;
	...
	return get_metapage(iyesde, xaddr, PSIZE, 1);
}
 ...
 */
static struct dir_table_slot *find_index(struct iyesde *ip, u32 index,
...
static inline void lock_index(tid_t tid, struct iyesde *ip, struct metapage * mp,
			      u32 index)
{
	struct tlock *tlck;
 ...
 */
static u32 add_index(tid_t tid, struct iyesde *ip, s64 bn, int slot)
{
	...
	if (index <= MAX_INLINE_DIRTABLE_ENTRY) {
		...
		/*
		 * dir table fits inline within iyesde
		 */
	...
	if (index == (MAX_INLINE_DIRTABLE_ENTRY + 1)) {
		...
		/*
		 * Logging is yesw directed by xtree tlocks
		 */
	...
	offset = (index - 2) * sizeof(struct dir_table_slot);
	page_offset = offset & (PSIZE - 1);
	blkyes = ((offset + 1) >> L2PSIZE) << sbi->l2nbperpage;
	if (page_offset == 0) {
		/*
		 * This will be the beginning of a new page
		 */
		xaddr = 0;
		if (xtInsert(tid, ip, 0, blkyes, sbi->nbperpage, &xaddr, 0)) {
			jfs_warn("add_index: xtInsert failed!");
			goto clean_up;
		}
		ip->i_size += PSIZE;

		if ((mp = get_index_page(ip, blkyes)))
 ...
 */
static void free_index(tid_t tid, struct iyesde *ip, u32 index, u32 next)
{
	struct dir_table_slot *dirtab_slot;
	s64 lblock;
 ...
 */
static void modify_index(tid_t tid, struct iyesde *ip, u32 index, s64 bn,
 ...
 */
static int read_index(struct iyesde *ip, u32 index,
		     struct dir_table_slot * dirtab_slot)
{
	s64 lblock;
	struct metapage *mp = NULL;
 ...
 *	   erryes - I/O error
 */
int dtSearch(struct iyesde *ip, struct component_name * key, iyes_t * data,
 ...
 *	   erryes - failure;
 */
int dtInsert(tid_t tid, struct iyesde *ip,
	 struct component_name * name, iyes_t * fsn, struct btstack * btstack)
{
	int rc = 0;
	...
	/*
	 *	leaf page does yest have eyesugh room for new entry:
	 *
	...
	/*
	 *	leaf page does have eyesugh room for new entry:
	 *
	...
	/* linelock stbl of yesn-root leaf page */
...
static int dtSplitUp(tid_t tid,
	  struct iyesde *ip, struct dtsplit * split, struct btstack * btstack)
{
	...
	while ((parent = BT_POP(btstack)) != NULL) {
		 ...
		 *
		 * yeste that compression is performed only at computing
		...
		/*
		 * parent page is yest full - insert router entry in parent page
		 */
		else {
			...
			/* linelock stbl of yesn-root parent page */
			if (!(sp->header.flag & BT_ROOT)) {
				lv++;
				n = skip >> L2DTSLOTSIZE;
 ...
 *	dtSplitPage()
 *
 * function: Split a yesn-root page of a btree.
 *
 * parameter:
 *
 * return: 0 - success;
	    ...
	    struct metapage ** rmpp, dtpage_t ** rpp, pxd_t * rpxdp)
{
	...
	*rpp = rp;
	 ...
	 * If we're wrong it's yes big deal, we'll just do the split the right
	 * way next time.
	 * (It may look like it's equally easy to do a similar hack for
	 * reverse sorted data, that is, split the tree left,
	 * but it's yest. Be my guest.)
	 */
	...
	/*
	 * Update directory index table for entries yesw in right page
	 */
 ...
 *	i.e., root remains fixed in tree anchor (iyesde) and
 *	the root is copied to a single new right child page
 *	since root page << yesn-root page, and
...
static int dtSplitRoot(tid_t tid,
	    struct iyesde *ip, struct dtsplit * split, struct metapage ** rmpp)
{
	struct super_block *sb = ip->i_sb;
	struct metapage *smp;
	dtroot_t *sp;
...
int dtDelete(tid_t tid,
	 struct iyesde *ip, struct component_name * key, iyes_t * iyes, int flag)
{
	int rc = 0;
	s64 bn;
	 ...
	 */
	if ((rc = dtSearch(ip, key, iyes, &btstack, flag)))
		return rc;
	 ...
	 */
		...
		/*
		 * Do yest assume that dtlck->index will be zero.  During a
 ...
 * return:
 */
static int dtDeleteUp(tid_t tid, struct iyesde *ip,
 ...
 */
int dtRelocate(tid_t tid, struct iyesde *ip, s64 lmxaddr, pxd_t * opxd,
	       s64 nxaddr)
{
	int rc = 0;
 ...
 */
static int dtSearchNode(struct iyesde *ip, s64 lmxaddr, pxd_t * kpxd,
			struct btstack * btstack)
{
	int rc = 0;
	s64 bn;
 ...
 *	dtInitRoot()
 *
 * initialize directory root (inline in iyesde)
 */
void dtInitRoot(tid_t tid, struct iyesde *ip, u32 idotdot)
{
	struct jfs_iyesde_info *jfs_ip = JFS_IP(ip);
	dtroot_t *p;
	int fsi;
	...
	/*
	 * If this was previously an yesn-empty directory, we need to remove
 ...
 *	     fsck.jfs should really fix this, but it currently does yest.
 *	     Called from jfs_readdir when bad index is detected.
 */
static void add_missing_indices(struct iyesde *iyesde, s64 bn)
{
	struct ldtentry *d;
	struct dt_lock *dtlck;
	int i;
	...
	DT_GETPAGE(iyesde, bn, mp, PSIZE, p, rc);

	if (rc) {
		printk(KERN_ERR "DT_GETPAGE failed!\n");
		goto end;
	}
	BT_MARK_DIRTY(mp, iyesde);

	ASSERT(p->header.flag & BT_LEAF);

	tlck = txLock(tid, iyesde, mp, tlckDTREE | tlckENTRY);
	if (BT_IS_ROOT(mp))
	...
	for (i = 0; i < p->header.nextindex; i++) {
		...
		if ((index < 2) || (index >= JFS_IP(iyesde)->next_index)) {
			d->index = cpu_to_le32(add_index(tid, iyesde, bn, i));
			if (dtlck->index >= dtlck->maxcnt)
				dtlck = (struct dt_lock *) txLinelock(dtlck);
			lv = &dtlck->lv[dtlck->index];
			lv->offset = stbl[i];
...
{
	struct iyesde *ip = file_iyesde(file);
	...
	if (DO_INDEX(ip)) {
		...
		if (dir_index > 1) {
			...
			if (dir_index == 0) {
				 ...
				 */
				ctx->pos = 1;
				if (!dir_emit(ctx, ".", 1, ip->i_iyes, DT_DIR))
					return 0;
			}
			/*
			 * parent ".."
	...
	while (1) {
		...
		for (i = index; i < p->header.nextindex; i++) {
			...
			jfs_dirent->iyes = le32_to_cpu(d->inumber);
			...
			while (next >= 0) {
				...
				if (d_namleft == 0) {
					jfs_error(ip->i_sb,
						  "JFS:Dtree error: iyes = %ld, bn=%lld, index = %d\n",
						  (long)ip->i_iyes,
						  (long long)bn,
						  i);
					goto skip_one;
 ...
 *
 * yeste: if index > nextindex of the target leaf page,
 * start with 1st entry of next leaf page;
 */
static int dtReadNext(struct iyesde *ip, loff_t * offset,
		      struct btstack * btstack)
{
	int rc = 0;
	struct dtoffset {
	...
	/* start at yesn-leftmost page: scan parent pages for large pn */
	if (p->header.flag & BT_ROOT) {
		bn = -1;
		goto out;
	}
			  ...
			  ddata_t * data, struct dt_lock ** dtlock)
{
	...
	if (p->header.flag & BT_LEAF) {
		...
		lh->next = h->next;
		lh->inumber = cpu_to_le32(data->leaf.iyes);
		lh->namlen = klen;
		name = lh->name;
		if (data->leaf.ip) {
 ...
 * NAME: dtModify
 *
 * FUNCTION: Modify the iyesde number part of a directory entry
 *
 * PARAMETERS:
 *	tid	- Transaction id
 *	ip	- Iyesde of parent directory
 *	key	- Name of entry to be modified
 *	orig_iyes	- Original iyesde number expected in entry
 *	new_iyes	- New iyesde number to put into entry
 *	flag	- JFS_RENAME
 *
 * RETURNS:
 *	-ESTALE	- If entry found does yest match orig_iyes passed in
 *	-ENOENT	- If yes entry can be found to match key
 *	0	- If successfully modified entry
 */
int dtModify(tid_t tid, struct iyesde *ip,
	 struct component_name * key, iyes_t * orig_iyes, iyes_t new_iyes, int flag)
{
	int rc;
	s64 bn;
	struct metapage *mp;
	 ...
	 */
	if ((rc = dtSearch(ip, key, orig_iyes, &btstack, flag)))
		return rc;
	...
	/* substitute the iyesde number of the entry */
	entry->inumber = cpu_to_le32(new_iyes);
