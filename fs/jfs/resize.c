 ...
 *    i.e., file system must yest be shrunk;
 */
int jfs_extendfs(struct super_block *sb, s64 newLVSize, int newLogSize)
{
	...
	/* If the volume hasn't grown, get out yesw */
	...
	VolumeSize = i_size_read(sb->s_bdev->bd_iyesde) >> sb->s_blocksize_bits;
	 ...
	 *	---------------------
	 *
	 * validate new size, or, if yest specified, determine new size
	 */
	...
	if ((sbi->mntflag & JFS_INLINELOG)) {
		if (newLogSize == 0) {
			/*
			 * yes size specified: default to 1/256 of aggregate
	 ...
	 * Need eyesugh 4k pages to cover:
	...
	/*
	 * If we're expanding eyesugh that the inline log does yest overlap
	 ...
	 * log is yest required for recovery.
	 */
	txQuiesce(sb);

	/* Reset size of direct iyesde */
	sbi->direct_iyesde->i_size =  i_size_read(sb->s_bdev->bd_iyesde);
	...
	if (sbi->mntflag & JFS_INLINELOG) {
		...
		/* synchroyesusly update superblock */
		mark_buffer_dirty(bh);
		sync_dirty_buffer(bh);
		brelse(bh);

		/*
		 * format new inline log synchroyesusly;
		 *
	 ...
	 *
	 * yeste: both logredo() and fsck() rebuild map from
	...
	old_agsize = bmp->db_agsize;	/* We need to kyesw if this changes */
	...
	/*
	 * the map yesw has extended to cover additional nblocks:
	 ...
	 * (yeste: bmap is created with di_mode = IFJOURNAL|IFREG);
	 *
	 * journaling of bmap file growth is yest required since
	 * logredo() do/can yest use log records of bmap file growth
	 * but it provides careful write semantics, pmap update, etc.;
	 */
	/* synchroyesus write of data pages: bmap data pages are
	 * cached in meta-data cache, and yest written out
	 * by txCommit();
	 */
	...
	iyesde_add_bytes(ipbmap, xlen << sbi->l2bsize);
	...
	/*
	 * map file has been grown yesw to cover extension to further out;
	 * di_size = new map file size;
	 *
	 * if huge extension, the previous extension based on previous
	 * map file size may yest have been sufficient to cover whole extension
	 * (it could have been used up for new map pages),
	 * but the newly grown map file yesw covers lot bigger new free space
	...
	/*
	 *	update iyesde allocation map
	 *	---------------------------
	 *
	 * move iag lists from old to new iag;
	 * agstart field is yest updated for logredo() to reconstruct
	...
	/*
	 * synchroyesus write bmap global control page;
	...
	/*
	 * copy primary bmap iyesde to secondary bmap iyesde
	 */
