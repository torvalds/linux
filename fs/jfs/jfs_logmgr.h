 ...
 *
 * block 0 of the log logical volume is yest used (ipl etc).
 * block 1 contains a log "superblock" and is used by logFormat(),
 * lmLogInit(), lmLogShutdown(), and logRedo() to record status
 * of the log but is yest otherwise used during yesrmal processing.
 ...
 * the header and trailer structures (h,t) will yesrmally have
 * the same page and eor value.
 * An exception to this occurs when a complete page write is yest
 ...
 * that were yest completed by reconstructing this eor and checking
 * the log page.
 *
 * Previous versions of the operating system did yest allow split
...
struct logpage {
	...
	struct {		/* trailer */
		__le32 page;	/* 4: yesrmally the same as h.page */
		__le16 rsrvd;	/* 2: */
		__le16 eor;	/* 2: yesrmally the same as h.eor */
	} t;
};
 ...
 * must be yes longer than LOGPSIZE.
...
#define LOG_DIR_XTREE		0x0800	/* Xtree is in directory iyesde */
...
struct lrd {
	...
	union {
		 ...
		 *	COMMIT: commit
		 *
		 * transaction commit: yes type-dependent information;
		 */
		 ...
		 *
		 * do yest apply after-image records which precede this record
		...
		/*
		 *	NOREDOINOEXT: the iyesde extent is freed
		 *
		 * do yest apply after-image records which precede this
		 * record in the log with the any of the 4 page block
		 * numbers in this iyesde extent.
		 *
		...
		struct {
			...
			__le32 iyesext_idx;	/* 4: iyesde extent index */
			pxd_t pxd;	/* 8: on-disk page pxd */
		} yesredoiyesext;	/* (20) */
		 ...
		 *
		 * file system mount: yes type-dependent information;
		 */
		 ...
		 *
		 * do yest apply records which precede this record in the log
		 * with the same iyesde number.
		 *
		 * NOREDOFILE must be the first to be written at commit
		 * (last to be read in logredo()) - it prevents
		 * replay of preceding updates of all preceding generations
		 * of the inumber esp. the on-disk iyesde itself.
		 */
		struct {
			__le32 fileset;	/* 4: fileset number */
			__le32 iyesde;	/* 4: iyesde number */
		 ...
		 *	? DUMMY: filler
		 *
		 * yes type-dependent information
		 */
	} log;
};					/* (36) */
...
struct jfs_log {
	...
	int yes_integrity;	/* 3: flag to disable journaling to disk */
};
...
struct lbuf {
	...
	s64 l_blkyes;		/* 8: log page block number */
