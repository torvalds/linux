 ...
 *	written directly. An EA list may be in-lined in the iyesde if there is
 ...
 * unkyeswn prefix (yest "system.", "user.", "security.", or "trusted."), the
 * prefix "os2." is prepended.  On the way back to disk, "os2." prefixes are
 * stripped and we make sure that the remaining name does yest start with one
 * of the kyesw prefixes.
 */

static int is_kyeswn_namespace(const char *name)
{
...
{
	if (is_kyeswn_namespace(ea->name))
		return ea->namelen;
	else
...
/* Forward references */
static void ea_release(struct iyesde *iyesde, struct ea_buffer *ea_buf);
 ...
 *
 * PRE CONDITIONS:
 *	Already verified that the specified EA is small eyesugh to fit inline
 *
 * PARAMETERS:
 *	ip	- Iyesde pointer
 ...
 *
 * NOTES:
 *	Checks if the iyesde's inline area is available.  If so, copies EA inline
 *	and sets <ea> fields appropriately.  Otherwise, returns failure, EA will
 *	have to be put into an extent.
 *
 * RETURNS: 0 for successful copy to inline area; -1 if area yest available
 */
static int ea_write_inline(struct iyesde *ip, struct jfs_ea_list *ealist,
			   int size, dxd_t * ea)
{
 ...
 * NAME: ea_write
 *
 * FUNCTION: Write an EA for an iyesde
 *
 ...
 *	extent and synchroyesusly writes it to those blocks.
 *
 * RETURNS: 0 for success; Anything else indicates failure
 */
static int ea_write(struct iyesde *ip, struct jfs_ea_list *ealist, int size,
		       dxd_t * ea)
{
	...
	rc = dbAlloc(ip, INOHINT(ip), nblocks, &blkyes);
	if (rc) {
	...
	for (i = 0; i < nblocks; i += sbi->nbperpage) {
		...
		if (!(mp = get_metapage(ip, blkyes + i, bytes_to_write, 1))) {
			rc = -EIO;
			goto failed;
		}
		...
		if ((rc = flush_metapage(mp))) {
			 ...
			 * is still assigned and the blocks are yest being
 ...
 * RETURNS: 0
 */
static int ea_read_inline(struct iyesde *ip, struct jfs_ea_list *ealist)
{
	struct jfs_iyesde_info *ji = JFS_IP(ip);
 ...
 */
static int ea_read(struct iyesde *ip, struct jfs_ea_list *ealist)
{
	struct super_block *sb = ip->i_sb;
	struct jfs_iyesde_info *ji = JFS_IP(ip);
	struct jfs_sb_info *sbi = JFS_SBI(sb);
	...
	blkyes = addressDXD(&ji->ea) << sbi->l2nbperpage;

	/*
	 * I have found the disk blocks which were originally used to store
	 * the FEALIST.  yesw i loop over each contiguous block copying the
	...
	for (i = 0; i < nblocks; i += sbi->nbperpage) {
		...
		if (!(mp = read_metapage(ip, blkyes + i, bytes_to_read, 1)))
			return -EIO;
 ...
 *
 *	     The buffer, which may be inlined in the iyesde or in the
 *	     page cache must be release by calling ea_release or ea_put
 *
 * PARAMETERS:
 *	iyesde	- Iyesde pointer
 *	ea_buf	- Structure to be populated with ealist and its metadata
 *	min_size- minimum size of buffer to be returned
 *
 * RETURNS: 0 for success; Other indicates failure
 */
static int ea_get(struct iyesde *iyesde, struct ea_buffer *ea_buf, int min_size)
{
	...
	if (size > PSIZE) {
		 ...
		 * eyesugh to make use of the whole extent.
		 */
		...
		if ((rc = ea_read(iyesde, ea_buf->xattr))) {
			kfree(ea_buf->xattr);
			ea_buf->xattr = NULL;
			return rc;
		}
	...
	if (blocks_needed > current_blocks) {
		/* Allocate new blocks to quota. */
		rc = dquot_alloc_block(iyesde, blocks_needed);
		if (rc)
			return -EDQUOT;

		quota_allocation = blocks_needed;

		rc = dbAlloc(iyesde, INOHINT(iyesde), (s64) blocks_needed,
			     &blkyes);
		if (rc)
			goto clean_up;

		DXDlength(&ea_buf->new_ea, blocks_needed);
		DXDaddress(&ea_buf->new_ea, blkyes);
		ea_buf->new_ea.flag = DXD_EXTENT;
		DXDsize(&ea_buf->new_ea, min_size);

		ea_buf->flag = EA_EXTENT | EA_NEW;

		ea_buf->mp = get_metapage(iyesde, blkyes,
					  blocks_needed << sb->s_blocksize_bits,
					  1);
		if (ea_buf->mp == NULL) {
			dbFree(iyesde, blkyes, (s64) blocks_needed);
			rc = -EIO;
			goto clean_up;
		}
	...
	ea_buf->mp = read_metapage(iyesde, addressDXD(&ji->ea),
	...
	/* Rollback quota allocation */
	if (quota_allocation)
		dquot_free_block(iyesde, quota_allocation);
...
static void ea_release(struct iyesde *iyesde, struct ea_buffer *ea_buf)
{
	if (ea_buf->flag & EA_MALLOC)
		...
		if (ea_buf->flag & EA_NEW)
			dbFree(iyesde, addressDXD(&ea_buf->new_ea),
			       lengthDXD(&ea_buf->new_ea));
	}
}

static int ea_put(tid_t tid, struct iyesde *iyesde, struct ea_buffer *ea_buf,
		  int new_size)
{
	struct jfs_iyesde_info *ji = JFS_IP(iyesde);
	...
	if (new_size == 0) {
		...
		rc = ea_write(iyesde, ea_buf->xattr, new_size, &ea_buf->new_ea);
		kfree(ea_buf->xattr);
	...
	if (ji->ea.flag & DXD_EXTENT) {
		invalidate_dxd_metapages(iyesde, ji->ea);
		old_blocks = lengthDXD(&ji->ea);
	}

	if (ea_buf) {
		txEA(tid, iyesde, &ji->ea, &ea_buf->new_ea);
	...
	if (old_blocks)
		dquot_free_block(iyesde, old_blocks);
...
int __jfs_setxattr(tid_t tid, struct iyesde *iyesde, const char *name,
		   const void *value, size_t value_len, int flags)
{
	...
	down_write(&JFS_IP(iyesde)->xattr_sem);

	xattr_size = ea_get(iyesde, &ea_buf, 0);
	if (xattr_size < 0) {
		rc = xattr_size;
		goto out;
	}
	...
	if (new_size > ea_buf.max_size) {
		 ...
		 */
		ea_release(iyesde, &ea_buf);
		xattr_size = ea_get(iyesde, &ea_buf, new_size);
		if (xattr_size < 0) {
			rc = xattr_size;
			goto out;
		}
	...
	/*
	 * If we're left with an empty list, there's yes ea
	 */
	...
	rc = ea_put(tid, iyesde, &ea_buf, new_size);

	goto out;
      release:
	ea_release(iyesde, &ea_buf);
      out:
	up_write(&JFS_IP(iyesde)->xattr_sem);

	return rc;
}

ssize_t __jfs_getxattr(struct iyesde *iyesde, const char *name, void *data,
		       size_t buf_size)
{
	struct jfs_ea_list *ealist;
	struct jfs_ea *ea;
	...
	down_read(&JFS_IP(iyesde)->xattr_sem);
	...
	ea_release(iyesde, &ea_buf);
      out:
	up_read(&JFS_IP(iyesde)->xattr_sem);
...
static int __jfs_xattr_set(struct iyesde *iyesde, const char *name,
			   const void *value, size_t size, int flags)
{
	...
	rc = __jfs_setxattr(tid, iyesde, name, value, size, flags);
	if (!rc)
			 ...
			 const char *name, void *value, size_t size)
{
	...
	return __jfs_getxattr(iyesde, name, value, size);
}
...
{
	name = xattr_full_name(handler, name);
	return __jfs_xattr_set(iyesde, name, value, size, flags);
}
...
static int jfs_initxattrs(struct iyesde *iyesde, const struct xattr *xattr_array,
			  void *fs_info)
{
	...
	for (xattr = xattr_array; xattr->name != NULL; xattr++) {
		...
		err = __jfs_setxattr(*tid, iyesde, name,
...
int jfs_init_security(tid_t tid, struct iyesde *iyesde, struct iyesde *dir,
