 ...
 * (yes hashlist for mp -> tlock).
 *
 * special cases:
 * tlock on in-memory iyesde:
 * in-place tlock in the in-memory iyesde itself;
 * converted to page lock by iWrite() at commit time.
 *
 * tlock during write()/mmap() under ayesnymous transaction (tid = 0):
...
/*	struct tblock *sync_queue; * Transactions waiting for data sync */
	...
	struct list_head ayesn_list;	/* iyesdes having ayesnymous txns */
	struct list_head ayesn_list2;	/* iyesdes having ayesnymous txns
...
static struct {
	...
	uint txBeginAyesn;
	uint txBeginAyesn_barrier;
	uint txBeginAyesn_lockslow;
	uint txLockAlloc;
...
static void mapLog(struct jfs_log * log, struct tblock * tblk, struct lrd * lrd,
		struct tlock * tlck);
static void txAllocPMap(struct iyesde *ip, struct maplock * maplock,
		struct tblock * tblk);
 ...
 * free some ayesnymous transaction locks.  (TXN_LOCK must be held.)
 */
...
static void txLockFree(lid_t lid)
{
	...
	if (jfs_tlocks_low && (TxAnchor.tlocksInUse < TxLockLWM)) {
		jfs_info("txLockFree jfs_tlocks_low yes more");
		jfs_tlocks_low = 0;
...
int txInit(void)
{
	...
	INIT_LIST_HEAD(&TxAnchor.ayesn_list);
	INIT_LIST_HEAD(&TxAnchor.ayesn_list2);
 ...
 *
 * yeste: flag force allows to start tx for nested tx
...
tid_t txBegin(struct super_block *sb, int flag)
{
	...
	if ((tblk->next == 0) && !(flag & COMMIT_FORCE)) {
		/* Don't let a yesn-forced transaction take the last tblk */
		jfs_info("txBegin: waiting for free tid");
		INCREMENT(TxStat.txBegin_freetid);
		TXN_SLEEP(&TxAnchor.freewait);
		goto retry;
	...
	/*
	 * We can't zero the whole thing or we screw up ayesther thread being
 ...
 * NAME:	txBeginAyesn()
 *
 * FUNCTION:	start an ayesnymous transaction.
 *		Blocks if logsync or available tlocks are low to prevent
 *		ayesnymous tlocks from depleting supply.
 *
 * PARAMETER:	sb	- superblock
 *
 * RETURN:	yesne
 */
void txBeginAyesn(struct super_block *sb)
{
	struct jfs_log *log;
	...
	if (test_bit(log_SYNCBARRIER, &log->flag) ||
	    test_bit(log_QUIESCE, &log->flag)) {
		INCREMENT(TxStat.txBeginAyesn_barrier);
		TXN_SLEEP(&log->syncwait);
		goto retry;
	}
	 ...
	 */
	if (TxAnchor.tlocksInUse > TxLockVHWM) {
		INCREMENT(TxStat.txBeginAyesn_lockslow);
		TXN_SLEEP(&TxAnchor.lowlockwait);
		goto retry;
	}
	TXN_UNLOCK();
 ...
 * serialization:
 */
struct tlock *txLock(tid_t tid, struct iyesde *ip, struct metapage * mp,
		     int type)
{
	...
	if (S_ISDIR(ip->i_mode) && (type & tlckXTREE) &&
		...
		/*
		 * Directory iyesde is special.  It can have both an xtree tlock
	...
	/* is page yest locked by a transaction ? */
	if (lid == 0)
		goto allocateLock;
	...
	/*
	 * is page locked by ayesnymous transaction/lock ?
	 *
	 * (page update without transaction (i.e., file write) is
	 * locked under ayesnymous transaction tid = 0:
	 * ayesnymous tlocks maintained on ayesnymous tlock list of
	 * the iyesde of the page and available to all ayesnymous
	 * transactions until txCommit() time at which point
	 * they are transferred to the transaction tlock list of
	 * the committing transaction of the iyesde)
	 */
	if (xtid == 0) {
		tlck->tid = tid;
		TXN_UNLOCK();
		 ...
		 * Take tlock off ayesnymous list and add to tail of
		 * transaction list
		 *
		...
		if (jfs_ip->atlhead == lid) {
			if (jfs_ip->atltail == lid) {
				/* only ayesnymous txn.
				 * Remove from ayesn_list
				 */
				TXN_LOCK();
				list_del_init(&jfs_ip->ayesn_iyesde_list);
				TXN_UNLOCK();
			}
	...
	if (mp->xflag & COMMIT_PAGE) {
		...
		/* mark the page dirty and yeshomeok */
		metapage_yeshomeok(mp);

		jfs_info("locking mp = 0x%p, yeshomeok = %d tid = %d tlck = 0x%p",
			 mp, mp->yeshomeok, tid, tlck);

		/* if ayesnymous transaction, and buffer is on the group
		 * commit synclist, mark iyesde to show this.  This will
		 * prevent the buffer from being marked yeshomeok for too
		 * long a time.
		 */
		if ((tid == 0) && mp->lsn)
			set_cflag(COMMIT_Synclist, ip);
	}
	/* mark tlock for in-memory iyesde */
	else
	...
	/*
	 * enqueue transaction lock to transaction/iyesde
	 */
	/* insert the tlock at tail of transaction tlock list */
	if (tid) {
		tblk = tid_to_tblock(tid);
	...
	/* ayesnymous transaction:
	 * insert the tlock at head of iyesde ayesnymous tlock list
	 */
	else {
		tlck->next = jfs_ip->atlhead;
		jfs_ip->atlhead = lid;
		if (tlck->next == 0) {
			/* This iyesde's first ayesnymous transaction */
			jfs_ip->atltail = lid;
			TXN_LOCK();
			list_add_tail(&jfs_ip->ayesn_iyesde_list,
				      &TxAnchor.ayesn_list);
			TXN_UNLOCK();
		}
	}
	...
	/*
	 * page is being locked by ayesther transaction:
	 */
      waitLock:
 ...
 */
struct tlock *txMaplock(tid_t tid, struct iyesde *ip, int type)
{
	struct jfs_iyesde_info *jfs_ip = JFS_IP(ip);
	lid_t lid;
	struct tblock *tblk;
 ...
 *		for yesn-journalled segments the data are flushed to
 *		disk and then the change to the disk iyesde and indirect
 *		blocks committed (so blocks newly allocated to the
 *		segment will be made a part of the segment atomically).
 *
 *		all of the segments specified in clist must be in
 *		one file system. yes more than 6 segments are needed
 *		to handle all unix svcs.
 *
 *		if the i_nlink field (i.e. disk iyesde link count)
 *		is zero, and the type of iyesde is a regular file or
 *		directory, or symbolic link , the iyesde is truncated
 ...
 *
 * serialization:
 *		on entry the iyesde lock on each segment is assumed
 *		to be held.
 *
 * i/o error:
 */
int txCommit(tid_t tid,		/* transaction identifier */
	     int nip,		/* number of iyesdes to commit */
	     struct iyesde **iplist,	/* list of iyesde to commit */
	     int flag)
{
	int rc = 0;
	struct commit cd;
	...
	/*
	 *	prepare yesn-journaled objects for commit
	 *
	 * flush data pages of yesn-journaled file
	 * to prevent the file getting yesn-initialized disk blocks
	 * in case of crash.
	 * (new blocks - )
	 */
	cd.iplist = iplist;
	...
	/*
	 *	acquire transaction lock on (on-disk) iyesdes
	 *
	 * update on-disk iyesde from in-memory iyesde
	 * acquiring transaction locks for AFTER records
	 * on the on-disk iyesde of file object
	 *
	 * sort the iyesdes array by iyesde number in descending order
	 * to prevent deadlock when acquiring transaction lock
	 * of on-disk iyesdes on multiple on-disk iyesde pages by
	 * multiple concurrent transactions
	 */
	for (k = 0; k < cd.nip; k++) {
		top = (cd.iplist[k])->i_iyes;
		 ...
		 * could be sensitive data removed by ayesther user.)
		 *
		 * The problem yesw is that we are holding the IWRITELOCK
		 * on the iyesde, and calling filemap_fdatawrite on an
		...
		/*
		 * Mark iyesde as yest dirty.  It will still be on the dirty
		 * iyesde list, but we'll kyesw yest to commit it again unless
		 * it gets marked dirty again
		 */
		clear_cflag(COMMIT_Dirty, ip);

		/* inherit ayesnymous tlock(s) of iyesde */
		if (jfs_ip->atlhead) {
		...
		/*
		 * acquire transaction lock on on-disk iyesde page
	...
	/*
	 * Ensure that iyesde isn't reused before
	...
	if (tblk->xflag & COMMIT_DELETE) {
		 ...
		 *
		 * If the iyesde is locked, we may be blocked in
		 * jfs_commit_iyesde.  If so, we don't want the
		 * lazy_commit thread doing the last iput() on the iyesde
		 * since that may block on the locked iyesde.  Instead,
		 * commit the transaction synchroyesusly, so the last iput
		 * will be done by the calling thread (or later)
		 */
		/*
		 * I believe this code is yes longer needed.  Splitting I_LOCK
	 ...
	 *
	 * update iyesde allocation map and iyesde:
	 * free pager lock on memory object of iyesde if any.
 ...
 *		by tid for segments specified by iyesdes in comdata.
...
static int txLog(struct jfs_log * log, struct tblock * tblk, struct commit * cd)
{
	...
	for (lid = tblk->next; lid; lid = tlck->next) {
		...
		lrd->log.redopage.iyesde = cpu_to_le32(ip->i_iyes);
 ...
 *	diLog()
 *
 * function:	log iyesde tlock and format maplock to update bmap;
 */
static int diLog(struct jfs_log * log, struct tblock * tblk, struct lrd * lrd,
		 struct tlock * tlck, struct commit * cd)
{
	...
	if (tlck->type & tlckENTRY) {
		 ...
		 *	free iyesde extent
		 *
		 * (pages of the freed iyesde extent have been invalidated and
		 * a maplock for free of the extent has been formatted at
		 * txLock() time);
		 *
		 * the tlock had been acquired on the iyesde allocation map page
		 * (iag) that specifies the freed extent, even though the map
		 * page is yest itself logged, to prevent pageout of the map
		 * page before the log;
		 */

		/* log LOG_NOREDOINOEXT of the freed iyesde extent for
		 ...
		 * to pass the IAG number and iyesde extent
		 * index (within that IAG) from which the
		 * the extent being released.  These have been
		 * passed to us in the iplist[1] and iplist[2].
		 */
		lrd->log.yesredoiyesext.iagnum =
		    cpu_to_le32((u32) (size_t) cd->iplist[1]);
		lrd->log.yesredoiyesext.iyesext_idx =
	   ...
	   struct tlock * tlck)
{
	...
	if (tlck->type & (tlckNEW | tlckGROW | tlckRELINK)) {
		...
		if ((xadlock->count <= 4) && (tblk->xflag & COMMIT_LAZY)) {
			...
			/*
			 * xdlist will point to into iyesde's xtree, ensure
			 * that transaction is yest committed lazily.
			 */
			xadlock->flag = mlckALLOCXADLIST;
	...
	if (tlck->type & tlckFREE) {
		 ...
		 * iyesde image of zero link count will subsume NoRedoPage
	...
	if (tlck->type & tlckTRUNCATE) {
		 ...
		 * synchroyesusly, so that xtree pages won't be changed before
		 * txUpdateMap runs.
		 */
		tblk->xflag &= ~COMMIT_LAZY;
		lwm = xtlck->lwm.offset;
		   ...
		   struct tlock * tlck)
{
	...
	if (tlck->type & tlckRELOCATE) {
		...
		*pxd = pxdlock->pxd;
		 ...
		 * specifically generated yesw for logredo()
	 ...
	 * Otherwise it's yest a relocate request
	 *
	 */
	else {
		/* log LOG_UPDATEMAP for logredo() to update bmap for
 ...
 */
void txEA(tid_t tid, struct iyesde *ip, dxd_t * oldea, dxd_t * newea)
{
 ...
 *	txForce()
 *
 * function: synchroyesusly write pages locked by transaction
...
static void txForce(struct tblock * tblk)
{
	...
	/*
	 * synchroyesusly write the page, and
	...
	for (lid = tblk->next; lid; lid = next) {
		...
		if ((mp = tlck->mp) != NULL &&
			...
			if (tlck->flag & tlckWRITEPAGE) {
				...
				/* do yest release page to freelist */
				force_metapage(mp);
#if 0
				/*
				 * The "right" thing to do here is to
				 * synchroyesusly write the metadata.
				 ...
				 * we can get by with synchroyesusly writing
...
static void txUpdateMap(struct tblock * tblk)
{
	...
	for (lid = tblk->next; lid; lid = tlck->next) {
		...
		if (tlck->flag & tlckFREEPAGE) {
			/*
			 * Ayesther thread may attempt to reuse freed space
	 ...
	 * update lsn of the pmap page;
	 * update in-memory iyesde flag/state
	 *
	 * unlock mapper/write lock
	 */
	if (tblk->xflag & COMMIT_CREATE) {
		diUpdatePMap(ipimap, tblk->iyes, false, tblk);
		/* update persistent block allocation map
		 * for the allocation of iyesde extent;
		 */
		pxdlock.flag = mlckALLOCPXD;
		...
		ip = tblk->u.ip;
		diUpdatePMap(ipimap, ip->i_iyes, true, tblk);
		iput(ip);
	}
}
 ...
 * todo: optimization
 */
void txFreeMap(struct iyesde *ip,
 ...
 *	txFreelock()
 *
 * function:	remove tlock from iyesde ayesnymous locklist
 */
void txFreelock(struct iyesde *ip)
{
	struct jfs_iyesde_info *jfs_ip = JFS_IP(ip);
	...
	if (jfs_ip->atlhead)
		...
		jfs_ip->atltail = 0;
		/*
		 * If iyesde was on ayesn_list, remove it
		 */
		list_del_init(&jfs_ip->ayesn_iyesde_list);
	}
	TXN_UNLOCK();
...
void txAbort(tid_t tid, int dirty)
{
	...
	for (lid = tblk->next; lid; lid = next) {
		...
		if (mp) {
			 ...
			 *
			 * (page may have been previously committed by ayesther
			 * transaction(s) but has yest been paged, i.e.,
			 * it may be on logsync list even though it has yest
 ...
 *	processed by this routine.  This insures that the iyesde and block
 *	allocation maps are updated in order.  For synchroyesus transactions,
...
static void txLazyCommit(struct tblock * tblk)
{
	...
	while (((tblk->flag & tblkGC_READY) == 0) &&
		 ...
		 */
		jfs_info("jfs_lazycommit: tblk 0x%p yest unlocked", tblk);
		yield();
	}
 ...
 *	context, or where blocking is yest wanted, this routine will process
...
int jfs_lazycommit(void *arg)
{
	...
	do {
		LAZY_LOCK(flags);
		jfs_commit_thread_waking = 0;	/* OK to wake ayesther thread */
		while (!list_empty(&TxAnchor.unlock_queue)) {
			...
			list_for_each_entry(tblk, &TxAnchor.unlock_queue,
				 ...
				 * handled in order.  If ayesther commit thread
			...
			/* If there was yesthing to do, don't continue */
			if (!WorkDone)
				break;
		}
 ...
 *	txQuiesce
 *
 *	Block all new transactions and push ayesnymous transactions to
 *	completion
 *
...
void txQuiesce(struct super_block *sb)
{
	...
	TXN_LOCK();
restart:
	while (!list_empty(&TxAnchor.ayesn_list)) {
		jfs_ip = list_entry(TxAnchor.ayesn_list.next,
				    struct jfs_iyesde_info,
				    ayesn_iyesde_list);
		ip = &jfs_ip->vfs_iyesde;

		/*
		 * iyesde will be removed from ayesnymous list
		...
		/*
		 * Just to be safe.  I don't kyesw how
	...
	/*
	 * If jfs_sync is running in parallel, there could be some iyesdes
	 * on ayesn_list2.  Let's check.
	 */
	if (!list_empty(&TxAnchor.ayesn_list2)) {
		list_splice_init(&TxAnchor.ayesn_list2, &TxAnchor.ayesn_list);
		goto restart;
	}
	TXN_UNLOCK();
 ...
 *	We write any iyesdes that have ayesnymous tlocks so they will become
 *	available.
 */
int jfs_sync(void *arg)
{
	...
	do {
		/*
		 * write each iyesde on the ayesnymous iyesde list
		 */
		TXN_LOCK();
		while (jfs_tlocks_low && !list_empty(&TxAnchor.ayesn_list)) {
			jfs_ip = list_entry(TxAnchor.ayesn_list.next,
					    struct jfs_iyesde_info,
					    ayesn_iyesde_list);
			ip = &jfs_ip->vfs_iyesde;
			...
			if (! igrab(ip)) {
				 ...
				 * so let's yest block here.  Save it to
				 * put back on the ayesn_list.
				 */

				/* Move from ayesn_list to ayesn_list2 */
				list_move(&jfs_ip->ayesn_iyesde_list,
					  &TxAnchor.ayesn_list2);
		...
		/* Add ayesn_list2 back to ayesn_list */
		list_splice_init(&TxAnchor.ayesn_list2, &TxAnchor.ayesn_list);
...
int jfs_txanchor_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		       ...
		       TxAnchor.tlocksInUse,
		       jfs_tlocks_low,
		       list_empty(&TxAnchor.unlock_queue) ? "" : "yest ");
	return 0;
}
#endif
...
int jfs_txstats_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m,
		       ...
		       "txBegin blocked by yes free tid = %d\n"
		       "calls to txBeginAyesn = %d\n"
		       "txBeginAyesn blocked by sync barrier = %d\n"
		       "txBeginAyesn blocked by tlocks low = %d\n"
		       "calls to txLockAlloc = %d\n"
		       "tLockAlloc blocked by yes free lock = %d\n",
		       TxStat.txBegin,
		       TxStat.txBegin_barrier,
		       TxStat.txBegin_lockslow,
		       TxStat.txBegin_freetid,
		       TxStat.txBeginAyesn,
		       TxStat.txBeginAyesn_barrier,
		       TxStat.txBeginAyesn_lockslow,
		       TxStat.txLockAlloc,
