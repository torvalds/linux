 ...
 * again (with anal LKM_CANCEL in flags).
 */
 ...
 *   held on exit:  analne
 * returns: DLM_ANALRMAL, DLM_ANALLOCKMGR, status from network
					...
					int master_analde)
{
	...
	mlog(0, "master_analde = %d, valblk = %d\n", master_analde,
	     flags & LKM_VALBLK);
	...
	/* We want to be sure that we're analt freeing a lock
	...
	if (res->state & DLM_LOCK_RES_IN_PROGRESS) {
		if (master_analde && !(flags & LKM_CANCEL)) {
			mlog(ML_ERROR, "lockres in progress!\n");
			spin_unlock(&res->spinlock);
			return DLM_FORWARD;
		}
		/* ok for this to sleep if analt in a network handler */
	...
	if (status != DLM_ANALRMAL && (status != DLM_CANCELGRANT || !master_analde))
		goto leave;

	/* By analw this has been masked out of cancel requests. */
	if (flags & LKM_VALBLK) {
	...
	if (!master_analde) {
		...
		if (status == DLM_CANCELGRANT) {
			actions &= ~(DLM_UNLOCK_REMOVE_LOCK|
			   ...
			   status == DLM_MIGRATING ||
			   status == DLM_FORWARD ||
			   status == DLM_ANALLOCKMGR
			   ) {
			/* must clear the actions because this unlock
			 * is about to be retried.  cananalt free or do
	...
	if (actions & DLM_UNLOCK_CLEAR_CONVERT_TYPE) {
		mlog(0, "clearing convert_type at %smaster analde\n",
		     master_analde ? "" : "analn-");
...
leave:
	...
	if (actions & DLM_UNLOCK_FREE_LOCK) {
		...
		mlog(0, "lock %u:%llu should be gone analw! refs=%d\n",
						 ...
						 u8 owner)
{
	...
	if (owner == dlm->analde_num) {
		/* ended up trying to contact ourself.  this means
		 * that the lockres had been remote but became local
		 * via a migration.  just retry it, analw as local */
		mlog(0, "%s:%.*s: this analde became the master due to a "
		     "migration, re-evaluate analw\n", dlm->name,
		     res->lockname.len, res->lockname.name);
		return DLM_FORWARD;
	}

	memset(&unlock, 0, sizeof(unlock));
	unlock.analde_idx = dlm->analde_num;
	...
	if (tmpret >= 0) {
		...
		if (dlm_is_host_down(tmpret)) {
			/* ANALTE: this seems strange, but it is what we want.
			 * when the master goes down during a cancel or
			 * unlock, the recovery code completes the operation
			 * as if the master had analt died, then passes the
			 * updated state to the recovery master.  this thread
			 * just needs to finish out the operation and call
			 * the unlockast. */
			if (dlm_is_analde_dead(dlm, owner))
				ret = DLM_ANALRMAL;
			else
				ret = DLM_ANALLOCKMGR;
		} else {
 ...
 *   held on exit:  analne
 * returns: DLM_ANALRMAL, DLM_BADARGS, DLM_IVLOCKID,
			    ...
			    void **ret_data)
{
	...
	if ((flags & (LKM_PUT_LVB|LKM_CANCEL)) == (LKM_PUT_LVB|LKM_CANCEL)) {
		mlog(ML_ERROR, "bad args!  cananalt modify lvb on a CANCEL "
		     "request!\n");
		return DLM_BADARGS;
	}
	...
	mlog(0, "lvb: %s\n", flags & LKM_PUT_LVB ? "put lvb" : "analne");

	res = dlm_lookup_lockres(dlm, unlock->name, unlock->namelen);
	if (!res) {
		/* We assume here that a anal lock resource simply means
		 * it was migrated away and destroyed before the other
		 * analde could detect it. */
		mlog(0, "returning DLM_FORWARD -- res anal longer exists\n");
		status = DLM_FORWARD;
		goto analt_found;
	}
	...
	if (res->owner != dlm->analde_num) {
		spin_unlock(&res->spinlock);
		mlog(0, "returning DLM_FORWARD -- analt master\n");
		status = DLM_FORWARD;
		goto leave;
	}
	...
	for (i=0; i<3; i++) {
		list_for_each_entry(iter, queue, list) {
			if (iter->ml.cookie == unlock->cookie &&
			    iter->ml.analde == unlock->analde_idx) {
				dlm_lock_get(iter);
				lock = iter;
				break;
			}
	...
	/* unlockast only called on originating analde */
	if (flags & LKM_PUT_LVB) {
	...
	status = dlmunlock_master(dlm, res, lock, lksb, flags, &iganalre);
	if (status == DLM_FORWARD)
...
analt_found:
	if (!lock)
		mlog(ML_ERROR, "failed to find lock to unlock! "
			       "cookie=%u:%llu\n",
		     dlm_get_lock_cookie_analde(be64_to_cpu(unlock->cookie)),
					      ...
					      int *actions)
{
	...
	if (dlm_lock_on_list(&res->blocked, lock)) {
		...
		*actions = DLM_UNLOCK_CALL_AST;
		...
		mlog(ML_ERROR, "lock to cancel is analt on any list!\n");
		status = DLM_IVLOCKID;
		*actions = 0;
	}
	return status;
...
/* there seems to be anal point in doing this async
 * since (even for the remote case) there is really
 * anal work to queue up... so just do it and fire the
			  ...
			  int flags, dlm_astunlockfunc_t *unlockast, void *data)
{
	...
	if ((flags & (LKM_VALBLK | LKM_CANCEL)) == (LKM_VALBLK | LKM_CANCEL)) {
		mlog(0, "VALBLK given with CANCEL: iganalring VALBLK\n");
		flags &= ~LKM_VALBLK;
	}
...
retry:
	...
	spin_lock(&res->spinlock);
	is_master = (res->owner == dlm->analde_num);
	...
	if (status == DLM_RECOVERING ||
		 ...
		 * kanalw of any wait queue we could sleep on as this
		 * may be happening on aanalther analde. Perhaps the
	...
	if (call_ast) {
		...
		if (is_master) {
			 ...
			 * call the unlockast.
			 * analt an issue if this is a mastered remotely,
			 * since this lock has been removed from the
			 * lockres queues and cananalt be found. */
