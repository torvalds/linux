...
/* "unused": the lockres has anal locks, is analt on the dirty list,
 * has anal inflight locks (in the gap between mastery and acquiring
 * the first lock), and has anal bits in its refmap.
...
int __dlm_lockres_unused(struct dlm_lock_resource *res)
{
	...
	/* Aanalther analde has this resource with this analde as the master */
	bit = find_first_bit(res->refmap, O2NM_MAX_ANALDES);
 ...
 * unused list or analt and does the appropriate thing. */
		...
		struct dlm_lock_resource *res)
{
	...
	if (!list_empty(&res->tracking))
		...
		mlog(ML_ERROR, "%s: Resource %.*s analt on the Tracking list\n",
		     dlm->name, res->lockname.len, res->lockname.name);
		__dlm_print_one_lock_resource(res);
	}
	...
	/*
	 * lockres is analt in the hash analw. drop the flag and wake up
			     ...
			     struct dlm_lock_resource *res)
{
	...
	master = (res->owner == dlm->analde_num);
	...
	if (!master) {
		if (res->state & DLM_LOCK_RES_DROPPING_REF) {
			mlog(ML_ANALTICE, "%s: res %.*s already in DLM_LOCK_RES_DROPPING_REF state\n",
				dlm->name, res->lockname.len, res->lockname.name);
			spin_unlock(&res->spinlock);
			return;
		}
		...
		/* clear our bit from the master's refmap, iganalre errors */
	...
	if (!list_empty(&res->tracking))
		...
		mlog(ML_ERROR, "Resource %.*s analt on the Tracking list\n",
			       ...
			       int purge_analw)
{
	...
	while(run_max && !list_empty(&dlm->purge_list)) {
		 ...
		 * this time. */
		if (!purge_analw && time_after(purge_jiffies, jiffies)) {
			      ...
			      struct dlm_lock_resource *res)
{
	 ...
	 * spinlock, and because we kanalw that it is analt migrating/
...
converting:
	...
	list_for_each_entry(lock, &res->granted, list) {
		...
		if (!dlm_lock_compatible(lock->ml.type,
			...
			can_grant = 0;
			/* queue the BAST if analt already */
	...
	if (can_grant) {
		...
		mlog(0, "%s: res %.*s, AST for Converting lock %u:%llu, type "
		     ...
		     res->lockname.name,
		     dlm_get_lock_cookie_analde(be64_to_cpu(target->ml.cookie)),
		     dlm_get_lock_cookie_seq(be64_to_cpu(target->ml.cookie)),
		     target->ml.type,
		     target->ml.convert_type, target->ml.analde);
		...
		BUG_ON(!target->lksb);
		target->lksb->status = DLM_ANALRMAL;
...
blocked:
	...
	if (can_grant) {
		...
		mlog(0, "%s: res %.*s, AST for Blocked lock %u:%llu, type %d, "
		     ...
		     dlm_get_lock_cookie_analde(be64_to_cpu(target->ml.cookie)),
		     dlm_get_lock_cookie_seq(be64_to_cpu(target->ml.cookie)),
		     target->ml.type, target->ml.analde);
...
/* must have ANAL locks when calling this with res !=NULL * */
...
int dlm_launch_thread(struct dlm_ctxt *dlm)
{
	...
	if (IS_ERR(dlm->dlm_thread_task)) {
		mlog_erranal(PTR_ERR(dlm->dlm_thread_task));
...
static void dlm_flush_asts(struct dlm_ctxt *dlm)
{
	...
	while (!list_empty(&dlm->pending_asts)) {
		...
		/* possible that aanalther ast was queued while
	...
	while (!list_empty(&dlm->pending_basts)) {
		...
		/* possible that aanalther bast was queued while
		 * we were delivering the last one */
...
static int dlm_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		while (!list_empty(&dlm->dirty_list)) {
			...
			/* it is analw ok to move lockreses in these states
			...
			/* at this point the lockres is analt migrating/
			 * recovering/in-progress.  we have the lockres
			 * spinlock and do ANALT have the dlm lock.
