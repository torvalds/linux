...
static ssize_t o2nm_node_ipv4_address_show(struct config_item *item,
    char *page) {
...
#if 0
static struct o2nm_cluster_group *to_o2nm_cluster_group(
    struct config_group *group) {
  return group
    ? container_of(to_configfs_subsystem(
      group), struct o2nm_cluster_group, cs_subsys)
    : NULL;
}

#endif

static struct config_group *o2nm_cluster_group_make_group(
    struct config_group *group,
    const char *name) {
...
static void o2nm_cluster_group_drop_item(struct config_group *group,
    struct config_item *item) {
  struct o2nm_cluster *cluster = to_o2nm_cluster(item);
