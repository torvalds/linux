...
static void o2hb_nego_timeout(struct work_struct *work) {
  ...
  if (master_node == o2nm_this_node()) {
    if (!test_bit(master_node, reg->hr_nego_node_bitmap)) {
      printk(
          KERN_NOTICE "o2hb: node %d hb write hung for %ds on region %s (%pg).\n",
    ...
    printk(
        KERN_NOTICE "o2hb: node %d hb write hung for %ds on region %s (%pg), negotiate timeout with node %d.\n",
        o2nm_this_node(), O2HB_NEGO_TIMEOUT_MS / 1000,
        config_item_name(&reg->hr_item),
...
static int o2hb_nego_timeout_handler(struct o2net_msg *msg, u32 len, void *data,
  ...
  printk(
      KERN_NOTICE "o2hb: receive negotiate timeout message from node %d on region %s (%pg).\n",
...
static int o2hb_nego_approve_handler(struct o2net_msg *msg, u32 len, void *data,
  ...
  struct o2hb_region *reg = data;
  printk(
      KERN_NOTICE "o2hb: negotiate timeout approved by master node on region %s (%pg).\n",
      config_item_name(&reg->hr_item), reg_bdev(reg));
  o2hb_arm_timeout(reg);
  return 0;
}
...
static inline void o2hb_prepare_block(struct o2hb_region *reg,
  ...
  hb_block->hb_dead_ms = cpu_to_le32(
...
static struct o2hb_heartbeat_group *to_o2hb_heartbeat_group(
...
static struct config_item *o2hb_heartbeat_group_make_item(
    struct config_group *group,
    const char *name) {
...
static ssize_t o2hb_heartbeat_group_dead_threshold_show(
    struct config_item *item,
    char *page) {
  return sprintf(page, "%u\n", o2hb_dead_threshold);
}

static ssize_t o2hb_heartbeat_group_dead_threshold_store(
    struct config_item *item,
