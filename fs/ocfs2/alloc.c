...
static enum ocfs2_contig_type ocfs2_extent_rec_contig(struct super_block *sb,
...
static void ocfs2_xattr_tree_fill_max_leaf_clusters(
    struct ocfs2_extent_tree *et) {
...
static enum ocfs2_contig_type ocfs2_refcount_tree_extent_contig(
    struct ocfs2_extent_tree *et,
    struct ocfs2_extent_rec *ext,
    struct ocfs2_extent_rec *insert_rec) {
  return CONTIG_NONE;
...
static inline enum ocfs2_contig_type ocfs2_et_extent_contig(
    struct ocfs2_extent_tree *et,
    struct ocfs2_extent_rec *rec,
    struct ocfs2_extent_rec *insert_rec) {
...
int ocfs2_num_free_extents(struct ocfs2_extent_tree *et) {
  ...
  if (el->l_tree_depth != 0) {
    retval = ocfs2_error(ocfs2_metadata_cache_get_super(
        et->et_ci),
...
static int ocfs2_find_branch_target(struct ocfs2_extent_tree *et,
  ...
  *target_bh = NULL;
  ...
  while (le16_to_cpu(el->l_tree_depth) > 1) {
    ...
    if (!blkno) {
      status = ocfs2_error(ocfs2_metadata_cache_get_super(
          et->et_ci),
...
static int __ocfs2_find_path(struct ocfs2_caching_info *ci,
  ...
  while (el->l_tree_depth) {
    ...
    if (le16_to_cpu(el->l_next_free_rec)
        > le16_to_cpu(el->l_count)) {
      ocfs2_error(ocfs2_metadata_cache_get_super(
          ci),
...
static void ocfs2_unlink_subtree(handle_t *handle,
  ...
  struct ocfs2_extent_block *eb;
  eb
    = (struct ocfs2_extent_block *) right_path->p_node[subtree_index
          + 1].bh->b_data;
  for (i = 1; i < le16_to_cpu(root_el->l_next_free_rec); i++) {
...
static int ocfs2_get_right_path(struct ocfs2_extent_tree *et,
  ...
  *ret_right_path = NULL;
  ...
  ret = ocfs2_find_cpos_for_right_leaf(ocfs2_metadata_cache_get_super(
      et->et_ci),
...
static int ocfs2_append_rec_to_path(handle_t *handle,
  ...
  *ret_left_path = NULL;
  ...
  if (next_free == 0
    ...
    u32 left_cpos;
    ret = ocfs2_find_cpos_for_left_leaf(ocfs2_metadata_cache_get_super(
        et->et_ci),
        right_path, &left_cpos);
    if (ret) {
      mlog_errno(ret);
...
static struct ocfs2_per_slot_free_list *ocfs2_find_per_slot_free_list(int type,
    int slot,
    struct ocfs2_cached_dealloc_ctxt *ctxt) {
...
static struct ocfs2_per_slot_free_list *ocfs2_find_preferred_free_list(int type,
    int preferred_slot,
    int *real_slot,
    struct ocfs2_cached_dealloc_ctxt *ctxt) {
  struct ocfs2_per_slot_free_list *fl = ctxt->c_first_suballocator;
