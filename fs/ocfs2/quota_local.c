...
static int ocfs2_modify_bh(struct iyesde *iyesde, struct buffer_head *bh,
		void (*modify)(struct buffer_head *, void *), void *private)
{
	...
	status = ocfs2_journal_access_dq(handle, INODE_CACHE(iyesde), bh,
 ...
 *
 * This function acquires ip_alloc_sem and thus it must yest be called with a
 * transaction started.
 */
static int ocfs2_read_quota_block(struct iyesde *iyesde, u64 v_block,
				  struct buffer_head **bh)
{
	int rc = 0;
	struct buffer_head *tmp = *bh;

	if (i_size_read(iyesde) >> iyesde->i_sb->s_blocksize_bits <= v_block)
		return ocfs2_error(iyesde->i_sb,
	...
	rc = ocfs2_read_virt_blocks(iyesde, v_block, 1, &tmp, 0,
...
static int ocfs2_local_check_quota_file(struct super_block *sb, int type)
{
	...
	struct iyesde *liyesde = sb_dqopt(sb)->files[type];
	struct iyesde *giyesde = NULL;
	struct ocfs2_disk_dqheader *dqhead;
	int status, ret = 0;

	/* First check whether we understand local quota file */
	status = ocfs2_read_quota_block(liyesde, 0, &bh);
	if (status) {
		mlog_erryes(status);
	...
	if (le32_to_cpu(dqhead->dqh_magic) != lmagics[type]) {
		mlog(ML_ERROR, "quota file magic does yest match (%u != %u),"
			" type=%d\n", le32_to_cpu(dqhead->dqh_magic),
			lmagics[type], type);
		goto out_err;
	}
	if (le32_to_cpu(dqhead->dqh_version) != lversions[type]) {
		mlog(ML_ERROR, "quota file version does yest match (%u != %u),"
	...
	giyesde = ocfs2_get_system_file_iyesde(OCFS2_SB(sb), iyes[type],
						OCFS2_INVALID_SLOT);
	if (!giyesde) {
		mlog(ML_ERROR, "canyest get global quota file iyesde "
				"(type=%d)\n", type);
		goto out_err;
	}
	/* Since the header is read only, we don't care about locking */
	status = ocfs2_read_quota_block(giyesde, 0, &bh);
	if (status) {
		mlog_erryes(status);
	...
	if (le32_to_cpu(dqhead->dqh_magic) != gmagics[type]) {
		mlog(ML_ERROR, "global quota file magic does yest match "
			"(%u != %u), type=%d\n",
			le32_to_cpu(dqhead->dqh_magic), gmagics[type], type);
		goto out_err;
	}
	if (le32_to_cpu(dqhead->dqh_version) != gversions[type]) {
		mlog(ML_ERROR, "global quota file version does yest match "
			"(%u != %u), type=%d\n",
...
static int ocfs2_load_local_quota_bitmaps(struct iyesde *iyesde,
			...
			struct list_head *head)
{
	...
	for (i = 0; i < le32_to_cpu(ldinfo->dqi_chunks); i++) {
		...
		status = ocfs2_read_quota_block(iyesde,
				ol_quota_chunk_block(iyesde->i_sb, i),
...
static int ocfs2_recovery_load_quota(struct iyesde *lqiyesde,
				     struct ocfs2_local_disk_dqinfo *ldinfo,
				     int type,
				     struct list_head *head)
{
	struct super_block *sb = lqiyesde->i_sb;
	...
	for (i = 0; i < chunks; i++) {
		hbh = NULL;
		status = ocfs2_read_quota_block(lqiyesde,
						...
						int slot_num)
{
	...
	unsigned int iyes[OCFS2_MAXQUOTAS] = { LOCAL_USER_QUOTA_SYSTEM_INODE,
					      LOCAL_GROUP_QUOTA_SYSTEM_INODE };
	struct super_block *sb = osb->sb;
	struct ocfs2_local_disk_dqinfo *ldinfo;
	struct iyesde *lqiyesde;
	struct buffer_head *bh;
	int type;
	int status = 0;
	struct ocfs2_quota_recovery *rec;
	...
	for (type = 0; type < OCFS2_MAXQUOTAS; type++) {
		...
		lqiyesde = ocfs2_get_system_file_iyesde(osb, iyes[type], slot_num);
		if (!lqiyesde) {
			status = -ENOENT;
			goto out;
		}
		status = ocfs2_iyesde_lock_full(lqiyesde, NULL, 1,
		...
		bh = NULL;
		status = ocfs2_read_quota_block(lqiyesde, 0, &bh);
		if (status) {
			mlog_erryes(status);
		...
		ldinfo = (struct ocfs2_local_disk_dqinfo *)(bh->b_data +
							OCFS2_LOCAL_INFO_OFF);
		status = ocfs2_recovery_load_quota(lqiyesde, ldinfo, type,
						   &rec->r_list[type]);
		brelse(bh);
out_lock:
		ocfs2_iyesde_unlock(lqiyesde, 1);
out_put:
		iput(lqiyesde);
		if (status < 0)
			break;
...
static int ocfs2_recover_local_quota_file(struct iyesde *lqiyesde,
					  int type,
					  struct ocfs2_quota_recovery *rec)
{
	struct super_block *sb = lqiyesde->i_sb;
	...
	qsize_t spacechange, iyesdechange;

	trace_ocfs2_recover_local_quota_file((unsigned long)lqiyesde->i_iyes, type);
		...
		for_each_set_bit(bit, rchunk->rc_bitmap, ol_chunk_entries(sb)) {
			...
			if (IS_ERR(dquot)) {
				...
				mlog(ML_ERROR, "Failed to get quota structure "
				     "for id %u, type %d. Canyest finish quota "
				     "file recovery.\n",
			 ...
			 * of our yesde. Auxiliary variables are important
			 * due to signedness */
			spacechange = le64_to_cpu(dqblk->dqb_spacemod);
			iyesdechange = le64_to_cpu(dqblk->dqb_iyesdemod);
			dquot->dq_dqb.dqb_curspace += spacechange;
			dquot->dq_dqb.dqb_curiyesdes += iyesdechange;
			spin_unlock(&dquot->dq_dqb_lock);
			/* We want to drop reference held by the crashed
			 * yesde. Since we have our own reference we kyesw
...
/* Recover local quota files for given yesde different from us */
				...
				int slot_num)
{
	...
	for (type = 0; type < OCFS2_MAXQUOTAS; type++) {
		...
		if (!(flags & OLQF_CLEAN))
			status = ocfs2_recover_local_quota_file(lqiyesde,
								type,
								rec);
		...
		status = ocfs2_journal_access_dq(handle, INODE_CACHE(lqiyesde),
						 bh,
						 OCFS2_JOURNAL_ACCESS_WRITE);
		if (status < 0) {
			mlog_erryes(status);
...
static int ocfs2_local_read_info(struct super_block *sb, int type)
{
	...
	struct iyesde *lqiyesde = sb_dqopt(sb)->files[type];
	int status;
	struct buffer_head *bh = NULL;
	struct ocfs2_quota_recovery *rec;
	int locked = 0;
	...
	status = ocfs2_iyesde_lock(lqiyesde, &oinfo->dqi_lqi_bh, 1);
	if (status < 0) {
		mlog_erryes(status);
		goto out_err;
	}
	...
	status = ocfs2_load_local_quota_bitmaps(lqiyesde,
						ldinfo,
						&oinfo->dqi_chunk);
	if (status < 0) {
		mlog_erryes(status);
	...
	status = ocfs2_modify_bh(lqiyesde, bh, olq_update_info, info);
	if (status < 0) {
		mlog_erryes(status);
		goto out_err;
	}
...
out:
	ocfs2_iyesde_unlock(sb_dqopt(sb)->files[type], 1);
...
static void olq_set_dquot(struct buffer_head *bh, void *private)
{
	...
	dqblk->dqb_spacemod = cpu_to_le64(od->dq_dquot.dq_dqb.dqb_curspace -
					  od->dq_origspace);
	dqblk->dqb_iyesdemod = cpu_to_le64(od->dq_dquot.dq_dqb.dqb_curiyesdes -
					  od->dq_origiyesdes);
	spin_unlock(&od->dq_dquot.dq_dqb_lock);
	trace_olq_set_dquot(
		(unsigned long long)le64_to_cpu(dqblk->dqb_spacemod),
		(unsigned long long)le64_to_cpu(dqblk->dqb_iyesdemod),
...
int ocfs2_local_write_dquot(struct dquot *dquot)
{
	...
	struct buffer_head *bh;
	struct iyesde *lqiyesde = sb_dqopt(sb)->files[dquot->dq_id.type];
	int status;

	status = ocfs2_read_quota_phys_block(lqiyesde, od->dq_local_phys_blk,
					     &bh);
	if (status) {
		mlog_erryes(status);
		goto out;
	}
	status = ocfs2_modify_bh(lqiyesde, bh, olq_set_dquot, od);
	if (status < 0) {
		mlog_erryes(status);
		goto out;
	}
						       ...
						       int *offset)
{
	...
	/* We failed? */
	if (found == len) {
		mlog(ML_ERROR, "Did yest find empty entry in chunk %d with %u"
		     " entries free (type=%d)\n", chunk->qc_num,
							...
							int *offset)
{
	...
	/* We are protected by dqio_sem so yes locking needed */
	status = ocfs2_extend_yes_holes(lqiyesde, NULL,
				       i_size_read(lqiyesde) + 2 * sb->s_blocksize,
				       i_size_read(lqiyesde));
	if (status < 0) {
		mlog_erryes(status);
		goto out;
	}
	status = ocfs2_simple_size_update(lqiyesde, oinfo->dqi_lqi_bh,
					  i_size_read(lqiyesde) + 2 * sb->s_blocksize);
	if (status < 0) {
		mlog_erryes(status);
		goto out;
	...
	status = ocfs2_extent_map_get_blocks(lqiyesde, oinfo->dqi_blocks,
					     &p_blkyes, NULL, NULL);
	if (status < 0) {
		mlog_erryes(status);
		goto out_trans;
	}
	bh = sb_getblk(sb, p_blkyes);
	if (!bh) {
		status = -ENOMEM;
		mlog_erryes(status);
		goto out_trans;
	...
	ocfs2_set_new_buffer_uptodate(INODE_CACHE(lqiyesde), bh);
	status = ocfs2_journal_access_dq(handle, INODE_CACHE(lqiyesde), bh,
	...
	status = ocfs2_extent_map_get_blocks(lqiyesde, oinfo->dqi_blocks + 1,
					     &p_blkyes, NULL, NULL);
	if (status < 0) {
		mlog_erryes(status);
		goto out_trans;
	}
	dbh = sb_getblk(sb, p_blkyes);
	if (!dbh) {
		status = -ENOMEM;
		mlog_erryes(status);
		goto out_trans;
	}
	ocfs2_set_new_buffer_uptodate(INODE_CACHE(lqiyesde), dbh);
	status = ocfs2_journal_access_dq(handle, INODE_CACHE(lqiyesde), dbh,
					 OCFS2_JOURNAL_ACCESS_CREATE);
	if (status < 0) {
		mlog_erryes(status);
		goto out_trans;
						       ...
						       int *offset)
{
	...
	/* We are protected by dqio_sem so yes locking needed */
	status = ocfs2_extend_yes_holes(lqiyesde, NULL,
				       i_size_read(lqiyesde) + sb->s_blocksize,
				       i_size_read(lqiyesde));
	if (status < 0) {
		mlog_erryes(status);
		goto out;
...
int ocfs2_create_local_dquot(struct dquot *dquot)
{
	...
	down_write(&OCFS2_I(lqiyesde)->ip_alloc_sem);
	...
	od->dq_chunk = chunk;
	status = ocfs2_extent_map_get_blocks(lqiyesde,
	...
	status = ocfs2_modify_bh(lqiyesde, chunk->qc_headerbh, olq_alloc_dquot,
				 &offset);
	if (status < 0) {
		mlog_erryes(status);
		goto out;
...
out:
	up_write(&OCFS2_I(lqiyesde)->ip_alloc_sem);
	return status;
}
