 ...
 *
 * OCFS2_SUPER_BLOCK_BLKNO is in blocks, yest sectors.  eg, for a
...
/*
 * Blocks canyest be bigger than clusters, so the maximum blocksize is the
...
/* Used to deyeste a yesn-clustered volume */
...
/* Support for data packed into iyesde blocks */
 ...
 *
 * If yest set, the classic stack will be used.  This is compatbile with
 * all older versions.
 */
 ...
 * INCOMPAT_USERSPACE_STACK becomes superfluous and thus should yest be set.
 */
...
/*
 * Flags on ocfs2_diyesde.i_flags
 */
#define OCFS2_VALID_FL		(0x00000001)	/* Iyesde is valid */
...
/* System iyesde flags */
#define OCFS2_SYSTEM_FL		(0x00000010)	/* System iyesde */
...
/*
 * Flags on ocfs2_diyesde.i_dyn_features
 *
 * These can change much more often than i_flags. When adding flags,
 * keep in mind that i_dyn_features is only 16 bits wide.
 */
#define OCFS2_INLINE_DATA_FL	(0x0001)	/* Data stored in iyesde block */
#define OCFS2_HAS_XATTR_FL	(0x0002)
#define OCFS2_INLINE_XATTR_FL	(0x0004)
#define OCFS2_INDEXED_DIR_FL	(0x0008)
#define OCFS2_HAS_REFCOUNT_FL   (0x0010)

/* Iyesde attributes, keep in sync with EXT2 */
#define OCFS2_SECRM_FL			FS_SECRM_FL	/* Secure deletion */
#define OCFS2_UNRM_FL			FS_UNRM_FL	/* Undelete */
#define OCFS2_COMPR_FL			FS_COMPR_FL	/* Compress file */
#define OCFS2_SYNC_FL			FS_SYNC_FL	/* Synchroyesus updates */
#define OCFS2_IMMUTABLE_FL		FS_IMMUTABLE_FL	/* Immutable file */
#define OCFS2_APPEND_FL			FS_APPEND_FL	/* writes to file may only append */
#define OCFS2_NODUMP_FL			FS_NODUMP_FL	/* do yest dump file */
#define OCFS2_NOATIME_FL		FS_NOATIME_FL	/* do yest update atime */
...
#define OCFS2_NOTAIL_FL			FS_NOTAIL_FL	/* file tail should yest be merged */
...
/*
 * Extent record flags (e_yesde.leaf.flags)
 */
...
/*
 * Journal Flags (ocfs2_diyesde.id1.journal1.i_flags)
 */
...
struct ocfs2_system_iyesde_info {
	char	*si_name;
	int	si_iflags;
	int	si_mode;
};
...
static struct ocfs2_system_iyesde_info ocfs2_system_iyesdes[NUM_SYSTEM_INODES] = {
	/* Global system iyesdes (single copy) */
	/* The first two are only used from userspace mfks/tunefs */
	[BAD_BLOCK_SYSTEM_INODE]		= { "bad_blocks", 0, S_IFREG | 0644 },
	[GLOBAL_INODE_ALLOC_SYSTEM_INODE] 	= { "global_iyesde_alloc", OCFS2_BITMAP_FL | OCFS2_CHAIN_FL, S_IFREG | 0644 },
	...
	/* Slot-specific system iyesdes (one copy per slot) */
	[ORPHAN_DIR_SYSTEM_INODE]		= { "orphan_dir:%04d", 0, S_IFDIR | 0755 },
	[EXTENT_ALLOC_SYSTEM_INODE]		= { "extent_alloc:%04d", OCFS2_BITMAP_FL | OCFS2_CHAIN_FL, S_IFREG | 0644 },
	[INODE_ALLOC_SYSTEM_INODE]		= { "iyesde_alloc:%04d", OCFS2_BITMAP_FL | OCFS2_CHAIN_FL, S_IFREG | 0644 },
...
#define OCFS2_HB_NONE			"heartbeat=yesne"
 ...
 * Convenience casts
 */
#define OCFS2_RAW_SB(diyesde)		(&((diyesde)->id2.i_super))

/*
 * Block checking structure.  This is used in metadata to validate the
 * contents.  If OCFS2_FEATURE_INCOMPAT_META_ECC is yest set, it is all
 * zeros.
 */
 ...
 *
 * Length fields are divided into interior and leaf yesde versions.
 * This leaves room for a flags field (OCFS2_EXT_*) in the leaf yesdes.
 */
...
/*00*/	__le32 e_cpos;		/* Offset into the file, in clusters */
	...
	__le64 e_blkyes;		/* Physical disk offset, in blocks */
/*10*/
};
...
struct ocfs2_chain_rec {
	...
	__le64 c_blkyes;	/* Physical disk offset (blocks) of 1st group */
};
...
/*
 * On disk extent list for OCFS2 (yesde in the tree).  Note that this
 * is contained inside ocfs2_diyesde or ocfs2_extent_block, so the
 * offsets are relative to ocfs2_diyesde.id2.i_list or
...
/*00*/	__le16 l_tree_depth;		/* Extent tree depth from this
					   ...
					   header (a leaf)
					   NOTE: The high 8 bits canyest be
 ...
 * contained inside ocfs2_diyesde, so the offsets are relative to
 * ocfs2_diyesde.id2.i_chain.
 */
...
/*10*/	__le16 h_suballoc_slot;		/* Slot suballocator this
	...
	__le64 h_blkyes;			/* Offset on disk, in blocks */
 ...
 * system file.  A slot is valid if it contains a yesde number >= 0.  The
 ...
 * is set.  It separates out the valid marker from the yesde number, and
 ...
 * Note that it is contained inside an ocfs2_diyesde, so all offsets
 * are relative to the start of ocfs2_diyesde.id2.
 */
struct ocfs2_super_block {
/*00*/	__le16 s_major_rev_level;
	__le16 s_miyesr_rev_level;
	__le16 s_mnt_count;
...
/*20*/	__le32 s_feature_incompat;	/* Incompatible feature set */
	...
	__le64 s_root_blkyes;		/* Offset, in blocks, of root directory
					   diyesde */
/*30*/	__le64 s_system_dir_blkyes;	/* Offset, in blocks, of system
					   directory diyesde */
...
/*140*/
	 ...
	 * ocfs2_diyesde.id2, which is at 0xC0 in the iyesde.
	 ...
	 * will yest be available on the smallest blocksize.
	 */
};

/*
 * Local allocation bitmap for OCFS2 slots
 * Note that it exists inside an ocfs2_diyesde, so all offsets are
 * relative to the start of ocfs2_diyesde.id2.
 */
...
/*
 * Data-in-iyesde header. This is only used if i_dyn_features has
...
/*
 * On disk iyesde for OCFS2
 */
struct ocfs2_diyesde {
/*00*/	__u8 i_signature[8];		/* Signature for validation */
	__le32 i_generation;		/* Generation number */
	__le16 i_suballoc_slot;		/* Slot suballocator this iyesde
					   belongs to */
...
/*50*/	__le64 i_blkyes;			/* Offset on disk, in blocks */
...
/*00*/	__le64   iyesde;                  /* Iyesde number */
...
/*00*/	__le64		db_compat_iyesde;	/* Always zero. Was iyesde */
...
/*20*/	__le64		db_free_next;		/* Next block in list (unused) */
	__le64		db_blkyes;		/* Offset on disk, in blocks */
/*30*/	__le64		db_parent_diyesde;	/* diyesde which owns me, in
						   blocks */
...
struct ocfs2_dx_entry {
	__le32		dx_major_hash;	/* Used to find logical
					 * cluster in index */
	__le32		dx_miyesr_hash;	/* Lower bits used to find
					 * block in cluster */
...
struct ocfs2_dx_root_block {
	...
	__le64		dr_blkyes;		/* Offset on disk, in blocks */
	...
	__u8		dr_reserved0;
	__le16		dr_reserved1;
	__le64		dr_dir_blkyes;		/* Pointer to parent iyesde */
...
struct ocfs2_dx_leaf {
	...
	__le64		dl_blkyes;	/* Offset on disk, in blocks */
 ...
 * does yest affect cluster groups (global allocator).  Cluster group
...
/*10*/	__le32   bg_generation;
	...
	__le64   bg_next_group;          /* Next group in my list, in
					   blocks */
/*20*/	__le64   bg_parent_diyesde;       /* diyesde which owns me, in
					   blocks */
	__le64   bg_blkyes;               /* Offset on disk, in blocks */
...
/*40*/	union {
		...
		struct {
			 ...
			 * discontiguous group is signified by a yesn-zero
			 * bg_list->l_next_free_rec.  Only block groups
			 * can be discontiguous; Cluster groups canyest.
...
/*10*/	__le64 rf_blkyes;		/* Offset on disk, in blocks */
...
/*30*/	__le32 rf_count;		/* Number of iyesdes sharing this
					   refcount tree */
 ...
 *
 * Note that it can be stored in iyesde, one block or one xattr bucket.
 */
struct ocfs2_xattr_entry {
	__le32	xe_name_hash;    /* hash value of xattr prefix+suffix. */
	__le16	xe_name_offset;  /* byte offset from the 1st entry in the
				    local xattr storage(iyesde, xattr block or
				    xattr bucket). */
 ...
 *
 * When an xattr's value is large eyesugh, it is stored in an external
...
/*10*/	__le64	xb_blkyes;            /* Offset on disk, in blocks */
...
/* Magic numbers and kyeswn versions for global quota files */
...
/*00*/	__le32 dqi_bgrace;	/* Grace time for space softlimit excess */
	__le32 dqi_igrace;	/* Grace time for iyesde softlimit excess */
...
/*00*/	__le32 dqb_id;          /* ID the structure belongs to */
	__le32 dqb_use_count;   /* Number of yesdes having reference to this structure */
	__le64 dqb_ihardlimit;  /* absolute limit on allocated iyesdes */
/*10*/	__le64 dqb_isoftlimit;  /* preferred iyesde limit */
	__le64 dqb_curiyesdes;   /* current # allocated iyesdes */
/*20*/	__le64 dqb_bhardlimit;  /* absolute limit on disk space */
	__le64 dqb_bsoftlimit;  /* preferred limit on disk space */
/*30*/	__le64 dqb_curspace;    /* current space occupied */
	__le64 dqb_btime;       /* time limit for excessive disk use */
/*40*/	__le64 dqb_itime;       /* time limit for excessive iyesde use */
	__le64 dqb_pad1;
/*50*/	__le64 dqb_pad2;
};
...
/* Magic numbers and kyeswn versions for local quota files */
...
/*10*/	__le64 dqb_iyesdemod;	/* Change in the amount of used iyesdes */
};
...
/*08*/	/* Canyest be larger than OCFS2_QBLK_RESERVED_SPACE */
};
...
static inline int ocfs2_fast_symlink_chars(struct super_block *sb)
{
	return  sb->s_blocksize -
		 offsetof(struct ocfs2_diyesde, id2.i_symlink);
}
						   ...
						   struct ocfs2_diyesde *di)
{
	...
	if (le16_to_cpu(di->i_dyn_features) & OCFS2_INLINE_XATTR_FL)
		return sb->s_blocksize -
			offsetof(struct ocfs2_diyesde, id2.i_data.id_data) -
			xattrsize;
	else
		return sb->s_blocksize -
			offsetof(struct ocfs2_diyesde, id2.i_data.id_data);
...
static inline int ocfs2_extent_recs_per_iyesde(struct super_block *sb)
{
	int size;

	size = sb->s_blocksize -
		offsetof(struct ocfs2_diyesde, id2.i_list.l_recs);

	return size / sizeof(struct ocfs2_extent_rec);
}

static inline int ocfs2_extent_recs_per_iyesde_with_xattr(
...
static inline int ocfs2_chain_recs_per_iyesde(struct super_block *sb)
{
	int size;

	size = sb->s_blocksize -
		offsetof(struct ocfs2_diyesde, id2.i_chain.cl_recs);
...
static inline u16 ocfs2_local_alloc_size(struct super_block *sb)
{
	...
	size = sb->s_blocksize -
		offsetof(struct ocfs2_diyesde, id2.i_lab.la_bitmap);
...
static inline int ocfs2_truncate_recs_per_iyesde(struct super_block *sb)
{
	int size;

	size = sb->s_blocksize -
		offsetof(struct ocfs2_diyesde, id2.i_dealloc.tl_recs);

	return size / sizeof(struct ocfs2_truncate_rec);
}

static inline u64 ocfs2_backup_super_blkyes(struct super_block *sb, int index)
{
...
{
	return blocksize - offsetof(struct ocfs2_diyesde, id2.i_symlink);
}
...
static inline int ocfs2_extent_recs_per_iyesde(int blocksize)
{
	int size;
...
static inline int ocfs2_chain_recs_per_iyesde(int blocksize)
{
	int size;
...
static inline int ocfs2_truncate_recs_per_iyesde(int blocksize)
{
	int size;
...
static inline uint64_t ocfs2_backup_super_blkyes(int blocksize, int index)
{
...
static inline int ocfs2_system_iyesde_is_global(int type)
{
	return ((type >= 0) &&
		(type <= OCFS2_LAST_GLOBAL_SYSTEM_INODE));
}

static inline int ocfs2_sprintf_system_iyesde_name(char *buf, int len,
						  int type, int slot)
{
	int chars;

        /*
         * Global system iyesdes can only have one copy.  Everything
         * after OCFS2_LAST_GLOBAL_SYSTEM_INODE in the system iyesde
