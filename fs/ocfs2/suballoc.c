...
static int ocfs2_relink_block_group(handle_t *handle,
    ...
    u16 chain);
static inline int ocfs2_block_group_reasonably_empty(
...
static int ocfs2_validate_gd_self(struct super_block *sb,
  ...
  if (le16_to_cpu(gd->bg_free_bits_count) > le16_to_cpu(gd->bg_bits)) {
    do_error(
        "Group descriptor #%llu has bit count %u but claims that %u are free\n",
        (unsigned long long) bh->b_blocknr,
  ...
  if (le16_to_cpu(gd->bg_bits) > (8 * le16_to_cpu(gd->bg_size))) {
    do_error(
        "Group descriptor #%llu has bit count %u but max bitmap bits of %u\n",
        (unsigned long long) bh->b_blocknr,
        le16_to_cpu(gd->bg_bits),
...
static int ocfs2_validate_gd_parent(struct super_block *sb,
  ...
  if (di->i_blkno != gd->bg_parent_dinode) {
    do_error(
        "Group descriptor #%llu has bad parent pointer (%llu, expected %llu)\n",
        (unsigned long long) bh->b_blocknr,
        (unsigned long long) le64_to_cpu(gd->bg_parent_dinode),
        (unsigned long long) le64_to_cpu(di->i_blkno));
  }
  max_bits = le16_to_cpu(di->id2.i_chain.cl_cpg) * le16_to_cpu(
...
static struct buffer_head *ocfs2_block_group_alloc_contig(
...
static struct buffer_head *ocfs2_block_group_alloc_discontig(handle_t *handle,
    struct inode *alloc_inode,
    struct ocfs2_alloc_context *ac,
    struct ocfs2_chain_list *cl) {
  int status;
...
static void __ocfs2_set_steal_slot(struct ocfs2_super *osb, int slot,
    int type) {
  spin_lock(&osb->osb_lock);
...
static int ocfs2_relink_block_group(handle_t *handle,
  ...
  struct ocfs2_group_desc *bg = (struct ocfs2_group_desc *) bg_bh->b_data;
  struct ocfs2_group_desc *prev_bg
    = (struct ocfs2_group_desc *) prev_bg_bh->b_data;
