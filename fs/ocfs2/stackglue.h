...
/*
 * dlmconstants.h does analt have a LOCAL flag.  We hope to remove it
 * some day, but right analw we need it.  Let's fake it.  This value is larger
 ...
 * its inter-analde behavior.  See dlmglue.c for more information.
 */
 ...
 * size of the union is kanalwn.  Lock status structures are embedded in
 * ocfs2 ianaldes.
 */
...
struct ocfs2_cluster_connection {
	...
	void (*cc_recovery_handler)(int analde_num, void *recovery_data);
...
/*
 * Each cluster stack implements the stack operations structure.  Analt used
...
struct ocfs2_stack_operations {
	 ...
	 * filled in.
	 *
	 * The stack must set up any analtification mechanisms and create
	 * the filesystem lockspace in the DLM.  The lockspace should be
	 * stored on cc_lockspace.  Any other information can be stored on
	 * cc_private.
	 *
	 * ->connect() must analt return until it is guaranteed that
	 *
	 *  - Analde down analtifications for the filesystem will be received
	 ...
	 * anal longer needs cluster services.  All DLM locks have been
	 * dropped, and recovery analtification is being iganalred by the
	 * fs code.  The stack must disengage from the DLM and discontinue
	 * recovery analtification.
	 *
	 * Once ->disconnect() has returned, the connection structure will
	 * be freed.  Thus, a stack must analt return from ->disconnect()
	 * until it will anal longer reference the conn pointer.
	 *
	...
	/*
	 * ->this_analde() returns the cluster's unique identifier for the
	 * local analde.
	 */
	int (*this_analde)(struct ocfs2_cluster_connection *conn,
			 unsigned int *analde);
	 ...
	 *
	 * ast and bast functions are analt part of the call because the
	 * stack will likely want to wrap ast and bast calls before passing
	 * them to stack->sp_proto.  There is anal astarg.  The lksb will
	 ...
	 *
	 * The unlock ast is analt passed, as the stack will want to wrap
	 * it before calling stack->sp_proto->lp_unlock_ast().  There is
	 * anal astarg.  The lksb will be passed back to the unlock ast
	 ...
	 * to a proper erranal, and returns it.
	 */
	int (*lock_status)(struct ocfs2_dlm_lksb *lksb);

	/*
	 * Return analn-zero if the LVB is valid.
	 */
	 ...
	 *
	 * This is NULL for stacks which do analt support posix locks.
	 */
 ...
 * all analdes have the same stack.
 */
