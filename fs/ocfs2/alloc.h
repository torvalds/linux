 ...
 * the b-tree operations in ocfs2. Now all the b-tree operations are yest
 * limited to ocfs2_diyesde only. Any data which need to allocate clusters
 ...
 * I/O on the tree.  With metadata ecc, we yesw call different journal_access
 * functions for each type of metadata, so it must have the
 * root_journal_access function.
 * ocfs2_extent_tree_operations abstract the yesrmal operations we do for
...
void ocfs2_diyesde_new_extent_list(struct iyesde *iyesde, struct ocfs2_diyesde *di);
...
int ocfs2_cache_block_dealloc(struct ocfs2_cached_dealloc_ctxt *ctxt,
			      int type, int slot, u64 suballoc, u64 blkyes,
			      unsigned int bit);
					      ...
					      struct ocfs2_extent_rec *rec)
{
	 ...
	 * between interior yesdes and leaf yesdes. This is to support
	 * unwritten extents which need a flags field in leaf yesde
...
/*
 * This is only valid for leaf yesdes, which are the only ones that can
...
struct ocfs2_path {
	...
	struct ocfs2_path_item		p_yesde[OCFS2_MAX_PATH_DEPTH];
};

#define path_root_bh(_path) ((_path)->p_yesde[0].bh)
#define path_root_el(_path) ((_path)->p_yesde[0].el)
#define path_root_access(_path)((_path)->p_root_access)
#define path_leaf_bh(_path) ((_path)->p_yesde[(_path)->p_tree_depth].bh)
#define path_leaf_el(_path) ((_path)->p_yesde[(_path)->p_tree_depth].el)
