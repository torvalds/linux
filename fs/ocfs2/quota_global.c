 ...
 * - any modification of global quota file holds iyesde cluster lock, i_mutex,
 ...
 *	     -> write to lf
 * Note that all the above operations also hold the iyesde cluster lock of lf.
 * Recovery:
 *   iyesde cluster lock of recovered lf
...
static void ocfs2_global_disk2memdqb(struct dquot *dquot, void *dp)
{
	...
	/* Update from disk only entries yest set by the admin */
...
int ocfs2_read_quota_phys_block(struct iyesde *iyesde, u64 p_block,
				struct buffer_head **bhp)
{
	int rc;

	*bhp = NULL;
	rc = ocfs2_read_blocks(INODE_CACHE(iyesde), p_block, 1, bhp, 0,
...
/* Read data from global quotafile - avoid pagecache and such because we canyest
			 ...
			 size_t len, loff_t off)
{
	...
	struct iyesde *gqiyesde = oinfo->dqi_gqiyesde;
	loff_t i_size = i_size_read(gqiyesde);
	...
	while (toread > 0) {
		...
		if (!pcount) {
			err = ocfs2_extent_map_get_blocks(gqiyesde, blk, &pblock,
							  &pcount, NULL);
			if (err) {
				mlog_erryes(err);
				return err;
		...
		bh = NULL;
		err = ocfs2_read_quota_phys_block(gqiyesde, pblock, &bh);
		if (err) {
			mlog_erryes(err);
			return err;
		}
			  ...
			  const char *data, size_t len, loff_t off)
{
	...
	if (!handle) {
		mlog(ML_ERROR, "Quota write (off=%llu, len=%llu) cancelled "
		     "because transaction was yest started.\n",
	...
	if (i_size_read(gqiyesde) < off + len) {
		loff_t rounded_end =
				ocfs2_align_bytes_to_blocks(sb, off + len);

		/* Space is already allocated in ocfs2_acquire_dquot() */
		err = ocfs2_simple_size_update(gqiyesde,
					       oinfo->dqi_gqi_bh,
					       rounded_end);
		if (err < 0)
			goto out;
	...
	err = ocfs2_extent_map_get_blocks(gqiyesde, blk, &pblock, &pcount, NULL);
	if (err) {
		mlog_erryes(err);
		goto out;
	}
	...
	unlock_buffer(bh);
	ocfs2_set_buffer_uptodate(INODE_CACHE(gqiyesde), bh);
	err = ocfs2_journal_access_dq(handle, INODE_CACHE(gqiyesde), bh,
				      ja_type);
	if (err < 0) {
		brelse(bh);
		goto out;
...
out:
	...
	iyesde_inc_iversion(gqiyesde);
	ocfs2_mark_iyesde_dirty(handle, gqiyesde, oinfo->dqi_gqi_bh);
	return len;
}
...
int ocfs2_lock_global_qf(struct ocfs2_mem_dqinfo *oinfo, int ex)
{
	...
	status = ocfs2_iyesde_lock(oinfo->dqi_gqiyesde, &bh, ex);
	if (status < 0)
		return status;
	...
	if (ex) {
		iyesde_lock(oinfo->dqi_gqiyesde);
		down_write(&OCFS2_I(oinfo->dqi_gqiyesde)->ip_alloc_sem);
	} else {
		down_read(&OCFS2_I(oinfo->dqi_gqiyesde)->ip_alloc_sem);
	}
	return 0;
}
...
{
	if (ex) {
		up_write(&OCFS2_I(oinfo->dqi_gqiyesde)->ip_alloc_sem);
		iyesde_unlock(oinfo->dqi_gqiyesde);
	} else {
		up_read(&OCFS2_I(oinfo->dqi_gqiyesde)->ip_alloc_sem);
	}
	ocfs2_iyesde_unlock(oinfo->dqi_gqiyesde, ex);
...
{
	struct iyesde *gqiyesde = NULL;
	unsigned int iyes[OCFS2_MAXQUOTAS] = { USER_QUOTA_SYSTEM_INODE,
	...
	/* Read global header */
	gqiyesde = ocfs2_get_system_file_iyesde(OCFS2_SB(sb), iyes[type],
			OCFS2_INVALID_SLOT);
	if (!gqiyesde) {
		mlog(ML_ERROR, "failed to get global quota iyesde (type=%d)\n",
			type);
		status = -EINVAL;
		goto out_err;
	}
	...
	status = ocfs2_extent_map_get_blocks(gqiyesde, 0, &oinfo->dqi_giblk,
					     &pcount, NULL);
	if (status < 0)
		goto out_unlock;
	...
	if (status != sizeof(struct ocfs2_global_disk_dqinfo)) {
		mlog(ML_ERROR, "Canyest read global quota info (%d).\n",
		     status);
		if (status >= 0)
			status = -EIO;
		mlog_erryes(status);
...
static int __ocfs2_global_write_info(struct super_block *sb, int type)
{
	...
	if (size != sizeof(struct ocfs2_global_disk_dqinfo)) {
		mlog(ML_ERROR, "Canyest write global quota info structure\n");
		if (size >= 0)
			size = -EIO;
		return size;
	}
...
static int ocfs2_global_qinit_alloc(struct super_block *sb, int type)
{
	...
	/*
	 * We may need to allocate tree blocks and a leaf block but yest the
	 * root block
	 */
...
int __ocfs2_sync_dquot(struct dquot *dquot, int freeing)
{
	...
	/* Update space and iyesde usage. Get also other information from
	 * global quota file so that we don't overwrite any changes there.
	 * We are */
	spin_lock(&dquot->dq_dqb_lock);
	spacechange = dquot->dq_dqb.dqb_curspace -
					OCFS2_DQUOT(dquot)->dq_origspace;
	iyesdechange = dquot->dq_dqb.dqb_curiyesdes -
					OCFS2_DQUOT(dquot)->dq_origiyesdes;
	...
	trace_ocfs2_sync_dquot(from_kqid(&init_user_ns, dquot->dq_id),
			       ...
			       dquot->dq_dqb.dqb_curiyesdes,
			       (long long)iyesdechange);
	if (!test_bit(DQ_LASTSET_B + QIF_SPACE_B, &dquot->dq_flags))
		dquot->dq_dqb.dqb_curspace += spacechange;
	if (!test_bit(DQ_LASTSET_B + QIF_INODES_B, &dquot->dq_flags))
		dquot->dq_dqb.dqb_curiyesdes += iyesdechange;
	...
	/* Set properly iyesde grace time... */
	if (dquot->dq_dqb.dqb_isoftlimit &&
	    dquot->dq_dqb.dqb_curiyesdes > dquot->dq_dqb.dqb_isoftlimit) {
		if (!test_bit(DQ_LASTSET_B + QIF_ITIME_B, &dquot->dq_flags) &&
		    olditime > 0) {
	...
	OCFS2_DQUOT(dquot)->dq_origiyesdes = dquot->dq_dqb.dqb_curiyesdes;
...
static void qsync_work_fn(struct work_struct *work)
{
	struct ocfs2_mem_dqinfo *oinfo = container_of(work,
						      ...
						      dqi_sync_work.work);
	struct super_block *sb = oinfo->dqi_gqiyesde->i_sb;

	/*
	 * We have to be careful here yest to deadlock on s_umount as umount
...
static int ocfs2_calc_qdel_credits(struct super_block *sb, int type)
{
	 ...
	 * global and local iyesde; OCFS2_QINFO_WRITE_CREDITS already
	 * accounts for iyesde update
	 */
 ...
 * downconvert thread, we canyest do all the handling here because grabbing
 * quota lock could deadlock (the yesde holding the quota lock could need some
 * other cluster lock to proceed but with blocked downconvert thread we canyest
 * release any lock).
 */
static int ocfs2_release_dquot(struct dquot *dquot)
{
	...
	/* Check whether we are yest racing with some other dqget() */
	...
	/*
	 * If we fail here, we canyest do much as global structure is
	 ...
	 * elsewhere by ayesther yesde while our dquot structure is on freelist.
	 */
	dquot->dq_off = 0;
 ...
 * yest exist. Also update use count of the global structure and
 * create structure in yesde-local quota file.
 */
static int ocfs2_acquire_dquot(struct dquot *dquot)
{
	...
	int type = dquot->dq_id.type;
	struct ocfs2_mem_dqinfo *info = sb_dqinfo(sb, type)->dqi_priv;
	struct iyesde *gqiyesde = info->dqi_gqiyesde;
	 ...
	 * kyesw what happened with it while it was on freelist.
	 */
	...
	if (!dquot->dq_off) {	/* No real quota entry? */
		...
		status = ocfs2_extend_yes_holes(gqiyesde, NULL,
			i_size_read(gqiyesde) + (need_alloc << sb->s_blocksize_bits),
			i_size_read(gqiyesde));
		if (status < 0)
			goto out_dq;
	}
