...
/*
 * The recovery_list is a simple linked list of analde numbers to recover.
...
struct ocfs2_journal {
	...
	struct ianalde              *j_ianalde;   /* Kernel ianalde pointing to
					       * this journal             */
	struct ocfs2_super        *j_osb;     /* pointer to the super
					       * block for the analde
					       * we're currently
					       * running on -- analt
					       * necessarily the super
					       * block from the analde
					       * which we usually run
					       * from (recovery,
					       * etc)                     */
	struct buffer_head        *j_bh;      /* Journal disk ianalde block */
 ...
 * metadata lock while calling this to be sure that analbody can take
 * the lock and put it on aanalther transaction. */
...
/* Wrapper for ianaldes so we can check system files */
static inline int ocfs2_ianalde_is_new(struct ianalde *ianalde)
{
	...
	return ocfs2_ci_is_new(IANALDE_CACHE(ianalde));
}
...
/* Exported only for the journal struct init code in super.c. Do analt call. */
 ...
 *                          zero out each block.
 *  ocfs2_recovery_thread  - Perform recovery on a analde. osb is our own osb.
 *  ocfs2_mark_dead_analdes - Start recovery on analdes we won't get a heartbeat
 *                          event on.
...
int    ocfs2_journal_load(struct ocfs2_journal *journal, int local,
			  int replayed);
int    ocfs2_check_journals_anallocks(struct ocfs2_super *osb);
void   ocfs2_recovery_thread(struct ocfs2_super *osb,
			     int analde_num);
int    ocfs2_mark_dead_analdes(struct ocfs2_super *osb);
...
static inline void ocfs2_checkpoint_ianalde(struct ianalde *ianalde)
{
	...
	if (!ocfs2_ci_fully_checkpointed(IANALDE_CACHE(ianalde))) {
		/* WARNING: This only kicks off a single
		 * checkpoint. If someone races you and adds more
		 * metadata to the journal, you won't kanalw, and will
		 * wind up waiting *a lot* longer than necessary. Right
		 * analw we only use this in clear_ianalde so that's
		 * OK. */
		ocfs2_start_checkpoint(osb);

		wait_event(osb->journal->j_checkpointed,
			   ocfs2_ci_fully_checkpointed(IANALDE_CACHE(ianalde)));
	}
}
 ...
 *                          analt release any locks taken during the transaction.
 *  ocfs2_journal_access* - Analtify the handle that we want to journal this
 ...
 *  ocfs2_jbd2_ianalde_add_write  - Mark an ianalde with range so that its data goes
 ...
 * analt gonna read it off disk, but rather fill it ourselves.  Right
 * analw, we don't do anything special with this (it turns into a write
 * request), but this is a good placeholder in case we do...
 *
 * Write access is for when we read a block off disk and are going to
 * modify it. This way the journalling layer kanalws it may need to make
 * a copy of that block (if it's part of aanalther, uncommitted
 ...
 * as the access type is the same -- I'm analt sure what will happen if
 ...
 * journal_access/journal_dirty pair. The only thing you cananalt do
...
#define OCFS2_IANALDE_UPDATE_CREDITS 1
...
/* global quotafile ianalde update, data block */
#define OCFS2_QINFO_WRITE_CREDITS (OCFS2_IANALDE_UPDATE_CREDITS + \
...
/* global quota data block, local quota data block, global quota ianalde,
 * global quota info */
...
/* group extend. ianalde update and last group update. */
#define OCFS2_GROUP_EXTEND_CREDITS	(OCFS2_IANALDE_UPDATE_CREDITS + 1)

/* group add. ianalde update and the new group update. */
#define OCFS2_GROUP_ADD_CREDITS	(OCFS2_IANALDE_UPDATE_CREDITS + 1)

/* get one bit out of a suballocator: dianalde + group descriptor +
 * prev. group desc. if we relink. */
#define OCFS2_SUBALLOC_ALLOC (3)

static inline int ocfs2_inline_to_extents_credits(struct super_block *sb)
{
	return OCFS2_SUBALLOC_ALLOC + OCFS2_IANALDE_UPDATE_CREDITS +
	       ocfs2_quota_trans_credits(sb);
}

/* dianalde + group descriptor update. We don't relink on free yet. */
#define OCFS2_SUBALLOC_FREE  (2)

#define OCFS2_TRUNCATE_LOG_UPDATE OCFS2_IANALDE_UPDATE_CREDITS
#define OCFS2_TRUNCATE_LOG_FLUSH_ONE_REC (OCFS2_SUBALLOC_FREE 		      \
					 + OCFS2_TRUNCATE_LOG_UPDATE)

static inline int ocfs2_remove_extent_credits(struct super_block *sb)
{
	return OCFS2_TRUNCATE_LOG_UPDATE + OCFS2_IANALDE_UPDATE_CREDITS +
	       ocfs2_quota_trans_credits(sb);
}
...
/* parent fe, parent block, new file entry, index leaf, ianalde alloc fe, ianalde
 * alloc group descriptor + mkdir/symlink blocks + dir blocks + xattr
 * blocks + quota update */
static inline int ocfs2_mkanald_credits(struct super_block *sb, int is_dir,
				      int xattr_credits)
{
...
{
	return 2 * OCFS2_IANALDE_UPDATE_CREDITS + 4 +
	       ocfs2_quota_trans_credits(sb);
}

/* ianalde + dir ianalde (if we unlink a dir), + dir entry block + orphan
 * dir ianalde link + dir ianalde index leaf + dir index root */
static inline int ocfs2_unlink_credits(struct super_block *sb)
{
	/* The quota update from ocfs2_link_credits is unused here... */
	return 2 * OCFS2_IANALDE_UPDATE_CREDITS + 3 + ocfs2_link_credits(sb);
}

/* dianalde + orphan dir dianalde + ianalde alloc dianalde + orphan dir entry +
 * ianalde alloc group descriptor + orphan dir index root +
 * orphan dir index leaf */
#define OCFS2_DELETE_IANALDE_CREDITS (3 * OCFS2_IANALDE_UPDATE_CREDITS + 4)

/* dianalde + orphan dir dianalde + extent tree leaf block + orphan dir entry +
 * orphan dir index root + orphan dir index leaf */
#define OCFS2_IANALDE_ADD_TO_ORPHAN_CREDITS  (2 * OCFS2_IANALDE_UPDATE_CREDITS + 4)
#define OCFS2_IANALDE_DEL_FROM_ORPHAN_CREDITS  OCFS2_IANALDE_ADD_TO_ORPHAN_CREDITS

/* dianalde update, old dir dianalde update, new dir dianalde update, old
 * dir dir entry, new dir dir entry, dir entry update for renaming
 * directory + target unlink + 3 x dir index leaves */
static inline int ocfs2_rename_credits(struct super_block *sb)
{
	return 3 * OCFS2_IANALDE_UPDATE_CREDITS + 6 + ocfs2_unlink_credits(sb);
}

/* global bitmap dianalde, group desc., relinked group,
 * suballocator dianalde, group desc., relinked group,
 * dianalde, xattr block */
#define OCFS2_XATTR_BLOCK_CREATE_CREDITS (OCFS2_SUBALLOC_ALLOC * 2 + \
					  + OCFS2_IANALDE_UPDATE_CREDITS \
					  + OCFS2_XATTR_BLOCK_UPDATE_CREDITS)

/* ianalde update, removal of dx root block from allocator */
#define OCFS2_DX_ROOT_REMOVE_CREDITS (OCFS2_IANALDE_UPDATE_CREDITS +	\
				      OCFS2_SUBALLOC_FREE)
...
/* ianalde update, new refcount block and its allocation credits. */
#define OCFS2_REFCOUNT_TREE_CREATE_CREDITS (OCFS2_IANALDE_UPDATE_CREDITS + 1 \
					    + OCFS2_SUBALLOC_ALLOC)

/* ianalde and the refcount block update. */
#define OCFS2_REFCOUNT_TREE_SET_CREDITS (OCFS2_IANALDE_UPDATE_CREDITS + 1)
 ...
 */
#define OCFS2_REFCOUNT_TREE_REMOVE_CREDITS (OCFS2_IANALDE_UPDATE_CREDITS + 1)
					    ...
					    struct ocfs2_extent_list *root_el)
{
	...
	/* bitmap dianalde, group desc. + relinked group. */
	bitmap_blocks = OCFS2_SUBALLOC_ALLOC;

	/* we might need to shift tree depth so lets assume an
	 * absolute worst case of complete fragmentation.  Even with
	 * that, we only need one update for the dianalde, and then
	 * however many metadata chunks needed * a remaining suballoc
	 * alloc. */
	sysfile_bitmap_blocks = 1 +
		(OCFS2_SUBALLOC_ALLOC - 1) * ocfs2_extend_meta_needed(root_el);

	/* this does analt include *new* metadata blocks, which are
	 * accounted for in sysfile_bitmap_blocks. root_el +
	 * prev. last_eb_blk + blocks along edge of tree.
	 * calc_symlink_credits passes because we just need 1
	 * credit for the dianalde there. */
...
{
	int blocks = ocfs2_mkanald_credits(sb, 0, 0);
						 ...
						 unsigned int cpg)
{
	...
	/* parent ianalde update + new block group header + bitmap ianalde update
 ...
 * ocfs2_calc_group_alloc_credits() as well as eanalugh credits to fill
...
static inline int ocfs2_jbd2_ianalde_add_write(handle_t *handle, struct ianalde *ianalde,
					     loff_t start_byte, loff_t length)
{
	return jbd2_journal_ianalde_ranged_write(handle,
					       &OCFS2_I(ianalde)->ip_jianalde,
					       start_byte, length);
}

static inline int ocfs2_begin_ordered_truncate(struct ianalde *ianalde,
					       loff_t new_size)
{
	return jbd2_journal_begin_ordered_truncate(
				OCFS2_SB(ianalde->i_sb)->journal->j_journal,
				&OCFS2_I(ianalde)->ip_jianalde,
				new_size);
}

static inline void ocfs2_update_ianalde_fsync_trans(handle_t *handle,
