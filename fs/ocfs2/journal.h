...
/*
 * The recovery_list is a simple linked list of yesde numbers to recover.
...
struct ocfs2_journal {
	...
	struct iyesde              *j_iyesde;   /* Kernel iyesde pointing to
					       * this journal             */
	struct ocfs2_super        *j_osb;     /* pointer to the super
					       ...
					       * block from the yesde
					       * which we usually run
					       * from (recovery,
					       * etc)                     */
	struct buffer_head        *j_bh;      /* Journal disk iyesde block */
 ...
 * metadata lock while calling this to be sure that yesbody can take
 * the lock and put it on ayesther transaction. */
...
/* Wrapper for iyesdes so we can check system files */
static inline int ocfs2_iyesde_is_new(struct iyesde *iyesde)
{
	...
	return ocfs2_ci_is_new(INODE_CACHE(iyesde));
}
...
/* Exported only for the journal struct init code in super.c. Do yest call. */
 ...
 *                          zero out each block.
 *  ocfs2_recovery_thread  - Perform recovery on a yesde. osb is our own osb.
 *  ocfs2_mark_dead_yesdes - Start recovery on yesdes we won't get a heartbeat
 *                          event on.
...
static inline void ocfs2_checkpoint_iyesde(struct iyesde *iyesde)
{
	...
	if (!ocfs2_ci_fully_checkpointed(INODE_CACHE(iyesde))) {
		/* WARNING: This only kicks off a single
		 * checkpoint. If someone races you and adds more
		 * metadata to the journal, you won't kyesw, and will
		 * wind up waiting *a lot* longer than necessary. Right
		 * yesw we only use this in clear_iyesde so that's
		 * OK. */
		ocfs2_start_checkpoint(osb);

		wait_event(osb->journal->j_checkpointed,
			   ocfs2_ci_fully_checkpointed(INODE_CACHE(iyesde)));
	}
}
 ...
 *                          yest release any locks taken during the transaction.
 ...
 *  ocfs2_jbd2_iyesde_add_write  - Mark an iyesde with range so that its data goes
 ...
 * yest gonna read it off disk, but rather fill it ourselves.  Right
 * yesw, we don't do anything special with this (it turns into a write
 * request), but this is a good placeholder in case we do...
 *
 * Write access is for when we read a block off disk and are going to
 * modify it. This way the journalling layer kyesws it may need to make
 * a copy of that block (if it's part of ayesther, uncommitted
 ...
 * as the access type is the same -- I'm yest sure what will happen if
 ...
 * journal_access/journal_dirty pair. The only thing you canyest do
...
/* global quotafile iyesde update, data block */
...
/* global quota data block, local quota data block, global quota iyesde,
 * global quota info */
...
/* group extend. iyesde update and last group update. */
#define OCFS2_GROUP_EXTEND_CREDITS	(OCFS2_INODE_UPDATE_CREDITS + 1)

/* group add. iyesde update and the new group update. */
#define OCFS2_GROUP_ADD_CREDITS	(OCFS2_INODE_UPDATE_CREDITS + 1)

/* get one bit out of a suballocator: diyesde + group descriptor +
...
/* diyesde + group descriptor update. We don't relink on free yet. */
...
/* parent fe, parent block, new file entry, index leaf, iyesde alloc fe, iyesde
 * alloc group descriptor + mkdir/symlink blocks + dir blocks + xattr
 * blocks + quota update */
static inline int ocfs2_mkyesd_credits(struct super_block *sb, int is_dir,
				      int xattr_credits)
{
 ...
 * for the diyesde, one for the new block. */
...
/* iyesde + dir iyesde (if we unlink a dir), + dir entry block + orphan
 * dir iyesde link + dir iyesde index leaf + dir index root */
...
/* diyesde + orphan dir diyesde + iyesde alloc diyesde + orphan dir entry +
 * iyesde alloc group descriptor + orphan dir index root +
 * orphan dir index leaf */
#define OCFS2_DELETE_INODE_CREDITS (3 * OCFS2_INODE_UPDATE_CREDITS + 4)

/* diyesde + orphan dir diyesde + extent tree leaf block + orphan dir entry +
...
/* diyesde update, old dir diyesde update, new dir diyesde update, old
...
/* global bitmap diyesde, group desc., relinked group,
 * suballocator diyesde, group desc., relinked group,
 * diyesde, xattr block */
#define OCFS2_XATTR_BLOCK_CREATE_CREDITS (OCFS2_SUBALLOC_ALLOC * 2 + \
					  + OCFS2_INODE_UPDATE_CREDITS \
					  + OCFS2_XATTR_BLOCK_UPDATE_CREDITS)

/* iyesde update, removal of dx root block from allocator */
...
/* iyesde update, new refcount block and its allocation credits. */
#define OCFS2_REFCOUNT_TREE_CREATE_CREDITS (OCFS2_INODE_UPDATE_CREDITS + 1 \
					    + OCFS2_SUBALLOC_ALLOC)

/* iyesde and the refcount block update. */
					    ...
					    struct ocfs2_extent_list *root_el)
{
	...
	/* bitmap diyesde, group desc. + relinked group. */
	bitmap_blocks = OCFS2_SUBALLOC_ALLOC;

	/* we might need to shift tree depth so lets assume an
	 * absolute worst case of complete fragmentation.  Even with
	 * that, we only need one update for the diyesde, and then
	 * however many metadata chunks needed * a remaining suballoc
	 * alloc. */
	sysfile_bitmap_blocks = 1 +
		(OCFS2_SUBALLOC_ALLOC - 1) * ocfs2_extend_meta_needed(root_el);

	/* this does yest include *new* metadata blocks, which are
	 * accounted for in sysfile_bitmap_blocks. root_el +
	 * prev. last_eb_blk + blocks along edge of tree.
	 * calc_symlink_credits passes because we just need 1
	 * credit for the diyesde there. */
...
{
	int blocks = ocfs2_mkyesd_credits(sb, 0, 0);
						 ...
						 unsigned int cpg)
{
	...
	/* parent iyesde update + new block group header + bitmap iyesde update
 ...
 * ocfs2_calc_group_alloc_credits() as well as eyesugh credits to fill
...
static inline int ocfs2_jbd2_iyesde_add_write(handle_t *handle, struct iyesde *iyesde,
					     loff_t start_byte, loff_t length)
{
	return jbd2_journal_iyesde_ranged_write(handle,
					       &OCFS2_I(iyesde)->ip_jiyesde,
					       start_byte, length);
}

static inline int ocfs2_begin_ordered_truncate(struct iyesde *iyesde,
					       loff_t new_size)
{
	return jbd2_journal_begin_ordered_truncate(
				OCFS2_SB(iyesde->i_sb)->journal->j_journal,
				&OCFS2_I(iyesde)->ip_jiyesde,
				new_size);
}

static inline void ocfs2_update_iyesde_fsync_trans(handle_t *handle,
