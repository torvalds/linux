...
static const char *ocfs2_filecheck_error(int errno) {
  if (!errno) {
...
static void ocfs2_filecheck_sysfs_free(
    struct ocfs2_filecheck_sysfs_entry *entry) {
...
static int ocfs2_filecheck_erase_entries(
  struct ocfs2_filecheck_sysfs_entry *ent,
  unsigned int count);
static int ocfs2_filecheck_adjust_max(struct ocfs2_filecheck_sysfs_entry *ent,
    unsigned int len) {
  int ret;
...
static int ocfs2_filecheck_args_get_long(const char *buf, size_t count,
    unsigned long *val) {
...
static int ocfs2_filecheck_type_parse(const char *name, unsigned int *type) {
...
static int ocfs2_filecheck_args_parse(const char *name, const char *buf,
    size_t count,
...
static inline int ocfs2_filecheck_is_dup_entry(
    struct ocfs2_filecheck_sysfs_entry *ent,
    unsigned long ino) {
  struct ocfs2_filecheck_entry *p;
  list_for_each_entry(p, &ent->fs_fcheck->fc_head, fe_list) {
...
static inline int ocfs2_filecheck_erase_entry(
    struct ocfs2_filecheck_sysfs_entry *ent) {
  struct ocfs2_filecheck_entry *p;
  list_for_each_entry(p, &ent->fs_fcheck->fc_head, fe_list) {
    if (p->fe_done) {
...
static void ocfs2_filecheck_done_entry(struct ocfs2_filecheck_sysfs_entry *ent,
...
static unsigned int ocfs2_filecheck_handle(struct ocfs2_super *osb,
...
static void ocfs2_filecheck_handle_entry(
    struct ocfs2_filecheck_sysfs_entry *ent,
    struct ocfs2_filecheck_entry *entry) {
