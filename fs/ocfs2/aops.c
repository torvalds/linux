  ...
  if (!ocfs2_sparse_alloc(osb)) {
    if (p_blkno == 0) {
      ...
      mlog(ML_ERROR,
          ...
          (unsigned long long) p_blkno,
          (unsigned long long) OCFS2_I(inode)->ip_blkno);
      mlog(ML_ERROR, "Size %llu, clusters %u\n", (unsigned long long) i_size_read(
          inode), OCFS2_I(inode)->ip_clusters);
      dump_stack();
      goto bail;
    }
  }
 ...
 */
static void ocfs2_zero_new_buffers(struct page *page, unsigned from,
    unsigned to) {
...
out_write_size:
  ...
  if (wc->w_type != OCFS2_WRITE_DIRECT) {
    ...
    di->i_mtime_nsec = di->i_ctime_nsec = cpu_to_le32(inode_get_mtime_nsec(
        inode));
    if (handle) {
...
static struct ocfs2_dio_write_ctxt *ocfs2_dio_alloc_write_ctx(
    struct buffer_head *bh, int *alloc) {
