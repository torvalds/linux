 ...
 * build_key
 */
static void hfs_ext_build_key(hfs_btree_key *key, u32 cnid, u16 block,
    u8 type) {
  key->key_len = 7;
  key->ext.FkType = type;
...
static int __hfs_ext_write_extent(struct inode *inode,
  ...
  if (HFS_I(inode)->flags & HFS_FLG_EXT_NEW) {
    ...
    hfs_bnode_write(fd->bnode, HFS_I(
        inode)->cached_extents, fd->entryoffset, fd->entrylength);
...
static inline int __hfs_ext_read_extent(struct hfs_find_data *fd,
...
static inline int __hfs_ext_cache_extent(struct hfs_find_data *fd,
  ...
  if (!res) {
    ...
    HFS_I(inode)->cached_blocks = hfs_ext_block_count(HFS_I(
        inode)->cached_extents);
  } else {
...
out:
  ...
  HFS_I(inode)->fs_blocks = (inode->i_size + sb->s_blocksize - 1) >>
      sb->s_blocksize_bits;
