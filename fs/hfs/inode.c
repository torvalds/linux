 ...
 * hfs_new_inode
 */
struct inode *hfs_new_inode(struct inode *dir, const struct qstr *name,
    umode_t mode) {
...
void hfs_inode_read_fork(struct inode *inode, struct hfs_extent *ext,
  ...
  HFS_I(inode)->fs_blocks = (log_size + sb->s_blocksize - 1) >>
      sb->s_blocksize_bits;
...
static int hfs_read_inode(struct inode *inode, void *data) {
  ...
  switch (rec->type) {
    ...
    case HFS_CDR_DIR:
      ...
      inode_set_mtime_to_ts(inode,
          inode_set_atime_to_ts(inode,
          inode_set_ctime_to_ts(inode, hfs_m_to_utime(rec->dir.MdDat))));
      inode->i_op = &hfs_dir_inode_operations;
      inode->i_fop = &hfs_dir_operations;
      break;
    default:
...
int hfs_write_inode(struct inode *inode, struct writeback_control *wbc) {
  ...
  if (S_ISDIR(main_inode->i_mode)) {
    ...
    hfs_inode_write_fork(inode, rec.file.ExtRec, &rec.file.LgLen,
        &rec.file.PyLen);
...
int hfs_inode_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
  ...
  if (attr->ia_valid & ATTR_MODE) {
    ...
    attr->ia_mode
      &= S_ISDIR(inode->i_mode) ? ~hsb->s_dir_umask : ~hsb->s_file_umask;
  }
