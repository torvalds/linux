...
static bool nfsd_support_version(int vers) {
...
static bool *nfsd_alloc_versions(void) {
...
static bool *nfsd_alloc_minorversions(void) {
...
void nfsd_netns_free_versions(struct nfsd_net *nn) {
...
static void nfsd_netns_init_versions(struct nfsd_net *nn) {
  if (!nn->nfsd_versions) {
...
static void nfsd_adjust_nfsd_versions4(struct nfsd_net *nn) {
  unsigned i;
  for (i = 0; i <= NFSD_SUPPORTED_MINOR_VERSION; i++) {
...
#if defined(CONFIG_NFSD_V2_ACL) || defined(CONFIG_NFSD_V3_ACL)
static bool nfsd_support_acl_version(int vers) {
...
static int nfsd_acl_rpcbind_set(struct net *net,
...
static __be32 nfsd_acl_init_request(struct svc_rqst *rqstp,
    const struct svc_program *progp,
...
static int nfsd_rpcbind_set(struct net *net, const struct svc_program *progp,
    u32 version, int family, unsigned short proto,
    unsigned short port) {
...
static __be32 nfsd_init_request(struct svc_rqst *rqstp,
    const struct svc_program *progp,
    struct svc_process_info *ret) {
  struct nfsd_net *nn = net_generic(SVC_NET(rqstp), nfsd_net_id);
  int i;
