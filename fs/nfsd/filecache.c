...
static bool nfsd_match_cred(const struct cred *c1, const struct cred *c2) {
  int i;
...
static void nfsd_file_schedule_laundrette(void) {
...
static void nfsd_file_slab_free(struct rcu_head *rcu) {
  struct nfsd_file *nf = container_of(rcu, struct nfsd_file, nf_rcu);
  put_cred(nf->nf_cred);
  kmem_cache_free(nfsd_file_slab, nf);
}

static void nfsd_file_mark_free(struct fsnotify_mark *mark) {
  struct nfsd_file_mark *nfm = container_of(mark, struct nfsd_file_mark,
      nfm_mark);
  kmem_cache_free(nfsd_file_mark_slab, nfm);
}

static struct nfsd_file_mark *nfsd_file_mark_get(struct nfsd_file_mark *nfm) {
...
static void nfsd_file_mark_put(struct nfsd_file_mark *nfm) {
...
static struct nfsd_file *nfsd_file_alloc(struct net *net, struct inode *inode,
    unsigned char need,
    bool want_gc) {
  struct nfsd_file *nf;
 ...
 * verifier if so.
 */
static void nfsd_file_check_write_error(struct nfsd_file *nf) {
...
static void nfsd_file_hash_remove(struct nfsd_file *nf) {
  trace_nfsd_file_unhash(nf);
  rhltable_remove(&nfsd_file_rhltable, &nf->nf_rlist,
      nfsd_file_rhash_params);
}

static bool nfsd_file_unhash(struct nfsd_file *nf) {
...
static void nfsd_file_free(struct nfsd_file *nf) {
...
static bool nfsd_file_check_writeback(struct nfsd_file *nf) {
  struct file *file = nf->nf_file;
   ...
   */
  if (!(file_inode(file)->i_sb->s_export_op->flags
...
static void nfsd_file_dispose_list(struct list_head *dispose) {
  struct nfsd_file *nf;
 ...
 */
static void nfsd_file_dispose_list_delayed(struct list_head *dispose) {
  while (!list_empty(dispose)) {
 ...
 */
static enum lru_status nfsd_file_lru_cb(struct list_head *item,
...
static void nfsd_file_gc_worker(struct work_struct *work) {
  nfsd_file_gc();
...
static unsigned long nfsd_file_lru_count(struct shrinker *s,
    struct shrink_control *sc) {
  return list_lru_count(&nfsd_file_lru);
}

static unsigned long nfsd_file_lru_scan(struct shrinker *s,
    struct shrink_control *sc) {
  LIST_HEAD(dispose);
  unsigned long ret;
 ...
 */
static void nfsd_file_cond_queue(struct nfsd_file *nf,
 ...
 */
static void nfsd_file_queue_for_close(struct inode *inode,
    struct list_head *dispose) {
 ...
 */
static void nfsd_file_close_inode(struct inode *inode) {
  LIST_HEAD(dispose);
...
static int nfsd_file_lease_notifier_call(struct notifier_block *nb,
    unsigned long arg,
    void *data) {
...
static int nfsd_file_fsnotify_handle_event(struct fsnotify_mark *mark, u32 mask,
 ...
 */
static void __nfsd_file_cache_purge(struct net *net) {
...
static struct nfsd_fcache_disposal *nfsd_alloc_fcache_disposal(void) {
...
static void nfsd_free_fcache_disposal(struct nfsd_fcache_disposal *l) {
  nfsd_file_dispose_list(&l->freeme);
  kfree(l);
}

static void nfsd_free_fcache_disposal_net(struct net *net) {
 ...
 *
 */
void nfsd_file_cache_purge(struct net *net) {
  lockdep_assert_held(&nfsd_mutex);
...
static struct nfsd_file *nfsd_file_lookup_locked(const struct net *net,
...
static __be32 nfsd_file_do_acquire(struct svc_rqst *rqstp, struct svc_fh *fhp,
