...
static int nfsd_lookup_parent(struct svc_rqst *rqstp, struct dentry *dparent,
...
__be32 nfsd_lookup_dentry(struct svc_rqst *rqstp, struct svc_fh *fhp,
 ...
 *
 */
__be32 nfsd_lookup(struct svc_rqst *rqstp, struct svc_fh *fhp, const char *name,
...
static void commit_reset_write_verifier(struct nfsd_net *nn,
 ...
 */
static int commit_inode_metadata(struct inode *inode) {
...
static int commit_metadata(struct svc_fh *fhp) {
 ...
 */
static void nfsd_sanitize_attrs(struct inode *inode, struct iattr *iap) {
...
static __be32 nfsd_get_write_access(struct svc_rqst *rqstp, struct svc_fh *fhp,
    struct iattr *iap) {
  struct inode *inode = d_inode(fhp->fh_dentry);
 ...
 * case.
 */
__be32 nfsd_setattr(struct svc_rqst *rqstp, struct svc_fh *fhp,
...
__be32 nfsd_access(struct svc_rqst *rqstp, struct svc_fh *fhp, u32 *access,
    u32 *supported) {
 ...
 */
static int __nfsd_open(struct svc_rqst *rqstp, struct svc_fh *fhp, umode_t type,
...
__be32 nfsd_open(struct svc_rqst *rqstp, struct svc_fh *fhp, umode_t type,
    int may_flags, struct file **filp) {
  __be32 err;
  int host_err;
  bool retried = false;
 ...
 */
static int nfsd_splice_actor(struct pipe_inode_info *pipe,
 ...
 * N.B. After this call fhp needs an fh_put
 */
__be32 nfsd_write(struct svc_rqst *rqstp, struct svc_fh *fhp, loff_t offset,
 ...
 */
__be32 nfsd_commit(struct svc_rqst *rqstp, struct svc_fh *fhp,
    struct nfsd_file *nf,
...
__be32 nfsd_create_locked(struct svc_rqst *rqstp, struct svc_fh *fhp,
 ...
 */
__be32 nfsd_create(struct svc_rqst *rqstp, struct svc_fh *fhp,
 ...
 * N.B. After this call fhp needs an fh_put
 */
__be32 nfsd_readlink(struct svc_rqst *rqstp, struct svc_fh *fhp, char *buf,
    int *lenp) {
  __be32 err;
  const char *link;
  struct path path;
 ...
 * Returns nfs_ok on success, or an nfsstat in network byte order.
 */
__be32 nfsd_symlink(struct svc_rqst *rqstp, struct svc_fh *fhp,
    char *fname, int flen,
 ...
 */
__be32 nfsd_link(struct svc_rqst *rqstp, struct svc_fh *ffhp,
...
static void nfsd_close_cached_files(struct dentry *dentry) {
  struct inode *inode = d_inode(dentry);
...
static bool nfsd_has_cached_files(struct dentry *dentry) {
  bool ret = false;
  struct inode *inode = d_inode(dentry);
  if (inode && S_ISREG(inode->i_mode)) {
 ...
 * N.B. After this call _both_ ffhp and tfhp need an fh_put
 */
__be32 nfsd_rename(struct svc_rqst *rqstp, struct svc_fh *ffhp, char *fname,
    int flen,
 ...
 * N.B. After this call fhp needs an fh_put
 */
__be32 nfsd_unlink(struct svc_rqst *rqstp, struct svc_fh *fhp, int type,
    char *fname, int flen) {
 ...
 * returned.
 */
__be32 nfsd_readdir(struct svc_rqst *rqstp, struct svc_fh *fhp, loff_t *offsetp,
 ...
 * N.B. After this call fhp needs an fh_put
 */
__be32 nfsd_statfs(struct svc_rqst *rqstp, struct svc_fh *fhp,
    struct kstatfs *stat, int access) {
  __be32 err;
 ...
 */
static __be32 nfsd_xattr_errno(int err) {
  switch (err) {
    case -ENODATA:
 ...
 */
__be32 nfsd_getxattr(struct svc_rqst *rqstp, struct svc_fh *fhp, char *name,
    void **bufp, int *lenp) {
  ssize_t len;
  __be32 err;
  char *buf;
 ...
 * this in inode_lock.
 */
__be32 nfsd_listxattr(struct svc_rqst *rqstp, struct svc_fh *fhp, char **bufp,
    int *lenp) {
  ssize_t len;
  __be32 err;
  char *buf;
...
__be32 nfsd_setxattr(struct svc_rqst *rqstp, struct svc_fh *fhp, char *name,
