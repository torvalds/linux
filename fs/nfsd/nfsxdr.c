...
static bool svcxdr_encode_fhandle(struct xdr_stream *xdr,
...
static __be32 *encode_timeval(__be32 *p, const struct timespec64 *time) {
...
static bool svcxdr_decode_filename(struct xdr_stream *xdr, char **name,
    unsigned int *len) {
  u32 size, i;
  __be32 *p;
  char *c;
...
static bool svcxdr_decode_diropargs(struct xdr_stream *xdr, struct svc_fh *fhp,
    char **name, unsigned int *len) {
  return svcxdr_decode_fhandle(xdr, fhp)
    && svcxdr_decode_filename(xdr, name, len);
}

static bool svcxdr_decode_sattr(struct svc_rqst *rqstp, struct xdr_stream *xdr,
    struct iattr *iap) {
  u32 tmp1, tmp2;
  __be32 *p;
...
static bool svcxdr_encode_entry_common(struct nfsd_readdirres *resp,
    const char *name,
 ...
 *   %-EINVAL: An encoding problem occured, secondary status code in
 *resp->common.err
 *
