 ...
 * the write call).
 */
static inline __be32 nfsd_mode_check(struct svc_rqst *rqstp,
...
static __be32 nfsd_set_fh_dentry(struct svc_rqst *rqstp, struct svc_fh *fhp) {
  ...
  if (fh->fh_fsid_type == FSID_MAJOR_MINOR) {
     ...
     */
    fh->fh_fsid[0]
      = new_encode_dev(MKDEV(ntohl((__force __be32) fh->fh_fsid[0]),
 ...
 * fs/nfsd/vfs.h.
 */
__be32 fh_verify(struct svc_rqst *rqstp, struct svc_fh *fhp, umode_t type,
    int access) {
...
static void set_version_and_fsid_type(struct svc_fh *fhp,
    struct svc_export *exp,
