 ...
 */
static unsigned int nfsd_cache_size_limit(void) {
  unsigned int limit;
 ...
 */
static unsigned int nfsd_hashsize(unsigned int limit) {
  return roundup_pow_of_two(limit / TARGET_BUCKET_SIZE);
}

static struct nfsd_cacherep *nfsd_cacherep_alloc(struct svc_rqst *rqstp,
    __wsum csum,
    struct nfsd_net *nn) {
  ...
  if (rp) {
    ...
    rpc_set_port((struct sockaddr *) &rp->c_key.k_addr,
...
static unsigned long nfsd_cacherep_dispose(struct list_head *dispose) {
  struct nfsd_cacherep *rp;
...
static void nfsd_cacherep_unlink_locked(struct nfsd_net *nn,
...
static void nfsd_reply_cache_free_locked(struct nfsd_drc_bucket *b,
    struct nfsd_cacherep *rp,
    struct nfsd_net *nn) {
...
static void nfsd_reply_cache_free(struct nfsd_drc_bucket *b,
    struct nfsd_cacherep *rp,
    struct nfsd_net *nn) {
 ...
 */
static void lru_put_end(struct nfsd_drc_bucket *b, struct nfsd_cacherep *rp) {
  rp->c_timestamp = jiffies;
  list_move_tail(&rp->c_lru, &b->lru_head);
}

static noinline struct nfsd_drc_bucket *nfsd_cache_bucket_find(__be32 xid,
    struct nfsd_net *nn) {
 ...
 */
static void nfsd_prune_bucket_locked(struct nfsd_net *nn,
    struct nfsd_drc_bucket *b,
...
static int nfsd_cache_key_cmp(const struct nfsd_cacherep *key,
 ...
 */
static struct nfsd_cacherep *nfsd_cache_insert(struct nfsd_drc_bucket *b,
    struct nfsd_cacherep *key,
    struct nfsd_net *nn) {
...
static int nfsd_cache_append(struct svc_rqst *rqstp, struct kvec *data) {
  __be32 *p;
