...
static bool lease_break_callback(struct file_lease *fl) {
  kill_fasync(&fl->fl_fasync, SIGIO, POLL_MSG);
  return false;
}

static void lease_setup(struct file_lease *fl, void **priv) {
 ...
 */
static int posix_same_owner(struct file_lock_core *fl1,
    struct file_lock_core *fl2) {
...
static unsigned long posix_owner_key(struct file_lock_core *flc) {
...
static void locks_insert_lock_ctx(struct file_lock_core *fl,
    struct list_head *before) {
  list_add_tail(&fl->flc_list, before);
  locks_insert_global_locks(fl);
}

static void locks_unlink_lock_ctx(struct file_lock_core *fl) {
  locks_delete_global_locks(fl);
  list_del_init(&fl->flc_list);
  locks_wake_up_blocks(fl);
}

static void locks_delete_lock_ctx(struct file_lock_core *fl,
...
static struct file_lock_core *what_owner_is_waiting_for(
...
static bool leases_conflict(struct file_lock_core *lc,
    struct file_lock_core *bc) {
  bool rc;
...
static bool any_leases_conflict(struct inode *inode,
    struct file_lease *breaker) {
  struct file_lock_context *ctx = inode->i_flctx;
  struct file_lock_core *flc;
  lockdep_assert_held(&ctx->flc_lock);
 ...
 */
static int check_conflicting_open(struct file *filp, const int arg, int flags) {
  struct inode *inode = file_inode(filp);
...
static int generic_add_lease(struct file *filp, int arg,
...
static inline void setlease_notifier(int arg, struct file_lease *lease) {
  if (arg != F_UNLCK) {
...
int kernel_setlease(struct file *filp, int arg, struct file_lease **lease,
    void **priv) {
  if (lease) {
 ...
 */
int vfs_setlease(struct file *filp, int arg, struct file_lease **lease,
    void **priv) {
  struct inode *inode = file_inode(filp);
...
SYSCALL_DEFINE2(flock, unsigned int, fd, unsigned int, cmd)
{
  ...
  if (cmd & LOCK_MAND) {
    pr_warn_once(
        "%s(%d): Attempt to set a LOCK_MAND lock via flock(2). This support has been removed and the request ignored.\n", current->comm,
        current->pid);
    return 0;
  }
 ...
 */
static pid_t locks_translate_pid(struct file_lock_core *fl,
...
/* The i_flctx must be valid when calling into here */
static void locks_remove_lease(struct file *filp,
    struct file_lock_context *ctx) {
  struct file_lease *fl, *tmp;
  LIST_HEAD(dispose);
  if (list_empty(&ctx->flc_lease)) {
...
static struct file_lock_core *get_next_blocked_member(
    struct file_lock_core *node) {
...
static int locks_show(struct seq_file *f, void *v) {
   ...
   * is the left child of current node, the next silibing in flc_blocked_member
   *is the
   * right child, we can alse get the parent of current node from flc_blocker,
   *so this
