...
#define SET_PAR_SHIFT_RIGHT \
  ...
  { \
    int to_r; \
   \
    to_r = (MAX_NR_KEY(Sh) + 1 - rpar + vn->vn_nr_item + 1) / 2 - (MAX_NR_KEY( \
    Sh) + 1 - rpar); \
   \
...
static int get_empty_nodes(struct tree_balance *tb, int h) {
  ...
  b_blocknr_t *blocknr, blocknrs[MAX_AMOUNT_NEEDED] = {
    0,
  };
...
static int dc_check_balance_leaf(struct tree_balance *tb, int h) {
  ...
  if (tb->lnum[0] >= vn->vn_nr_item && tb->lbytes == -1) {
    if (is_left_neighbor_in_cache(tb,
        h) || ((tb->rnum[0] - ((tb->rbytes == -1) ? 0 : 1)) < vn->vn_nr_item)     /*
                                                                                   * S
                                                                                   * can
                                                                                   * not
                                                                                   * be
...
static int get_virtual_node_size(struct super_block *sb,
    struct buffer_head *bh) {
