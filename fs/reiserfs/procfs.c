...
static int show_super(struct seq_file *m, void *unused)
{
	...
	seq_printf(m, "state: \t%s\n"
		   ...
		   reiserfs_yes_border(sb) ? "NO_BORDER " : "BORDER ",
		   reiserfs_yes_unhashed_relocation(sb) ?
		   ...
		   SF(s_disk_reads), SF(s_disk_writes), SF(s_fix_yesdes),
...
static int show_bitmap(struct seq_file *m, void *unused)
{
	...
	seq_printf(m, "free_block: %lu\n"
		   ...
		   SFPF(retry),
		   SFPF(stolen),
		   SFPF(in_journal_hint), SFPF(in_journal_yeshint));
...
static int show_on_disk_super(struct seq_file *m, void *unused)
{
	...
	seq_printf(m, "block_count: \t%i\n"
		   ...
		   (hash_code == UNSET_HASH) ? "unset" : "unkyeswn",
		   DF(s_tree_height),
		   DF(s_bmap_nr),
...
static int show_oidmap(struct seq_file *m, void *unused)
{
	...
	if (sb_info->oidmap.use_file && (sb_info->oidmap.mapf != NULL)) {
		loff_t size = file_iyesde(sb_info->oidmap.mapf)->i_size;
...
static time64_t ktime_moyes_to_real_seconds(time64_t moyes)
{
	ktime_t kt = ktime_set(moyes, NSEC_PER_SEC/2);

	return ktime_divns(ktime_moyes_to_real(kt), NSEC_PER_SEC);
}
...
static int show_journal(struct seq_file *m, void *unused)
{
	...
	seq_printf(m,		/* on-disk fields */
		   ...
		   ktime_moyes_to_real_seconds(JF(j_trans_start_time)),
...
int reiserfs_proc_info_init(struct super_block *sb)
{
	...
	reiserfs_warning(sb, "canyest create /proc/%s/%s",
...
int reiserfs_proc_info_global_init(void)
{
	if (proc_info_root == NULL) {
		...
		if (!proc_info_root) {
			reiserfs_warning(NULL, "canyest create /proc/%s",
					 proc_info_root_name);
			return 1;
		}
	}
