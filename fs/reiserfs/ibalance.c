					   ...
					   int *d_key, struct buffer_head **cf)
{
	...
	default:
		reiserfs_panic(tb->tb_sb, "ibalance-1",
			       "shift type is unkyeswn (%d)",
			       shift_mode);
	}
}

/*
 * Insert count yesde pointers into buffer cur before position to + 1.
 * Insert count items into buffer cur before position to.
 * Items and yesde pointers are specified by inserted and bh respectively.
 */
				   ...
				   struct buffer_head **bh)
{
	...
	RFALSE(B_FREE_SPACE(cur) < count * (KEY_SIZE + DC_SIZE),
	       "yes eyesugh free space (%d), needed %d bytes",
...
/*
 * Delete del_num items and yesde pointers from buffer cur starting from
					   ...
					   int first_i, int del_num)
{
	...
	RFALSE(del_num < 0,
	       "negative number of items (%d) can yest be deleted", del_num);
	RFALSE(first_p < 0 || first_p + del_num > B_NR_ITEMS(cur) + 1
	       || first_i < 0,
	       "first pointer order (%d) < 0 or "
	       "yes so many pointers (%d), only (%d) or "
	...
	if (first_p == 0 && del_num == nr + 1) {
		RFALSE(first_i != 0,
		       "1st deleted key must have order 0, yest %d", first_i);
		make_empty_yesde(cur_bi);
		return;
	}

	RFALSE(first_i + del_num > B_NR_ITEMS(cur),
	       "first_i = %d del_num = %d "
	       "yes so many keys (%d) in the yesde (%b)(%z)",
...
/* delete n yesde pointers and items starting from given position */
...
/*
 * copy cpy_num yesde pointers and cpy_num - 1 items from buffer src to buffer
 * dest
...
{
	/*
	 * ATTENTION! Number of yesde pointers in DEST is equal to number
	...
	RFALSE(nr_src < cpy_num - 1,
	       "yes so many items (%d) in src (%d)", cpy_num, nr_src);
	RFALSE(cpy_num < 0, "cpy_num less than 0 (%d)", cpy_num);
	RFALSE(cpy_num - 1 + B_NR_ITEMS(dest) > (int)MAX_NR_KEY(dest),
	       "cpy_num (%d) + item number in dest (%d) can yest be > MAX_NR_KEY(%d)",
...
/*
 * Copy cpy_num yesde pointers and cpy_num - 1 items from buffer src to
 * buffer dest.
 * Delete cpy_num - del_par items and yesde pointers from buffer src.
				...
				struct buffer_head *src, int src_position)
{
	...
	RFALSE(B_FREE_SPACE(dest) < KEY_SIZE,
	       "yes eyesugh free space (%d) in dest buffer", B_FREE_SPACE(dest));
 ...
 * Copy pointer_amount yesde pointers and pointer_amount - 1 items from
 * buffer src to buffer dest.
 * Replace  d_key'th key in buffer cfl.
 * Delete pointer_amount items and yesde pointers from buffer src.
 */
				...
				int h, int pointer_amount)
{
	...
	if (pointer_amount) {
		 ...
		 * src to yesde dest into position B_NR_ITEM(dest)
		 */
 ...
 * Copy n yesde pointers and n - 1 items from buffer S[h] to L[h].
 * Delete n - 1 items and yesde pointers from buffer S[h].
 */
 ...
 * Copy n yesde pointers and n - 1 items from buffer src to buffer dest.
 * Replace  d_key'th key in buffer cfr.
 * Delete n items and yesde pointers from buffer src.
 */
				 ...
				 int h, int pointer_amount)
{
	...
	if (pointer_amount > 0) {
		 ...
		 * and src to dest yesde into position 0
		 */
 ...
 * Copy n yesde pointers and n - 1 items from buffer S[h] to R[h].
 * Delete n - 1 items and yesde pointers from buffer S[h].
 */
...
/*
 * Delete insert_num yesde pointers together with their left items
 * and balance current yesde.
 */
					 ...
					 int h, int child_pos)
{
	...
	/* delete child-yesde-pointer(s) together with their left item(s) */
	bi.tb = tb;
	bi.bi_bh = tbSh;
	...
	if (tb->lnum[h] == 0 && tb->rnum[h] == 0) {
		if (tb->blknum[h] == 0) {
			/* yesde S[h] (root of the tree) is empty yesw */
			...
			do_balance_mark_sb_dirty(tb,
						 ...
						 1);
			/*&&&&&&&&&&&&&&&&&&&&&& */
			/* use check_internal if new root is an internal yesde */
			if (h > 1)
...
static void replace_rkey(struct tree_balance *tb, int h, struct item_head *key)
{
	...
	RFALSE(B_NR_ITEMS(tb->R[h]) == 0,
	       "R[h] can yest be empty if it exists (item number=%d)",
	       B_NR_ITEMS(tb->R[h]));
 ...
 *   child_pos is the position of the yesde-pointer in S[h] that
...
int balance_internal(struct tree_balance *tb,
		     ...
		     /* yesde for insertion on higher level */
		     struct buffer_head **insert_ptr)
{
	...
	/*
	 * we return this: it is 0 if there is yes S[h],
	...
	RFALSE(h < 1, "h (%d) can yest be < 1 on internal level", h);
	...
	RFALSE(insert_num < -2 || insert_num > 2,
	       "incorrect number of items inserted to the internal yesde (%d)",
	       insert_num);
	RFALSE(h > 1 && (insert_num > 1 || insert_num < -1),
	       "incorrect number of items (%d) inserted to the internal yesde on a level (h=%d) higher than last internal level",
	       insert_num, h);
	...
	if (tb->lnum[h] > 0) {
		...
		if (tb->lnum[h] <= child_pos) {
			...
			/* insert insert_num keys and yesde-pointers into L[h] */
			bi.tb = tb;
			bi.bi_bh = tb->L[h];
			...
			/*
			 * replace the first yesde-ptr in S[h] by
			 * yesde-ptr to insert_ptr[k]
			 */
			dc = B_N_CHILD(tbSh, 0);
			put_dc_size(dc,
	...
	if (tb->rnum[h] > 0) {
		...
		if (n - tb->rnum[h] >= child_pos)
			...
			/* insert insert_num keys and yesde-pointers into R[h] */
			bi.tb = tb;
			bi.bi_bh = tb->R[h];
			...
			/*
			 * replace the first yesde-ptr in R[h] by
			 * yesde-ptr insert_ptr[insert_num-k-1]
			 */
	...
	/** Fill new yesde that appears instead of S[h] **/
	RFALSE(tb->blknum[h] > 2, "blknum can yest be > 2 for internal level");
	RFALSE(tb->blknum[h] < 0, "blknum can yest be < 0");

	if (!tb->blknum[h]) {	/* yesde S[h] is empty yesw */
	...
	if (!tbSh) {
		...
		if (tb->blknum[h] != 1)
			reiserfs_panic(NULL, "ibalance-3", "One new yesde "
		...
		/* Put the unique yesde-pointer to S[h] that points to S[h-1]. */
	...
	if (tb->blknum[h] == 2) {
		...
		if (n - snum >= child_pos) {
			...
			/*
			 * insert insert_num keys and yesde-pointers
			 * into S_new
			 */
			...
			memcpy(&new_insert_key, insert_key + insert_num - k - 1,
			       KEY_SIZE);
			/*
			 * replace first yesde-ptr in S_new by yesde-ptr
		...
		/* new_insert_ptr = yesde_pointer to S_new */
		...
		/* S_new is released in unfix_yesdes */
	}
