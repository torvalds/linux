...
static void internal_delete_childs(struct buffer_info *cur_bi, int from,
    int n) {
  int i_from;
...
static void balance_internal_when_delete(struct tree_balance *tb,
  ...
  if (tb->rnum[h] < 0) {
    RFALSE(tb->lnum[h] != 0,
        ...
        h, tb->lnum[h]);
    internal_shift_left(INTERNAL_SHIFT_FROM_R_TO_S, tb, h, -tb->rnum[h]); /*tb->S[h],
                                                                           * tb->CFR[h],
                                                                           * tb->rkey[h],
                                                                           * tb->R[h],
                                                                           * -tb->rnum[h]);
  ...
  if (tb->lnum[h] > 0) {
    ...
    internal_shift_left(INTERNAL_SHIFT_FROM_S_TO_L, tb, h, tb->lnum[h]);  /*tb->L[h],
                                                                           * tb->CFL[h],
                                                                           * tb->lkey[h],
                                                                           * tb->S[h],
                                                                           * tb->lnum[h]);
...
static void replace_lkey(struct tree_balance *tb, int h,
...
static void replace_rkey(struct tree_balance *tb, int h,
    struct item_head *key) {
...
int balance_internal(struct tree_balance *tb,
  ...
  if (0 <= child_pos && child_pos <= n && insert_num > 0) {
    ...
    internal_insert_childs(&bi, /*tbSh, */
        /*          ( tb->S[h-1]->b_parent == tb->S[h] ) ? tb->S[h-1]->b_next :
         *  tb->S[h]->b_child->b_next, */
