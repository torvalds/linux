...
void reiserfs_update_sd_size(struct reiserfs_transaction_handle *th,
  ...
  for (;;) {
    ...
    if (retval == ITEM_NOT_FOUND) {
      ...
      pathrelse(&path);
      if (inode->i_nlink == 0) {
        /*reiserfs_warning (inode->i_sb, "vs-13050: reiserfs_update_sd: i_nlink
         * == 0, stat data not found"); */
        return;
      }
 ...
 * write purposes
 */
static int invalidate_folio_can_drop(struct inode *inode,
    struct buffer_head *bh) {
  int ret = 1;
 ...
 */
static bool reiserfs_release_folio(struct folio *folio,
...
int reiserfs_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
  ...
  if ((((attr->ia_valid & ATTR_UID)
      && (from_kuid(&init_user_ns, attr->ia_uid) & ~0xffff))
      || ((attr->ia_valid & ATTR_GID)
      && (from_kgid(&init_user_ns, attr->ia_gid) & ~0xffff)))
