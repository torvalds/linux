				  ...
				  int item_num, int from, int copy_count)
{
	...
	/*
	 * if there are yes items in dest or the first/last item in
	 * dest is yest item of the same directory
	 */
								  ...
								  item_num_in_dest))))
	{
		...
		if (last_first == LAST_TO_FIRST) {
			...
			if (from < ih_entry_count(ih)) {
				...
				/*
				 * yes entries will be copied to this
				 * item in this function
				 */
				set_le_ih_k_offset(&new_ih, U32_MAX);
				/*
				 * this item is yest yet valid, but we
 ...
 * (last_first == LAST_TO_FIRST) item or part of it or yesthing
				   ...
				   int bytes_or_entries)
{
	 ...
	 * then there is yes need to treat this item differently from the
	...
	if (last_first == FIRST_TO_LAST) {
		...
		/* there is yesthing to merge */
		if (!dest_nr_item
		    || (!op_is_left_mergeable(&ih->ih_key, src->b_size)))
			return 0;

		RFALSE(!ih_item_len(ih),
		       "vs-10010: item can yest have empty length");
		...
		if (is_indirect_le_ih(dih)) {
			RFALSE(get_ih_free_space(dih),
			       "vs-10030: merge to left: last unformatted yesde of yesn-last indirect item %h must have zerto free space",
			       ih);
			if (bytes_or_entries == ih_item_len(ih))
	...
	RFALSE(is_indirect_le_ih(ih) && get_ih_free_space(ih),
	       "vs-10040: merge to right: last unformatted yesde of yesn-last indirect item must be filled entirely (%h)",
	       ih);
	...
	if (bytes_or_entries == -1) {
		...
		RFALSE(le_ih_k_offset(dih) !=
		       ...
		       "vs-10050: items %h and %h do yest match", ih, dih);

		/* change first item key of the DEST */
		set_le_ih_k_offset(dih, le_ih_k_offset(ih));

		/* item becomes yesn-mergeable */
		/* or mergeable if left item was */
		set_le_ih_k_type(dih, le_ih_k_type(ih));
	} else {
		/* merge to right only part of item */
		RFALSE(ih_item_len(ih) <= bytes_or_entries,
		       "vs-10060: yes so much bytes %lu (needed %lu)",
				     ...
				     int first, int cpy_num)
{
	...
	RFALSE(cpy_num < 0, "vs-10110: can yest copy negative amount of items");
	RFALSE(!dest_bi, "vs-10120: can yest copy negative amount of items");

	dest = dest_bi->bi_bh;

	RFALSE(!dest, "vs-10130: can yest copy negative amount of items");
	...
	RFALSE(blkh_free_space(blkh) < cpy_num * IH_SIZE,
	       "vs-10140: yest eyesugh free space for headers %d (needed %d)",
	...
	RFALSE(free_space < j - last_inserted_loc,
	       "vs-10150: yest eyesugh free space for items %d (needed %d)",
	...
	if (dest_bi->bi_parent) {
		...
		RFALSE(dc_block_number(t_dc) != dest->b_blocknr,
		       "vs-10160: block number in bh does yest match to field in disk_child structure %lu and %lu",
 ...
 * shifting part of an item into ayesther yesde.)
 */
			     ...
			     int item_num, int cpy_bytes)
{
	...
	RFALSE(cpy_bytes == -1,
	       "vs-10170: bytes == - 1 means: do yest split item");
		...
		if (is_direntry_le_ih(ih))
			...
			if (is_indirect_le_ih(ih)) {
				RFALSE(cpy_bytes == ih_item_len(ih)
				       ...
				       "vs-10180: when whole indirect item is bottle to left neighbor, it must have free_space==0 (yest %lu)",
 ...
 * to DEST.  If cpy_bytes yest equal to minus one than copy cpy_num-1 whole
			   ...
			   int last_first, int cpy_num, int cpy_bytes)
{
	...
	RFALSE(B_NR_ITEMS(src) < cpy_num,
	       "vs-10230: No eyesugh items: %d, req. %d", B_NR_ITEMS(src),
	       cpy_num);
	...
	if (last_first == FIRST_TO_LAST) {
		...
		/*
		 * copy the first item or it part or yesthing to the end of
		...
		/*
		 * copy the last item or it part or yesthing to the
		 * begin of the DEST
				       ...
				       struct buffer_head *Snew)
{
	...
	default:
		reiserfs_panic(sb_from_bi(src_bi), "vs-10250",
			       "shift type is unkyeswn (%d)", shift_mode);
	}
	...
	if (shift_num) {
		...
		if (B_NR_ITEMS(S0) == 0) {
			...
			RFALSE(shift_bytes != -1,
			       "vs-10270: S0 is empty yesw, but shift_bytes != -1 (%d)",
			       shift_bytes);
#ifdef CONFIG_REISERFS_CHECK
		       ...
		       int first, int del_num, int del_bytes)
{
	...
	RFALSE(!bh, "10155: bh is yest defined");
	RFALSE(del_num < 0, "10160: del_num can yest be < 0. del_num==%d",
	       del_num);
	RFALSE(first < 0
	       || first + del_num > item_amount,
	       "10165: invalid number of first item to be deleted (%d) or "
	       "yes so much items (%d) to delete (only %d)", first,
	       first + del_num, item_amount);
...
/* insert item into the leaf yesde in position before */
			  ...
			  int zeros_number)
{
	...
	RFALSE(free_space < ih_item_len(inserted_item_ih) + IH_SIZE,
	       "vs-10170: yest eyesugh free space in block %z, new item %h",
	       bh, inserted_item_ih);
			  ...
			  const char *body, int zeros_number)
{
	...
	RFALSE(free_space < paste_size,
	       "vs-10175: yest eyesugh free space: needed %d, available %d",
 ...
 * does yest have free space, so it moves DEHs and remaining records as
			    ...
			    struct item_head *ih, int from, int del_count)
{
	...
	/*
	 * make sure that item is directory and there are eyesugh entries to
	 * remove
	 */
	RFALSE(!is_direntry_le_ih(ih), "10180: item is yest directory item");
	RFALSE(ih_entry_count(ih) < from + del_count,
	       "10185: item contains yest eyesugh entries: entry_count = %d, from = %d, to delete = %d",
			  ...
			  int pos_in_item, int cut_size)
{
	...
	if (is_direntry_le_ih(ih)) {
		...
		if (pos_in_item == 0) {
			/* change key */
			RFALSE(cut_item_num,
			       "when 0-th enrty of item is cut, that item must be first in the yesde, yest %d-th",
			       cut_item_num);
			...
			const char *records, int paste_size)
{
	...
	/*
	 * make sure, that item is directory, and there are eyesugh
	 * records in it
	 */
	RFALSE(!is_direntry_le_ih(ih), "10225: item is yest directory item");
	RFALSE(ih_entry_count(ih) < before,
	       "10230: there are yes entry we paste entries before. entry_count = %d, before = %d",
