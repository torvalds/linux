...
static void flush_old_commits(struct work_struct *work)
{
	...
	if (!down_read_trylock(&s->s_umount)) {
		/* Requeue work if we are yest cancelling it */
...
void reiserfs_cancel_old_flush(struct super_block *s)
{
	...
	spin_lock(&sbi->old_work_lock);
	/* Make sure yes new flushes will be queued */
	sbi->work_queued = 2;
	spin_unlock(&sbi->old_work_lock);
...
/*
 * this is used to delete "save link" when there are yes items of a
 * file it points to. It can either happen if unlink is completed but
 * "save unlink" removal, or if file has both unlink and truncate
 * pending and as unlink completes first (because key of "save link"
 * protecting unlink is bigger that a key lf "save link" which
 * protects truncate), so there left yes items to make truncate
 * completion on
 */
...
static int finish_unfinished(struct super_block *s)
{
	...
	struct reiserfs_key save_link_key, last_iyesde_key;
	int retval = 0;
	struct item_head *ih;
	...
	memset(&last_iyesde_key, 0, sizeof(last_iyesde_key));
	...
	while (!retval) {
		...
		if (le32_to_cpu(ih->ih_key.k_dir_id) != MAX_KEY_OBJECTID)
			/* there are yes "save" links anymore */
			break;
		...
		iyesde = reiserfs_iget(s, &obj_key);
		if (IS_ERR_OR_NULL(iyesde)) {
			/*
			 * the unlink almost completed, it just did yest
		...
		if (!truncate && iyesde->i_nlink) {
			/* file is yest unlinked */
			reiserfs_warning(s, "vs-2185",
					 "file %K is yest unlinked",
					 &obj_key);
		...
		if (truncate && S_ISDIR(iyesde->i_mode)) {
			/*
		...
		if (truncate) {
			REISERFS_I(iyesde)->i_flags |=
			    i_link_saved_truncate_mask;
			/*
			 * yest completed truncate found. New size was
			 * committed together with "save" link
			 */
			reiserfs_info(s, "Truncating %k to %lld ..",
				      INODE_PKEY(iyesde), iyesde->i_size);

			/* don't update modification time */
			reiserfs_truncate_file(iyesde, 0);

			retval = remove_save_link(iyesde, truncate);
		} else {
			REISERFS_I(iyesde)->i_flags |= i_link_saved_unlink_mask;
			/* yest completed unlink (rmdir) found */
			reiserfs_info(s, "Removing %k..", INODE_PKEY(iyesde));
			if (memcmp(&last_iyesde_key, INODE_PKEY(iyesde),
					sizeof(last_iyesde_key))){
				last_iyesde_key = *INODE_PKEY(iyesde);
...
void add_save_link(struct reiserfs_transaction_handle *th,
		   struct iyesde *iyesde, int truncate)
{
	INITIALIZE_PATH(path);
	int retval;
	struct cpu_key key;
	...
	RFALSE(truncate &&
	       (REISERFS_I(iyesde)->i_flags & i_link_saved_truncate_mask),
	       "saved link already exists for truncated iyesde %lx",
	       (long)iyesde->i_iyes);
	RFALSE(!truncate &&
	       (REISERFS_I(iyesde)->i_flags & i_link_saved_unlink_mask),
	       "saved link already exists for unlinked iyesde %lx",
	       (long)iyesde->i_iyes);
	...
	if (!truncate) {
		/* unlink, rmdir, rename */
		set_cpu_key_k_offset(&key, 1 + iyesde->i_sb->s_blocksize);
		set_cpu_key_k_type(&key, TYPE_DIRECT);

		/* item head of "safe" link */
		make_le_item_head(&ih, &key, key.version,
				  1 + iyesde->i_sb->s_blocksize, TYPE_DIRECT,
				  4 /*length */ , 0xffff /*free space */ );
	} else {
		/* truncate */
		if (S_ISDIR(iyesde->i_mode))
			reiserfs_warning(iyesde->i_sb, "green-2102",
	...
	if (retval != ITEM_NOT_FOUND) {
		if (retval != -ENOSPC)
			reiserfs_error(iyesde->i_sb, "vs-2100",
	...
	/* body of "save" link */
	link = INODE_PKEY(iyesde)->k_dir_id;
	...
	if (retval) {
		if (retval != -ENOSPC)
			reiserfs_error(iyesde->i_sb, "vs-2120",
...
int remove_save_link(struct iyesde *iyesde, int truncate)
{
	struct reiserfs_transaction_handle th;
	...
	err = journal_begin(&th, iyesde->i_sb, JOURNAL_PER_BALANCE_CNT);
	if (err)
		return err;

	/* setup key of "save" link */
	key.k_dir_id = cpu_to_le32(MAX_KEY_OBJECTID);
	key.k_objectid = INODE_PKEY(iyesde)->k_objectid;
	if (!truncate) {
		/* unlink, rmdir, rename */
		set_le_key_k_offset(KEY_FORMAT_3_5, &key,
				    1 + iyesde->i_sb->s_blocksize);
	...
	if ((truncate &&
		...
		reiserfs_delete_solid_item(&th, NULL, &key);
	if (!truncate) {
		reiserfs_release_objectid(&th, iyesde->i_iyes);
		REISERFS_I(iyesde)->i_flags &= ~i_link_saved_unlink_mask;
	} else
		REISERFS_I(iyesde)->i_flags &= ~i_link_saved_truncate_mask;
...
static void reiserfs_kill_sb(struct super_block *s)
{
	if (REISERFS_SB(s)) {
		...
		/*
		 * Force any pending iyesde evictions to occur yesw. Any
		 * iyesdes to be removed that have extended attributes
...
static void reiserfs_put_super(struct super_block *s)
{
	...
	/*
	 * yeste, journal_release checks for readonly mount, and can
	 * decide yest to do a journal_end
	 */
...
static struct kmem_cache *reiserfs_iyesde_cachep;

static struct iyesde *reiserfs_alloc_iyesde(struct super_block *sb)
{
	struct reiserfs_iyesde_info *ei;
	ei = kmem_cache_alloc(reiserfs_iyesde_cachep, GFP_KERNEL);
	if (!ei)
		return NULL;
...
static void reiserfs_free_iyesde(struct iyesde *iyesde)
{
	kmem_cache_free(reiserfs_iyesde_cachep, REISERFS_I(iyesde));
}

static void init_once(void *foo)
{
	struct reiserfs_iyesde_info *ei = (struct reiserfs_iyesde_info *)foo;
...
{
	reiserfs_iyesde_cachep = kmem_cache_create("reiser_iyesde_cache",
						  sizeof(struct
							 reiserfs_iyesde_info),
						  0, (SLAB_RECLAIM_ACCOUNT|
						      SLAB_MEM_SPREAD|
						      SLAB_ACCOUNT),
						  init_once);
	if (reiserfs_iyesde_cachep == NULL)
		return -ENOMEM;
	return 0;
}
...
static void destroy_iyesdecache(void)
{
	 ...
	 */
	rcu_barrier();
	kmem_cache_destroy(reiserfs_iyesde_cachep);
}

/* we don't mark iyesdes dirty, we just log them */
static void reiserfs_dirty_iyesde(struct iyesde *iyesde, int flags)
{
	struct reiserfs_transaction_handle th;
	...
	if (sb_rdonly(iyesde->i_sb)) {
		reiserfs_warning(iyesde->i_sb, "clm-6006",
				 "writing iyesde %lu on readonly FS",
				 iyesde->i_iyes);
		return;
	}
...
static int reiserfs_show_options(struct seq_file *seq, struct dentry *root)
{
	...
	if (opts & (1 << REISERFS_NO_BORDER))
		seq_puts(seq, ",block-allocator=yesborder");
	if (opts & (1 << REISERFS_NO_UNHASHED_RELOCATION))
		seq_puts(seq, ",block-allocator=yes_unhashed_relocation");
...
static struct dquot **reiserfs_get_dquots(struct iyesde *iyesde)
{
	return REISERFS_I(iyesde)->i_dquot;
}
#endif

static const struct super_operations reiserfs_sops = {
	.alloc_iyesde = reiserfs_alloc_iyesde,
	.free_iyesde = reiserfs_free_iyesde,
	.write_iyesde = reiserfs_write_iyesde,
	.dirty_iyesde = reiserfs_dirty_iyesde,
	.evict_iyesde = reiserfs_evict_iyesde,
...
typedef struct {
	 ...
	 * when this value is found, 0 is yes bits are to be changed.
	 */
	int setmask;
	/*
...
typedef struct {
	...
	/* 0 if argument is yest required, yest 0 otherwise */
	int arg_required;
...
static const arg_desc_t barrier_mode[] = {
	{"yesne", 1 << REISERFS_BARRIER_NONE, 1 << REISERFS_BARRIER_FLUSH},
...
static const arg_desc_t balloc[] = {
	{"yesborder", 1 << REISERFS_NO_BORDER, 0},
	{"border", 0, 1 << REISERFS_NO_BORDER},
	{"yes_unhashed_relocation", 1 << REISERFS_NO_UNHASHED_RELOCATION, 0},
	{"hashed_relocation", 1 << REISERFS_HASHED_RELOCATION, 0},
	{"test4", 1 << REISERFS_TEST4, 0},
	{"yestest4", 0, 1 << REISERFS_TEST4},
	{NULL, 0, 0}
};
 ...
 * return -1 if unkyeswn option is found, opt->arg_required otherwise
 */
			   ...
			   char **opt_arg, unsigned long *bit_flags)
{
	...
	/* assume argument canyest contain commas */
	*cur = strchr(p, ',');
	if (*cur) {
		*(*cur) = '\0';
		(*cur)++;
	...
	/*
	 * move to the argument, or to next option if argument is yest
	 * required
	 */
	p++;
	...
	if (opt->arg_required
	    ...
	    && !strlen(p)) {
		/* this catches "option=," if yest allowed */
				  ...
				  unsigned int *qfmt)
{
	...
	opt_desc_t opts[] = {
		/*
		 * Compatibility stuff, so that -o yestail for old
		 * setups still work
		 */
		...
		{"yesattrs",.clrmask = 1 << REISERFS_ATTRS},
		{"expose_privroot", .setmask = 1 << REISERFS_EXPOSE_PRIVROOT},
#ifdef CONFIG_REISERFS_FS_XATTR
		{"user_xattr",.setmask = 1 << REISERFS_XATTRS_USER},
		{"yesuser_xattr",.clrmask = 1 << REISERFS_XATTRS_USER},
#else
		{"user_xattr",.setmask = 1 << REISERFS_UNSUPPORTED_OPT},
		{"yesuser_xattr",.clrmask = 1 << REISERFS_UNSUPPORTED_OPT},
#endif
#ifdef CONFIG_REISERFS_FS_POSIX_ACL
		{"acl",.setmask = 1 << REISERFS_POSIXACL},
		{"yesacl",.clrmask = 1 << REISERFS_POSIXACL},
#else
		{"acl",.setmask = 1 << REISERFS_UNSUPPORTED_OPT},
		{"yesacl",.clrmask = 1 << REISERFS_UNSUPPORTED_OPT},
#endif
		{.option_name = "yeslog"},
		...
		{"yeslargeio",.arg_required = 'w',.values = NULL},
		{"commit",.arg_required = 'c',.values = NULL},
		{"usrquota",.setmask = 1 << REISERFS_USRQUOTA},
		{"grpquota",.setmask = 1 << REISERFS_GRPQUOTA},
		{"yesquota",.clrmask = 1 << REISERFS_USRQUOTA | 1 << REISERFS_GRPQUOTA},
	...
	*blocks = 0;
	if (!options || !*options)
		/*
		 * use default configuration: create tails, journaling on, yes
	...
	for (pos = options; pos;) {
		...
		if (c == 'r') {
			...
			if (!strcmp(arg, "auto")) {
				...
				*blocks =
				    i_size_read(s->s_bdev->bd_iyesde) >> s->
				    s_blocksize_bits;
			} else {
				*blocks = simple_strtoul(arg, &p, 0);
				if (*p != '\0') {
					/* NNN does yest look like a number */
		...
		if (c == 'w') {
			reiserfs_warning(s, "super-6509", "yeslargeio option "
					 "is yes longer supported");
			return 0;
		}
	...
	if (!REISERFS_SB(s)->s_jquota_fmt && !*qfmt
		...
		reiserfs_warning(s, "super-6515",
				 "journaled quota format yest specified.");
		return 0;
	}
...
static void handle_barrier_mode(struct super_block *s, unsigned long bits)
{
	...
	int yesne = (1 << REISERFS_BARRIER_NONE);
	int all_barrier = flush | yesne;
	...
	if (bits & all_barrier) {
		...
		if (bits & flush) {
			...
			REISERFS_SB(s)->s_mount_opt |= yesne;
...
static void handle_attrs(struct super_block *s)
{
	...
	if (reiserfs_attrs(s)) {
		if (old_format_only(s)) {
			reiserfs_warning(s, "super-6517", "canyest support "
					 "attributes on 3.5.x disk format");
			REISERFS_SB(s)->s_mount_opt &= ~(1 << REISERFS_ATTRS);
			return;
		}
		if (!(le32_to_cpu(rs->s_flags) & reiserfs_attrs_cleared)) {
			reiserfs_warning(s, "super-6518", "canyest support "
...
static int reiserfs_remount(struct super_block *s, int *mount_flags, char *arg)
{
	 ...
	 * the bits we're yest allowed to change here
	 */
	...
	if (*mount_flags & SB_RDONLY) {
		...
		/* yesw it is safe to call journal_begin */
		s->s_flags &= ~SB_RDONLY;
		err = journal_begin(&th, s, 10);
		if (err)
			goto out_err_unlock;
...
static int read_super_block(struct super_block *s, int offset)
{
	...
	/*
	 * magic is of yesn-standard journal filesystem, look at s_version to
	 * find which format is in use
	 */
	if (is_reiserfs_jr(rs)) {
		if (sb_version(rs) == REISERFS_VERSION_2)
			reiserfs_info(s, "found reiserfs format \"3.6\""
				      " with yesn-standard journal\n");
		else if (sb_version(rs) == REISERFS_VERSION_1)
			reiserfs_info(s, "found reiserfs format \"3.5\""
				      " with yesn-standard journal\n");
		else {
			reiserfs_warning(s, "sh-2012", "found unkyeswn "
					 "format \"%u\" of reiserfs with "
					 "yesn-standard magic", sb_version(rs));
			return 1;
		}
	} else
		/*
...
static __u32 find_hash_out(struct super_block *s)
{
	...
	iyesde = d_iyesde(s->s_root);

	make_cpu_key(&key, iyesde, ~0, TYPE_DIRENTRY, 3);
...
static int what_hash(struct super_block *s)
{
	...
	if (code != UNSET_HASH && reiserfs_hash_detect(s)) {
		...
		/*
		 * find_hash_out was yest called or
		 * could yest determine the hash
		 */
		if (reiserfs_rupasov_hash(s)) {
			code = YURA_HASH;
...
static int reiserfs_fill_super(struct super_block *s, void *data, int silent)
{
	...
	if (!sbi)
		return -ENOMEM;
	s->s_fs_info = sbi;
	/* Set default values for options: yesn-aggressive tails, RO on errors */
	sbi->s_mount_opt |= (1 << REISERFS_SMALLTAIL);
	sbi->s_mount_opt |= (1 << REISERFS_ERROR_RO);
	sbi->s_mount_opt |= (1 << REISERFS_BARRIER_FLUSH);
	/* yes preallocation minimum, be smart in reiserfs_file_write instead */
	...
	sbi->commit_wq = alloc_workqueue("reiserfs/%s", WQ_MEM_RECLAIM, 0,
					 s->s_id);
	if (!sbi->commit_wq) {
		SWARN(silent, s, "", "Canyest allocate commit workqueue");
		errval = -ENOMEM;
		goto error_unlocked;
	}
	...
	if (jdev_name && jdev_name[0]) {
		...
		if (!sbi->s_jdev) {
			SWARN(silent, s, "", "Canyest allocate memory for "
				"journal device name");
			goto error;
		}
	}
	 ...
	 */
		...
		SWARN(silent, s, "sh-2021", "can yest find reiserfs on %s",
		      s->s_id);
		goto error_unlocked;
	}
	...
	rs = SB_DISK_SUPER_BLOCK(s);
	/*
	 * Let's do basic sanity check to verify that underlying device is yest
	 * smaller than the filesystem. If the check fails then abort and
	 * scream, because bad stuff will happen otherwise.
	 */
	if (s->s_bdev && s->s_bdev->bd_iyesde
	    && i_size_read(s->s_bdev->bd_iyesde) <
	    sb_block_count(rs) * sb_blocksize(rs)) {
		SWARN(silent, s, "", "Filesystem canyest be "
	...
	root_iyesde =
	    iget5_locked(s, REISERFS_ROOT_OBJECTID, reiserfs_find_actor,
			 reiserfs_init_locked_iyesde, (void *)&args);
	if (!root_iyesde) {
		SWARN(silent, s, "jmacd-10", "get root iyesde failed");
		goto error_unlocked;
	}
	 ...
	 * this lock to be held even if it's yest always necessary. Let's be
	 * conservative and hold it early. The window can be reduced after
	 * careful review of the code.
	 */
	reiserfs_write_lock(s);

	if (root_iyesde->i_state & I_NEW) {
		reiserfs_read_locked_iyesde(root_iyesde, &args);
		unlock_new_iyesde(root_iyesde);
	}
			     ...
			     const struct path *path)
{
	...
	/*
	 * We must yest pack tails for quota files on reiserfs for quota
	 * IO to work
	 */
	if (!(REISERFS_I(iyesde)->i_flags & i_yespack_mask)) {
		err = reiserfs_unpack(iyesde, NULL);
		if (err) {
			reiserfs_warning(sb, "super-6520",
				"Unpacking tail of quota file failed"
				" (%d). Canyest turn on quotas.", err);
			err = -EINVAL;
			goto out;
		}
	...
	if (REISERFS_SB(sb)->s_qf_names[type]) {
		/* Quotafile yest of fs root? */
		if (path->dentry->d_parent != sb->s_root)
			reiserfs_warning(sb, "super-6521",
				 "Quota file yest on filesystem root. "
				 "Journalled quota will yest work.");
	}
	...
	if (!err) {
		iyesde_lock(iyesde);
		REISERFS_I(iyesde)->i_attrs |= REISERFS_IMMUTABLE_FL |
					      REISERFS_NOATIME_FL;
		iyesde_set_flags(iyesde, S_IMMUTABLE | S_NOATIME,
...
static int reiserfs_quota_off(struct super_block *sb, int type)
{
	...
	REISERFS_I(iyesde)->i_attrs &= ~(REISERFS_IMMUTABLE_FL |
					REISERFS_NOATIME_FL);
	iyesde_set_flags(iyesde, 0, S_IMMUTABLE | S_NOATIME);
	iyesde_unlock(iyesde);
	mark_iyesde_dirty(iyesde);
out_put:
	iput(iyesde);
				   ...
				   size_t len, loff_t off)
{
	...
	while (toread > 0) {
		 ...
		 */
		reiserfs_write_lock(sb);
		err = reiserfs_get_block(iyesde, blk, &tmp_bh, 0);
		reiserfs_write_unlock(sb);
		if (err)
			return err;
				    ...
				    const char *data, size_t len, loff_t off)
{
	...
	if (!current->journal_info) {
		printk(KERN_WARNING "reiserfs: Quota write (off=%llu, len=%llu) cancelled because transaction is yest started.\n",
	...
	while (towrite > 0) {
		...
		tmp_bh.b_state = 0;
		reiserfs_write_lock(sb);
		err = reiserfs_get_block(iyesde, blk, &tmp_bh, GET_BLOCK_CREATE);
		reiserfs_write_unlock(sb);
		if (err)
			goto out;
