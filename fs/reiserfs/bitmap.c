...
#define  concentrating_formatted_nodes(s) test_bit( \
    _ALLOC_concentrating_formatted_nodes, &SB_ALLOC_OPTS(s))
#define  displacing_large_files(s)    test_bit(_ALLOC_displacing_large_files, &SB_ALLOC_OPTS( \
    s))
#define  displacing_new_packing_localities(s) test_bit( \
    _ALLOC_displacing_new_packing_localities, &SB_ALLOC_OPTS(s))
...
static inline int this_blocknr_allocation_would_make_it_a_large_file(reiserfs_blocknr_hint_t *
    hint) {
  return hint->block
