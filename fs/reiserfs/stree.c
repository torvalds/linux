...
inline void le_key2cpu_key(struct cpu_key *to,
...
static inline const struct reiserfs_key *get_lkey(
    const struct treepath *chk_path,
...
static int has_valid_deh_location(struct buffer_head *bh,
    struct item_head *ih) {
      ...
      *cut_size = 0;
      ...
      while (le_ih_k_offset(&s_ih) + (pos - 1) * blk_size > new_file_length) {
        ...
        if (!delete && (*cut_size) != 0
            && reiserfs_transaction_free_space(th)
