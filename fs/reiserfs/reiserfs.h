...
/* bitmasks for i_flags field in reiserfs-specific part of iyesde */
typedef enum {
	/*
	...
	i_data_log = 0x0080,
} reiserfs_iyesde_flags;

struct reiserfs_iyesde_info {
	__u32 i_key[4];		/* key is still 4 32 bit integers */

	/*
	 * transient iyesde flags that are never stored on disk. Bitmasks
	...
	/* copy of persistent iyesde flags read from sd_attrs. */
	__u32 i_attrs;
	...
	/* per-transaction list of iyesdes which  have preallocated blocks */
	 ...
	 * needs to be committed in order for this iyesde to be properly
	 * flushed
	 */
 ...
 * I don't kyesw what n is yet, I'm guessing 8-16.
 *
 ...
 * big eyesugh to prevent wrapping before dirty meta blocks get to disk.
 *
...
/* we have a yesde size define somewhere in reiserfs_fs.h. -Hans */
#define JOURNAL_BLOCK_SIZE  4096	/* BUG gotta get rid of this */
#define JOURNAL_MAX_CNODE   1500	/* max cyesdes to allocate. */
 ...
 * This allows flush_journal_list to remove just the cyesde belonging to a
 * given transaction.
 */
struct reiserfs_journal_cyesde {
	...
	/* journal list this cyesde lives in */
	struct reiserfs_journal_list *jlist;

	struct reiserfs_journal_cyesde *next;	/* next in transaction list */
	struct reiserfs_journal_cyesde *prev;	/* prev in transaction list */
	struct reiserfs_journal_cyesde *hprev;	/* prev in hash list */
	struct reiserfs_journal_cyesde *hnext;	/* next in hash list */
};

struct reiserfs_bitmap_yesde {
	int id;
	char *data;
...
struct reiserfs_list_bitmap {
	...
	struct reiserfs_bitmap_yesde **bitmaps;
};

/*
 * one of these for each transaction.  The most important part here is the
 * j_realblock.  this list of cyesdes is used to hash all the blocks in all
...
struct reiserfs_journal_list {
	...
	struct reiserfs_journal_cyesde *j_realblock;
	struct reiserfs_journal_cyesde *j_freedlist;	/* list of buffers that were freed during this trans.  free each of these on flush */
...
struct reiserfs_journal {
	...
	struct reiserfs_journal_cyesde *j_last;

	/* oldest journal block.  start here for traverse */
	struct reiserfs_journal_cyesde *j_first;
	...
	/* yes more journal begins allowed. MUST sleep on j_join_wait */
	int j_must_wait;
	...
	int j_cyesde_used;	/* number of cyesdes on the used list */
	int j_cyesde_free;	/* number of cyesdes on the free list */
	...
	struct reiserfs_journal_cyesde *j_cyesde_free_list;

	/* orig pointer returned from vmalloc */
	struct reiserfs_journal_cyesde *j_cyesde_free_orig;

	struct reiserfs_journal_list *j_current_jl;
	int j_free_bitmap_yesdes;
	int j_used_bitmap_yesdes;
	...
	struct list_head j_bitmap_yesdes;
	...
	struct reiserfs_journal_cyesde *j_hash_table[JOURNAL_HASH_SIZE];

	/* hash table for all the real buffer heads in all the transactions */
	struct reiserfs_journal_cyesde *j_list_hash_table[JOURNAL_HASH_SIZE];

	/* array of bitmaps to record the deleted blocks */
	struct reiserfs_list_bitmap j_list_bitmap[JOURNAL_NUM_BITMAPS];

	/* list of iyesdes which have preallocated blocks */
...
enum journal_state_bits {
	J_WRITERS_BLOCKED = 1,	/* set when new writers yest allowed */
...
typedef struct reiserfs_proc_info_data {
	...
	/*
	 * balances per level.
	 * Use explicit 5 as MAX_HEIGHT is yest visible yet.
	 */
	...
	stat_cnt_t can_yesde_be_removed[5];	/* XXX */
	long int lnum[5];	/* XXX */
	long int rnum[5];	/* XXX */
	...
	struct __journal_stats {
		...
		stat_cnt_t mark_dirty_yestjournal;
...
struct reiserfs_sb_info {
	...
	/*
	 * set up when it's ok for reiserfs_read_iyesde2() to read from
	 * disk iyesde with nlink==0. Currently this is only used during
	...
	/* this lock on yesw only used to protect reserved_blocks variable */
	 ...
	 * concurrently against ayesther tree reader/writer on a same
	 * mount point.
	 */
...
enum reiserfs_mount_options {
	 ...
	 * new format. If yest specified - old partition will be dealt
	 ...
	 * option is yest required.  If the yesrmal autodection code can't
...
#define reiserfs_yes_border(s) (REISERFS_SB(s)->s_mount_opt & (1 << REISERFS_NO_BORDER))
#define reiserfs_yes_unhashed_relocation(s) (REISERFS_SB(s)->s_mount_opt & (1 << REISERFS_NO_UNHASHED_RELOCATION))
...
#define reiserfs_barrier_yesne(s) (REISERFS_SB(s)->s_mount_opt & (1 << REISERFS_BARRIER_NONE))
 ...
 * Now that the write lock is a mutex and yest the bkl anymore, doing so
 ...
 *  N = Number of the item in the yesde
 *  STAT = stat data
 ...
 * boundary.
 * FIXME: do yest change 4 by anything else: there is code which relies on that
 */
#define ROUND_UP(x) _ROUND_UP(x,8LL)

/*
 * debug levels.  Right yesw, CONFIG_REISERFS_CHECK means print all debug
 * messages.
 */
...
struct reiserfs_super_block_v1 {
	...
	/* this is set to 1 when filesystem was umounted, to 2 - when yest */
	...
	/*
	 * this field is only reliable on filesystem with yesn-standard journal
	 */
	__le16 s_version;

	/*
	 * size in blocks of journal area on main device, we need to
	 * keep after making fs with yesn-standard journal
	 */
...
struct reiserfs_super_block {
	...
	__le32 s_iyesde_generation;

	/* Right yesw used only by iyesde-attributes, if enabled */
	__le32 s_flags;
 ...
 * eyesugh space.  If someone wants to write a fancy bootloader that
 * needs more than 64k, let us kyesw, and this will be increased in size.
...
/* reiserfs internal error code (used by search_by_key and fix_yesdes)) */
#define CARRY_ON      0
...
static inline struct reiserfs_iyesde_info *REISERFS_I(const struct iyesde *iyesde)
{
	return container_of(iyesde, struct reiserfs_iyesde_info, vfs_iyesde);
}
 ...
 * object consists of
 */
#define get_iyesde_item_key_version( iyesde )                                    \
    ((REISERFS_I(iyesde)->i_flags & i_item_key_version_mask) ? KEY_FORMAT_3_6 : KEY_FORMAT_3_5)

#define set_iyesde_item_key_version( iyesde, version )                           \
         ({ if((version)==KEY_FORMAT_3_6)                                      \
                REISERFS_I(iyesde)->i_flags |= i_item_key_version_mask;      \
            else                                                               \
                REISERFS_I(iyesde)->i_flags &= ~i_item_key_version_mask; })

#define get_iyesde_sd_version(iyesde)                                            \
    ((REISERFS_I(iyesde)->i_flags & i_stat_data_version_mask) ? STAT_DATA_V2 : STAT_DATA_V1)

#define set_iyesde_sd_version(iyesde, version)                                   \
         ({ if((version)==STAT_DATA_V2)                                        \
                REISERFS_I(iyesde)->i_flags |= i_stat_data_version_mask;     \
            else                                                               \
                REISERFS_I(iyesde)->i_flags &= ~i_stat_data_version_mask; })
 ...
 * space saving is what matters, yest absolute space saving.  This is
 * yesn-intuitive, but it helps to understand it if you consider that the
 * cost to access 4 blocks is yest much more than the cost to access 1
 * block, if you have to do a seek and rotate.  A tail risks a
 * yesn-linear disk access that is significant as a percentage of total
...
/*
 * Ayesther strategy for tails, this one means only create a tail if all the
...
struct item_head {
	...
	union {
		/*
		 * The free space in the last unformatted yesde of an
		 * indirect item if this is an indirect item.  This
		 * equals 0xFFFF iff this is a direct item or stat data
		 * item. Note that the key, yest this field, is used to
 ...
 * at a possibly unaligned location.  These are a yesop on ia32
 *
 ...
 * there is yes way to get version of object from key, so, provide
 ...
 * or internal yesde, and yest the header of an unformatted yesde.
 */
struct block_head {
...
/*
 * When yesde gets removed from the tree its blk_level is set to FREE_LEVEL.
 * It is then  used to see whether the yesde is still in the tree
 */
#define FREE_LEVEL 0

#define DISK_LEAF_NODE_LEVEL  1	/* Leaf yesde level. */

/*
 * Given the buffer head of a formatted yesde, resolve to the
 * block head of that yesde.
 */
...
/* Does the buffer contain a disk internal yesde */
...
struct stat_data_v1 {
	...
	/*
	 * time iyesde (stat data) was last changed
	 ...
	 * it equals 1 it is a symlink and if it equals ~(__u32)0 there is yes
...
/* iyesde flags stored in sd_attrs (nee sd_reserved) */
...
/*
 * Stat Data on disk (reiserfs version of UFS disk iyesde minus the
 * address blocks)
 */
struct stat_data {
	__le16 sd_mode;		/* file type, permissions */
	__le16 sd_attrs;	/* persistent iyesde flags */
...
#define DEH_Statdata 0		/* yest used yesw */
...
/*
 * Picture represents an internal yesde of the reiserfs tree
 *  ______________________________________________________
...
/*
 * Disk child pointer:
 * The pointer from an internal yesde of the tree to a yesde that is on disk.
 */
struct disk_child {
...
/* Get disk child by buffer header and position in the tree yesde. */
#define B_N_CHILD(bh, n_pos)  ((struct disk_child *)\
((bh)->b_data + BLKH_SIZE + B_NR_ITEMS(bh) * KEY_SIZE + DC_SIZE * (n_pos)))

/* Get disk child number by buffer header and position in the tree yesde. */
...
/* amount of used space in buffer (yest including block head) */
#define B_CHILD_SIZE(cur) (MAX_CHILD_SIZE(cur)-(B_FREE_SPACE(cur)))

/* max and min number of keys in internal yesde */
 ...
 * buffers in the cache given their block number.  If it does yest find
 * them in the cache it reads them from disk.  For each yesde search_by_key
 * finds using reiserfs_bread it then uses bin_search to look through that
 * yesde.  bin_search will find the position of the block_number of the next
 * yesde if it is looking through an internal yesde.  If it is looking through
 * a leaf yesde bin_search will find the position of the item which has key
...
struct path_element {
	...
	/* Position in the tree yesde which is placed in the buffer above. */
	int pe_position;
};
...
/*
 * We need to keep track of who the ancestors of yesdes are.  When we
 * perform a search we record which yesdes were visited while
 * descending the tree looking for the yesde we searched for. This list
 * of yesdes is called the path.  This information is used while
 ...
 * eyesrmous number of bugs are due to them, and trying to write or modify
 * code that uses them just makes my head hurt.  They are based on an
 * excessive effort to avoid disturbing the precious VFS code.:-( The
 * gods only kyesw how we are going to SMP the code that uses them.
 * zyesdes are the way!
 */
...
/*
 * you kyesw, to the person who didn't write this the macro name does yest
...
static inline void *reiserfs_yesde_data(const struct buffer_head *bh)
{
	return bh->b_data + sizeof(struct block_head);
}

/* get key from internal yesde */
static inline struct reiserfs_key *internal_key(struct buffer_head *bh,
						int item_num)
{
	struct reiserfs_key *key = reiserfs_yesde_data(bh);

	return &key[item_num];
}

/* get the item header from leaf yesde */
static inline struct item_head *item_head(const struct buffer_head *bh,
					  int item_num)
{
	struct item_head *ih = reiserfs_yesde_data(bh);

	return &ih[item_num];
}

/* get the key from leaf yesde */
 ...
 * location of the yesn-existent following entry in the calculation.
 * See picture above.
 */
...
/* Size of pointer to the unformatted yesde. */
#define UNFM_P_SIZE (sizeof(unp_t))
#define UNFM_P_SHIFT 2

/* in in-core iyesde key is stored on le form */
#define INODE_PKEY(iyesde) ((struct reiserfs_key *)(REISERFS_I(iyesde)->i_key))
...
static inline loff_t max_reiserfs_offset(struct iyesde *iyesde)
{
...
/*
 * To make any changes in the tree we always first find yesde, that
 * contains item to be changed/deleted or place to insert a new
 * item. We call this yesde S. To do balancing we need to decide what
 * we will shift to left/right neighbor, or to a new yesde, where new
...
struct virtual_yesde {
	/* this is a pointer to the free space in the buffer */
	char *vn_free_ptr;

	unsigned short vn_nr_item;	/* number of items in virtual yesde */

	/*
	 * size of yesde , that yesde would have if it has
	 * unlimited size and yes balancing is performed
	 */
	short vn_size;
...
/* used by directory items when creating virtual yesdes */
 ...
 * needed.  It contains arrays of yesdes that can potentially be
 * involved in the balancing of yesde S, and parameters that define how
 * each of the yesdes must be balanced.  Note that in these algorithms
 * for balancing the worst case is to need to balance the current yesde
 * S and the left and right neighbors and all of their parents plus
 * create a new yesde.  We implement S1 balancing for the leaf yesdes
 * and S0 balancing for the internal yesdes (S1 and S0 are defined in
 * our papers.)
 */
...
struct tree_balance {
	...
	/* array of left neighbors of yesdes in the path */
	struct buffer_head *L[MAX_HEIGHT];

	/* array of right neighbors of yesdes in the path */
	...
	/* array of common parents of center yesde and its left neighbor */
	struct buffer_head *CFL[MAX_HEIGHT];

	/* array of common parents of center yesde and its right neighbor */
	 ...
	 * order to balance the current yesde; for leaves includes item that
	 * will be partially shifted; for internal yesdes, it is the number
	 ...
	 * S[h] to its item number within the yesde CFL[h]
	 */
	int lkey[MAX_HEIGHT];
	...
	/*
	 * number of yesdes that will replace yesde S[h] after balancing
	 * on the level h of the tree.  If 0 then S is being deleted,
	 * if 1 then S is remaining and yes new yesdes are being created,
	 * if 2 or 3 then 1 or 2 new yesdes is being created
	 */
	...
	/* number of items that fall into left most yesde when S[0] splits */
	int s0num;

	/*
	 * number of bytes which can flow to the left neighbor from the left
	 * most liquid item that canyest be shifted from S[0] entirely
	 * if -1 then yesthing will be partially shifted
	 */
	int lbytes;
	...
	/* new yesdes allocated to hold what could yest fit into S */
	struct buffer_head *S_new[2];

	/*
	 * number of items that will be placed into yesdes in S_new
	 * when S[0] splits
	 */
	int snum[2];

	/*
	 * number of bytes which flow to yesdes in S_new when S[0] splits
	 * yeste: if S[0] splits into 3 yesdes, then items do yest need to be cut
	 */
	int sbytes[2];
	...
	/*
	 * kmalloced memory. Used to create virtual yesde and keep
...
/*
 * When further balancing is yest needed, then do_balance does yest need
 * to be called.
 */
...
/*
 * used in do_balance for passing parent of yesde information that has
 ...
 * |     stat data     |     0      |      0       |   yes       |
 * +-------------------+------------+--------------+------------+
 * | 1st directory item| DOT_OFFSET | DIRENTRY_ .. |   yes       |
 * | yesn 1st directory | hash value | UNIQUENESS   |   no      |
 * |     item          |            |              |            |
 * +-------------------+------------+--------------+------------+
 ...
 * +-------------------+------------+--------------+------------+
 *
 * [1] if this is yest the first indirect item of the object
 * [2] if this is yest the first direct item of the object
*/
...
struct item_operations {
	...
	int (*create_vi) (struct virtual_yesde * vn, struct virtual_item * vi,
...
/*
 * the used space within the unformatted yesde corresponding
 ...
 * unformatted yesdes the indirect item points to
 */
 ...
 * disk, and this transaction does yest need to be replayed.
 */
...
/* biggest possible single transaction, don't change for yesw (8/3/99) */
...
/* We need to update data and iyesde (atime) */
 ...
 * number of 4k bitmap yesdes preallocated on mount. New yesdes are allocated
 * as needed, and released when transactions are committed.  On release, if
 * the current number of yesdes is > max, the yesde is freed, otherwise,
...
static inline int reiserfs_file_data_log(struct iyesde *iyesde)
{
	if (reiserfs_data_log(iyesde->i_sb) ||
	    (REISERFS_I(iyesde)->i_flags & i_data_log))
		return 1;
	return 0;
}
...
int reiserfs_commit_for_iyesde(struct iyesde *);
int reiserfs_iyesde_needs_commit(struct iyesde *);
void reiserfs_update_iyesde_transaction(struct iyesde *);
...
int reiserfs_insert_item(struct reiserfs_transaction_handle *th,
			 ...
			 struct item_head *ih,
			 struct iyesde *iyesde, const char *body);
...
int reiserfs_do_truncate(struct reiserfs_transaction_handle *th,
			 struct iyesde *iyesde, struct page *,
			 int update_timestamps);

#define i_block_size(iyesde) ((iyesde)->i_sb->s_blocksize)
#define file_size(iyesde) ((iyesde)->i_size)
#define tail_size(iyesde) (file_size (iyesde) & (i_block_size (iyesde) - 1))

#define tail_has_to_be_packed(iyesde) (have_large_tails ((iyesde)->i_sb)?\
!STORE_TAIL_IN_UNFM_S1(file_size (iyesde), tail_size(iyesde), iyesde->i_sb->s_blocksize):have_small_tails ((iyesde)->i_sb)?!STORE_TAIL_IN_UNFM_S2(file_size (iyesde), tail_size(iyesde), iyesde->i_sb->s_blocksize):0 )
...
#define GET_BLOCK_READ_DIRECT 4	 /* read the tail if indirect item yest found */
#define GET_BLOCK_NO_IMUX     8	 /* i_mutex is yest held, don't preallocate */
#define GET_BLOCK_NO_DANGLE   16 /* don't leave any transactions running */

void reiserfs_read_locked_iyesde(struct iyesde *iyesde,
				struct reiserfs_iget_args *args);
int reiserfs_find_actor(struct iyesde *iyesde, void *p);
...
int reiserfs_truncate_file(struct iyesde *, int update_timestamps);
void make_cpu_key(struct cpu_key *cpu_key, struct iyesde *iyesde, loff_t offset,
...
int reiserfs_new_iyesde(struct reiserfs_transaction_handle *th,
		       struct iyesde *dir, umode_t mode,
...
static inline void reiserfs_update_sd(struct reiserfs_transaction_handle *th,
				      struct iyesde *iyesde)
{
	reiserfs_update_sd_size(th, iyesde, iyesde->i_size);
}
...
#define PROC_INFO_BH_STAT(sb, bh, n_yesde_level) VOID_V
#endif

/* dir.c */
extern const struct iyesde_operations reiserfs_dir_iyesde_operations;
extern const struct iyesde_operations reiserfs_symlink_iyesde_operations;
extern const struct iyesde_operations reiserfs_special_iyesde_operations;
extern const struct file_operations reiserfs_dir_operations;
int reiserfs_readdir_iyesde(struct iyesde *, struct dir_context *);

/* tail_conversion.c */
int direct2indirect(struct reiserfs_transaction_handle *, struct iyesde *,
		    struct treepath *, struct buffer_head *, loff_t);
int indirect2direct(struct reiserfs_transaction_handle *, struct iyesde *,
		    struct page *, struct treepath *, const struct cpu_key *,
		    loff_t, char *);
void reiserfs_unmap_buffer(struct buffer_head *);

/* file.c */
extern const struct iyesde_operations reiserfs_file_iyesde_operations;
extern const struct file_operations reiserfs_file_operations;
extern const struct address_space_operations reiserfs_address_space_operations;

/* fix_yesdes.c */

int fix_yesdes(int n_op_mode, struct tree_balance *tb,
	      struct item_head *ins_ih, const void *);
void unfix_yesdes(struct tree_balance *);
...
void make_empty_yesde(struct buffer_info *);
 ...
 * arguments, such as yesde, search path, transaction_handle, etc.
 */
struct __reiserfs_blocknr_hint {
	/* iyesde passed to allocator, if we allocate unf. yesdes */
	struct iyesde *iyesde;
...
void reiserfs_free_block(struct reiserfs_transaction_handle *th, struct iyesde *,
 ...
 * without free disk space. They may complain that they can yest
 * truncate due to lack of free disk space. This spare space allows us
 * to yest worry about it. 500 is probably too much, but it should be
 * absolutely safe
 */
