...
#define set_sb_block_count(sbp, \
      v)   ((sbp)->s_v1.s_block_count = cpu_to_le32(v))
#define sb_free_blocks(sbp)         (le32_to_cpu((sbp)->s_v1.s_free_blocks))
#define set_sb_free_blocks(sbp, \
      v)   ((sbp)->s_v1.s_free_blocks = cpu_to_le32(v))
...
#define set_sb_umount_state(sbp, \
      v) ((sbp)->s_v1.s_umount_state = cpu_to_le16(v))
...
struct reiserfs_journal_list {
  ...
  struct reiserfs_journal_cnode *j_freedlist; /* list of buffers that were freed
                                               * during this trans.  free each
                                               * of these on flush */
...
#define reiserfs_rupasov_hash(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << FORCE_RUPASOV_HASH))
#define reiserfs_tea_hash(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << FORCE_TEA_HASH))
#define reiserfs_hash_detect(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << FORCE_HASH_DETECT))
#define reiserfs_no_border(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_NO_BORDER))
#define reiserfs_no_unhashed_relocation(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_NO_UNHASHED_RELOCATION))
#define reiserfs_hashed_relocation(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_HASHED_RELOCATION))
#define reiserfs_test4(s) (REISERFS_SB(s)->s_mount_opt & (1 << REISERFS_TEST4))

#define have_large_tails(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_LARGETAIL))
#define have_small_tails(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_SMALLTAIL))
#define replay_only(s) (REISERFS_SB(s)->s_mount_opt & (1 << REPLAYONLY))
#define reiserfs_attrs(s) (REISERFS_SB(s)->s_mount_opt & (1 << REISERFS_ATTRS))
#define old_format_only(s) (REISERFS_SB(s)->s_properties & (1 << REISERFS_3_5))
#define convert_reiserfs(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_CONVERT))
#define reiserfs_data_log(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_DATA_LOG))
#define reiserfs_data_ordered(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_DATA_ORDERED))
#define reiserfs_data_writeback(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_DATA_WRITEBACK))
#define reiserfs_xattrs_user(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_XATTRS_USER))
#define reiserfs_posixacl(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_POSIXACL))
#define reiserfs_expose_privroot(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_EXPOSE_PRIVROOT))
#define reiserfs_xattrs_optional(s) (reiserfs_xattrs_user(s) \
  || reiserfs_posixacl(s))
#define reiserfs_barrier_none(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_BARRIER_NONE))
#define reiserfs_barrier_flush(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_BARRIER_FLUSH))

#define reiserfs_error_panic(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_ERROR_PANIC))
#define reiserfs_error_ro(s) (REISERFS_SB(s)->s_mount_opt \
  & (1 << REISERFS_ERROR_RO))
...
#define reiserfs_is_journal_aborted(journal) (unlikely( \
    __reiserfs_is_journal_aborted(journal)))
...
static inline void reiserfs_mutex_lock_nested_safe(struct mutex *m,
...
static inline void reiserfs_down_read_safe(struct rw_semaphore *sem,
    struct super_block *s) {
  int depth;
  depth = reiserfs_write_unlock_nested(s);
  down_read(sem);
...
#define RFALSE(cond, format, args ...) __RASSERT(!(cond), "!(" #cond ")", \
    format, ## args)
#else
...
#define get_inode_item_key_version(inode)                                    \
  ...
  & i_item_key_version_mask) ? KEY_FORMAT_3_6 : KEY_FORMAT_3_5)
...
#define get_inode_sd_version(inode)                                            \
  ((REISERFS_I(inode)->i_flags \
  & i_stat_data_version_mask) ? STAT_DATA_V2 : STAT_DATA_V1)
...
#define put_ih_free_space(ih, \
      val)   do { (ih)->u.ih_free_space_reserved = cpu_to_le16(val); } while (0)
#define put_ih_version(ih, val)      do { (ih)->ih_version = cpu_to_le16(val); \
} while (0)
#define put_ih_entry_count(ih, val)  do { (ih)->u.ih_entry_count = cpu_to_le16( \
    val); } while (0)
#define put_ih_location(ih, val)     do { (ih)->ih_item_location = cpu_to_le16( \
    val); } while (0)
...
#define get_ih_free_space(ih) (ih_version(ih) \
  == KEY_FORMAT_3_6 ? 0 : ih_free_space(ih))
#define set_ih_free_space(ih, val) put_ih_free_space((ih), \
...
static inline loff_t le_key_k_type(int version,
    const struct reiserfs_key *key) {
...
#define set_blkh_nr_item(p_blkh, \
      val)  ((p_blkh)->blk_nr_item = cpu_to_le16(val))
#define set_blkh_free_space(p_blkh, \
      val) ((p_blkh)->blk_free_space = cpu_to_le16(val))
#define set_blkh_reserved(p_blkh, val) ((p_blkh)->blk_reserved = cpu_to_le16( \
    val))
#define blkh_right_delim_key(p_blkh)  ((p_blkh)->blk_right_delim_key)
#define set_blkh_right_delim_key(p_blkh, \
      val)  ((p_blkh)->blk_right_delim_key = val)
...
#define PUT_B_NR_ITEMS(bh, val)   do { set_blkh_nr_item(B_BLK_HEAD(bh), val); \
} while (0)
#define PUT_B_LEVEL(bh, val)    do { set_blkh_level(B_BLK_HEAD(bh), val); \
} while (0)
#define PUT_B_FREE_SPACE(bh, val) do { set_blkh_free_space(B_BLK_HEAD(bh), val); \
} while (0)
...
#define aligned_address(addr)           ((void *) ((long) (addr) \
  & ~((1UL << ADDR_UNALIGNED_BITS) - 1)))
#define unaligned_offset(addr)          (((int) ((long) (addr) \
  & ((1 << ADDR_UNALIGNED_BITS) - 1))) << 3)
...
#define SET_GENERATION_NUMBER(offset, \
      gen_number) (GET_HASH_VALUE(offset) | (gen_number))
...
#define put_dc_block_number(dc_p, \
      val)   do { (dc_p)->dc_block_number = cpu_to_le32(val); } while (0)
#define put_dc_size(dc_p, val)   do { (dc_p)->dc_size = cpu_to_le16(val); \
} while (0)
...
#define INITIALIZE_PATH(var) \
  struct treepath var \
    = {.path_length = ILLEGAL_PATH_ELEMENT_OFFSET, .reada = 0, }

/* Get path element by path and path position. */
#define PATH_OFFSET_PELEMENT(path, \
      n_offset)  ((path)->path_elements + (n_offset))

/* Get buffer header at the path by path and path position. */
#define PATH_OFFSET_PBUFFER(path, \
      n_offset)   (PATH_OFFSET_PELEMENT(path, n_offset)->pe_buffer)

/* Get position in the element at the path by path and path position. */
#define PATH_OFFSET_POSITION(path, \
      n_offset) (PATH_OFFSET_PELEMENT(path, n_offset)->pe_position)

#define PATH_PLAST_BUFFER(path) (PATH_OFFSET_PBUFFER((path), \
    (path)->path_length))
 ...
 */
#define PATH_LAST_POSITION(path) (PATH_OFFSET_POSITION((path), \
    (path)->path_length))
...
#define op_bytes_number(ih, \
      bsize)                    item_ops[le_ih_k_type(ih)]->bytes_number(ih, \
    bsize)
#define op_is_left_mergeable(key, \
      bsize)              item_ops[le_key_k_type(le_key_version(key), \
    key)]->is_left_mergeable(key, bsize)
#define op_print_item(ih, \
      item)                       item_ops[le_ih_k_type(ih)]->print_item(ih, \
    item)
#define op_check_item(ih, \
      item)                       item_ops[le_ih_k_type(ih)]->check_item(ih, \
    item)
#define op_create_vi(vn, vi, is_affected, \
      insert_size)  item_ops[le_ih_k_type((vi)->vi_ih)]->create_vi(vn, vi, \
    is_affected, \
    insert_size)
#define op_check_left(vi, free, start_skip, \
      end_skip) item_ops[(vi)->vi_index]->check_left(vi, free, start_skip, \
    end_skip)
#define op_check_right(vi, \
      free)                      item_ops[(vi)->vi_index]->check_right(vi, free)
#define op_part_size(vi, from, \
      to)                     item_ops[(vi)->vi_index]->part_size(vi, from, to)
#define op_unit_num(vi)            item_ops[(vi)->vi_index]->unit_num(vi)
#define op_print_vi(vi)                              item_ops[(vi)->vi_index]-> \
  print_vi(vi)
 ...
 */
#define I_POS_UNFM_SIZE(ih, pos, \
      size) (((pos) \
  == I_UNFM_NUM(ih) - 1) ? (size) - ih_free_space(ih) : (size))
...
#define B_I_STAT_DATA(bh, \
      ih) ((struct stat_data *) ((bh)->b_data + ih_location(ih)))
...
#define set_desc_trans_len(d, val)      do { (d)->j_len = cpu_to_le32(val); \
} while (0)
...
#define set_commit_trans_len(c, val)    do { (c)->j_len = cpu_to_le32(val); \
} while (0)
...
#ifdef CONFIG_QUOTA
#define REISERFS_QUOTA_OPTS ((1 << \
      REISERFS_USRQUOTA) | (1 << REISERFS_GRPQUOTA))
/* We need to update data and inode (atime) */
#define REISERFS_QUOTA_TRANS_BLOCKS(s) (REISERFS_SB(s)->s_mount_opt \
  & REISERFS_QUOTA_OPTS ? 2 : 0)
/* 1 balancing, 1 bitmap, 1 data per write + stat data update */
#define REISERFS_QUOTA_INIT_BLOCKS(s) (REISERFS_SB(s)->s_mount_opt \
  & REISERFS_QUOTA_OPTS   \
  ? (DQUOT_INIT_ALLOC * (JOURNAL_PER_BALANCE_CNT + 2) + DQUOT_INIT_REWRITE \
  + 1) : 0)
/* same as with INIT */
#define REISERFS_QUOTA_DEL_BLOCKS(s) (REISERFS_SB(s)->s_mount_opt \
  & REISERFS_QUOTA_OPTS   \
  ? (DQUOT_DEL_ALLOC * (JOURNAL_PER_BALANCE_CNT + 2) + DQUOT_DEL_REWRITE \
  + 1) : 0)
#else
...
#define _jhashfn(sb, block)  \
  ...
  ^ (((block) << \
      (JBH_HASH_SHIFT \
  - 6)) ^ ((block) >> 13) ^ ((block) << (JBH_HASH_SHIFT - 12))))
#define journal_hash(t, sb, \
      block) ((t)[_jhashfn((sb), (block)) & JBH_HASH_MASK])
...
static inline int reiserfs_transaction_free_space(
...
int search_by_key(struct super_block *, const struct cpu_key *,
    struct treepath *, int);
#define search_item(s, key, path) search_by_key(s, key, path, \
...
#define tail_has_to_be_packed(inode) (have_large_tails((inode)->i_sb) \
  ? !STORE_TAIL_IN_UNFM_S1(file_size(inode), tail_size(inode), \
    inode->i_sb->s_blocksize) : have_small_tails((inode)->i_sb) ? ! \
  STORE_TAIL_IN_UNFM_S2(file_size(inode), tail_size(inode), \
