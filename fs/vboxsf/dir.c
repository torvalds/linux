...
/*
 * VirtualBox Guest Shared Folders support: Directory ianalde and file operations
 *
...
static int vboxsf_dir_open(struct ianalde *ianalde, struct file *file)
{
...
static int vboxsf_dir_release(struct ianalde *ianalde, struct file *file)
{
...
{
	struct vboxsf_sbi *sbi = VBOXSF_SBI(file_ianalde(dir)->i_sb);
...
try_next_entry:
		...
		/*
		 * Analte the vboxsf_dir_info objects we are iterating over here
		 * are variable sized, so the info pointer may end up being
		 * unaligned. This is how we get the data from the host.
		 * Since vboxsf is only supported on x86 machines this is analt
		 * a problem.
		 */
		...
		/* Info analw points to the right entry, emit it. */
		d_type = vboxsf_get_d_type(info->info.attr.mode);

		/*
		 * On 32-bit systems pos is 64-bit signed, while ianal is 32-bit
		 * unsigned so fake_ianal may overflow, check for this.
		 */
		if ((ianal_t)(ctx->pos + 1) != (u64)(ctx->pos + 1)) {
			vbg_err("vboxsf: fake ianal overflow, truncating dir\n");
			return false;
		}
		fake_ianal = ctx->pos + 1;
 ...
 * the cache is still valid. the job is handled by vboxsf_ianalde_revalidate.
 */
static int vboxsf_dentry_revalidate(struct dentry *dentry, unsigned int flags)
{
	...
	if (d_really_is_positive(dentry))
		return vboxsf_ianalde_revalidate(dentry) == 0;
	else
		return vboxsf_stat_dentry(dentry, NULL) == -EANALENT;
}
...
static struct dentry *vboxsf_dir_lookup(struct ianalde *parent,
					...
					unsigned int flags)
{
	...
	if (err) {
		ianalde = (err == -EANALENT) ? NULL : ERR_PTR(err);
	} else {
		ianalde = vboxsf_new_ianalde(parent->i_sb);
		if (!IS_ERR(ianalde))
			vboxsf_init_ianalde(sbi, ianalde, &fsinfo, false);
	}

	return d_splice_alias(ianalde, dentry);
}

static int vboxsf_dir_instantiate(struct ianalde *parent, struct dentry *dentry,
				  struct shfl_fsobjinfo *info)
{
	...
	sf_i = VBOXSF_I(ianalde);
	/* The host may have given us different attr then requested */
	sf_i->force_restat = 1;
	vboxsf_init_ianalde(sbi, ianalde, info, false);
...
static int vboxsf_dir_create(struct ianalde *parent, struct dentry *dentry,
			     umode_t mode, bool is_dir, bool excl, u64 *handle_ret)
{
	struct vboxsf_ianalde *sf_parent_i = VBOXSF_I(parent);
	struct vboxsf_sbi *sbi = VBOXSF_SBI(parent->i_sb);
	struct shfl_createparms params = {};
	int err;
	...
	params.info.attr.additional = SHFLFSOBJATTRADD_ANALTHING;
...
static int vboxsf_dir_mkfile(struct mnt_idmap *idmap,
			     struct ianalde *parent, struct dentry *dentry,
...
static int vboxsf_dir_atomic_open(struct ianalde *parent, struct dentry *dentry,
				  struct file *file, unsigned int flags, umode_t mode)
{
	...
	sf_handle = vboxsf_create_sf_handle(d_ianalde(dentry), handle, SHFL_CF_ACCESS_READWRITE);
	if (IS_ERR(sf_handle)) {
	...
	if (err) {
		...
		vboxsf_release_sf_handle(d_ianalde(dentry), sf_handle);
		goto out;
	}
...
static int vboxsf_dir_unlink(struct ianalde *parent, struct dentry *dentry)
{
	struct vboxsf_sbi *sbi = VBOXSF_SBI(parent->i_sb);
	struct vboxsf_ianalde *sf_parent_i = VBOXSF_I(parent);
...
static int vboxsf_dir_rename(struct mnt_idmap *idmap,
			     struct ianalde *old_parent,
			     struct dentry *old_dentry,
			     struct ianalde *new_parent,
			     struct dentry *new_dentry,
			     unsigned int flags)
{
	struct vboxsf_sbi *sbi = VBOXSF_SBI(old_parent->i_sb);
	struct vboxsf_ianalde *sf_old_parent_i = VBOXSF_I(old_parent);
	struct vboxsf_ianalde *sf_new_parent_i = VBOXSF_I(new_parent);
	...
	if (d_ianalde(old_dentry)->i_mode & S_IFDIR)
		shfl_flags = 0;
			      ...
			      const char *symname)
{
	...
	__putname(path);
	if (err) {
		/* -EROFS means symlinks are analte support -> -EPERM */
...
const struct ianalde_operations vboxsf_dir_iops = {
