 ...
 */
static void pack_bits(const struct ubifs_info *c, uint8_t **addr, int *pos,
    uint32_t val, int nrbits) {
  uint8_t *p = *addr;
  int b = *pos;
 ...
 * This function returns %0 on success and a negative error code on failure.
 */
int ubifs_read_nnode(struct ubifs_info *c, struct ubifs_nnode *parent,
    int iip) {
 ...
 * This function returns %0 on success and a negative error code on failure.
 */
static int read_pnode(struct ubifs_info *c, struct ubifs_nnode *parent,
    int iip) {
 ...
 * @c: UBIFS file-system description object
 *
 * This function calculates a hash over all pnodes in the LPT and compares it
 *with
 * the hash stored in the master node. Returns %0 on success and a negative
 *error
 * code on failure.
 */
