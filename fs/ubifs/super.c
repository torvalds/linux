...
static int ubifs_default_version_set(const char *val,
 ...
 */
static int ubifs_write_inode(struct inode *inode,
 ...
 */
static int bud_wbuf_callback(struct ubifs_info *c, int lnum, int free,
    int pad) {
      ...
      case Opt_override_compr:
        {
          ...
          if (!strcmp(name, "none")) {
            ...
            ubifs_err(c, "unknown compressor \"%s\"", name); //FIXME: is c
                                                             // ready?
            kfree(name);
            return -EINVAL;
          }
...
static int __init ubifs_init(void) {
  ...
  if (PAGE_SIZE < UBIFS_BLOCK_SIZE) {
    pr_err(
        "UBIFS error (pid %d): VFS page cache size is %u bytes, but UBIFS requires at least 4096 bytes",
