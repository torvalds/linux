 ...
 * is found.
 */
static bool inode_still_linked(struct ubifs_info *c,
...
static int noinline_for_stack authenticate_sleb_hash(struct ubifs_info *c,
 ...
 * validation failed.
 */
static int validate_ref(struct ubifs_info *c,
    const struct ubifs_ref_node *ref) {
  struct ubifs_bud *bud;
 ...
 * failure.
 */
static int replay_log_leb(struct ubifs_info *c, int lnum, int offs,
    void *sbuf) {
  int err;
  struct ubifs_scan_leb *sleb;
  struct ubifs_scan_node *snod;
