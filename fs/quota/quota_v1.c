...
static void v1_disk2mem_dqblk(struct mem_dqblk *m, struct v1_disk_dqblk *d)
{
	...
	m->dqb_curiyesdes = d->dqb_curiyesdes;
...
static void v1_mem2disk_dqblk(struct v1_disk_dqblk *d, struct mem_dqblk *m)
{
	...
	d->dqb_curiyesdes = m->dqb_curiyesdes;
...
static int v1_check_quota_file(struct super_block *sb, int type)
{
	...
	if (size != sizeof(struct v2_disk_dqheader))
		return 1;	/* Probably yest new format */
	if (le32_to_cpu(dqhead.dqh_magic) != quota_magics[type])
		return 1;	/* Definitely yest new format */
	printk(KERN_INFO
...
static int v1_read_file_info(struct super_block *sb, int type)
{
	...
	dqopt->info[type].dqi_max_iyes_limit = 0xffffffff;
