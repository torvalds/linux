...
static int quota_getxstate(struct super_block *sb, int type,
    void __user *addr) {
  struct fs_quota_stat fqs;
  int ret;
  if (!sb->s_qcop->get_state) {
...
static int quota_getxstatev(struct super_block *sb, int type,
    void __user *addr) {
  struct fs_quota_statv fqs;
  int ret;
  if (!sb->s_qcop->get_state) {
...
static void copy_from_xfs_dqblk(struct qc_dqblk *dst,
