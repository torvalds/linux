 ...
 */
static ssize_t nlm_end_grace_write(struct file *file, const char __user *buf,
    size_t size,
    loff_t *pos) {
  char *data;
...
static ssize_t nlm_end_grace_read(struct file *file, char __user *buf,
    size_t size,
    loff_t *pos) {
  struct lockd_net *ln = net_generic(current->nsproxy->net_ns,
      lockd_net_id);
...
int __init lockd_create_procfs(void) {
...
void __exit lockd_remove_procfs(void) {
