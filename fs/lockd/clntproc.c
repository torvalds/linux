...
static void nlmclnt_locks_init_private(struct file_lock *fl,
...
static struct nlm_lockowner *nlmclnt_get_lockowner(
...
static struct nlm_lockowner *__nlmclnt_find_lockowner(struct nlm_host *host,
    fl_owner_t owner) {
  struct nlm_lockowner *lockowner;
  list_for_each_entry(lockowner, &host->h_lockowners, list) {
...
static struct nlm_lockowner *nlmclnt_find_lockowner(struct nlm_host *host,
    fl_owner_t owner) {
 ...
 * Generic NLM call
 */
static int nlmclnt_call(const struct cred *cred, struct nlm_rqst *req,
    u32 proc) {
 ...
 */
static struct rpc_task *__nlm_async_call(struct nlm_rqst *req, u32 proc,
    struct rpc_message *msg,
...
static int nlm_do_async_call(struct nlm_rqst *req, u32 proc,
    struct rpc_message *msg,
    const struct rpc_call_ops *tk_ops) {
 ...
 */
int nlm_async_call(struct nlm_rqst *req, u32 proc,
    const struct rpc_call_ops *tk_ops) {
  struct rpc_message msg = {
    .rpc_argp = &req->a_args,
    .rpc_resp = &req->a_res,
...
int nlm_async_reply(struct nlm_rqst *req, u32 proc,
    const struct rpc_call_ops *tk_ops) {
  struct rpc_message msg = {
    .rpc_argp = &req->a_res,
  };
 ...
 *      state.
 */
static int nlmclnt_async_call(const struct cred *cred, struct nlm_rqst *req,
    u32 proc, const struct rpc_call_ops *tk_ops) {
  struct rpc_message msg = {
    .rpc_argp = &req->a_args,
    .rpc_resp = &req->a_res,
    .rpc_cred = cred,
 ...
 */
static int nlmclnt_test(struct nlm_rqst *req, struct file_lock *fl) {
  int status;
...
static void nlmclnt_locks_copy_lock(struct file_lock *new,
    struct file_lock *fl) {
  spin_lock(&fl->fl_u.nfs_fl.owner->host->h_lock);
  new->fl_u.nfs_fl.state = fl->fl_u.nfs_fl.state;
  new->fl_u.nfs_fl.owner = nlmclnt_get_lockowner(fl->fl_u.nfs_fl.owner);
  list_add_tail(&new->fl_u.nfs_fl.list,
 ...
 */
static int nlmclnt_lock(struct nlm_rqst *req, struct file_lock *fl) {
 ...
 */
static int nlmclnt_unlock(struct nlm_rqst *req, struct file_lock *fl) {
  struct nlm_host *host = req->a_host;
  struct nlm_res *resp = &req->a_res;
  int status;
  unsigned char flags = fl->c.flc_flags;
...
static void nlmclnt_unlock_prepare(struct rpc_task *task, void *data) {
  ...
  if (nlmclnt_ops && nlmclnt_ops->nlmclnt_unlock_prepare) {
    defer_call
      = nlmclnt_ops->nlmclnt_unlock_prepare(task, req->a_callback_data);
  }
  if (!defer_call) {
    rpc_call_start(task);
  }
 ...
 */
static int nlm_stat_to_errno(__be32 status) {
  switch (ntohl(status)) {
    case NLM_LCK_GRANTED:
      return 0;
    case NLM_LCK_DENIED:
