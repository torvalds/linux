 ...
 */
static void nlmsvc_insert_block_locked(struct nlm_block *block,
    unsigned long when) {
  struct nlm_block *b;
 ...
 */
static inline void nlmsvc_remove_block(struct nlm_block *block) {
  spin_lock(&nlm_blocked_lock);
  if (!list_empty(&block->b_list)) {
    list_del_init(&block->b_list);
    spin_unlock(&nlm_blocked_lock);
 ...
 */
static struct nlm_block *nlmsvc_lookup_block(struct nlm_file *file,
 ...
 */
static inline struct nlm_block *nlmsvc_find_block(struct nlm_cookie *cookie) {
  struct nlm_block *block;
  spin_lock(&nlm_blocked_lock);
  list_for_each_entry(block, &nlm_blocked, b_list) {
 ...
 * address. --okir
 */
static struct nlm_block *nlmsvc_create_block(struct svc_rqst *rqstp,
...
static struct nlm_lockowner *nlmsvc_get_lockowner(
...
static struct nlm_lockowner *__nlmsvc_find_lockowner(struct nlm_host *host,
    pid_t pid) {
  struct nlm_lockowner *lockowner;
...
static struct nlm_lockowner *nlmsvc_find_lockowner(struct nlm_host *host,
    pid_t pid) {
...
void nlmsvc_release_lockowner(struct nlm_lock *lock) {
 ...
 */
static __be32 nlmsvc_defer_lock_rqst(struct svc_rqst *rqstp,
    struct nlm_block *block) {
 ...
 * if required.
 */
__be32 nlmsvc_lock(struct svc_rqst *rqstp, struct nlm_file *file,
 ...
 * must be removed.
 */
__be32 nlmsvc_unlock(struct net *net, struct nlm_file *file,
    struct nlm_lock *lock) {
  int error = 0;
 ...
 */
__be32 nlmsvc_cancel_blocked(struct net *net, struct nlm_file *file,
    struct nlm_lock *lock) {
  struct nlm_block *block;
  int status = 0;
  int mode;
 ...
 */
static void nlmsvc_update_deferred_block(struct nlm_block *block, int result) {
 ...
 */
static void nlmsvc_notify_blocked(struct file_lock *fl) {
  struct nlm_block *block;
 ...
 */
static void nlmsvc_grant_blocked(struct nlm_block *block) {
  struct nlm_file *file = block->b_file;
 ...
 */
static void retry_deferred_block(struct nlm_block *block) {
