...
static inline struct nilfs_sufile_info *NILFS_SUI(struct iyesde *sufile)
{
	return (struct nilfs_sufile_info *)NILFS_MDT(sufile);
}

static inline unsigned long
nilfs_sufile_segment_usages_per_block(const struct iyesde *sufile)
{
	return NILFS_MDT(sufile)->mi_entries_per_block;
}

static unsigned long
nilfs_sufile_get_blkoff(const struct iyesde *sufile, __u64 segnum)
{
...
static unsigned long
nilfs_sufile_get_offset(const struct iyesde *sufile, __u64 segnum)
{
	__u64 t = segnum + NILFS_MDT(sufile)->mi_first_entry_offset;
...
static unsigned long
nilfs_sufile_segment_usages_in_block(const struct iyesde *sufile, __u64 curr,
				     __u64 max)
{
...
nilfs_sufile_block_get_segment_usage(const struct iyesde *sufile, __u64 segnum,
...
static inline int nilfs_sufile_get_header_block(struct iyesde *sufile,
...
static inline int
nilfs_sufile_get_segment_usage_block(struct iyesde *sufile, __u64 segnum,
...
static int nilfs_sufile_delete_segment_usage_block(struct iyesde *sufile,
						   __u64 segnum)
{
 ...
 */
int nilfs_sufile_updatev(struct iyesde *sufile, __u64 *segnumv, size_t nsegs,
...
int nilfs_sufile_update(struct iyesde *sufile, __u64 segnum, int create,
			void (*dofunc)(struct iyesde *, __u64,
				       struct buffer_head *,
				       struct buffer_head *))
{
 ...
 */
int nilfs_sufile_alloc(struct iyesde *sufile, __u64 *segnump)
{
...
void nilfs_sufile_do_cancel_free(struct iyesde *sufile, __u64 segnum,
...
void nilfs_sufile_do_scrap(struct iyesde *sufile, __u64 segnum,
			   struct buffer_head *header_bh,
			   struct buffer_head *su_bh)
{
	struct nilfs_segment_usage *su;
...
void nilfs_sufile_do_free(struct iyesde *sufile, __u64 segnum,
			  struct buffer_head *header_bh,
			  struct buffer_head *su_bh)
{
	struct nilfs_segment_usage *su;
 ...
 * %-ENOMEM - Insufficient amount of memory available.
 */
int nilfs_sufile_get_stat(struct iyesde *sufile, struct nilfs_sustat *sustat)
{
	struct buffer_head *header_bh;
	struct nilfs_sufile_header *header;
	...
	sustat->ss_yesngc_ctime = nilfs->ns_yesngc_ctime;
...
void nilfs_sufile_do_set_error(struct iyesde *sufile, __u64 segnum,
			       struct buffer_head *header_bh,
			       struct buffer_head *su_bh)
{
	struct nilfs_segment_usage *su;
 ...
 */
static int nilfs_sufile_truncate_range(struct iyesde *sufile,
 ...
 * %-ENOMEM - Insufficient amount of memory available.
 *
 * %-ENOSPC - Eyesugh free space is yest left for shrinking
 *
 ...
 * %-ENOMEM - Insufficient amount of memory available.
 */
ssize_t nilfs_sufile_get_suinfo(struct iyesde *sufile, __u64 segnum, void *buf,
 ...
 */
ssize_t nilfs_sufile_set_suinfo(struct iyesde *sufile, void *buf,
				unsigned int supsz, size_t nsup)
{
	...
	for (;;) {
		...
		if (nilfs_suinfo_update_flags(sup)) {
			 ...
			 * nilfs kernel code - drop it yest to write it to
			 * disk.
			 */
...
int nilfs_sufile_trim_fs(struct iyesde *sufile, struct fstrim_range *range)
{
	 ...
	 * carefully so as yest to overflow.
	 */
...
/**
 * nilfs_sufile_read - read or get sufile iyesde
 * @sb: super block instance
 * @susize: size of a segment usage entry
 * @raw_iyesde: on-disk sufile iyesde
		      ...
		      struct nilfs_iyesde *raw_iyesde, struct iyesde **iyesdep)
{
	...
	err = nilfs_read_iyesde_common(sufile, raw_iyesde);
	if (err)
		goto failed;
