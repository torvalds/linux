 ...
 * @ri_ri_cyes: Number of the last checkpoint
 ...
 * @flags: State flags
 * @dirty_file_ptr: Pointer on dirty_files list, or iyesde of a target file
 * @gc_iyesde_ptr: Pointer on the list of gc-iyesdes
 */
struct nilfs_cstage {
	int			scnt;
	unsigned int		flags;
	struct nilfs_iyesde_info *dirty_file_ptr;
	struct nilfs_iyesde_info *gc_iyesde_ptr;
};
 ...
 * @sc_gc_iyesdes: List of GC iyesdes having blocks to be written
 * @sc_iput_queue: list of iyesdes for which iput should be done
 * @sc_iput_work: work struct to defer iput call
 * @sc_freesegs: array of segment numbers to be freed
 * @sc_nfreesegs: number of segments on @sc_freesegs
 * @sc_dsync_iyesde: iyesde whose data pages are written for a sync operation
 ...
 * @sc_cyes: checkpoint number of current log
 * @sc_flags: Internal flags
 * @sc_state_lock: spinlock for sc_state and so on
 * @sc_state: Segctord state flags
 * @sc_flush_request: iyesde bitmap of metadata files to be flushed
...
struct nilfs_sc_info {
	...
	struct list_head	sc_gc_iyesdes;
	...
	struct nilfs_iyesde_info *sc_dsync_iyesde;
...
enum {
	...
	NILFS_SC_UNCLOSED,	/* Logical segment is yest closed */
...
extern int nilfs_construct_dsync_segment(struct super_block *, struct iyesde *,
					 loff_t, loff_t);
extern void nilfs_flush_segment(struct super_block *, iyes_t);
