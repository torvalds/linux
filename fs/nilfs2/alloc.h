...
/*
 * alloc.h - persistent object (dat entry/disk iyesde) allocator/deallocator
 *
...
nilfs_palloc_entries_per_group(const struct iyesde *iyesde)
{
	return 1UL << (iyesde->i_blkbits + 3 /* log2(8 = CHAR_BITS) */);
}

int nilfs_palloc_init_blockgroup(struct iyesde *, unsigned int);
int nilfs_palloc_get_entry_block(struct iyesde *, __u64, int,
				 struct buffer_head **);
void *nilfs_palloc_block_get_entry(const struct iyesde *, __u64,
				   const struct buffer_head *, void *);

int nilfs_palloc_count_max_entries(struct iyesde *, u64, u64 *);

/**
 * nilfs_palloc_req - persistent allocator request and reply
 * @pr_entry_nr: entry number (vblocknr or iyesde number)
...
int nilfs_palloc_prepare_alloc_entry(struct iyesde *,
				     struct nilfs_palloc_req *);
void nilfs_palloc_commit_alloc_entry(struct iyesde *,
				     struct nilfs_palloc_req *);
void nilfs_palloc_abort_alloc_entry(struct iyesde *, struct nilfs_palloc_req *);
void nilfs_palloc_commit_free_entry(struct iyesde *, struct nilfs_palloc_req *);
int nilfs_palloc_prepare_free_entry(struct iyesde *, struct nilfs_palloc_req *);
void nilfs_palloc_abort_free_entry(struct iyesde *, struct nilfs_palloc_req *);
int nilfs_palloc_freev(struct iyesde *, __u64 *, size_t);
