...
#include <linux/nilfs2_ondisk.h>	/* nilfs_binfo, nilfs_iyesde, etc */
#include "alloc.h"
#include "dat.h"
 ...
 * @data: cached raw block mapping of on-disk iyesde
...
int nilfs_bmap_read(struct nilfs_bmap *, struct nilfs_iyesde *);
void nilfs_bmap_write(struct nilfs_bmap *, struct nilfs_iyesde *);
 ...
 * Internal use only
 */
struct iyesde *nilfs_bmap_get_dat(const struct nilfs_bmap *);
