 ...
 */
static inline struct netfs_group *netfs_get_group(
 ...
 * Dispose of a netfs group attached to a dirty page (e.g. a ceph snap).
 */
static inline void netfs_put_group_many(struct netfs_group *netfs_group,
    int nr) {
...
static inline enum fscache_cache_state fscache_cache_state(
    const struct fscache_cache *cache) {
 ...
 * fscache-main.c
 */
extern unsigned int fscache_hash(unsigned int salt, const void *data,
    size_t len);
#ifdef CONFIG_FSCACHE
int __init fscache_init(void);
void __exit fscache_exit(void);
#else
static inline int fscache_init(void) {
  return 0;
}
...
static inline int fscache_stats_show(struct seq_file *m) {
  return 0;
}
