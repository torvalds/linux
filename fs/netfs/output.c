 ...
 * been completed.
 */
static void netfs_write_terminated(struct netfs_io_request *wreq,
    bool was_async) {
  struct netfs_io_subrequest *subreq;
 ...
 */
struct netfs_io_request *netfs_begin_writethrough(struct kiocb *iocb,
    size_t len) {
  struct netfs_io_request *wreq;
  ...
  iov_iter_xarray(&wreq->iter, ITER_SOURCE, &wreq->mapping->i_pages,
      wreq->start, 0);
  wreq->io_iter = wreq->iter;
  /* ->outstanding > 0 carries a ref */
  netfs_get_request(wreq, netfs_rreq_trace_get_for_outstanding);
...
static void netfs_submit_writethrough(struct netfs_io_request *wreq,
    bool final) {
 ...
 * subrequest.
 */
int netfs_advance_writethrough(struct netfs_io_request *wreq, size_t copied,
    bool to_page_end) {
