...
struct ckpt_req_control {
  ...
  unsigned int cur_time;    /* cur wait time in msec for currently issued
                             * checkpoint */
...
struct discard_cmd_control {
  ...
  unsigned int max_discard_issue_time;  /* max. interval between discard issue
                                         * */
  unsigned int discard_io_aware_gran; /* minimum discard granularity not be
                                       * aware of I/O */
  unsigned int discard_urgent_util; /* utilization which issue discard
                                     * proactively */
  unsigned int discard_granularity; /* discard granularity */
  unsigned int max_ordered_discard; /* maximum discard granularity issued by lba
                                     * order */
...
struct f2fs_inode_info {
  ...
  struct f2fs_rwsem i_xattr_sem; /* avoid racing between reading and changing
                                  * EAs */
...
struct f2fs_nm_info {
  ...
  struct list_head free_nid_list;   /* list for free nids excluding preallocated
                                     * nids */
...
struct f2fs_bio_info {
  ...
  struct completion zone_wait;  /* condition value for the previous open zone to
                                 * close */
...
struct f2fs_sb_info {
  ...
  unsigned long seq_file_ra_mul;    /* multiplier for ra_pages of seq. files in
                                     * fadvise */
...
static inline bool is_sbi_flag_set(struct f2fs_sb_info *sbi,
    unsigned int type) {
...
static inline void __set_ckpt_flags(struct f2fs_checkpoint *cp,
    unsigned int f) {
...
static inline void f2fs_down_write_nested(struct f2fs_rwsem *sem,
    int subclass) {
...
static inline void set_page_private_data(struct page *page,
    unsigned long data) {
  if (!PagePrivate(page)) {
    attach_page_private(page, (void *) 0);
  }
...
#define stat_inc_total_hit(sbi, \
      type)   (atomic64_inc(&(sbi)->total_hit_ext[type]))
#define stat_inc_rbtree_node_hit(sbi, \
      type) (atomic64_inc(&(sbi)->read_hit_rbtree[type]))
#define stat_inc_largest_node_hit(sbi)  (atomic64_inc(&(sbi)->read_hit_largest))
#define stat_inc_cached_node_hit(sbi, \
      type) (atomic64_inc(&(sbi)->read_hit_cached[type]))
...
static inline bool f2fs_is_compress_level_valid(int alg, int lvl) {
  return false;
}
...
static inline int __init f2fs_init_compress_mempool(void) {
  return 0;
}
...
static inline unsigned int f2fs_cluster_blocks_are_contiguous(
    struct dnode_of_data *dn, unsigned int ofs_in_node) {
  return 0;
}

static inline bool f2fs_sanity_check_cluster(struct dnode_of_data *dn) {
  return false;
}

static inline int f2fs_init_compress_inode(struct f2fs_sb_info *sbi) {
  return 0;
}
