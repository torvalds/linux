...
static int f2fs_ioc_set_encryption_policy(struct file *filp,
    unsigned long arg) {
  struct inode *inode = file_inode(filp);
...
static int f2fs_ioc_get_encryption_policy(struct file *filp,
    unsigned long arg) {
...
static int f2fs_ioc_get_encryption_pwsalt(struct file *filp,
    unsigned long arg) {
  struct inode *inode = file_inode(filp);
  struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
...
static int f2fs_ioc_remove_encryption_key(struct file *filp,
    unsigned long arg) {
  if (!f2fs_sb_has_encrypt(F2FS_I_SB(file_inode(filp)))) {
    return -EOPNOTSUPP;
  }
...
static long __f2fs_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
  switch (cmd) {
