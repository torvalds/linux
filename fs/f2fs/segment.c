...
void f2fs_balance_fs_bg(struct f2fs_sb_info *sbi, bool from_bg) {
  ...
  if (is_inflight_io(sbi, REQ_TIME)
      || (!f2fs_time_over(sbi,
          REQ_TIME) && f2fs_rwsem_is_locked(&sbi->cp_rwsem))) {
    return;
  }
 ...
 *
 * When UMOUNT_DISCARD_TIMEOUT is exceeded, all remaining discard commands will
 *be dropped
 *
 * Return true if issued all discard cmd or no discard cmd need issue, otherwise
 *return false.
 */
...
static bool __mark_sit_entry_dirty(struct f2fs_sb_info *sbi,
    unsigned int segno) {
...
static void update_sit_entry(struct f2fs_sb_info *sbi, block_t blkaddr,
    int del) {
  struct seg_entry *se;
...
static unsigned short f2fs_curseg_valid_blocks(struct f2fs_sb_info *sbi,
    int type) {
...
static void write_compacted_summaries(struct f2fs_sb_info *sbi,
    block_t blkaddr) {
  struct page *page;
  unsigned char *kaddr;
