...
err_level:
  f2fs_warn(sbi,
      "%s: inode (ino=%lx) has unsupported compress level: %u, run fsck to fix",
...
static bool sanity_check_inode(struct inode *inode, struct page *node_page) {
  ...
  if (f2fs_has_extra_attr(inode)) {
    ...
    if (f2fs_sb_has_flexible_inline_xattr(sbi)
      ...
      f2fs_warn(sbi,
          "%s: inode (ino=%lx) has corrupted i_inline_xattr_size: %d, max: %lu",
  ...
  if (!f2fs_sb_has_extra_attr(sbi)) {
    if (f2fs_sb_has_project_quota(sbi)) {
      f2fs_warn(sbi,
          "%s: corrupted inode ino=%lx, wrong feature flag: %u, run fsck to fix.",
