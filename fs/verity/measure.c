 ...
 */
__bpf_kfunc int bpf_get_fsverity_digest(struct file *file,
  ...
  memcpy(arg->digest, vi->file_digest,
      min_t(int, hash_alg->digest_size, out_digest_sz));
...
static int bpf_get_fsverity_digest_filter(const struct bpf_prog *prog,
    u32 kfunc_id) {
