...
struct RESTART_HDR {
  ...
  __le32 sys_page_size; // 0x10: Page size of the system which initialized the
                        // log.
...
struct LFS_RECORD {
  ...
  u8 align[6];    // 0x02:
  __le64 last_end_lsn;  // 0x08: lsn for the last log record which ends on the
                        // page,
};
 ...
 */
static void log_init_pg_hdr(struct ntfs_log *log, u16 major_ver,
    u16 minor_ver) {
 ...
 */
static int find_client_next_lsn(struct ntfs_log *log, struct lcb *lcb,
    u64 *lsn) {
  int err;
  u64 next_lsn;
...
static inline bool check_index_buffer(const struct INDEX_BUFFER *ib,
    u32 bytes) {
  u16 fo;
