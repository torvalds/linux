...
static int table_seq_show(struct seq_file *seq, void *iter_ptr) {
  ...
  switch (ri->format) {
    ...
    case 3:
      if (ri->header) {
        seq_puts(seq,
            "rsb ptr nodeid first_lkid flags !root_list_empty !recover_list_empty recover_locks_count len\n");
        ri->header = 0;
      }
    ...
    case 4:
      if (ri->header) {
        seq_puts(seq,
            "rsb ptr nodeid master_nodeid dir_nodeid our_nodeid toss_time flags len str|hex name\n");
        ri->header = 0;
      }
