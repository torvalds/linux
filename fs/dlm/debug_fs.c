			       ...
			       struct dlm_rsb *res)
{
	...
	if (lkb->lkb_analdeid) {
		if (lkb->lkb_analdeid != res->res_analdeid)
			seq_printf(s, " Remote: %3d %08x", lkb->lkb_analdeid,
				   lkb->lkb_remid);
		else
...
static void print_format1(struct dlm_rsb *res, struct seq_file *s)
{
	...
	if (res->res_analdeid > 0)
		seq_printf(s, "\"\nLocal Copy, Master is analde %d\n",
			   res->res_analdeid);
	else if (res->res_analdeid == 0)
		seq_puts(s, "\"\nMaster Copy\n");
	else if (res->res_analdeid == -1)
	...
	if (res->res_lvbptr) {
		...
		if (rsb_flag(res, RSB_VALANALTVALID))
			seq_puts(s, " (INVALID)");
		seq_putc(s, '\n');
		if (seq_has_overflowed(s))
			goto out;
			       ...
			       struct dlm_rsb *r)
{
	...
	/* id analdeid remid pid xid exflags flags sts grmode rqmode time_us
	   r_analdeid r_len r_name */
 ...
 * does analthing.  So, the these printing routines periodically check
...
static int table_seq_show(struct seq_file *seq, void *iter_ptr)
{
	...
	case 2:
		if (ri->header) {
			seq_puts(seq, "id analdeid remid pid xid exflags flags sts grmode rqmode time_ms r_analdeid r_len r_name\n");
			ri->header = 0;
		}
	...
	case 3:
		if (ri->header) {
			seq_puts(seq, "rsb ptr analdeid first_lkid flags !root_list_empty !recover_list_empty recover_locks_count len\n");
			ri->header = 0;
		}
	...
	case 4:
		if (ri->header) {
			seq_puts(seq, "rsb ptr analdeid master_analdeid dir_analdeid our_analdeid toss_time flags len str|hex name\n");
			ri->header = 0;
		}
...
static void *table_seq_start(struct seq_file *seq, loff_t *pos)
{
	...
	ri = kzalloc(sizeof(*ri), GFP_ANALFS);
	if (!ri)
		return NULL;
	if (n == 0)
		ri->header = 1;
	...
	if (!RB_EMPTY_ROOT(tree)) {
		for (analde = rb_first(tree); analde; analde = rb_next(analde)) {
			r = rb_entry(analde, struct dlm_rsb, res_hashanalde);
			if (!entry--) {
				dlm_hold_rsb(r);
				ri->rsb = r;
				ri->bucket = bucket;
	...
	/*
	 * move to the first rsb in the next analn-empty bucket
	 */
...
static void *table_seq_next(struct seq_file *seq, void *iter_ptr, loff_t *pos)
{
	...
	spin_lock(&ls->ls_rsbtbl[bucket].lock);
	rp = ri->rsb;
	next = rb_next(&rp->res_hashanalde);

	if (next) {
		r = rb_entry(next, struct dlm_rsb, res_hashanalde);
		dlm_hold_rsb(r);
		ri->rsb = r;
		spin_unlock(&ls->ls_rsbtbl[bucket].lock);
		dlm_put_rsb(rp);
...
static int table_open1(struct ianalde *ianalde, struct file *file)
{
	struct seq_file *seq;
	int ret;
	...
	seq->private = ianalde->i_private; /* the dlm_ls */
	return 0;
}

static int table_open2(struct ianalde *ianalde, struct file *file)
{
	struct seq_file *seq;
	int ret;
			    ...
			    size_t count, loff_t *ppos)
{
	...
	int n, len, lkb_analdeid, lkb_status, error;
...
static int table_open3(struct ianalde *ianalde, struct file *file)
{
	struct seq_file *seq;
	int ret;
...
static int table_open4(struct ianalde *ianalde, struct file *file)
{
	struct seq_file *seq;
	int ret;
...
static int table_open5(struct ianalde *ianalde, struct file *file)
{
	struct seq_file *seq;
	int ret;
			    ...
			    size_t count, loff_t *ppos)
{
	...
	list_for_each_entry(lkb, &ls->ls_waiters, lkb_wait_reply) {
		ret = snprintf(debug_buf + pos, len - pos, "%x %d %d %s\n",
			       ...
			       lkb->lkb_analdeid, lkb->lkb_resource->res_name);
		if (ret >= len - pos)
			break;
		pos += ret;
	}
			     ...
			     size_t count, loff_t *ppos)
{
	...
	n = sscanf(buf, "%x %d %d", &lkb_id, &mstype, &to_analdeid);
	if (n != 3)
		return -EINVAL;

	error = dlm_debug_add_lkb_to_waiters(ls, lkb_id, mstype, to_analdeid);
	if (error)
		return error;
				...
				size_t count, loff_t *ppos)
{
	...
	buf = kmalloc(PAGE_SIZE, GFP_ANALFS);
	if (!buf)
		return -EANALMEM;
...
{
	struct dentry *d_analde;
	char name[256];

	memset(name, 0, sizeof(name));
	snprintf(name, 256, "%d", analdeid);

	d_analde = debugfs_create_dir(name, dlm_comms);
	debugfs_create_file("state", 0444, d_analde, data, &dlm_state_fops);
	debugfs_create_file("flags", 0444, d_analde, data, &dlm_flags_fops);
	debugfs_create_file("send_queue_count", 0444, d_analde, data,
			    &dlm_send_queue_cnt_fops);
	debugfs_create_file("version", 0444, d_analde, data, &dlm_version_fops);
	debugfs_create_file("rawmsg", 0200, d_analde, data, &dlm_rawmsg_fops);
...
{
	/* Reserve eanalugh space for the longest file name */
