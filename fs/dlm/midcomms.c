 ...
 * all unacknowledged dlm messages again. The receiving side might drop any
 *already
...
static void dlm_midcomms_receive_buffer(const union dlm_packet *p,
  ...
  if (is_expected_seq) {
    ...
    log_print_ratelimited(
        "ignore dlm msg because seq mismatch, seq: %u, expected: %u, nodeid: %d",
...
static int dlm_opts_check_msglen(const union dlm_packet *p, uint16_t msglen,
  ...
  switch (p->opts.o_nextcmd) {
    ...
    default:
      log_print(
          "unsupported o_nextcmd received: %u, will skip this message from node %d",
...
static void dlm_midcomms_receive_buffer_3_2(const union dlm_packet *p,
    int nodeid) {
  ...
  switch (node->version) {
    ...
    default:
      log_print_ratelimited(
          "version mismatch detected, assumed 0x%08x but node %d has 0x%08x",
  ...
  switch (p->header.h_cmd) {
    case DLM_RCOM:
      ...
      switch (p->rcom.rc_type) {
        ...
        default:
          log_print(
              "unsupported rcom type received: %u, will skip this message from node %d",
      ...
      switch (p->header.h_cmd) {
        case DLM_RCOM:
          if (msglen < sizeof(struct dlm_rcom)) {
            log_print(
                "inner rcom msg too small: %u, will skip this message from node %d",
                msglen, nodeid);
            goto out;
          }
          break;
        case DLM_MSG:
          if (msglen < sizeof(struct dlm_message)) {
            log_print(
                "inner msg too small: %u, will skip this message from node %d",
                msglen, nodeid);
            goto out;
          }
          break;
        case DLM_FIN:
          if (msglen < sizeof(struct dlm_header)) {
            log_print(
                "inner fin too small: %u, will skip this message from node %d",
                msglen, nodeid);
            goto out;
          }
          break;
        default:
          log_print(
              "unsupported inner h_cmd received: %u, will skip this message from node %d",
              msglen, nodeid);
          goto out;
      }
    ...
    default:
      log_print(
          "unsupported h_cmd received: %u, will skip this message from node %d",
...
static void dlm_midcomms_receive_buffer_3_1(const union dlm_packet *p,
    int nodeid) {
  uint16_t msglen = le16_to_cpu(p->header.h_length);
  struct midcomms_node *node;
  int idx;
...
int dlm_validate_incoming_buffer(int nodeid, unsigned char *buf, int len) {
  ...
  while (len >= sizeof(struct dlm_header)) {
    ...
    if (msglen > DLM_MAX_SOCKET_BUFSIZE
      ...
      log_print(
          "received invalid length header: %u from node %d, will abort message parsing",
          msglen, nodeid);
      return -EBADMSG;
    }
...
int dlm_process_incoming_buffer(int nodeid, unsigned char *buf, int len) {
  ...
  while (len >= sizeof(struct dlm_header)) {
    ...
    switch (hd->h_version) {
      ...
      default:
        log_print(
            "received invalid version header: %u from node %d, will skip this message",
...
static struct dlm_msg *dlm_midcomms_get_msg_3_2(struct dlm_mhandle *mh,
    int nodeid,
