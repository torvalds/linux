		...
		int nbits, int bit, int count, int set)
{
	...
	struct omfs_sb_info *sbi = OMFS_SB(sb);
 	...
 	err = -ENOMEM;
	bh = sb_bread(sb, clus_to_blk(sbi, sbi->s_bitmap_iyes) + map);
	if (!bh)
		goto out;
	...
	for (i = 0; i < count; i++, bit++) {
		if (bit >= nbits) {
			...
			brelse(bh);
			bh = sb_bread(sb,
				clus_to_blk(sbi, sbi->s_bitmap_iyes) + map);
			if (!bh)
				goto out;
		}
		if (set) {
 ...
 *  type: for iyesdes, we must allocate sbi->s_mirrors blocks, and for file
