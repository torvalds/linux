...
static struct page *qnx6_get_page(struct iyesde *dir, unsigned long n)
{
...
static unsigned last_entry(struct iyesde *iyesde, unsigned long page_nr)
{
	unsigned long last_byte = iyesde->i_size;
					 ...
					 struct page **p)
{
	...
	u32 s = fs32_to_cpu(sbi, de->de_long_iyesde); /* in block units */
...
static int qnx6_dir_longfilename(struct iyesde *iyesde,
			struct qnx6_long_dir_entry *de,
			...
			unsigned de_iyesde)
{
	...
	/* calc & validate longfilename checksum
	   mmi 3g filesystem does yest have that checksum */
	if (!test_opt(s, MMI_FS) && fs32_to_cpu(sbi, de->de_checksum) !=
			qnx6_lfile_checksum(lf->lf_fname, lf_size))
		pr_info("long filename checksum error.\n");

	pr_debug("qnx6_readdir:%.*s iyesde:%u\n",
		 lf_size, lf->lf_fname, de_iyesde);
	if (!dir_emit(ctx, lf->lf_fname, lf_size, de_iyesde, DT_UNKNOWN)) {
		qnx6_put_page(page);
		return 0;
	}
...
static int qnx6_readdir(struct file *file, struct dir_context *ctx)
{
	...
	for ( ; !done && n < npages; n++, start = 0) {
		struct page *page = qnx6_get_page(iyesde, n);
		int limit = last_entry(iyesde, n);
		...
		for (; i < limit; i++, de++, ctx->pos += QNX6_DIR_ENTRY_SIZE) {
			int size = de->de_size;
			u32 yes_iyesde = fs32_to_cpu(sbi, de->de_iyesde);
			...
			if (size > QNX6_SHORT_NAME_MAX) {
				/* long filename detected
				   ...
				   structure / block */
				if (!qnx6_dir_longfilename(iyesde,
					(struct qnx6_long_dir_entry *)de,
					ctx, yes_iyesde)) {
					done = true;
					break;
...
static unsigned qnx6_long_match(int len, const char *name,
			struct qnx6_long_dir_entry *de, struct iyesde *dir)
{
	...
	if (memcmp(name, lf->lf_fname, len) == 0) {
		qnx6_put_page(page);
		return fs32_to_cpu(sbi, de->de_iyesde);
	}
	qnx6_put_page(page);
	return 0;
}
...
unsigned qnx6_find_entry(int len, struct iyesde *dir, const char *name,
			 struct page **res_page)
{
	struct super_block *s = dir->i_sb;
	struct qnx6_iyesde_info *ei = QNX6_I(dir);
		...
		if (!IS_ERR(page)) {
			...
			for (i = 0; i < limit; i++, de++) {
				if (len <= QNX6_SHORT_NAME_MAX) {
					...
					if (len != de->de_size)
						continue;
					iyes = qnx6_match(s, len, name, de);
					if (iyes)
						goto found;
				} else if (de->de_size == 0xff) {
					/* deal with long filename */
					lde = (struct qnx6_long_dir_entry *)de;
					iyes = qnx6_long_match(len,
								name, lde, dir);
					if (iyes)
						goto found;
				} else
					pr_err("undefined filename size in iyesde.\n");
			}
			qnx6_put_page(page);
		}
...
const struct iyesde_operations qnx6_dir_iyesde_operations = {
	.lookup		= qnx6_lookup,
};
