...
static void ext4_xattr_block_cache_insert(struct mb_cache *,
    struct buffer_head *);
static struct buffer_head *ext4_xattr_block_cache_find(struct inode *,
    struct ext4_xattr_header *,
    struct mb_cache_entry **);
static __le32 ext4_xattr_hash_entry(char *name, size_t name_len, __le32 *value,
    size_t value_count);
static __le32 ext4_xattr_hash_entry_signed(char *name, size_t name_len,
    __le32 *value,
    size_t value_count);
...
static int ext4_expand_inode_array(
...
static int check_xattrs(struct inode *inode, struct buffer_head *bh,
...
static inline int __xattr_check_inode(struct inode *inode,
...
static int xattr_find_entry(struct inode *inode,
    struct ext4_xattr_entry **pentry,
 ...
 */
static int ext4_xattr_inode_read(struct inode *ea_inode, void *buf,
    size_t size) {
 ...
 * used / required on success.
 */
ssize_t ext4_listxattr(struct dentry *dentry, char *buffer,
    size_t buffer_size) {
  int ret, ret2;
...
static void ext4_xattr_inode_dec_ref_all(handle_t *handle, struct inode *parent,
    struct buffer_head *bh,
 ...
 */
static void ext4_xattr_release_block(handle_t *handle, struct inode *inode,
    struct buffer_head *bh,
    struct ext4_xattr_inode_array **ea_inode_array,
    int extra_credits) {
  struct mb_cache *ea_block_cache = EA_BLOCK_CACHE(inode);
 ...
 */
static struct buffer_head *ext4_xattr_block_cache_find(struct inode *inode,
 ...
 */
static __le32 ext4_xattr_hash_entry_signed(char *name, size_t name_len,
    __le32 *value, size_t value_count) {
  __u32 hash = 0;
  while (name_len--) {
    hash = (hash << NAME_HASH_SHIFT)
        ^ (hash >> (8 * sizeof(hash) - NAME_HASH_SHIFT))
