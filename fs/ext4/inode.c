 ...
 * make data stable.
 */
static int ext4_dirty_journalled_data(handle_t *handle,
    struct buffer_head *bh) {
...
static bool ext4_dirty_folio(struct address_space *mapping,
    struct folio *folio) {
...
static int ext4_fill_raw_inode(struct inode *inode,
 ...
 * set.
 */
static inline void ext4_inode_set_iversion_queried(struct inode *inode,
    u64 val) {
  if (unlikely(EXT4_I(inode)->i_flags & EXT4_EA_INODE_FL)) {
...
static const char *check_igot_inode(struct inode *inode,
...
/*
 * In data=journal mode ext4_journalled_invalidate_folio() may fail to
 *invalidate
