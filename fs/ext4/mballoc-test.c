...
#define MBT_SB(_sb) (container_of((_sb)->s_fs_info, struct mbt_ext4_super_block, \
    sbi))
...
static struct buffer_head *ext4_read_block_bitmap_nowait_stub(
...
static struct ext4_group_desc *ext4_get_group_desc_stub(struct super_block *sb,
    ext4_group_t block_group,
    struct buffer_head **bh) {
  struct mbt_grp_ctx *grp_ctx = MBT_GRP_CTX(sb, block_group);
  if (bh != NULL) {
...
static int ext4_mb_mark_context_stub(handle_t *handle, struct super_block *sb,
    bool state,
...
static void test_new_blocks_simple(struct kunit *test) {
  ...
  ar.goal = ext4_group_first_block_no(sb, goal_group);
  found = ext4_mb_new_blocks_simple(&ar, &err);
  KUNIT_ASSERT_EQ_MSG(test, ar.goal + EXT4_C2B(sbi,
      1), found,
...
static void mbt_generate_test_ranges(struct super_block *sb,
...
static void validate_free_blocks_simple(struct kunit *test,
    struct super_block *sb,
...
static void test_free_blocks_simple_range(struct kunit *test,
...
static void test_mark_diskspace_used_range(struct kunit *test,
...
static void mbt_validate_group_info(struct kunit *test,
    struct ext4_group_info *grp1,
...
static void do_test_generate_buddy(struct kunit *test, struct super_block *sb,
    void *bitmap,
...
static void test_mb_mark_used_range(struct kunit *test, struct ext4_buddy *e4b,
...
static void test_mb_free_blocks_range(struct kunit *test,
    struct ext4_buddy *e4b,
    ext4_grpblk_t start, ext4_grpblk_t len, void *bitmap,
    void *buddy, struct ext4_group_info *grp) {
  struct super_block *sb = (struct super_block *) test->priv;
