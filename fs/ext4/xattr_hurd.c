...
static bool ext4_xattr_hurd_list(struct dentry *dentry) {
  return test_opt(dentry->d_sb, XATTR_USER);
}

static int ext4_xattr_hurd_get(const struct xattr_handler *handler,
...
static int ext4_xattr_hurd_set(const struct xattr_handler *handler,
