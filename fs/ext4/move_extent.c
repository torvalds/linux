 ...
 * on failure.
 */
static inline int get_ext_path(struct inode *inode, ext4_lblk_t lblock,
 ...
 */
void ext4_double_up_write_data_sem(struct inode *orig_inode,
 ...
 */
static int mext_check_coverage(struct inode *inode, ext4_lblk_t from,
    ext4_lblk_t count,
...
static int mext_page_mkuptodate(struct folio *folio, unsigned from,
    unsigned to) {
 ...
 */
static int move_extent_per_page(struct file *o_filp, struct inode *donor_inode,
...
again:
  ...
  *err = mext_folio_double_lock(orig_inode, donor_inode, orig_page_offset,
  ...
  VM_BUG_ON_FOLIO(folio_nr_pages(folio[0]) != folio_nr_pages(folio[1]),
      folio[1]);
  if (unwritten) {
 ...
 */
static int mext_check_arguments(struct inode *orig_inode,
  ...
  if (IS_SWAPFILE(orig_inode) || IS_SWAPFILE(donor_inode)) {
    ext4_debug(
        "ext4 move extent: The argument files should not be swap files [ino:orig %lu, donor %lu]\n",
        orig_inode->i_ino, donor_inode->i_ino);
    return -ETXTBSY;
  }
  if (ext4_is_quota_file(orig_inode) && ext4_is_quota_file(donor_inode)) {
    ext4_debug(
        "ext4 move extent: The argument files should not be quota files [ino:orig %lu, donor %lu]\n",
        orig_inode->i_ino, donor_inode->i_ino);
    return -EOPNOTSUPP;
  }
