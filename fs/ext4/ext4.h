 ...
 */
#define TEST_FLAG_VALUE(FLAG) (EXT4_ ## FLAG ## _FL \
  == (1U << EXT4_INODE_ ## FLAG))
...
struct ext4_inode {
  ...
  union {
    ...
    struct {
      __le16 h_i_reserved1;  /* Obsoleted fragment number/size which are removed
                              * in ext4 */
      __u16 h_i_mode_high;
      __u16 h_i_uid_high;
      __u16 h_i_gid_high;
...
static inline struct timespec64 ext4_decode_extra_time(__le32 base,
  ...
  struct timespec64 ts = {
    .tv_sec = (signed) le32_to_cpu(base)
  };
...
#define EXT4_INODE_SET_XTIME_VAL(xtime, inode, raw_inode, ts)     \
  do {                    \
    ...
    (raw_inode)->xtime \
      = cpu_to_le32(clamp_t(int32_t, (ts).tv_sec, S32_MIN, S32_MAX));  \
  } while (0)
...
#define EXT4_MOUNT_INIT_INODE_TABLE 0x80000000 /* Initialize uninitialized
                                                * itables */
...
#define EXTN_FEATURE_FUNCS(ver) \
  static inline bool ext4_has_unknown_ext ## ver ## _compat_features( \
    struct super_block *sb) \
  { \
    return (EXT4_SB(sb)->s_es->s_feature_compat   \
      & cpu_to_le32(~EXT ## ver ## _FEATURE_COMPAT_SUPP)) != 0; \
  } \
  static inline bool ext4_has_unknown_ext ## ver ## _ro_compat_features( \
    struct super_block *sb) \
  { \
    return (EXT4_SB(sb)->s_es->s_feature_ro_compat   \
      & cpu_to_le32(~EXT ## ver ## _FEATURE_RO_COMPAT_SUPP)) != 0; \
  } \
  static inline bool ext4_has_unknown_ext ## ver ## _incompat_features( \
    struct super_block *sb) \
  { \
    return (EXT4_SB(sb)->s_es->s_feature_incompat   \
 ...
 */
static inline unsigned int ext4_rec_len_from_disk(__le16 dlen,
    unsigned blocksize) {
...
static inline ext4_fsblk_t ext4_group_first_block_no(struct super_block *sb,
...
extern int ext4_read_bh(struct buffer_head *bh, blk_opf_t op_flags,
    bh_end_io_t *end_io);
extern int ext4_read_bh_lock(struct buffer_head *bh, blk_opf_t op_flags,
    bool wait);
