...
static int mb_avg_fragment_size_order(struct super_block *sb,
    ext4_grpblk_t len) {
  int order;
  /*
...
static void mb_update_avg_fragment_size(struct super_block *sb,
    struct ext4_group_info *grp) {
  struct ext4_sb_info *sbi = EXT4_SB(sb);
  int new_order;
 ...
 */
static void ext4_mb_choose_next_group_p2_aligned(
 ...
 */
static struct ext4_group_info *ext4_mb_find_good_group_avg_frag_lists(
    struct ext4_allocation_context *ac, int order) {
  struct ext4_sb_info *sbi = EXT4_SB(ac->ac_sb);
...
/*
 * We couldn't find a group in CR_GOAL_LEN_FAST so try to find the highest free
 *fragment
...
static void ext4_mb_choose_next_group_best_avail(
  ...
  /* Reset goal length to original goal length before falling into
   * CR_GOAL_LEN_SLOW */
 ...
 */
static ext4_group_t next_linear_group(struct ext4_allocation_context *ac,
    ext4_group_t group,
 ...
 * calling this routine!
 */
static noinline_for_stack int ext4_mb_load_buddy_gfp(struct super_block *sb,
    ext4_group_t group,
  ...
  while (buddy) {
    ...
    if (!(last & 1)) {
      e4b->bd_info->bb_counters[order]
        += mb_buddy_adjust_border(&last, buddy, 1);
    }
    if (first > last) {
      break;
    }
...
static noinline_for_stack int ext4_mb_regular_allocator(
    struct ext4_allocation_context *ac) {
...
static void *ext4_mb_seq_groups_next(struct seq_file *seq, void *v,
    loff_t *pos) {
  struct super_block *sb = pde_data(file_inode(seq->file));
  ext4_group_t group;
  ++*pos;
...
static void *ext4_mb_seq_structs_summary_next(struct seq_file *seq, void *v,
    loff_t *pos) {
  struct super_block *sb = pde_data(file_inode(seq->file));
  unsigned long position;
  ++*pos;
...
static int ext4_mb_mark_context(handle_t *handle, struct super_block *sb,
    bool state,
 ...
 */
static noinline_for_stack int ext4_mb_mark_diskspace_used(
    struct ext4_allocation_context *ac,
 ...
 */
static inline struct rb_node *ext4_mb_pa_rb_next_iter(ext4_lblk_t new_start,
...
static inline void ext4_mb_pa_assert_overlap(struct ext4_allocation_context *ac,
 ...
 */
static inline void ext4_mb_pa_adjust_overlap(struct ext4_allocation_context *ac,
    ext4_lblk_t *start, loff_t *end) {
  struct ext4_inode_info *ei = EXT4_I(ac->ac_inode);
  struct ext4_sb_info *sbi = EXT4_SB(ac->ac_sb);
 ...
 * size and alignment
 */
static noinline_for_stack void ext4_mb_normalize_request(
    struct ext4_allocation_context *ac,
 ...
 * from the goal block.
 */
static struct ext4_prealloc_space *ext4_mb_check_group_pa(
 ...
 */
static bool ext4_mb_pa_goal_check(struct ext4_allocation_context *ac,
    struct ext4_prealloc_space *pa) {
  struct ext4_sb_info *sbi = EXT4_SB(ac->ac_sb);
  ext4_fsblk_t start;
 ...
 */
static noinline_for_stack bool ext4_mb_use_preallocated(
    struct ext4_allocation_context *ac) {
  struct ext4_sb_info *sbi = EXT4_SB(ac->ac_sb);
  int order, i;
  struct ext4_inode_info *ei = EXT4_I(ac->ac_inode);
 ...
 */
static noinline_for_stack void ext4_mb_new_inode_pa(
    struct ext4_allocation_context *ac) {
  struct super_block *sb = ac->ac_sb;
  struct ext4_sb_info *sbi = EXT4_SB(sb);
  struct ext4_prealloc_space *pa;
 ...
 */
static noinline_for_stack void ext4_mb_new_group_pa(
    struct ext4_allocation_context *ac) {
  struct super_block *sb = ac->ac_sb;
  struct ext4_locality_group *lg;
  struct ext4_prealloc_space *pa;
 ...
 */
static noinline_for_stack void ext4_mb_release_inode_pa(struct ext4_buddy *e4b,
...
static noinline_for_stack void ext4_mb_release_group_pa(struct ext4_buddy *e4b,
    struct ext4_prealloc_space *pa) {
  struct super_block *sb = e4b->bd_sb;
  ext4_group_t group;
  ext4_grpblk_t bit;
 ...
 */
static noinline_for_stack int ext4_mb_discard_group_preallocations(
    struct super_block *sb,
...
static noinline_for_stack void ext4_mb_initialize_context(
    struct ext4_allocation_context *ac,
    struct ext4_allocation_request *ar) {
...
static noinline_for_stack void ext4_mb_discard_lg_preallocations(
    struct super_block *sb,
    struct ext4_locality_group *lg,
...
static noinline_for_stack void ext4_mb_free_metadata(handle_t *handle,
    struct ext4_buddy *e4b,
    struct ext4_free_data *new_entry) {
  ...
  spin_lock(&sbi->s_md_lock);
  list_add_tail(&new_entry->efd_list,
      &sbi->s_freed_data_list[new_entry->efd_tid & 1]);
 ...
 */
static ext4_grpblk_t ext4_trim_all_free(struct super_block *sb,
    ext4_group_t group,
