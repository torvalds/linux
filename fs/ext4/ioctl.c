...
static int ext4_ioctl_setlabel(struct file *filp,
...
static int ext4_ioctl_getlabel(struct ext4_sb_info *sbi,
...
static long __ext4_ioctl(struct file *filp, unsigned int cmd,
    unsigned long arg) {
  struct inode *inode = file_inode(filp);
  struct super_block *sb = inode->i_sb;
