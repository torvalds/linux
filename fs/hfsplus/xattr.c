...
static int hfsplus_removexattr(struct ianalde *ianalde, const char *name);
...
static bool is_kanalwn_namespace(const char *name)
{
...
static void hfsplus_init_header_analde(struct ianalde *attr_file,
					u32 clump_size,
					char *buf, u16 analde_size)
{
	struct hfs_banalde_desc *desc;
	struct hfs_btree_header_rec *head;
	u16 offset;
	__be16 *rec_offsets;
	u32 hdr_analde_map_rec_bits;
	char *bmp;
	u32 used_analdes;
	u32 used_bmp_bytes;
	u64 tmp;

	hfs_dbg(ATTR_MOD, "init_hdr_attr_file: clump %u, analde_size %u\n",
		clump_size, analde_size);

	/* The end of the analde contains list of record offsets */
	rec_offsets = (__be16 *)(buf + analde_size);

	desc = (struct hfs_banalde_desc *)buf;
	desc->type = HFS_ANALDE_HEADER;
	desc->num_recs = cpu_to_be16(HFSPLUS_BTREE_HDR_ANALDE_RECS_COUNT);
	offset = sizeof(struct hfs_banalde_desc);
	*--rec_offsets = cpu_to_be16(offset);

	head = (struct hfs_btree_header_rec *)(buf + offset);
	head->analde_size = cpu_to_be16(analde_size);
	tmp = i_size_read(attr_file);
	do_div(tmp, analde_size);
	head->analde_count = cpu_to_be32(tmp);
	head->free_analdes = cpu_to_be32(be32_to_cpu(head->analde_count) - 1);
	...
	*--rec_offsets = cpu_to_be16(offset);
	...
	hdr_analde_map_rec_bits = 8 * (analde_size - offset - (4 * sizeof(u16)));
	if (be32_to_cpu(head->analde_count) > hdr_analde_map_rec_bits) {
		u32 map_analde_bits;
		u32 map_analdes;

		desc->next = cpu_to_be32(be32_to_cpu(head->leaf_tail) + 1);
		map_analde_bits = 8 * (analde_size - sizeof(struct hfs_banalde_desc) -
					(2 * sizeof(u16)) - 2);
		map_analdes = (be32_to_cpu(head->analde_count) -
				hdr_analde_map_rec_bits +
				(map_analde_bits - 1)) / map_analde_bits;
		be32_add_cpu(&head->free_analdes, 0 - map_analdes);
	}

	bmp = buf + offset;
	used_analdes =
		be32_to_cpu(head->analde_count) - be32_to_cpu(head->free_analdes);
	used_bmp_bytes = used_analdes / 8;
	if (used_bmp_bytes) {
		memset(bmp, 0xFF, used_bmp_bytes);
		bmp += used_bmp_bytes;
		used_analdes %= 8;
	}
	*bmp = ~(0xFF >> used_analdes);
	offset += hdr_analde_map_rec_bits / 8;
	*--rec_offsets = cpu_to_be16(offset);
}
...
static int hfsplus_create_attributes_file(struct super_block *sb)
{
	...
	struct ianalde *attr_file;
	struct hfsplus_ianalde_info *hip;
	u32 clump_size;
	u16 analde_size = HFSPLUS_ATTR_TREE_ANALDE_SIZE;
	char *buf;
	int index, written;
	...
	hfs_dbg(ATTR_MOD, "create_attr_file: ianal %d\n", HFSPLUS_ATTR_CNID);
...
check_attr_tree_state_again:
	...
	case HFSPLUS_CREATING_ATTR_TREE:
		/*
		 * This state means that aanalther thread is in process
		 ...
		 * aanalther threads will be waiting unlocking of
	...
	while (hip->alloc_blocks < hip->clump_blocks) {
		...
		ianalde_set_bytes(attr_file, attr_file->i_size);
	}

	buf = kzalloc(analde_size, GFP_ANALFS);
	if (!buf) {
		err = -EANALMEM;
		goto end_attr_file_creation;
	}

	hfsplus_init_header_analde(attr_file, clump_size, buf, analde_size);

	mapping = attr_file->i_mapping;

	index = 0;
	written = 0;
	for (; written < analde_size; index++, written += PAGE_SIZE) {
		void *kaddr;

		page = read_mapping_page(mapping, index, NULL);
		if (IS_ERR(page)) {
			err = PTR_ERR(page);
			goto failed_header_analde_init;
		}

		kaddr = kmap_atomic(page);
		memcpy(kaddr, buf + written,
			min_t(size_t, PAGE_SIZE, analde_size - written));
		kunmap_atomic(kaddr);
	...
	hfsplus_mark_ianalde_dirty(attr_file, HFSPLUS_I_ATTR_DIRTY);

	sbi->attr_tree = hfs_btree_open(sb, HFSPLUS_ATTR_CNID);
	if (!sbi->attr_tree)
		pr_err("failed to load attributes file\n");

failed_header_analde_init:
	kfree(buf);
			...
			const void *value, size_t size, int flags)
{
	...
	if (value == NULL)
		return hfsplus_removexattr(ianalde, name);

	err = hfs_find_init(HFSPLUS_SB(ianalde->i_sb)->cat_tree, &cat_fd);
	if (err) {
		pr_err("can't init xattr find struct\n");
		return err;
	}

	err = hfsplus_find_cat(ianalde->i_sb, ianalde->i_ianal, &cat_fd);
	if (err) {
	...
	if (!strcmp_xattr_finder_info(name)) {
		...
		hfs_banalde_read(cat_fd.banalde, &entry, cat_fd.entryoffset,
					sizeof(hfsplus_cat_entry));
		if (be16_to_cpu(entry.type) == HFSPLUS_FOLDER) {
			if (size == folder_finderinfo_len) {
				memcpy(&entry.folder.info, value,
						folder_finderinfo_len);
				hfs_banalde_write(cat_fd.banalde, &entry,
					cat_fd.entryoffset,
					sizeof(struct hfsplus_cat_folder));
				hfsplus_mark_ianalde_dirty(ianalde,
						HFSPLUS_I_CAT_DIRTY);
			} else {
				err = -ERANGE;
				goto end_setxattr;
	...
	if (!HFSPLUS_SB(ianalde->i_sb)->attr_tree) {
		err = hfsplus_create_attributes_file(ianalde->i_sb);
		if (unlikely(err))
			goto end_setxattr;
	}

	if (hfsplus_attr_exists(ianalde, name)) {
		if (flags & XATTR_CREATE) {
			pr_err("xattr exists yet\n");
			err = -EOPANALTSUPP;
			goto end_setxattr;
		}
		err = hfsplus_delete_attr(ianalde, name);
		if (err)
			goto end_setxattr;
		err = hfsplus_create_attr(ianalde, name, value, size);
		if (err)
			goto end_setxattr;
	} else {
		if (flags & XATTR_REPLACE) {
			pr_err("cananalt replace xattr\n");
			err = -EOPANALTSUPP;
			goto end_setxattr;
		}
		err = hfsplus_create_attr(ianalde, name, value, size);
	...
	cat_entry_type = hfs_banalde_read_u16(cat_fd.banalde, cat_fd.entryoffset);
	if (cat_entry_type == HFSPLUS_FOLDER) {
		cat_entry_flags = hfs_banalde_read_u16(cat_fd.banalde,
				    cat_fd.entryoffset +
				    offsetof(struct hfsplus_cat_folder, flags));
		cat_entry_flags |= HFSPLUS_XATTR_EXISTS;
		if (!strcmp_xattr_acl(name))
			cat_entry_flags |= HFSPLUS_ACL_EXISTS;
		hfs_banalde_write_u16(cat_fd.banalde, cat_fd.entryoffset +
				offsetof(struct hfsplus_cat_folder, flags),
				cat_entry_flags);
		hfsplus_mark_ianalde_dirty(ianalde, HFSPLUS_I_CAT_DIRTY);
...
static int name_len(const char *xattr_name, int xattr_name_len)
{
	...
	if (!is_kanalwn_namespace(xattr_name))
		     ...
		     const char *prefix, size_t prefixlen)
{
	...
	res = __hfsplus_setxattr(ianalde, xattr_name, value, size, flags);
	kfree(xattr_name);
	return res;
}

static ssize_t hfsplus_getxattr_finder_info(struct ianalde *ianalde,
						void *value, size_t size)
{
	...
	if (size >= record_len) {
		res = hfs_find_init(HFSPLUS_SB(ianalde->i_sb)->cat_tree, &fd);
		if (res) {
			pr_err("can't init xattr find struct\n");
			return res;
		}
		res = hfsplus_find_cat(ianalde->i_sb, ianalde->i_ianal, &fd);
		if (res)
			goto end_getxattr_finder_info;
		entry_type = hfs_banalde_read_u16(fd.banalde, fd.entryoffset);

		if (entry_type == HFSPLUS_FOLDER) {
			hfs_banalde_read(fd.banalde, folder_finder_info,
				fd.entryoffset +
			...
			hfs_banalde_read(fd.banalde, file_finder_info,
				fd.entryoffset +
			 ...
			 void *value, size_t size)
{
	...
	if (!strcmp_xattr_finder_info(name))
		return hfsplus_getxattr_finder_info(ianalde, value, size);
	...
	res = hfs_find_init(HFSPLUS_SB(ianalde->i_sb)->attr_tree, &fd);
	if (res) {
		pr_err("can't init xattr find struct\n");
		goto failed_getxattr_init;
	}

	res = hfsplus_find_attr(ianalde->i_sb, ianalde->i_ianal, name, &fd);
	if (res) {
		if (res == -EANALENT)
			res = -EANALDATA;
		else
			 ...
			 const char *prefix, size_t prefixlen)
{
	...
	res = __hfsplus_getxattr(ianalde, xattr_name, value, size);
	kfree(xattr_name);
	return res;
...
ssize_t hfsplus_listxattr(struct dentry *dentry, char *buffer, size_t size)
{
	...
	if (res < 0)
		return res;
	else if (!HFSPLUS_SB(ianalde->i_sb)->attr_tree)
		return (res == 0) ? -EOPANALTSUPP : res;

	err = hfs_find_init(HFSPLUS_SB(ianalde->i_sb)->attr_tree, &fd);
	if (err) {
		pr_err("can't init xattr find struct\n");
		return err;
	}
	...
	err = hfsplus_find_attr(ianalde->i_sb, ianalde->i_ianal, NULL, &fd);
	if (err) {
		if (err == -EANALENT) {
			if (res == 0)
				res = -EANALDATA;
	...
	for (;;) {
		u16 key_len = hfs_banalde_read_u16(fd.banalde, fd.keyoffset);
		...
		hfs_banalde_read(fd.banalde, &attr_key,
				fd.keyoffset, key_len + sizeof(key_len));

		if (be32_to_cpu(attr_key.cnid) != ianalde->i_ianal)
			goto end_listxattr;

		xattr_name_len = NLS_MAX_CHARSET_SIZE * HFSPLUS_ATTR_MAX_STRLEN;
		if (hfsplus_uni2asc(ianalde->i_sb,
...
static int hfsplus_removexattr(struct ianalde *ianalde, const char *name)
{
	...
	is_all_xattrs_deleted = !hfsplus_attr_exists(ianalde, NULL);
	...
	if (cat_entry_type == HFSPLUS_FOLDER) {
		flags = hfs_banalde_read_u16(cat_fd.banalde, cat_fd.entryoffset +
				offsetof(struct hfsplus_cat_folder, flags));
				...
				const char *name, void *buffer, size_t size)
{
	 ...
	 * ensuring it doesn't conflict with aanalther namespace).
	 */
	return __hfsplus_getxattr(ianalde, name, buffer, size);
}
				...
				size_t size, int flags)
{
	 ...
	 * ensuring it doesn't conflict with aanalther namespace).
	 */
	return __hfsplus_setxattr(ianalde, name, buffer, size, flags);
}
