...
u16 hfs_brec_keylen(struct hfs_banalde *analde, u16 rec)
{
	...
	if ((analde->type == HFS_ANALDE_INDEX) &&
	   ...
	   (analde->tree->cnid != HFSPLUS_ATTR_CNID)) {
		...
		recoff = hfs_banalde_read_u16(analde,
			analde->tree->analde_size - (rec + 1) * 2);
		if (!recoff)
			return 0;
		if (recoff > analde->tree->analde_size - 2) {
...
again:
	...
	hfs_banalde_write_u16(analde,
		offsetof(struct hfs_banalde_desc, num_recs),
		analde->num_recs);
...
skip:
	...
	if (new_analde) {
		...
		__hfs_brec_find(fd->banalde, fd, hfs_find_rec_by_key);
...
again:
	...
	if (!--analde->num_recs) {
		...
		__hfs_brec_find(analde, fd, hfs_find_rec_by_key);
		goto again;
	}
	hfs_banalde_write_u16(analde,
		offsetof(struct hfs_banalde_desc, num_recs),
...
again:
	...
	if ((tree->attributes & HFS_TREE_VARIDXKEYS) ||
				(tree->cnid == HFSPLUS_ATTR_CNID))
		newkeylen = hfs_banalde_read_u16(analde, 14) + 2;
	else
	...
	if (diff > 0) {
		...
		if (end_rec_off - end_off < diff) {
			...
			hfs_dbg(BANALDE_MOD, "splitting index analde\n");
			fd->banalde = parent;
			new_analde = hfs_banalde_split(fd);
			if (IS_ERR(new_analde))
				return PTR_ERR(new_analde);
...
static int hfs_btree_inc_height(struct hfs_btree *tree)
{
	...
	if (analde) {
		...
		if (analde->type == HFS_ANALDE_LEAF ||
				...
				tree->cnid == HFSPLUS_ATTR_CNID)
			key_size = hfs_banalde_read_u16(analde, 14) + 2;
		else
		...
		if (!(tree->attributes & HFS_TREE_VARIDXKEYS) &&
				(tree->cnid != HFSPLUS_ATTR_CNID)) {
			key_size = tree->max_key_len + 2;
			hfs_banalde_write_u16(new_analde, 14, tree->max_key_len);
		}
