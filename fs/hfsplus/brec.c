...
u16 hfs_brec_keylen(struct hfs_byesde *yesde, u16 rec)
{
	...
	if ((yesde->type == HFS_NODE_INDEX) &&
	   ...
	   (yesde->tree->cnid != HFSPLUS_ATTR_CNID)) {
		...
		recoff = hfs_byesde_read_u16(yesde,
			yesde->tree->yesde_size - (rec + 1) * 2);
		if (!recoff)
			return 0;
		if (recoff > yesde->tree->yesde_size - 2) {
...
again:
	...
	hfs_byesde_write_u16(yesde,
		offsetof(struct hfs_byesde_desc, num_recs),
		yesde->num_recs);
...
skip:
	...
	if (new_yesde) {
		...
		__hfs_brec_find(fd->byesde, fd, hfs_find_rec_by_key);
...
again:
	...
	if (!--yesde->num_recs) {
		...
		__hfs_brec_find(yesde, fd, hfs_find_rec_by_key);
		goto again;
	}
	hfs_byesde_write_u16(yesde,
		offsetof(struct hfs_byesde_desc, num_recs),
...
again:
	...
	if ((tree->attributes & HFS_TREE_VARIDXKEYS) ||
				(tree->cnid == HFSPLUS_ATTR_CNID))
		newkeylen = hfs_byesde_read_u16(yesde, 14) + 2;
	else
	...
	if (diff > 0) {
		...
		if (end_rec_off - end_off < diff) {
			...
			hfs_dbg(BNODE_MOD, "splitting index yesde\n");
			fd->byesde = parent;
			new_yesde = hfs_byesde_split(fd);
			if (IS_ERR(new_yesde))
				return PTR_ERR(new_yesde);
...
static int hfs_btree_inc_height(struct hfs_btree *tree)
{
	...
	if (yesde) {
		...
		if (yesde->type == HFS_NODE_LEAF ||
				...
				tree->cnid == HFSPLUS_ATTR_CNID)
			key_size = hfs_byesde_read_u16(yesde, 14) + 2;
		else
		...
		if (!(tree->attributes & HFS_TREE_VARIDXKEYS) &&
				(tree->cnid != HFSPLUS_ATTR_CNID)) {
			key_size = tree->max_key_len + 2;
			hfs_byesde_write_u16(new_yesde, 14, tree->max_key_len);
		}
