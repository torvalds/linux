...
static int internal_create_group(struct kobject *kobj, int update,
  ...
  if (!grp->attrs && !grp->bin_attrs) {
    pr_debug(
        "sysfs: (bin_)attrs not set by subsystem for group: %s/%s, skipping\n",
        kobj->name, grp->name ? : "");
    return 0;
  }
...
/**
 * sysfs_create_groups - given a directory kobject, create a bunch of attribute
 *groups
 * @kobj: The kobject to create the group on
...
/**
 * sysfs_update_groups - given a directory kobject, create a bunch of attribute
 *groups
