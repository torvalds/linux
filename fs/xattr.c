 ...
 */
static int xattr_permission(struct mnt_idmap *idmap, struct inode *inode,
 ...
 */
int __vfs_setxattr_locked(struct mnt_idmap *idmap, struct dentry *dentry,
...
static ssize_t xattr_getsecurity(struct mnt_idmap *idmap, struct inode *inode,
 ...
 */
int vfs_getxattr_alloc(struct mnt_idmap *idmap, struct dentry *dentry,
...
int __vfs_removexattr(struct mnt_idmap *idmap, struct dentry *dentry,
    const char *name) {
  struct inode *inode = d_inode(dentry);
  const struct xattr_handler *handler;
  if (is_posix_acl_xattr(name)) {
 ...
 *  a delegation was broken on, NULL if none.
 */
int __vfs_removexattr_locked(struct mnt_idmap *idmap,
    struct dentry *dentry, const char *name,
 ...
 */
ssize_t do_getxattr(struct mnt_idmap *idmap, struct dentry *d,
    struct xattr_ctx *ctx) {
  ssize_t error;
...
static ssize_t getxattr(struct mnt_idmap *idmap, struct dentry *d,
 ...
 */
static long removexattr(struct mnt_idmap *idmap, struct dentry *d,
...
ssize_t generic_listxattr(struct dentry *dentry, char *buffer,
    size_t buffer_size) {
  const struct xattr_handler *handler,
  * const *handlers = dentry->d_sb->s_xattr;
 ...
 */
static int rbtree_simple_xattr_cmp(const void *key,
