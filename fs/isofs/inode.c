 ...
 */
static int isofs_hashi_common(const struct dentry *dentry, struct qstr *qstr,
    int ms) {
  const char *name;
  int len;
  char c;
...
static int isofs_hashi(const struct dentry *dentry, struct qstr *qstr) {
 ...
 * Compute the hash for the isofs name corresponding to the dentry.
 */
static int isofs_hash_common(const struct dentry *dentry, struct qstr *qstr,
    int ms) {
  const char *name;
  int len;
  len = qstr->len;
...
static int isofs_hash_ms(const struct dentry *dentry, struct qstr *qstr) {
  return isofs_hash_common(dentry, qstr, 1);
}

static int isofs_hashi_ms(const struct dentry *dentry, struct qstr *qstr) {
...
static int isofs_show_options(struct seq_file *m, struct dentry *root) {
  ...
  if (sbi->s_nocompress) {
    seq_puts(m, ",nocompress");
  }
  if (sbi->s_overriderockperm) {
    seq_puts(m, ",overriderockperm");
  }
  if (sbi->s_showassoc) {
    seq_puts(m, ",showassoc");
  }
  if (sbi->s_check) {
    seq_printf(m, ",check=%c", sbi->s_check);
  }
  if (sbi->s_mapping) {
    seq_printf(m, ",map=%c", sbi->s_mapping);
  }
  if (sbi->s_session != 255) {
    seq_printf(m, ",session=%u", sbi->s_session - 1);
  }
  if (sbi->s_sbsector != -1) {
    seq_printf(m, ",sbsector=%u", sbi->s_sbsector);
  }
...
static unsigned int isofs_get_last_session(struct super_block *sb,
    s32 session) {
