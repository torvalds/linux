 ...
 *     <path> is optional, but if present must begin with '/'
 */
static int ceph_parse_source(struct fs_parameter *param,
    struct fs_context *fc) {
  struct ceph_parse_opts_ctx *pctx = fc->fs_private;
  struct ceph_mount_options *fsopt = pctx->opts;
 ...
 */
static int extra_mon_dispatch(struct ceph_client *client,
    struct ceph_msg *msg) {
...
module_param_cb(disable_send_metrics, &param_ops_metrics, &disable_send_metrics,
    0644);
MODULE_PARM_DESC(disable_send_metrics,
