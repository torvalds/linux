 ...
 */
static struct ceph_mds_request *prepare_open_request(struct super_block *sb,
    int flags,
    int create_mode) {
...
static void cache_file_layout(struct inode *dst, struct inode *src) {
...
static ssize_t ceph_direct_read_write(struct kiocb *iocb, struct iov_iter *iter,
    struct ceph_snap_context *snapc,
...
static int ceph_zero_objects(struct inode *inode, loff_t offset,
    loff_t length) {
  int ret = 0;
  struct ceph_inode_info *ci = ceph_inode(inode);
...
static struct ceph_osd_request *ceph_alloc_copyfrom_request(
...
static ssize_t ceph_do_objects_copy(struct ceph_inode_info *src_ci,
    u64 *src_off,
