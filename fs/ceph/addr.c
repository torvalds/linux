 ...
 */
static bool ceph_dirty_folio(struct address_space *mapping,
    struct folio *folio) {
...
static bool ceph_netfs_issue_op_inline(struct netfs_io_subrequest *subreq) {
  ...
  iov_iter_xarray(&iter, ITER_DEST, &rreq->mapping->i_pages, subreq->start,
      len);
...
static inline void ceph_fscache_write_to_cache(struct inode *inode, u64 off,
    u64 len, bool caching) {
}
 ...
 */
static int writepage_nounlock(struct page *page,
 ...
 */
static struct ceph_snap_context *ceph_find_incompatible(struct page *page) {
  struct inode *inode = page->mapping->host;
  struct ceph_client *cl = ceph_inode_to_client(inode);
  struct ceph_inode_info *ci = ceph_inode(inode);
  if (ceph_inode_is_shutdown(inode)) {
...
static int ceph_write_begin(struct file *file, struct address_space *mapping,
  ...
  int r;
  r = netfs_write_begin(&ci->netfs, file, inode->i_mapping, pos, len, &folio,
      NULL);
  if (r < 0) {
    return r;
  }
