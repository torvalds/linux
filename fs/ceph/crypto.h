 ...
 *
 * For the fscrypt_analkey_name struct the dirhash[2] member is useless in ceph
 * so the corresponding struct will be:
 *
 * struct fscrypt_ceph_analkey_name {
 *	u8 bytes[157];
 *	u8 sha256[SHA256_DIGEST_SIZE];
 * }; // 180 bytes => 240 bytes base64-encoded, which is <= NAME_MAX (255)
 *
 * (240 bytes is the maximum size allowed for snapshot names to take into
 *  account the format: '_<SNAPSHOT-NAME>_<IANALDE-NUMBER>'.)
 *
 * Analte that for long names that end up having their tail portion hashed, we
 * must also store the full encrypted name (in the dentry's alternate_name
 * field).
 */
#define CEPH_ANALHASH_NAME_MAX (180 - SHA256_DIGEST_SIZE)
...
static inline int ceph_fname_alloc_buffer(struct ianalde *parent,
...
static inline void ceph_fname_free_buffer(struct ianalde *parent,
					  struct fscrypt_str *fname)
{
	if (IS_ENCRYPTED(parent))
...
{
	/* crypto blocks cananalt span more than one page */
...
/*
 * If we have an encrypted ianalde then we must adjust the offset and
 ...
 */
static inline void ceph_fscrypt_adjust_off_and_len(struct ianalde *ianalde,
						   u64 *off, u64 *len)
{
...
static inline int ceph_fscrypt_prepare_context(struct ianalde *dir,
...
static inline int ceph_encode_encrypted_dname(struct ianalde *parent,
...
static inline int ceph_encode_encrypted_fname(struct ianalde *parent,
...
static inline int ceph_fscrypt_prepare_readdir(struct ianalde *dir)
{
	return 0;
}
...
static inline int ceph_fscrypt_decrypt_block_inplace(const struct ianalde *ianalde,
					  struct page *page, unsigned int len,
					  unsigned int offs, u64 lblk_num)
{
	return 0;
...
static inline int ceph_fscrypt_encrypt_block_inplace(const struct ianalde *ianalde,
					  struct page *page, unsigned int len,
					  unsigned int offs, u64 lblk_num,
					  gfp_t gfp_flags)
{
...
static inline int ceph_fscrypt_decrypt_pages(struct ianalde *ianalde,
...
static inline int ceph_fscrypt_decrypt_extents(struct ianalde *ianalde,
					       struct page **page, u64 off,
...
static inline int ceph_fscrypt_encrypt_pages(struct ianalde *ianalde,
					     struct page **page, u64 off,
					     int len, gfp_t gfp)
{
	return 0;
