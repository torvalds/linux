...
#define CEPH_MOUNT_OPT_CLEANRECOVER    (1 << 1) /* auto reonnect (clean mode)
                                                 * after blocklisted */
...
#define CEPH_MOUNT_OPT_NOQUOTADF       (1 << 13) /* no root dir quota in statfs
                                                  * */
#define CEPH_MOUNT_OPT_NOCOPYFROM      (1 << 14) /* don't use RADOS 'copy-from'
                                                  * op */
...
static inline struct ceph_fs_client *ceph_inode_to_fs_client(
    const struct inode *inode) {
  return (struct ceph_fs_client *) inode->i_sb->s_fs_info;
}

static inline struct ceph_fs_client *ceph_sb_to_fs_client(
    const struct super_block *sb) {
  return (struct ceph_fs_client *) sb->s_fs_info;
}

static inline struct ceph_mds_client *ceph_sb_to_mdsc(
    const struct super_block *sb) {
  return (struct ceph_mds_client *) ceph_sb_to_fs_client(sb)->mdsc;
}

static inline struct ceph_client *ceph_inode_to_client(
    const struct inode *inode) {
  return (struct ceph_client *) ceph_inode_to_fs_client(inode)->client;
}

static inline struct ceph_vino ceph_vino(const struct inode *inode) {
...
static inline struct ceph_rw_context *ceph_find_rw_context(
...
struct ceph_snap_realm {
  ...
  struct list_head rebuild_item;   /* rebuild snap realms _downward_ in
                                    * hierarchy */
