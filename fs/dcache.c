 ...
 */
#define D_FLAG_VERIFY(dentry, \
      x) WARN_ON_ONCE(((dentry)->d_flags \
...
void shrink_dcache_for_umount(struct super_block *sb) {
  ...
  while (!hlist_bl_empty(&sb->s_roots)) {
    dentry
      = dget(hlist_bl_entry(hlist_bl_first(
        &sb->s_roots), struct dentry, d_hash));
    do_one_tree(dentry);
  }
}
...
static struct dentry *__d_alloc(struct super_block *sb,
