 ...
 */
static inline s32 ufs_get_fs_state(struct super_block *sb,
...
static inline void ufs_set_fs_state(struct super_block *sb,
    struct ufs_super_block_first *usb1,
...
static inline u32 ufs_get_fs_npsect(struct super_block *sb,
    struct ufs_super_block_first *usb1,
    struct ufs_super_block_third *usb3) {
...
static inline u64 ufs_get_fs_qbmask(struct super_block *sb,
    struct ufs_super_block_third *usb3) {
  __fs64 tmp;
  switch (UFS_SB(sb)->s_flags & UFS_ST_MASK) {
    case UFS_ST_SUNOS:
...
static inline u64 ufs_get_fs_qfmask(struct super_block *sb,
    struct ufs_super_block_third *usb3) {
  __fs64 tmp;
  switch (UFS_SB(sb)->s_flags & UFS_ST_MASK) {
    case UFS_ST_SUNOS:
...
static inline u16 ufs_get_de_namlen(struct super_block *sb,
...
static inline void ufs_set_de_namlen(struct super_block *sb,
    struct ufs_dir_entry *de, u16 value) {
  if ((UFS_SB(sb)->s_flags & UFS_DE_MASK) == UFS_DE_OLD) {
...
static inline void ufs_set_de_type(struct super_block *sb,
    struct ufs_dir_entry *de, int mode) {
...
static inline u32 ufs_get_inode_uid(struct super_block *sb,
...
static inline void ufs_set_inode_uid(struct super_block *sb,
    struct ufs_inode *inode, u32 value) {
  switch (UFS_SB(sb)->s_flags & UFS_UID_MASK) {
    case UFS_UID_44BSD:
...
static inline u32 ufs_get_inode_gid(struct super_block *sb,
    struct ufs_inode *inode) {
  switch (UFS_SB(sb)->s_flags & UFS_UID_MASK) {
    case UFS_UID_44BSD:
...
static inline void ufs_set_inode_gid(struct super_block *sb,
    struct ufs_inode *inode, u32 value) {
  switch (UFS_SB(sb)->s_flags & UFS_UID_MASK) {
    case UFS_UID_44BSD:
...
extern struct ufs_buffer_head *_ubh_bread_(struct ufs_sb_private_info *,
    struct super_block *, u64, u64);
extern struct ufs_buffer_head *ubh_bread_uspi(struct ufs_sb_private_info *,
    struct super_block *, u64, u64);
...
extern void _ubh_ubhcpymem_(struct ufs_sb_private_info *, unsigned char *,
    struct ufs_buffer_head *, unsigned);
#define ubh_memcpyubh(ubh, mem, size) _ubh_memcpyubh_(uspi, ubh, mem, size)
extern void _ubh_memcpyubh_(struct ufs_sb_private_info *,
    struct ufs_buffer_head *, unsigned char *,
    unsigned);
...
/*
 * macros and inline function to get important structures from
 *ufs_sb_private_info
 */
...
#define ubh_blkmap(ubh, begin, bit) \
  ((*ubh_get_addr(ubh, \
    (begin) + ((bit) >> 3)) >> \
  ((bit) & 7)) & (0xff >> (UFS_MAXFRAG - uspi->s_fpb)))

static inline u64 ufs_freefrags(struct ufs_sb_private_info *uspi) {
...
#define ubh_find_first_zero_bit(ubh, begin, size) _ubh_find_next_zero_bit_(uspi, \
    ubh, \
    begin, size, \
    0)

#define ubh_find_next_zero_bit(ubh, begin, size, \
      offset) _ubh_find_next_zero_bit_(uspi, ubh, begin, size, offset)
...
#define ubh_find_last_zero_bit(ubh, begin, size, \
      offset) _ubh_find_last_zero_bit_(uspi, ubh, begin, size, offset)
...
#define ubh_isblockclear(ubh, begin, \
      block) (!_ubh_isblockset_(uspi, ubh, begin, block))

#define ubh_isblockset(ubh, begin, block) _ubh_isblockset_(uspi, ubh, begin, \
    block)
...
static inline void _ubh_clrblock_(struct ufs_sb_private_info *uspi,
  ...
  switch (uspi->s_fpb) {
    ...
    case 4:
      *ubh_get_addr(ubh,
          begin + (block >> 1)) &= ~(0x0f << ((block & 0x01) << 2));
      return;
    case 2:
      *ubh_get_addr(ubh,
          begin + (block >> 2)) &= ~(0x03 << ((block & 0x03) << 1));
      return;
    case 1:
...
static inline void ufs_cpu_to_data_ptr(struct super_block *sb, void *p,
    u64 val) {
