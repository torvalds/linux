...
static struct inode *ufs_nfs_get_inode(struct super_block *sb, u64 ino,
    u32 generation) {
...
static int ufs_read_cylinder_structures(struct super_block *sb) {
  ...
  for (i = 0; i < UFS_MAX_GROUP_LOADED; i++) {
    if (!(sbi->s_ucpi[i]
          = kmalloc(sizeof(struct ufs_cg_private_info), GFP_NOFS))) {
      goto failed;
    }
    sbi->s_cgno[i] = UFS_CGNO_EMPTY;
  }
...
static int ufs_fill_super(struct super_block *sb, void *data, int silent) {
  ...
  switch (sbi->s_mount_opt & UFS_MOUNT_UFSTYPE) {
    ...
    case UFS_MOUNT_UFSTYPE_UFS2:
      ...
      flags |= UFS_TYPE_UFS2 | UFS_DE_44BSD | UFS_UID_44BSD | UFS_ST_44BSD
          | UFS_CG_44BSD;
      break;
...
magic_found:
  ...
  if (((flags & UFS_ST_MASK) == UFS_ST_44BSD)
      ...
      || (flags & UFS_ST_MASK) == UFS_ST_SUNOS
      || (flags & UFS_ST_MASK) == UFS_ST_SUNx86)
      && (ufs_get_fs_state(sb, usb1,
          usb3) == (UFS_FSOK - fs32_to_cpu(sb, usb1->fs_time))))) {
