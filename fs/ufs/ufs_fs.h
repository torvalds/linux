...
#define ufs_inotofsba(x)  (((u64) ufs_cgimin(ufs_inotocg(x))) + ufs_inotocgoff( \
    x) / uspi->s_inopf)
...
#define ufs_ocg_blktot(sb, ucg)      fs32_to_cpu((sb), \
    ((struct ufs_old_cylinder_group *) (ucg))->cg_btot)
#define ufs_ocg_blks(sb, ucg, cylno) fs32_to_cpu((sb), \
    ((struct ufs_old_cylinder_group *) (ucg))->cg_b[cylno])
#define ufs_ocg_inosused(sb, ucg)    fs32_to_cpu((sb), \
    ((struct ufs_old_cylinder_group *) (ucg))->cg_iused)
#define ufs_ocg_blksfree(sb, ucg)    fs32_to_cpu((sb), \
    ((struct ufs_old_cylinder_group *) (ucg))->cg_free)
#define ufs_ocg_chkmagic(sb, ucg) \
  (fs32_to_cpu((sb), \
    ((struct ufs_old_cylinder_group *) (ucg))->cg_magic) == CG_MAGIC)
