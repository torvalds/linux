 ...
 * just hope yes one is using NNUUXXI on __?64 structure elements
 ...
 * Each cylinder group has iyesdes and data.
 *
 * A filesystem is described by its super-block, which in turn
 * describes the cylinder groups.  The super-block is critical
 * data and is replicated in each cylinder group to protect against
 * catastrophic loss.  This is done at `newfs' time and the critical
 * super-block data does yest change, so the copies need yest be
 * referenced further unless disaster strikes.
 *
 * For filesystem fs, the offsets of the various blocks of interest
 * are given in the super block as:
 *      [fs->fs_sblkyes]         Super-block
 *      [fs->fs_cblkyes]         Cylinder group block
 *      [fs->fs_iblkyes]         Iyesde blocks
 *      [fs->fs_dblkyes]         Data blocks
 ...
 * given in byte-offset form, so they do yest imply a sector size. The
 ...
 * be set to 0 if yes reserve of free blocks is deemed necessary,
 * however throughput drops by fifty percent if the file system
 * is run at between 95% and 100% full; thus the minimum default
 * value of fs_minfree is 5%. However, to get good clustering
 * performance, 10% is a better choice. hence we use 10% as our
 * default value. With 10% free space, fragmentation is yest a
...
#define	ufs_cgsblock(c)	(ufs_cgstart(c) + uspi->s_sblkyes)	/* super blk */
#define	ufs_cgcmin(c)	(ufs_cgstart(c) + uspi->s_cblkyes)	/* cg block */
#define	ufs_cgimin(c)	(ufs_cgstart(c) + uspi->s_iblkyes)	/* iyesde blk */
#define	ufs_cgdmin(c)	(ufs_cgstart(c) + uspi->s_dblkyes)	/* 1st data */

/*
 * Macros for handling iyesde numbers:
 *     iyesde number to file system block offset.
 *     iyesde number to cylinder group number.
 *     iyesde number to file system block address.
 */
#define	ufs_iyestocg(x)		((x) / uspi->s_ipg)
#define	ufs_iyestocgoff(x)	((x) % uspi->s_ipg)
#define	ufs_iyestofsba(x)	(((u64)ufs_cgimin(ufs_iyestocg(x))) + ufs_iyestocgoff(x) / uspi->s_iyespf)
#define	ufs_iyestofsbo(x)	((x) % uspi->s_iyespf)

/*
 * Compute the cylinder and rotational position of a cyl block addr.
 */
#define ufs_cbtocylyes(byes) \
	((byes) * uspi->s_nspf / uspi->s_spc)
#define ufs_cbtorpos(byes)				      \
	((UFS_SB(sb)->s_flags & UFS_CG_SUN) ?		      \
	 (((((byes) * uspi->s_nspf % uspi->s_spc) %	      \
	    uspi->s_nsect) *				      \
	   uspi->s_nrpos) / uspi->s_nsect)		      \
	 :						      \
	((((byes) * uspi->s_nspf % uspi->s_spc / uspi->s_nsect \
	* uspi->s_trackskew + (byes) * uspi->s_nspf % uspi->s_spc \
...
#define ufs_lblkyes(loc)		((loc) >> uspi->s_bshift)
...
struct ufs_dir_entry {
	__fs32  d_iyes;			/* iyesde number of this entry */
...
struct ufs_csum_core {
	...
	__u64	cs_nifree;	/* number of free iyesdes */
 ...
 * File system flags
 */
#define UFS_UNCLEAN      0x01    /* file system yest clean at mount (unused) */
...
struct ufs_super_block {
	...
	__fs32	fs_sblkyes;	/* addr of super-block in filesys */
	__fs32	fs_cblkyes;	/* offset of cyl-block in filesys */
	__fs32	fs_iblkyes;	/* offset of iyesde-blocks in filesys */
	__fs32	fs_dblkyes;	/* offset of first data after cg */
...
/* these fields can be computed from the others */
	...
	__fs32	fs_bshift;	/* ``lblkyes'' calc of logical blkyes */
...
/* these fields can be computed from the others */
	...
	__fs32	fs_iyespb;	/* value of INOPB */
	__fs32	fs_nspf;	/* value of NSPF */
/* yet ayesther configuration parameter */
...
/* Neither of those fields is used in the Tahoe code right yesw but */
...
/* these fields can be computed from the others */
	...
	__fs32	fs_ipg;		/* iyesdes per cylinder group */
...
/* these fields are cleared at mount time */
	...
	union {
		...
		struct {
			...
			__fs32    fs_pendingiyesdes;/*iyesdes in process of being freed */
		} fs_u2;
	}  fs_u11;
	union {
		struct {
		...
		struct {
			...
			__fs32	fs_iyesdefmt;	/* format of on-disk iyesdes */
...
#define ufs_ocg_blks(sb, ucg, cylyes) fs32_to_cpu((sb), ((struct ufs_old_cylinder_group *)(ucg))->cg_b[cylyes])
#define ufs_ocg_iyessused(sb, ucg)    fs32_to_cpu((sb), ((struct ufs_old_cylinder_group *)(ucg))->cg_iused)
...
struct	ufs_cylinder_group {
	...
	__fs16	cg_niblk;		/* number of iyesde blocks this cg */
	...
	__fs32	cg_irotor;		/* position of last used iyesde */
	...
	__fs32	cg_iusedoff;		/* (char) used iyesde map */
	...
	union {
		...
		struct {
			...
			__fs32	cg_clusteroff;	/* (u_int8) free cluster map */
			__fs32	cg_nclusterblks;/* number of clusters this cg */
			__fs32   cg_niblk; /* number of iyesde blocks this cg */
			__fs32   cg_initediblk;	/* last initialized iyesde */
...
struct ufs_old_cylinder_group {
	...
	__u8	cg_iused[256];		/* used iyesde map */
	__fs32	cg_magic;		/* magic number */
...
/*
 * structure of an on-disk iyesde
 */
struct ufs_iyesde {
	__fs16	ui_mode;		/*  0x0 */
	__fs16	ui_nlink;		/*  0x2 */
	union {
		...
		__fs32	ui_inumber;		/*  0x4 lsf: iyesde number */
	...
	union {
		struct {
			__fs32	ui_shadow;	/* 0x70 shadow iyesde with security data */
...
#define UFS_NXADDR  2            /* External addresses in iyesde. */
struct ufs2_iyesde {
	...
	__fs32     ui_blksize;     /*  12: Iyesde blocksize. */
	...
	__fs64   ui_ctime;       /*  48: Last iyesde change time. */
	__fs64   ui_birthtime;   /*  56: Iyesde creation time. */
	__fs32     ui_mtimensec;   /*  64: Last modified time. */
	__fs32     ui_atimensec;   /*  68: Last access time. */
	__fs32     ui_ctimensec;   /*  72: Last iyesde change time. */
	__fs32     ui_birthnsec;   /*  76: Iyesde creation time. */
...
#define UFS_UF_NODUMP     0x00000001  /* do yest dump */
...
struct ufs_cg_private_info {
	...
	__u16	c_niblk;	/* number of iyesde blocks this cg */
...
struct ufs_sb_private_info {
	...
	__u32	s_sblkyes;	/* offset of super-blocks in filesys */
	__u32	s_cblkyes;	/* offset of cg-block in filesys */
	__u32	s_iblkyes;	/* offset of iyesde-blocks in filesys */
	__u32	s_dblkyes;	/* offset of first data after cg */
	...
	__u32	s_bshift;	/* `lblkyes'' calc of logical blkyes */
	...
	__u32	s_ipg;		/* iyesdes per cylinder group */
	...
	__u32	s_iyespf;	/* iyesdes per fragment */
