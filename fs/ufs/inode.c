...
static int ufs_block_to_path(struct inode *inode, sector_t i_block,
    unsigned offsets[4]) {
 ...
 * ones.
 */
static bool ufs_extend_tail(struct inode *inode, u64 writes_to,
    int *err, struct page *locked_page) {
  struct ufs_inode_info *ufsi = UFS_I(inode);
  struct super_block *sb = inode->i_sb;
  struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
  unsigned lastfrag = ufsi->i_lastfrag; /* it's a short file, so unsigned is
                                         * enough */
...
static int ufs_getfrag_block(struct inode *inode, sector_t fragment,
...
static void ufs1_update_inode(struct inode *inode,
...
static void ufs2_update_inode(struct inode *inode,
...
static void free_branch_tail(struct inode *inode, unsigned from,
    struct ufs_buffer_head *ubh, int depth) {
  struct super_block *sb = inode->i_sb;
  struct ufs_sb_private_info *uspi = UFS_SB(sb)->s_uspi;
  unsigned i;
  if (--depth) {
