 ...
 */
static int afs_find_contig_bits(union afs_xdr_dir_block *block,
...
void afs_edit_dir_add(struct afs_vnode *vnode,
  ...
  for (b = 0; b < nr_blocks + 1; b++) {
    ...
    block
      = kmap_local_folio(folio,
        b * AFS_DIR_BLOCK_SIZE - folio_file_pos(folio));
   ...
   */
  trace_afs_edit_dir(vnode, why, afs_edit_dir_create_nospc, 0, 0, 0, 0,
      name->name);
  clear_bit(AFS_VNODE_DIR_VALID, &vnode->flags);
  goto out_unmap;
new_directory:
...
out_unmap:
  ...
  return;
invalidated:
  trace_afs_edit_dir(vnode, why, afs_edit_dir_create_inval, 0, 0, 0, 0,
      name->name);
  clear_bit(AFS_VNODE_DIR_VALID, &vnode->flags);
  kunmap_local(block);
  if (folio != folio0) {
  ...
  goto out_unmap;
error:
  trace_afs_edit_dir(vnode, why, afs_edit_dir_create_error, 0, 0, 0, 0,
      name->name);
  clear_bit(AFS_VNODE_DIR_VALID, &vnode->flags);
  goto out_unmap;
}
...
void afs_edit_dir_remove(struct afs_vnode *vnode,
  ...
  for (b = 0; b < nr_blocks; b++) {
    ...
    if (b > AFS_DIR_BLOCKS_WITH_CTR
        || meta->meta.alloc_ctrs[b]
        <= AFS_DIR_SLOTS_PER_BLOCK - 1 - need_slots) {
