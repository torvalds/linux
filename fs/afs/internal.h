 ...
 * allows whole-file locks and yes upgrading/downgrading).
 */
enum afs_flock_mode {
...
struct afs_fs_context {
	...
	bool			yes_cell;	/* T if the source is "yesne" (for dynroot) */
...
struct afs_call {
	...
	struct afs_vyesde	*lvyesde;	/* vyesde being locked */
	...
	u32			epoch;
	unsigned int		max_lifespan;	/* Maximum lifespan to set if yest 0 */
...
struct afs_wb_key {
	refcount_t		usage;
	struct key		*key;
	struct list_head	vyesde_link;	/* Link in vyesde->wb_keys */
};
...
/*
 * Record of an outstanding read operation on a vyesde.
 */
struct afs_read {
...
struct afs_net {
	...
	/* Kyeswn servers.  Theoretically each fileserver can only be in one
	 * cell, but in practice, people create aliases and subsets and there's
	 * yes easy way to distinguish them.
	 */
 ...
 * for authentication and encryption.  The cell name is yest typically used in
 * the protocol.
 *
 * There is yes easy way to determine if two cells are aliases or one is a
 * subset of ayesther.
 */
struct afs_cell {
	union {
		struct rcu_head	rcu;
		struct rb_yesde	net_yesde;	/* Node in net->cells */
	};
	struct afs_net		*net;
	struct key		*ayesnymous_key;	/* ayesnymous user key for this cell */
	struct work_struct	manager;	/* Manager for init/deinit/dns */
	struct hlist_yesde	proc_link;	/* /proc cell list link */
...
#define AFS_VLSERVER_FL_IS_YFS	2		/* Server is YFS yest AFS */
...
struct afs_server {
	...
	struct rb_yesde		uuid_rb;	/* Link in net->servers */
	struct hlist_yesde	addr4_link;	/* Link in net->fs_addresses4 */
	struct hlist_yesde	addr6_link;	/* Link in net->fs_addresses6 */
	struct hlist_yesde	proc_link;	/* Link in net->fs_proc */
	struct afs_server	*gc_next;	/* Next server in manager's list */
	time64_t		put_time;	/* Time at which last put */
	time64_t		update_at;	/* Time at which to next update the record */
	unsigned long		flags;
#define AFS_SERVER_FL_NOT_READY	1		/* The record is yest ready for use */
#define AFS_SERVER_FL_NOT_FOUND	2		/* VL server says yes such server */
...
#define AFS_SERVER_FL_IS_YFS	9		/* Server is YFS yest AFS */
...
struct afs_vol_interest {
	struct hlist_yesde	srv_link;	/* Link in server->cb_volumes */
...
struct afs_cb_interest {
	struct hlist_yesde	cb_vlink;	/* Link in vol_interest->cb_interests */
	struct afs_vol_interest	*vol_interest;
	struct afs_server	*server;	/* Server on which this interest resides */
	struct super_block	*sb;		/* Superblock on which iyesdes reside */
	union {
		struct rcu_head	rcu;
		afs_volid_t	vid;		/* Volume ID to match */
	};
...
struct afs_server_list {
	...
	unsigned short		vyesvol_mask;	/* Servers to be skipped due to VNOVOL */
...
#define AFS_VOLUME_OFFLINE	4	/* - T if volume offline yestice given */
#define AFS_VOLUME_BUSY		5	/* - T if volume busy yestice given */
#ifdef CONFIG_AFS_FSCACHE
	struct fscache_cookie	*cache;		/* caching cookie */
#endif
...
enum afs_lock_state {
	AFS_VNODE_LOCK_NONE,		/* The vyesde has yes lock on the server */
	...
	AFS_VNODE_LOCK_DELETED,		/* The vyesde has been deleted whilst we have a lock */
};

/*
 * AFS iyesde private data.
 *
 * Note that afs_alloc_iyesde() *must* reset anything that could incorrectly
 * leak from one iyesde to ayesther.
 */
struct afs_vyesde {
	struct iyesde		vfs_iyesde;	/* the VFS's iyesde record */

	struct afs_volume	*volume;	/* volume on which vyesde resides */
	struct afs_fid		fid;		/* the file identifier for this iyesde */
	...
	struct rw_semaphore	validate_lock;	/* lock for validating this vyesde */
	struct rw_semaphore	rmdir_lock;	/* Lock for rmdir vs sillyrename */
	struct key		*silly_key;	/* Silly rename key */
	spinlock_t		wb_lock;	/* lock for wb_keys */
	spinlock_t		lock;		/* waitqueue/flags lock */
	unsigned long		flags;
#define AFS_VNODE_CB_PROMISED	0		/* Set if vyesde has a callback promise */
#define AFS_VNODE_UNSET		1		/* set if vyesde attributes yest yet set */
#define AFS_VNODE_DIR_VALID	2		/* Set if dir contents are valid */
#define AFS_VNODE_ZAP_DATA	3		/* set if vyesde's data should be invalidated */
#define AFS_VNODE_DELETED	4		/* set if vyesde deleted on server */
#define AFS_VNODE_MOUNTPOINT	5		/* set if vyesde is a mountpoint symlink */
#define AFS_VNODE_AUTOCELL	6		/* set if Vyesde is an auto mount point */
#define AFS_VNODE_PSEUDODIR	7 		/* set if Vyesde is a pseudo directory */
#define AFS_VNODE_NEW_CONTENT	8		/* Set if file has new content (create/trunc-0) */
	...
	/* outstanding callback yestification on this file */
	struct afs_cb_interest __rcu *cb_interest; /* Server on which this resides */
	unsigned int		cb_s_break;	/* Mass break counter on ->server */
	unsigned int		cb_v_break;	/* Mass break counter on ->volume */
	unsigned int		cb_break;	/* Break counter on vyesde */
	seqlock_t		cb_lock;	/* Lock for ->cb_interest, ->status, ->cb_*break */

	time64_t		cb_expires_at;	/* time at which callback expires */
};

static inline struct fscache_cookie *afs_vyesde_cache(struct afs_vyesde *vyesde)
{
#ifdef CONFIG_AFS_FSCACHE
	return vyesde->cache;
#else
...
/*
 * cached security record for one user's attempt to access a vyesde
 */
struct afs_permit {
...
/*
 * Immutable cache of CallerAccess records from attempts to access vyesdes.
 * These may be shared between multiple vyesdes.
 */
struct afs_permits {
	struct rcu_head		rcu;
	struct hlist_yesde	hash_yesde;	/* Link in hash */
...
struct afs_fs_cursor {
	...
	unsigned int		cb_break_2;	/* cb_break + cb_s_break (2nd vyesde) */
	short			index;		/* Current server */
	short			error;
	unsigned short		flags;
...
extern struct fscache_cookie_def afs_vyesde_cache_index_def;
#else
#define afs_cell_cache_index_def	(*(struct fscache_cookie_def *) NULL)
#define afs_volume_cache_index_def	(*(struct fscache_cookie_def *) NULL)
#define afs_vyesde_cache_index_def	(*(struct fscache_cookie_def *) NULL)
#endif
...
extern void __afs_break_callback(struct afs_vyesde *, enum afs_cb_break_reason);
extern void afs_break_callback(struct afs_vyesde *, enum afs_cb_break_reason);
extern void afs_break_callbacks(struct afs_server *, size_t, struct afs_callback_break *);

extern int afs_register_server_cb_interest(struct afs_vyesde *,
...
static inline unsigned int afs_calc_vyesde_cb_break(struct afs_vyesde *vyesde)
{
	return vyesde->cb_break + vyesde->cb_v_break;
}

static inline bool afs_cb_is_broken(unsigned int cb_break,
				    const struct afs_vyesde *vyesde,
				    const struct afs_cb_interest *cbi)
{
	return !cbi || cb_break != (vyesde->cb_break +
				    vyesde->volume->cb_v_break);
}
...
extern const struct iyesde_operations afs_dir_iyesde_operations;
 ...
 * dir_edit.c
 */
extern void afs_edit_dir_add(struct afs_vyesde *, struct qstr *, struct afs_fid *,
			     enum afs_edit_dir_reason);
extern void afs_edit_dir_remove(struct afs_vyesde *, struct qstr *, enum afs_edit_dir_reason);

/*
 * dir_silly.c
 */
extern int afs_sillyrename(struct afs_vyesde *, struct afs_vyesde *,
			   struct dentry *, struct key *);
extern int afs_silly_iput(struct dentry *, struct iyesde *);

/*
 * dynroot.c
 */
extern const struct iyesde_operations afs_dynroot_iyesde_operations;
extern const struct dentry_operations afs_dynroot_dentry_operations;

extern struct iyesde *afs_try_auto_mntpt(struct dentry *, struct iyesde *);
...
extern const struct iyesde_operations afs_file_iyesde_operations;
extern const struct file_operations afs_file_operations;

extern int afs_cache_wb_key(struct afs_vyesde *, struct afs_file *);
extern void afs_put_wb_key(struct afs_wb_key *);
extern int afs_open(struct iyesde *, struct file *);
extern int afs_release(struct iyesde *, struct file *);
extern int afs_fetch_data(struct afs_vyesde *, struct key *, struct afs_read *);
...
extern void afs_lock_may_be_available(struct afs_vyesde *);
...
extern int afs_fs_remove(struct afs_fs_cursor *, struct afs_vyesde *, const char *, bool,
			 struct afs_status_cb *);
extern int afs_fs_link(struct afs_fs_cursor *, struct afs_vyesde *, const char *,
		       struct afs_status_cb *, struct afs_status_cb *);
extern int afs_fs_symlink(struct afs_fs_cursor *, const char *, const char *,
			  struct afs_status_cb *, struct afs_fid *, struct afs_status_cb *);
extern int afs_fs_rename(struct afs_fs_cursor *, const char *,
			 struct afs_vyesde *, const char *,
			 struct afs_status_cb *, struct afs_status_cb *);
 ...
 * iyesde.c
 */
extern void afs_vyesde_commit_status(struct afs_fs_cursor *,
				    struct afs_vyesde *,
				    unsigned int,
				    const afs_dataversion_t *,
				    struct afs_status_cb *);
extern int afs_fetch_status(struct afs_vyesde *, struct key *, bool, afs_access_t *);
extern int afs_iget5_test(struct iyesde *, void *);
extern struct iyesde *afs_iget_pseudo_dir(struct super_block *, bool);
extern struct iyesde *afs_iget(struct super_block *, struct key *,
			      struct afs_iget_data *, struct afs_status_cb *,
			      struct afs_cb_interest *,
			      struct afs_vyesde *);
extern void afs_zap_data(struct afs_vyesde *);
extern bool afs_check_validity(struct afs_vyesde *);
extern int afs_validate(struct afs_vyesde *, struct key *);
extern int afs_getattr(const struct path *, struct kstat *, u32, unsigned int);
extern int afs_setattr(struct dentry *, struct iattr *);
extern void afs_evict_iyesde(struct iyesde *);
extern int afs_drop_iyesde(struct iyesde *);
...
static inline struct afs_net *afs_i2net(struct iyesde *iyesde)
{
	return afs_sb2net(iyesde->i_sb);
}

static inline struct afs_net *afs_v2net(struct afs_vyesde *vyesde)
{
	return afs_i2net(&vyesde->vfs_iyesde);
}
...
#define afs_stat_v(vyesde, n) __afs_stat(&afs_v2net(vyesde)->n)
 ...
 * mntpt.c
 */
extern const struct iyesde_operations afs_mntpt_iyesde_operations;
extern const struct iyesde_operations afs_autocell_iyesde_operations;
 ...
 * rotate.c
 */
extern bool afs_begin_vyesde_operation(struct afs_fs_cursor *, struct afs_vyesde *,
				      struct key *, bool);
extern bool afs_select_fileserver(struct afs_fs_cursor *);
extern bool afs_select_current_fileserver(struct afs_fs_cursor *);
extern int afs_end_vyesde_operation(struct afs_fs_cursor *);
...
extern void afs_clear_permits(struct afs_vyesde *);
extern void afs_cache_permit(struct afs_vyesde *, struct key *, unsigned int,
			     struct afs_status_cb *);
extern void afs_zap_permits(struct rcu_head *);
extern struct key *afs_request_key(struct afs_cell *);
extern struct key *afs_request_key_rcu(struct afs_cell *);
extern int afs_check_permit(struct afs_vyesde *, struct key *, afs_access_t *);
extern int afs_permission(struct iyesde *, int);
...
extern struct afs_server_list *afs_alloc_server_list(struct afs_cell *, struct key *,
						     struct afs_vldb_entry *,
						     u8);
extern bool afs_anyestate_server_list(struct afs_server_list *, struct afs_server_list *);
...
extern void afs_prune_wb_keys(struct afs_vyesde *);
...
extern int yfs_fs_make_dir(struct afs_fs_cursor *, const char *, umode_t, struct afs_status_cb *,
			   struct afs_fid *, struct afs_status_cb *);
extern int yfs_fs_remove_file2(struct afs_fs_cursor *, struct afs_vyesde *, const char *,
			       struct afs_status_cb *, struct afs_status_cb *);
extern int yfs_fs_remove(struct afs_fs_cursor *, struct afs_vyesde *, const char *, bool,
			 struct afs_status_cb *);
extern int yfs_fs_link(struct afs_fs_cursor *, struct afs_vyesde *, const char *,
		       struct afs_status_cb *, struct afs_status_cb *);
extern int yfs_fs_symlink(struct afs_fs_cursor *, const char *, const char *,
			  struct afs_status_cb *, struct afs_fid *, struct afs_status_cb *);
extern int yfs_fs_rename(struct afs_fs_cursor *, const char *, struct afs_vyesde *, const char *,
			 struct afs_status_cb *, struct afs_status_cb *);
 ...
 */
static inline struct afs_vyesde *AFS_FS_I(struct iyesde *iyesde)
{
	return container_of(iyesde, struct afs_vyesde, vfs_iyesde);
}

static inline struct iyesde *AFS_VNODE_TO_I(struct afs_vyesde *vyesde)
{
						 ...
						 struct afs_vyesde *vyesde)
{
	if (fc->ac.error == -ENOENT) {
		...
		afs_break_callback(vyesde, afs_cb_break_for_deleted);
	}
}
...
static inline int afs_bad(struct afs_vyesde *vyesde, enum afs_file_error where)
{
	trace_afs_file_error(vyesde, -EIO, where);
	return -EIO;
}
