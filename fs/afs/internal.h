...
struct afs_read {
  ...
  struct netfs_io_subrequest *subreq; /* Fscache helper read request this
                                       * belongs to */
...
struct afs_net {
  ...
  struct list_head fs_probe_fast;  /* List of afs_server to probe at 30s
                                    * intervals */
  struct list_head fs_probe_slow;  /* List of afs_server to probe at 5m
                                    * intervals */
...
#define AFS_VLSERVER_PROBE_RESPONDED    0x01 /* At least once response (may be
                                              * abort) */
...
#define AFS_VLSERVER_PROBE_LOCAL_FAILURE  0x08 /* A local failure prevented a
                                                * probe */
  } probe;
...
#define AFS_VOL_VTM_RO  0x02 /* R/O version of the volume is available (on this
                              * server) */
#define AFS_VOL_VTM_BAK 0x04 /* backup version of the volume is available (on
                              * this server) */
...
#define AFS_ESTATE_LOCAL_FAILURE 4    /* Set if there was a local failure (eg.
                                       * ENOMEM) */
};
...
#define AFS_SERVER_FL_NEEDS_UPDATE 2    /* Fileserver address list is out of
                                         * date */
...
#define AFS_SERVER_FL_NO_RM2  18    /* Fileserver doesn't support
                                     * YFS.RemoveFile2 */
#define AFS_SERVER_FL_HAS_FS64  19    /* Fileserver supports
                                       * FS.{Fetch,Store}Data64 */
  ...
  /* Probe state */
  struct afs_endpoint_state __rcu *endpoint_state; /* Latest endpoint/probe
                                                    * state */
...
struct afs_server_list {
  ...
  enum afs_ro_replicating ro_replicating; /* RW->RO update (probably) in
                                           * progress */
...
#define AFS_VOLUME_MAYBE_NO_IBULK 4 /* - T if some servers don't have
                                     * InlineBulkStatus */
#define AFS_VOLUME_RM_TREE  5 /* - Set if volume removed from cell->volumes */
#ifdef CONFIG_AFS_FSCACHE
  struct fscache_volume *cache;   /* Caching cookie */
#endif
  struct afs_server_list __rcu *servers;  /* List of servers on which volume
                                           * resides */
...
enum afs_lock_state {
  ...
  AFS_VNODE_LOCK_WAITING_FOR_CB,  /* We're waiting for the server to break the
                                   * callback */
...
#define AFS_VNODE_NEW_CONTENT 8   /* Set if file has new content
                                   * (create/trunc-0) */
...
struct afs_permits {
  ...
  struct afs_permit permits[] __counted_by(nr_permits); /* List of permits
                                                         * sorted by key pointer
                                                         * */
};
...
struct afs_operation {
  ...
  struct afs_server *server;  /* Server we're using (ref pinned by server_list)
                               * */
  struct afs_endpoint_state *estate;  /* Current endpoint state (doesn't pin
                                       * ref) */
...
#define AFS_OPERATION_CUR_ONLY    0x0010  /* Set if current server only (file
                                           * lock held) */
#define AFS_OPERATION_NO_VSLEEP   0x0020  /* Set to prevent sleep on VBUSY,
                                           * VOFFLINE, ... */
...
#define AFS_OPERATION_TRIED_ALL   0x0400  /* Set if we've tried all the
                                           * fileservers */
#define AFS_OPERATION_RETRY_SERVER  0x0800  /* Set if we should retry the
                                             * current server */
#define AFS_OPERATION_DIR_CONFLICT  0x1000  /* Set if we detected a 3rd-party
                                             * dir change */
};
...
static inline void afs_invalidate_cache(struct afs_vnode *vnode,
    unsigned int flags) {
  struct afs_vnode_cache_aux aux;
 ...
 * addr_list.c
 */
struct afs_addr_list *afs_get_addrlist(struct afs_addr_list *alist,
    enum afs_alist_trace reason);
extern struct afs_addr_list *afs_alloc_addrlist(unsigned int nr);
extern void afs_put_addrlist(struct afs_addr_list *alist,
    enum afs_alist_trace reason);
...
extern struct afs_cell *afs_find_cell(struct afs_net *, const char *, unsigned,
    enum afs_cell_trace);
extern struct afs_cell *afs_lookup_cell(struct afs_net *, const char *,
    unsigned,
    const char *, bool);
 ...
 * dir_edit.c
 */
extern void afs_edit_dir_add(struct afs_vnode *, struct qstr *,
    struct afs_fid *,
    enum afs_edit_dir_reason);
extern void afs_edit_dir_remove(struct afs_vnode *, struct qstr *,
    enum afs_edit_dir_reason);
 ...
 * fs_operation.c
 */
extern struct afs_operation *afs_alloc_operation(struct key *,
    struct afs_volume *);
 ...
 * fs_probe.c
 */
struct afs_endpoint_state *afs_get_endpoint_state(
  struct afs_endpoint_state *estate,
  enum afs_estate_trace where);
void afs_put_endpoint_state(struct afs_endpoint_state *estate,
    enum afs_estate_trace where);
extern void afs_fileserver_probe_result(struct afs_call *);
void afs_fs_probe_fileserver(struct afs_net *net, struct afs_server *server,
    struct afs_addr_list *new_addrs, struct key *key);
int afs_wait_for_fs_probes(struct afs_operation *op,
    struct afs_server_state *states, bool intr);
extern void afs_probe_fileserver(struct afs_net *, struct afs_server *);
extern void afs_fs_probe_dispatcher(struct work_struct *);
int afs_wait_for_one_fs_probe(struct afs_server *server,
    struct afs_endpoint_state *estate,
...
static inline void afs_op_accumulate_error(struct afs_operation *op, int error,
    s32 abort_code) {
...
#else
static inline int afs_proc_init(struct afs_net *net) {
  return 0;
}
...
static inline void afs_make_op_call(struct afs_operation *op,
    struct afs_call *call,
    gfp_t gfp) {
...
static inline void afs_extract_begin(struct afs_call *call, void *buf,
    size_t size) {
  call->iov_len = size;
...
extern struct afs_server *afs_find_server_by_uuid(struct afs_net *,
    const uuid_t *);
extern struct afs_server *afs_lookup_server(struct afs_cell *, struct key *,
    const uuid_t *, u32);
extern struct afs_server *afs_get_server(struct afs_server *,
    enum afs_server_trace);
extern struct afs_server *afs_use_server(struct afs_server *,
    enum afs_server_trace);
extern void afs_unuse_server(struct afs_net *, struct afs_server *,
    enum afs_server_trace);
extern void afs_unuse_server_notime(struct afs_net *, struct afs_server *,
    enum afs_server_trace);
...
bool afs_check_server_record(struct afs_operation *op,
    struct afs_server *server, struct key *key);
 ...
 * server_list.c
 */
static inline struct afs_server_list *afs_get_serverlist(
...
extern bool afs_annotate_server_list(struct afs_server_list *,
    struct afs_server_list *);
void afs_attach_volume_to_servers(struct afs_volume *volume,
    struct afs_server_list *slist);
void afs_reattach_volume_to_servers(struct afs_volume *volume,
    struct afs_server_list *slist,
    struct afs_server_list *old);
void afs_detach_volume_from_servers(struct afs_volume *volume,
    struct afs_server_list *slist);
...
extern struct afs_vldb_entry *afs_vl_get_entry_by_name_u(struct afs_vl_cursor *,
    const char *, int);
extern struct afs_addr_list *afs_vl_get_addrs_u(struct afs_vl_cursor *,
    const uuid_t *);
...
extern int afs_send_vl_probes(struct afs_net *, struct key *,
    struct afs_vlserver_list *);
 ...
 * vlserver_list.c
 */
static inline struct afs_vlserver *afs_get_vlserver(
    struct afs_vlserver *vlserver) {
  refcount_inc(&vlserver->ref);
  return vlserver;
}

static inline struct afs_vlserver_list *afs_get_vlserverlist(
...
extern struct afs_vlserver *afs_alloc_vlserver(const char *, size_t,
    unsigned short);
...
bool afs_try_get_volume(struct afs_volume *volume,
    enum afs_volume_trace reason);
extern struct afs_volume *afs_get_volume(struct afs_volume *,
