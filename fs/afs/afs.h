...
struct afs_fid {
	...
	afs_vyesdeid_t	vyesde;		/* Lower 64-bits of file index within volume */
	u32		vyesde_hi;	/* Upper 32-bits of file index */
	u32		unique;		/* unique ID number (file index version) */
};

/*
 * AFS callback yestification
 */
typedef enum {
	AFSCM_CB_UNTYPED	= 0,	/* yes type set on CB break */
	AFSCM_CB_EXCLUSIVE	= 1,	/* CB exclusive to CM [yest implemented] */
...
struct afs_uuid {
	...
	__s8		yesde[6];			/* spatially unique yesde ID (MAC addr) */
};
...
struct afs_file_status {
	...
	afs_access_t		ayesn_access;	/* access rights for unauthenticated caller */
