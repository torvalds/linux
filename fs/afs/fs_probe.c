...
static void afs_endpoint_state_rcu(struct rcu_head *rcu) {
  struct afs_endpoint_state *estate = container_of(rcu,
...
static void afs_schedule_fs_probe(struct afs_net *net,
  ...
  atj
    += fast ? afs_fs_probe_fast_poll_interval : afs_fs_probe_slow_poll_interval;
 ...
 */
static void afs_finished_fs_probe(struct afs_net *net,
 ...
 */
static void afs_done_one_fs_probe(struct afs_net *net,
    struct afs_server *server,
    struct afs_endpoint_state *estate) {
  _enter("");
...
out:
  spin_unlock(&server->probe_lock);
  trace_afs_fs_probe(server, false, estate, index, call->error,
      call->abort_code, rtt_us);
...
int afs_wait_for_fs_probes(struct afs_operation *op,
  ...
  for (i = 0; i < slist->nr_servers; i++) {
    add_wait_queue(&slist->servers[i].server->probe_wq,
        &states[i].probe_waiter);
  }
  for (;;) {
    still_probing = false;
...
stop:
  ...
  for (i = 0; i < slist->nr_servers; i++) {
    remove_wait_queue(&slist->servers[i].server->probe_wq,
        &states[i].probe_waiter);
  }
 ...
 */
static void afs_dispatch_fs_probe(struct afs_net *net,
    struct afs_server *server)
