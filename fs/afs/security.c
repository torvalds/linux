...
struct key *afs_request_key(struct afs_cell *cell)
{
	...
	_enter("{%x}", key_serial(cell->ayesnymous_key));

	_debug("key %s", cell->ayesnymous_key->description);
	key = request_key_net(&key_type_rxrpc, cell->ayesnymous_key->description,
			      cell->net->net, NULL);
	if (IS_ERR(key)) {
		...
		/* act as ayesnymous user */
		_leave(" = {%x} [ayesn]", key_serial(cell->ayesnymous_key));
		return key_get(cell->ayesnymous_key);
	} else {
...
struct key *afs_request_key_rcu(struct afs_cell *cell)
{
	...
	key = request_key_net_rcu(&key_type_rxrpc,
				  cell->ayesnymous_key->description,
				  cell->net->net);
	if (IS_ERR(key)) {
		if (PTR_ERR(key) != -ENOKEY) {
			_leave(" = %ld", PTR_ERR(key));
...
void afs_put_permits(struct afs_permits *permits)
{
	if (permits && refcount_dec_and_test(&permits->usage)) {
		...
		hash_del_rcu(&permits->hash_yesde);
 ...
 */
void afs_clear_permits(struct afs_vyesde *vyesde)
{
	struct afs_permits *permits;

	spin_lock(&vyesde->lock);
	permits = rcu_dereference_protected(vyesde->permit_cache,
					    lockdep_is_held(&vyesde->lock));
	RCU_INIT_POINTER(vyesde->permit_cache, NULL);
 ...
 * that returned a vyesde status for a particular key.  If a callback break
 * occurs whilst the operation was in progress then we have to ditch the cache
 * as the ACL *may* have changed.
 */
void afs_cache_permit(struct afs_vyesde *vyesde, struct key *key,
		      unsigned int cb_break, struct afs_status_cb *scb)
{
	...
	_enter("{%llx:%llu},%x,%x",
	       vyesde->fid.vid, vyesde->fid.vyesde, key_serial(key), caller_access);
	 ...
	 */
	permits = rcu_dereference(vyesde->permit_cache);
	if (permits) {
		if (!permits->invalidated) {
			for (i = 0; i < permits->nr_permits; i++) {
				...
				if (afs_cb_is_broken(cb_break, vyesde,
						     rcu_dereference(vyesde->cb_interest))) {
					changed = true;
					break;
				}
		...
		/* If this set of permits is yesw wrong, clear the permits
		 * pointer so that yes one tries to use the stale information.
		 */
		if (changed) {
			spin_lock(&vyesde->lock);
			if (permits != rcu_access_pointer(vyesde->permit_cache))
	...
	if (afs_cb_is_broken(cb_break, vyesde, rcu_dereference(vyesde->cb_interest)))
		goto someone_else_changed_it;

	/* We need a ref on any permits list we want to copy as we'll have to
	 * drop the lock to do memory allocation.
	 */
	if (permits && !refcount_inc_yest_zero(&permits->usage))
		goto someone_else_changed_it;
	...
	hash_for_each_possible(afs_permits_cache, xpermits, hash_yesde, new->h) {
		if (xpermits->h != new->h ||
		...
		if (refcount_inc_yest_zero(&xpermits->usage)) {
	...
	hash_add_rcu(afs_permits_cache, &new->hash_yesde, new->h);
	replacement = new;
	new = NULL;
...
found:
	...
	rcu_read_lock();
	spin_lock(&vyesde->lock);
	zap = rcu_access_pointer(vyesde->permit_cache);
	if (!afs_cb_is_broken(cb_break, vyesde, rcu_dereference(vyesde->cb_interest)) &&
	    zap == permits)
		rcu_assign_pointer(vyesde->permit_cache, replacement);
	else
		zap = replacement;
	spin_unlock(&vyesde->lock);
	rcu_read_unlock();
...
static bool afs_check_permit_rcu(struct afs_vyesde *vyesde, struct key *key,
				 afs_access_t *_access)
{
	...
	_enter("{%llx:%llu},%x",
	       vyesde->fid.vid, vyesde->fid.vyesde, key_serial(key));

	/* check the permits to see if we've got one yet */
	if (key == vyesde->volume->cell->ayesnymous_key) {
		*_access = vyesde->status.ayesn_access;
		_leave(" = t [ayesn %x]", *_access);
		return true;
	}
 ...
 * is granted
 */
int afs_check_permit(struct afs_vyesde *vyesde, struct key *key,
		     afs_access_t *_access)
{
	struct afs_permits *permits;
	bool valid = false;
	...
	if (!valid) {
		 ...
		 */
		_debug("yes valid permit");

		ret = afs_fetch_status(vyesde, key, false, _access);
		if (ret < 0) {
			*_access = 0;
			_leave(" = %d", ret);
			return ret;
 ...
 */
int afs_permission(struct iyesde *iyesde, int mask)
{
	...
	_enter("{{%llx:%llu},%lx},%x,",
	       vyesde->fid.vid, vyesde->fid.vyesde, vyesde->flags, mask);

	if (mask & MAY_NOT_BLOCK) {
		key = afs_request_key_rcu(vyesde->volume->cell);
		if (IS_ERR(key))
			return -ECHILD;

		ret = -ECHILD;
		if (!afs_check_validity(vyesde) ||
		    !afs_check_permit_rcu(vyesde, key, &access))
			goto error;
	} else {
	...
	_debug("REQ %x ACC %x on %s",
	       mask, access, S_ISDIR(iyesde->i_mode) ? "dir" : "file");
	...
	if (S_ISDIR(iyesde->i_mode)) {
		...
		if (!(access & AFS_ACE_LOOKUP))
			goto permission_denied;
		if ((mask & MAY_EXEC) && !(iyesde->i_mode & S_IXUSR))
			goto permission_denied;
