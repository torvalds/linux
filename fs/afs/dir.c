...
expand:
  ...
  iov_iter_xarray(&req->def_iter, ITER_DEST,
      &dvnode->netfs.inode.i_mapping->i_pages,
      0, i_size);
...
out_op:
  ...
  if (op->file[0].scb.have_status) {
    dentry->d_fsdata
      = (void *) (unsigned long) op->file[0].scb.status.data_version;
  } else {
...
static void afs_unlink_put(struct afs_operation *op) {
  _enter("op=%08x", op->debug_id);
  if (op->unlink.need_rehash && afs_op_error(op) < 0
      && afs_op_error(op) != -ENOENT) {
    d_rehash(op->dentry);
  }
}
...
static void afs_rename_edit_dir(struct afs_operation *op) {
  ...
  if (test_bit(AFS_VNODE_DIR_VALID, &orig_dvnode->flags)
      && orig_dvnode->status.data_version
      == orig_dvp->dv_before + orig_dvp->dv_delta) {
  ...
  if (test_bit(AFS_VNODE_DIR_VALID, &new_dvnode->flags)
      && new_dvnode->status.data_version
      == new_dvp->dv_before + new_dvp->dv_delta) {
...
static bool afs_dir_release_folio(struct folio *folio, gfp_t gfp_flags) {
  ...
  _enter("{{%llx:%llu}[%lu]}", dvnode->fid.vid, dvnode->fid.vnode,
      folio->index);
