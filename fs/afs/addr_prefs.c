...
static int afs_insert_address_pref(struct afs_addr_preference_list **_preflist,
  ...
  _enter("{%u/%u/%u},%u", preflist->ipv6_off, preflist->nr, preflist->max_prefs,
      index);
  if (preflist->nr == 255) {
    return -ENOSPC;
  }
 ...
 */
static int afs_add_address_pref(struct afs_net *net,
    struct afs_addr_preference_list **_preflist,
    int argc, char **argv) {
 ...
 */
static int afs_del_address_pref(struct afs_net *net,
    struct afs_addr_preference_list **_preflist,
    int argc, char **argv) {
  struct afs_addr_preference_list *preflist = *_preflist;
  struct afs_addr_preference pref;
...
int afs_proc_addr_prefs_write(struct file *file, char *buf, size_t size) {
  ...
  max_prefs
    = min_t(size_t, (psize - sizeof(*old)) / sizeof(old->prefs[0]), 255);
  ret = -ENOMEM;
 ...
 */
void afs_get_address_preferences_rcu(struct afs_net *net,
 ...
 */
void afs_get_address_preferences(struct afs_net *net,
    struct afs_addr_list *alist) {
  if (!net->address_prefs
      || /* Load version before prefs */
      smp_load_acquire(&net->address_pref_version)
