 ...
 */
void vxfs_dumpi(struct vxfs_inode_info *vip, ino_t ino) {
  printk(KERN_DEBUG "\n\n");
  if (ino) {
 ...
 */
static __inline__ umode_t vxfs_transmod(struct vxfs_inode_info *vip) {
 ...
 */
struct inode *vxfs_blkiget(struct super_block *sbp, u_long extent, ino_t ino) {
 ...
 */
static int __vxfs_iget(struct inode *ilistp, struct vxfs_inode_info *vip,
    ino_t ino) {
  struct page *pp;
  u_long offset;
 ...
 */
struct inode *vxfs_stiget(struct super_block *sbp, ino_t ino) {
  struct inode *inode;
  int error;
  inode = new_inode(sbp);
  if (!inode) {
