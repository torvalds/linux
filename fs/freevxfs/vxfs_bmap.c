...
#ifdef DIAGNOSTIC
static void vxfs_typdump(struct vxfs_typed *typ) {
 ...
 */
static daddr_t vxfs_bmap_ext4(struct inode *ip, long bn) {
 ...
 */
static daddr_t vxfs_bmap_indir(struct inode *ip, long indir, int size,
    long block) {
 ...
 *   The physical block number on success, else Zero.
 */
static daddr_t vxfs_bmap_typed(struct inode *ip, long iblock) {
  struct vxfs_inode_info *vip = VXFS_INO(ip);
  struct vxfs_sb_info *sbi = VXFS_SBI(ip->i_sb);
  daddr_t pblock = 0;
  int i;
