...
inline void vxfs_put_page(struct page *pp) {
  kunmap(pp);
  put_page(pp);
}
 ...
 */
static int vxfs_getblk(struct inode *ip, sector_t iblock,
 ...
 */
static sector_t vxfs_bmap(struct address_space *mapping, sector_t block) {
