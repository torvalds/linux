...
#define NFS_SB_MASK (SB_RDONLY | SB_NOSUID | SB_NODEV | SB_NOEXEC \
  | SB_SYNCHRONOUS)
...
static inline void nfs_attr_check_mountpoint(struct super_block *parent,
...
int nfs_create_rpc_client(struct nfs_client *,
    const struct nfs_client_initdata *, rpc_authflavor_t);
struct nfs_client *nfs_get_client(const struct nfs_client_initdata *);
int nfs_probe_server(struct nfs_server *, struct nfs_fh *);
void nfs_server_insert_lists(struct nfs_server *);
void nfs_server_remove_lists(struct nfs_server *);
void nfs_init_timeout_values(struct rpc_timeout *to, int proto, int timeo,
    int retrans);
...
extern struct nfs_client *nfs4_find_client_sessionid(struct net *,
...
extern struct nfs4_label *nfs4_label_alloc(struct nfs_server *server,
    gfp_t flags);
static inline struct nfs4_label *nfs4_label_copy(struct nfs4_label *dst,
    struct nfs4_label *src) {
  if (!dst || !src) {
    return NULL;
  }
...
#else
static inline struct nfs4_label *nfs4_label_alloc(struct nfs_server *server,
    gfp_t flags) {
  return NULL;
}
...
#ifdef CONFIG_NFS_V4_1
static inline void pnfs_bucket_clear_pnfs_ds_commit_verifiers(
...
static inline int nfs_write_verifier_cmp(const struct nfs_write_verifier *v1,
    const struct nfs_write_verifier *v2) {
  return memcmp(v1->data, v2->data, sizeof(v1->data));
}

static inline bool nfs_write_match_verf(const struct nfs_writeverf *verf,
    struct nfs_page *req) {
...
/* unlink.c */
extern struct rpc_task *nfs_async_rename(struct inode *old_dir,
