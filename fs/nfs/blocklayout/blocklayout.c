		...
		struct parallel_io *par, unsigned int offset, int *len)
{
	struct pnfs_block_dev *dev =
		container_of(be->be_device, struct pnfs_block_dev, yesde);
	u64 disk_addr, end;
...
bl_read_pagelist(struct nfs_pgio_header *header)
{
	...
	if ((isect << SECTOR_SHIFT) >= header->iyesde->i_size) {
		header->res.eof = 1;
		header->res.count = header->iyesde->i_size - header->args.offset;
	} else {
...
static struct pnfs_layout_hdr *__bl_alloc_layout_hdr(struct iyesde *iyesde,
...
static struct pnfs_layout_hdr *bl_alloc_layout_hdr(struct iyesde *iyesde,
						   gfp_t gfp_flags)
{
	return __bl_alloc_layout_hdr(iyesde, gfp_flags, false);
}

static struct pnfs_layout_hdr *sl_alloc_layout_hdr(struct iyesde *iyesde,
						   gfp_t gfp_flags)
{
	return __bl_alloc_layout_hdr(iyesde, gfp_flags, true);
}
...
struct layout_verification {
	u32 mode;	/* R or RW */
	u64 start;	/* Expected start of next yesn-COW extent */
...
static int decode_sector_number(__be32 **rp, sector_t *sp)
{
	...
	*rp = xdr_decode_hyper(*rp, &s);
	if (s & 0x1ff) {
		printk(KERN_WARNING "NFS: %s: sector yest aligned\n", __func__);
		return -1;
	}
	*sp = s >> SECTOR_SHIFT;
	return 0;
...
bl_find_get_deviceid(struct nfs_server *server,
		...
		gfp_t gfp_mask)
{
	struct nfs4_deviceid_yesde *yesde;
	unsigned long start, end;

retry:
	yesde = nfs4_find_get_deviceid(server, id, cred, gfp_mask);
	if (!yesde)
		return ERR_PTR(-ENODEV);

	if (test_bit(NFS_DEVICEID_UNAVAILABLE, &yesde->flags) == 0)
		return yesde;

	end = jiffies;
	start = end - PNFS_DEVICE_RETRY_TIMEOUT;
	if (!time_in_range(yesde->timestamp_unavailable, start, end)) {
		nfs4_delete_deviceid(yesde->ld, yesde->nfs_client, id);
		goto retry;
	}
	return ERR_PTR(-ENODEV);
}
		...
		gfp_t gfp_mask)
{
	...
	be->be_device = bl_find_get_deviceid(NFS_SERVER(lo->plh_iyesde), &id,
		...
		struct pnfs_layout_range *range)
{
	...
	if (range->offset % 8) {
		dprintk("%s: offset %lld yest block size aligned\n",
	...
	if (range->length != NFS4_MAX_UINT64) {
		if (range->length % 8) {
			dprintk("%s: length %lld yest block size aligned\n",
...
bl_set_layoutdriver(struct nfs_server *server, const struct nfs_fh *fh)
{
	...
	if (server->pnfs_blksize == 0) {
		dprintk("%s Server did yest return blksize\n", __func__);
		return -EINVAL;
	}
	if (server->pnfs_blksize > PAGE_SIZE) {
		printk(KERN_ERR "%s: pNFS blksize %d yest supported.\n",
		...
		struct nfs_page *req, unsigned int alignment, bool is_write)
{
	...
	if (is_write &&
	    (req_offset(req) + req->wb_bytes == i_size_read(pgio->pg_iyesde))) {
		/*
		 * If the write goes up to the iyesde size, just write
		 * the full page.  Data past the iyesde size is
...
bl_pg_init_read(struct nfs_pageio_descriptor *pgio, struct nfs_page *req)
{
	...
	if (pgio->pg_lseg &&
		...
		pnfs_error_mark_layout_for_return(pgio->pg_iyesde, pgio->pg_lseg);
...
/*
 * Return the number of contiguous bytes for a given iyesde
 * starting at page frame idx.
 */
static u64 pnfs_num_cont_bytes(struct iyesde *iyesde, pgoff_t idx)
{
	struct address_space *mapping = iyesde->i_mapping;
	pgoff_t end;

	/* Optimize common case that writes from 0 to end of file */
	end = DIV_ROUND_UP(i_size_read(iyesde), PAGE_SIZE);
	if (end != iyesde->i_mapping->nrpages) {
		rcu_read_lock();
	...
	if (!end)
		return i_size_read(iyesde) - (idx << PAGE_SHIFT);
	else
...
bl_pg_init_write(struct nfs_pageio_descriptor *pgio, struct nfs_page *req)
{
	...
	if (pgio->pg_dreq == NULL)
		wb_size = pnfs_num_cont_bytes(pgio->pg_iyesde,
					      req->wb_index);
	else
...
static struct pnfs_layoutdriver_type blocklayout_type = {
	...
	.alloc_deviceid_yesde		= bl_alloc_deviceid_yesde,
	.free_deviceid_yesde		= bl_free_deviceid_yesde,
