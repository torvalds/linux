...
static struct pnfs_layout_segment *pnfs_free_bucket_lseg(
...
static struct pnfs_commit_array *pnfs_find_commit_array_by_lseg(
...
struct pnfs_commit_array *pnfs_add_commit_array(
    struct pnfs_ds_commit_info *fl_cinfo,
...
static struct pnfs_commit_array *pnfs_lookup_commit_array(
    struct pnfs_ds_commit_info *fl_cinfo,
    struct pnfs_layout_segment *lseg) {
  struct pnfs_commit_array *array;
  rcu_read_lock();
...
static void pnfs_release_commit_array_locked(struct pnfs_commit_array *array) {
  list_del_rcu(&array->cinfo_list);
  list_del(&array->lseg_list);
  pnfs_free_commit_array(array);
}

static void pnfs_put_commit_array_locked(struct pnfs_commit_array *array) {
  if (refcount_dec_and_test(&array->refcount)) {
    pnfs_release_commit_array_locked(array);
  }
}

static void pnfs_put_commit_array(struct pnfs_commit_array *array,
    struct inode *inode) {
...
static struct pnfs_commit_array *pnfs_get_commit_array(
    struct pnfs_commit_array *array) {
...
static void pnfs_remove_and_free_commit_array(struct pnfs_commit_array *array) {
  array->lseg = NULL;
 ...
 * @bucket->committing.
 */
static int pnfs_bucket_scan_ds_commit_list(struct pnfs_commit_bucket *bucket,
    struct nfs_commit_info *cinfo,
    int max) {
...
static unsigned int pnfs_bucket_recover_commit_reqs(struct list_head *dst,
    struct pnfs_commit_bucket *buckets,
    unsigned int nbuckets,
    struct nfs_commit_info *cinfo) {
  struct pnfs_commit_bucket *b;
...
static struct nfs_page *pnfs_bucket_search_commit_reqs(
    struct pnfs_commit_bucket *buckets,
...
static struct pnfs_layout_segment *pnfs_bucket_get_committing(
...
static struct nfs_commit_data *pnfs_bucket_fetch_commitdata(
    struct pnfs_commit_bucket *bucket,
    struct nfs_commit_info *cinfo) {
...
static unsigned int pnfs_bucket_alloc_ds_commits(struct list_head *list,
    struct pnfs_commit_bucket *buckets,
    unsigned int nbuckets,
    struct nfs_commit_info *cinfo) {
  struct pnfs_commit_bucket *bucket;
...
static unsigned int pnfs_alloc_ds_commits_list(struct list_head *list,
    struct pnfs_ds_commit_info *fl_cinfo,
    struct nfs_commit_info *cinfo) {
  struct pnfs_commit_array *array;
  unsigned int ret = 0;
...
/* Debug routines */
static void print_ds(struct nfs4_pnfs_ds *ds) {
  if (ds == NULL) {
 ...
 * declare a match.
 */
static bool _same_data_server_addrs_locked(const struct list_head *dsaddrs1,
 ...
 */
static struct nfs4_pnfs_ds *_data_server_lookup_locked(
