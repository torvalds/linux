...
static struct nfs_open_dir_context *alloc_nfs_open_dir_context(
    struct inode *dir) {
...
static void put_nfs_open_dir_context(struct inode *dir,
    struct nfs_open_dir_context *ctx) {
  spin_lock(&dir->i_lock);
...
static void nfs_set_dtsize(struct nfs_readdir_descriptor *desc,
    unsigned int sz) {
...
static struct folio *nfs_readdir_folio_array_alloc(u64 last_cookie,
    gfp_t gfp_flags) {
...
static void nfs_readdir_folio_unlock_and_put_cached(
    struct nfs_readdir_descriptor *desc) {
  folio_unlock(desc->folio);
  nfs_readdir_folio_put(desc);
}

static struct folio *nfs_readdir_folio_get_cached(
    struct nfs_readdir_descriptor *desc) {
  struct address_space *mapping = desc->file->f_mapping;
...
static int nfs_lookup_revalidate_done(struct inode *dir, struct dentry *dentry,
...
static int nfs_lookup_revalidate_negative(struct inode *dir,
    struct dentry *dentry,
    unsigned int flags) {
  int ret = 1;
...
static int nfs_lookup_revalidate_delegated(struct inode *dir,
    struct dentry *dentry,
    struct inode *inode) {
 ...
 */
static int nfs_do_lookup_revalidate(struct inode *dir, struct dentry *dentry,
    unsigned int flags) {
  struct inode *inode;
  int error;
...
static int __nfs_lookup_revalidate(struct dentry *dentry, unsigned int flags,
...
struct dentry *nfs_lookup(struct inode *dir, struct dentry *dentry,
    unsigned int flags) {
  struct dentry *res;
...
static int nfs4_do_lookup_revalidate(struct inode *dir, struct dentry *dentry,
    unsigned int flags) {
  struct inode *inode;
  trace_nfs_lookup_revalidate_enter(dir, dentry, flags);
...
struct dentry *nfs_add_or_obtain(struct dentry *dentry, struct nfs_fh *fhandle,
    struct nfs_fattr *fattr) {
 ...
 */
int nfs_mknod(struct mnt_idmap *idmap, struct inode *dir,
...
int nfs_link(struct dentry *old_dentry, struct inode *dir,
    struct dentry *dentry) {
...
static void nfs_unblock_rename(struct rpc_task *task,
...
MODULE_PARM_DESC(nfs_access_max_cachesize,
...
static unsigned long nfs_do_access_cache_scan(unsigned int nr_to_scan) {
  LIST_HEAD(head);
  struct nfs_inode *nfsi, *next;
  struct nfs_access_entry *cache;
  long freed = 0;
  spin_lock(&nfs_access_lru_lock);
  list_for_each_entry_safe(nfsi, next, &nfs_access_lru_list,
...
unsigned long nfs_access_cache_scan(struct shrinker *shrink,
...
unsigned long nfs_access_cache_count(struct shrinker *shrink,
    struct shrink_control *sc) {
  return vfs_pressure_ratio(atomic_long_read(&nfs_access_nr_entries));
}

static void nfs_access_cache_enforce_limit(void) {
...
static void __nfs_access_zap_cache(struct nfs_inode *nfsi,
...
static int nfs_access_get_cached_locked(struct inode *inode,
...
static int nfs_access_get_cached_rcu(struct inode *inode,
    const struct cred *cred, u32 *mask) {
...
static int nfs_access_calc_mask(u32 access_result, umode_t umode) {
  int mask = 0;
...
static int nfs_do_access(struct inode *inode, const struct cred *cred,
    int mask) {
  struct nfs_access_entry cache;
