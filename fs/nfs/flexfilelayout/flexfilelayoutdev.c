...
void nfs4_ff_layout_put_deviceid(struct nfs4_ff_layout_ds *mirror_ds)
{
	if (!IS_ERR_OR_NULL(mirror_ds))
		nfs4_put_deviceid_yesde(&mirror_ds->id_yesde);
}

void nfs4_ff_layout_free_deviceid(struct nfs4_ff_layout_ds *mirror_ds)
{
	nfs4_print_deviceid(&mirror_ds->id_yesde.deviceid);
	nfs4_pnfs_ds_put(mirror_ds->ds);
	kfree(mirror_ds->ds_versions);
	kfree_rcu(mirror_ds, id_yesde.rcu);
}

/* Decode opaque device data and construct new_ds using it */
struct nfs4_ff_layout_ds *
nfs4_ff_alloc_deviceid_yesde(struct nfs_server *server, struct pnfs_device *pdev,
			    gfp_t gfp_flags)
{
	...
	nfs4_init_deviceid_yesde(&new_ds->id_yesde,
				server,
				&pdev->dev_id);
	...
	for (i = 0; i < version_count; i++) {
		/* 20 = version(4) + miyesr_version(4) + rsize(4) + wsize(4) +
		 * tightly_coupled(4) */
		p = xdr_inline_decode(&stream, 20);
		if (unlikely(!p))
			goto out_err_drain_dsaddrs;
		ds_versions[i].version = be32_to_cpup(p++);
		ds_versions[i].miyesr_version = be32_to_cpup(p++);
		...
		/*
		 * check for valid major/miyesr combination.
		 * currently we support dataserver which talk:
		 *   v3, v4.0, v4.1, v4.2
		 */
		if (!((ds_versions[i].version == 3 && ds_versions[i].miyesr_version == 0) ||
			(ds_versions[i].version == 4 && ds_versions[i].miyesr_version < 3))) {
			dprintk("%s: [%d] unsupported ds version %d-%d\n", __func__,
				i, ds_versions[i].version,
				ds_versions[i].miyesr_version);
			ret = -EPROTONOSUPPORT;
			goto out_err_drain_dsaddrs;
		}

		dprintk("%s: [%d] vers %u miyesr_ver %u rsize %u wsize %u coupled %d\n",
			     ...
			     gfp_t gfp_flags)
{
	...
	memcpy(&dserr->deviceid, &mirror->mirror_ds->id_yesde.deviceid,
	       NFS4_DEVICEID4_SIZE);

	spin_lock(&flo->generic_hdr.plh_iyesde->i_lock);
	ff_layout_add_ds_error_locked(flo, dserr);
	spin_unlock(&flo->generic_hdr.plh_iyesde->i_lock);
	return 0;
}
...
{
	/* FIXME: For yesw assume there is only 1 version available for the DS */
			 ...
			 struct nfs4_ff_layout_mirror *mirror)
{
	...
	if (mirror->mirror_ds == NULL) {
		...
		yesde = nfs4_find_get_deviceid(NFS_SERVER(lo->plh_iyesde),
				&mirror->devid, lo->plh_lc_cred,
				GFP_KERNEL);
		if (yesde)
			mirror_ds = FF_LAYOUT_MIRROR_DS(yesde);

		/* check for race with ayesther call to this function */
		if (cmpxchg(&mirror->mirror_ds, NULL, mirror_ds) &&
		    mirror_ds != ERR_PTR(-ENODEV))
			nfs4_put_deviceid_yesde(yesde);
	}
 ...
 * selecting a mirror to use and connecting the client to it if it's yest
 * already connected.
 *
			  ...
			  bool fail_return)
{
	...
	struct iyesde *iyes = lseg->pls_layout->plh_iyesde;
	struct nfs_server *s = NFS_SERVER(iyes);
	...
	/* FIXME: For yesw we assume the server sent only one version of NFS
	 * to use for the DS.
	 */
	status = nfs4_pnfs_ds_connect(s, ds, &mirror->mirror_ds->id_yesde,
			     dataserver_timeo, dataserver_retrans,
			     mirror->mirror_ds->ds_versions[0].version,
			     mirror->mirror_ds->ds_versions[0].miyesr_version);
...
yesconnect:
	...
	if (fail_return || !ff_layout_has_available_ds(lseg))
		pnfs_error_mark_layout_for_return(iyes, lseg);
	ds = NULL;
out:
	return ds;
}
...
nfs4_ff_find_or_create_ds_client(struct nfs4_ff_layout_mirror *mirror,
				 struct nfs_client *ds_clp, struct iyesde *iyesde)
{
	...
	case 4:
		return nfs4_find_or_create_ds_client(ds_clp, iyesde);
	default:
		BUG();
	}
}
...
/* called with iyesde i_lock held */
...
{
	struct nfs4_ff_layout_mirror *mirror;
	struct nfs4_deviceid_yesde *devid;
	u32 idx;
	...
	for (idx = 0; idx < FF_LAYOUT_MIRROR_COUNT(lseg); idx++) {
		...
		if (mirror) {
			...
			if (IS_ERR(mirror->mirror_ds))
				continue;
			devid = &mirror->mirror_ds->id_yesde;
...
{
	return ff_layout_yes_fallback_to_mds(lseg) ||
