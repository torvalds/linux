...
/* XXX: Let's filter out insanely large mirror count for yesw to avoid oom
...
FF_LAYOUT_MIRROR_DS(struct nfs4_deviceid_yesde *yesde)
{
	return container_of(yesde, struct nfs4_ff_layout_ds, id_yesde);
}
...
static inline bool
ff_layout_yes_fallback_to_mds(struct pnfs_layout_segment *lseg)
{
	return FF_LAYOUT_LSEG(lseg)->flags & FF_FLAGS_NO_IO_THRU_MDS;
}

static inline bool
ff_layout_yes_read_on_rw(struct pnfs_layout_segment *lseg)
{
