...
static inline unsigned long nfs_fattr_to_ino_t(struct nfs_fattr *fattr) {
...
struct inode *nfs_ilookup(struct super_block *sb, struct nfs_fattr *fattr,
    struct nfs_fh *fh) {
 ...
 */
struct inode *nfs_fhget(struct super_block *sb, struct nfs_fh *fh,
    struct nfs_fattr *fattr) {
  struct nfs_find_desc desc = {
    .fh = fh,
    .fattr = fattr
...
#define NFS_VALID_ATTRS (ATTR_MODE | ATTR_UID | ATTR_GID | ATTR_SIZE \
  | ATTR_ATIME | ATTR_ATIME_SET | ATTR_MTIME | ATTR_MTIME_SET | ATTR_FILE \
  | ATTR_OPEN)

int nfs_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
    struct iattr *attr) {
 ...
 */
static int nfs_check_inode_attributes(struct inode *inode,
    struct nfs_fattr *fattr) {
  struct nfs_inode *nfsi = NFS_I(inode);
  ...
  if ((fattr->valid & NFS_ATTR_FATTR_TYPE)
      && inode_wrong_type(inode, fattr->mode)) {
    return -ESTALE;
  }
  if (!nfs_file_has_buffered_writers(nfsi)) {
    ...
    ts = inode_get_mtime(inode);
    if ((fattr->valid & NFS_ATTR_FATTR_MTIME)
        && !timespec64_equal(&ts, &fattr->mtime)) {
      invalid |= NFS_INO_INVALID_MTIME;
    }
    ts = inode_get_ctime(inode);
    if ((fattr->valid & NFS_ATTR_FATTR_CTIME)
        && !timespec64_equal(&ts, &fattr->ctime)) {
  ...
  if ((fattr->valid & NFS_ATTR_FATTR_MODE)
      && (inode->i_mode & S_IALLUGO) != (fattr->mode & S_IALLUGO)) {
    invalid |= NFS_INO_INVALID_MODE;
  }
  if ((fattr->valid & NFS_ATTR_FATTR_OWNER)
      && !uid_eq(inode->i_uid, fattr->uid)) {
    invalid |= NFS_INO_INVALID_OTHER;
  }
  if ((fattr->valid & NFS_ATTR_FATTR_GROUP)
      && !gid_eq(inode->i_gid, fattr->gid)) {
    invalid |= NFS_INO_INVALID_OTHER;
  }
  ...
  if ((fattr->valid & NFS_ATTR_FATTR_ATIME)
      && !timespec64_equal(&ts, &fattr->atime)) {
 ...
 */
int nfs_post_op_update_inode_force_wcc_locked(struct inode *inode,
    struct nfs_fattr *fattr) {
  int attr_cmp = nfs_inode_attrs_cmp(fattr, inode);
  int status;
