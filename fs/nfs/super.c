 ...
 * - directory type roots are spliced into the tree when a path from one root
 *reaches the root
...
static void show_implementation_id(struct seq_file *m,
...
static int nfs_compare_remount_data(struct nfs_server *nfss,
  ...
  if ((ctx->flags ^ nfss->flags) & NFS_REMOUNT_CMP_FLAGMASK
      ...
      != (nfss->options & NFS_OPTION_FSCACHE)
...
int nfs_get_tree_common(struct fs_context *fc) {
  ...
  int (*compare_super)(struct super_block *,
      struct fs_context *) = nfs_compare_super;
