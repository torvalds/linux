...
static struct nfs_delegation *nfs_get_delegation(
...
static struct inode *nfs_delegation_grab_inode(
    struct nfs_delegation *delegation) {
...
static struct nfs_delegation *nfs_start_delegation_return_locked(
...
static struct nfs_delegation *nfs_start_delegation_return(
    struct nfs_inode *nfsi) {
  struct nfs_delegation *delegation;
  rcu_read_lock();
...
static struct nfs_delegation *nfs_detach_delegation_locked(
    struct nfs_inode *nfsi,
    struct nfs_delegation *delegation,
...
static void nfs_client_mark_return_unused_delegation_types(
    struct nfs_client *clp,
    fmode_t flags) {
  struct nfs_server *server;
  rcu_read_lock();
...
static struct inode *nfs_delegation_find_inode_server(struct nfs_server *server,
...
/**
 * nfs_delegation_reap_unclaimed - reap unclaimed delegations after reboot
 *recovery is done
 * @clp: nfs_client to process
 *
 */
