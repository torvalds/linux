...
/*
 * Let's break subtree checking for now... otherwise we'll have to embed parent
 *fh
 * but there might not be enough space.
 */
static int nfs_encode_fh(struct inode *inode, __u32 *p, int *max_len,
    struct inode *parent) {
...
static struct dentry *nfs_fh_to_dentry(struct super_block *sb, struct fid *fid,
...
static struct dentry *nfs_get_parent(struct dentry *dentry) {
  int ret;
