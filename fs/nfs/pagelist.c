...
static void nfs_page_iter_page_advance(struct nfs_page_iter_page *i,
    size_t sz) {
...
static struct nfs_pgio_mirror *nfs_pgio_get_mirror(
...
struct nfs_pgio_mirror *nfs_pgio_current_mirror(
...
static u32 nfs_pgio_set_current_mirror(struct nfs_pageio_descriptor *desc,
    u32 idx) {
...
static inline void nfs_page_free(struct nfs_page *p) {
 ...
 */
bool nfs_async_iocounter_wait(struct rpc_task *task,
 ...
 */
struct nfs_page *nfs_page_group_lock_head(struct nfs_page *req) {
 ...
 */
static void nfs_unroll_locks(struct nfs_page *head, struct nfs_page *req) {
  struct nfs_page *tmp;
 ...
 */
static int nfs_page_group_lock_subreq(struct nfs_page *head,
 ...
 */
int nfs_page_set_headlock(struct nfs_page *req) {
 ...
 * @req: request that is to be locked
 */
void nfs_page_clear_headlock(struct nfs_page *req) {
 ...
 * return 0 on success, < 0 on error
 */
int nfs_page_group_lock(struct nfs_page *req) {
  int ret;
 ...
 */
static bool nfs_page_group_sync_on_bit_locked(struct nfs_page *req,
    unsigned int bit) {
  struct nfs_page *head = req->wb_head;
  struct nfs_page *tmp;
 ...
 */
static inline void nfs_page_group_init(struct nfs_page *req,
...
static struct nfs_page *nfs_create_subreq(struct nfs_page *req,
    unsigned int pgbase,
    unsigned int offset,
    unsigned int count) {
...
static struct nfs_pgio_mirror *nfs_pageio_alloc_mirrors(
    struct nfs_pageio_descriptor *desc,
 ...
 */
static unsigned int nfs_pageio_do_add_request(
    struct nfs_pageio_descriptor *desc,
    struct nfs_page *req) {
  struct nfs_pgio_mirror *mirror = nfs_pgio_current_mirror(desc);
...
static void nfs_pageio_cleanup_request(struct nfs_pageio_descriptor *desc,
    struct nfs_page *req) {
  LIST_HEAD(head);
