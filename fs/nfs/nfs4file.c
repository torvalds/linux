...
static int nfs4_file_open(struct inode *inode, struct file *filp) {
 ...
 */
static int nfs4_file_flush(struct file *file, fl_owner_t id) {
...
static long nfs42_fallocate(struct file *filep, int mode, loff_t offset,
    loff_t len) {
...
static loff_t nfs42_remap_file_range(struct file *src_file, loff_t src_off,
  ...
  if (!ret) {
    truncate_inode_pages_range(&dst_inode->i_data, dst_off,
        dst_off + count - 1);
  }
out_unlock:
  if (same_inode) {
