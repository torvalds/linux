...
static inline void nfs_confirm_seqid(struct nfs_seqid_counter *seqid,
    int status) {
...
struct nfs4_state_maintenance_ops {
  int (*sched_state_renewal)(struct nfs_client *, const struct cred *,
      unsigned);
...
size_t nfs_parse_server_name(char *string, size_t len,
    struct sockaddr_storage *ss,
    size_t salen, struct net *net, int port);
/* nfs4proc.c */
extern int nfs4_handle_exception(struct nfs_server *, int,
    struct nfs4_exception *);
...
extern void nfs4_init_sequence(struct nfs4_sequence_args *,
...
extern int nfs4_proc_bind_conn_to_session(struct nfs_client *,
    const struct cred *cred);
extern int nfs4_proc_exchange_id(struct nfs_client *clp,
    const struct cred *cred);
...
extern int nfs4_server_capabilities(struct nfs_server *server,
    struct nfs_fh *fhandle);
extern int nfs4_proc_fs_locations(struct rpc_clnt *, struct inode *,
    const struct qstr *,
...
extern struct rpc_clnt *nfs4_proc_lookup_mountpoint(struct inode *,
    ...
    struct nfs_fh *,
    struct nfs_fattr *);
extern int nfs4_proc_secinfo(struct inode *, const struct qstr *,
...
static inline bool is_ds_only_client(struct nfs_client *clp) {
  return (clp->cl_exchange_flags & EXCHGID4_FLAG_MASK_PNFS)
    == EXCHGID4_FLAG_USE_PNFS_DS;
}

static inline bool is_ds_client(struct nfs_client *clp) {
  return clp->cl_exchange_flags & EXCHGID4_FLAG_USE_PNFS_DS;
}

static inline bool _nfs4_state_protect(struct nfs_client *clp,
    unsigned long sp4_mode,
 ...
 */
static inline void nfs4_state_protect(struct nfs_client *clp,
    unsigned long sp4_mode,
    struct rpc_clnt **clntp, struct rpc_message *msg) {
 ...
 */
static inline void nfs4_state_protect_write(struct nfs_client *clp,
    struct rpc_clnt **clntp,
...
extern struct nfs4_state *nfs4_get_open_state(struct inode *,
    struct nfs4_state_owner *);
...
extern int nfs4_schedule_stateid_recovery(const struct nfs_server *,
    struct nfs4_state *);
...
extern void nfs4_free_lock_state(struct nfs_server *server,
    struct nfs4_lock_state *lsp);
extern int nfs4_proc_commit(struct file *dst, __u64 offset, __u32 count,
...
static inline void nfs4_stateid_copy(nfs4_stateid *dst,
    const nfs4_stateid *src) {
  memcpy(dst->data, src->data, sizeof(dst->data));
  dst->type = src->type;
}

static inline bool nfs4_stateid_match(const nfs4_stateid *dst,
    const nfs4_stateid *src) {
...
static inline bool nfs4_stateid_match_other(const nfs4_stateid *dst,
    const nfs4_stateid *src) {
  return memcmp(dst->other, src->other, NFS4_STATEID_OTHER_SIZE) == 0;
}

static inline bool nfs4_stateid_is_newer(const nfs4_stateid *s1,
    const nfs4_stateid *s2) {
  return (s32) (be32_to_cpu(s1->seqid) - be32_to_cpu(s2->seqid)) > 0;
}

static inline bool nfs4_stateid_is_next(const nfs4_stateid *s1,
    const nfs4_stateid *s2) {
...
static inline bool nfs4_stateid_match_or_older(const nfs4_stateid *dst,
    const nfs4_stateid *src) {
...
static inline bool nfs4_state_match_open_stateid_other(
