 ...
 */
static void nfs4_xattr_hash_init(struct nfs4_xattr_cache *cache) {
  unsigned int i;
 ...
 */
static bool nfs4_xattr_entry_lru_add(struct nfs4_xattr_entry *entry) {
  struct list_lru *lru;
...
static bool nfs4_xattr_entry_lru_del(struct nfs4_xattr_entry *entry) {
  struct list_lru *lru;
  lru = (entry->flags & NFS4_XATTR_ENTRY_EXTVAL)
      ? &nfs4_xattr_large_entry_lru : &nfs4_xattr_entry_lru;
 ...
 */
static struct nfs4_xattr_entry *nfs4_xattr_alloc_entry(const char *name,
    const void *value,
...
static void nfs4_xattr_free_entry(struct nfs4_xattr_entry *entry) {
...
static void nfs4_xattr_free_entry_cb(struct kref *kref) {
  struct nfs4_xattr_entry *entry;
...
static void nfs4_xattr_free_cache_cb(struct kref *kref) {
  struct nfs4_xattr_cache *cache;
  int i;
...
static struct nfs4_xattr_cache *nfs4_xattr_alloc_cache(void) {
  struct nfs4_xattr_cache *cache;
 ...
 */
static int nfs4_xattr_set_listcache(struct nfs4_xattr_cache *cache,
 ...
 */
static struct nfs4_xattr_cache *nfs4_xattr_cache_unlink(struct inode *inode) {
 ...
 * shrinker callback.
 */
static void nfs4_xattr_discard_cache(struct nfs4_xattr_cache *cache) {
  unsigned int i;
  struct nfs4_xattr_entry *entry;
  struct nfs4_xattr_bucket *bucket;
  struct hlist_node *n;
...
static struct nfs4_xattr_cache *nfs4_xattr_get_cache(struct inode *inode,
    int add) {
  struct nfs_inode *nfsi;
...
static inline struct nfs4_xattr_bucket *nfs4_xattr_hash_bucket(
    struct nfs4_xattr_cache *cache, const char *name) {
  return &cache->buckets[jhash(name, strlen(name), 0)
        & (ARRAY_SIZE(cache->buckets) - 1)];
}

static struct nfs4_xattr_entry *nfs4_xattr_get_entry(
    struct nfs4_xattr_bucket *bucket, const char *name) {
  struct nfs4_xattr_entry *entry;
  entry = NULL;
...
static int nfs4_xattr_hash_add(struct nfs4_xattr_cache *cache,
    struct nfs4_xattr_entry *entry) {
  struct nfs4_xattr_bucket *bucket;
...
static void nfs4_xattr_hash_remove(struct nfs4_xattr_cache *cache,
    const char *name) {
  struct nfs4_xattr_bucket *bucket;
  struct nfs4_xattr_entry *entry;
...
static struct nfs4_xattr_entry *nfs4_xattr_hash_find(
    struct nfs4_xattr_cache *cache, const char *name) {
  struct nfs4_xattr_bucket *bucket;
  struct nfs4_xattr_entry *entry;
  bucket = nfs4_xattr_hash_bucket(cache, name);
...
static enum lru_status cache_lru_isolate(struct list_head *item,
...
static enum lru_status entry_lru_isolate(struct list_head *item,
    struct list_lru_one *lru, spinlock_t *lru_lock, void *arg) {
  struct list_head *dispose = arg;
  struct nfs4_xattr_bucket *bucket;
  struct nfs4_xattr_cache *cache;
