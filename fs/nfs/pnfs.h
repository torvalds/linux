...
struct pnfs_layoutdriver_type {
  ...
  struct pnfs_layout_segment *(*alloc_lseg)(struct pnfs_layout_hdr *layoutid,
  ...
  struct nfs4_deviceid_node *(*alloc_deviceid_node)(struct nfs_server *server,
...
extern struct pnfs_layout_segment *nfs4_proc_layoutget(
...
void pnfs_set_layoutcommit(struct inode *, struct pnfs_layout_segment *,
    loff_t);
...
struct nfs4_deviceid_node *nfs4_find_get_deviceid(struct nfs_server *server,
    ...
    gfp_t gfp_mask);
void nfs4_delete_deviceid(const struct pnfs_layoutdriver_type *,
...
static inline struct nfs4_deviceid_node *nfs4_get_deviceid(
    struct nfs4_deviceid_node *d) {
  atomic_inc(&d->ref);
  return d;
}

static inline struct pnfs_layout_segment *pnfs_get_lseg(
    struct pnfs_layout_segment *lseg) {
  if (lseg) {
...
static inline bool pnfs_is_valid_lseg(struct pnfs_layout_segment *lseg) {
...
static inline int pnfs_commit_list(struct inode *inode,
    struct list_head *mds_pages, int how,
    struct nfs_commit_info *cinfo) {
...
static inline struct pnfs_ds_commit_info *pnfs_get_ds_info(struct inode *inode)
{
...
static inline void pnfs_init_ds_commit_info_ops(
    struct pnfs_ds_commit_info *fl_cinfo, struct inode *inode) {
...
static inline void pnfs_init_ds_commit_info(
    struct pnfs_ds_commit_info *fl_cinfo) {
  INIT_LIST_HEAD(&fl_cinfo->commits);
  fl_cinfo->ops = NULL;
}

static inline void pnfs_release_ds_info(struct pnfs_ds_commit_info *fl_cinfo,
    struct inode *inode) {
...
static inline void pnfs_generic_mark_devid_invalid(
    struct nfs4_deviceid_node *node) {
  set_bit(NFS_DEVICEID_INVALID, &node->flags);
}

static inline bool pnfs_mark_request_commit(struct nfs_page *req,
    struct pnfs_layout_segment *lseg,
...
static inline bool pnfs_clear_request_commit(struct nfs_page *req,
    struct nfs_commit_info *cinfo) {
  struct pnfs_ds_commit_info *fl_cinfo = cinfo->ds;
...
static inline int pnfs_scan_commit_lists(struct inode *inode,
    struct nfs_commit_info *cinfo,
    int max) {
  struct pnfs_ds_commit_info *fl_cinfo = cinfo->ds;
...
static inline void pnfs_recover_commit_reqs(struct list_head *head,
    struct nfs_commit_info *cinfo) {
  struct pnfs_ds_commit_info *fl_cinfo = cinfo->ds;
  if (fl_cinfo && fl_cinfo->nwritten != 0) {
...
static inline struct nfs_page *pnfs_search_commit_reqs(struct inode *inode,
    struct nfs_commit_info *cinfo,
    struct folio *folio) {
  struct pnfs_ds_commit_info *fl_cinfo = cinfo->ds;
...
static inline bool pnfs_ld_layoutret_on_setattr(struct inode *inode) {
...
static inline bool pnfs_ld_read_whole_page(struct inode *inode) {
  if (!pnfs_enabled_sb(NFS_SERVER(inode))) {
    return false;
  }
...
static inline int pnfs_sync_inode(struct inode *inode, bool datasync) {
  if (!pnfs_enabled_sb(NFS_SERVER(inode))) {
    return 0;
  }
...
static inline bool pnfs_layoutcommit_outstanding(struct inode *inode) {
...
static inline bool pnfs_use_threshold(struct nfs4_threshold **dst,
...
static inline u64 pnfs_calc_offset_end(u64 offset, u64 len) {
...
static inline u64 pnfs_calc_offset_length(u64 offset, u64 end) {
...
static inline void pnfs_copy_range(struct pnfs_layout_range *dst,
    const struct pnfs_layout_range *src) {
  memcpy(dst, src, sizeof(*dst));
}

static inline u64 pnfs_end_offset(u64 start, u64 len) {
 ...
 *                                [----------------)
 */
static inline bool pnfs_is_range_intersecting(u64 start1, u64 end1, u64 start2,
    u64 end2) {
  return (end1 == NFS4_MAX_UINT64 || start2 < end1)
    && (end2 == NFS4_MAX_UINT64 || start1 < end2);
}

static inline bool pnfs_lseg_range_intersecting(
    const struct pnfs_layout_range *l1,
...
static inline bool pnfs_lseg_request_intersecting(
    struct pnfs_layout_segment *lseg, struct nfs_page *req) {
  u64 seg_last
    = pnfs_end_offset(lseg->pls_range.offset, lseg->pls_range.length);
...
static inline int pnfs_roc_done(struct rpc_task *task,
...
static inline void pnfs_roc_release(struct nfs4_layoutreturn_args *args,
    struct nfs4_layoutreturn_res *res,
    int ret) {
}

static inline bool pnfs_wait_on_layoutreturn(struct inode *ino,
...
#else
static inline int pnfs_report_layoutstat(struct inode *inode, gfp_t gfp_flags) {
  return 0;
}
