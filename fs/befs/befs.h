 ...
 */
static inline struct befs_sb_info *BEFS_SB(const struct super_block *super) {
  return (struct befs_sb_info *) super->s_fs_info;
}

static inline struct befs_inode_info *BEFS_I(const struct inode *inode) {
  return container_of(inode, struct befs_inode_info, vfs_inode);
}

static inline befs_blocknr_t iaddr2blockno(struct super_block *sb,
    const befs_inode_addr *iaddr) {
  return (iaddr->allocation_group << BEFS_SB(sb)->ag_shift)
    + iaddr->start;
}

static inline befs_inode_addr blockno2iaddr(struct super_block *sb,
...
static inline unsigned int befs_iaddrs_per_block(struct super_block *sb) {
