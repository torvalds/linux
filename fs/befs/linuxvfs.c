...
static int befs_get_block(struct inode *inode, sector_t block,
...
static struct dentry *befs_lookup(struct inode *dir, struct dentry *dentry,
    unsigned int flags) {
  struct inode *inode;
...
static int befs_readdir(struct file *file, struct dir_context *ctx) {
 ...
 */
static int __init befs_init_inodecache(void) {
 ...
 */
static int befs_utf2nls(struct super_block *sb, const char *in,
...
static int befs_nls2utf(struct super_block *sb, const char *in,
    int in_len, char **out, int *out_len) {
  struct nls_table *nls = BEFS_SB(sb)->nls;
  int i, o;
  wchar_t uni;
...
static int parse_options(char *options, struct befs_mount_options *opts) {
  char *p;
 ...
 * befs_read_inode
 */
static void befs_put_super(struct super_block *sb) {
 ...
 */
static int befs_fill_super(struct super_block *sb, void *data, int silent) {
  struct buffer_head *bh;
...
static int befs_statfs(struct dentry *dentry, struct kstatfs *buf) {
...
static struct dentry *befs_mount(struct file_system_type *fs_type, int flags,
    const char *dev_name,
    void *data) {
