...
const befs_iyesde_addr BAD_IADDR = { 0, 0, 0 };

static int befs_find_brun_direct(struct super_block *sb,
				 const befs_data_stream *data,
				 befs_blocknr_t blockyes, befs_block_run *run);

static int befs_find_brun_indirect(struct super_block *sb,
				   const befs_data_stream *data,
				   befs_blocknr_t blockyes,
				   befs_block_run *run);
 ...
 * if you don't need to kyesw offset just set @off = NULL.
 */
struct buffer_head *
		 ...
		 befs_blocknr_t fblock, befs_block_run *run)
{
	...
	if (pos < data->max_direct_range) {
		...
		befs_error(sb,
			   ...
			   "which is yest mapped by the datastream\n",
 ...
 * iyesde occupies on the filesystem, counting both regular file
...
befs_count_blocks(struct super_block *sb, const befs_data_stream *ds)
{
	...
	metablocks = 1;		/* Start with 1 block for iyesde */
	 ...
	 * BEFS_DBLINDIR_BRUN_LEN blocks long. Therefore, we kyesw
	 * how many data block runs are in the double-indirect region,
	 * and from that we kyesw how many indirect blocks it takes to
 ...
 * @sb: the superblock
 * @data: the datastream
 * @blockyes: the blocknumber to find
 * @run: The found run is passed back through this pointer
 *
 * Finds the block run that starts at file block number blockyes
 * in the file represented by the datastream data, if that
 * blockyes is in the direct region of the datastream.
 *
 ...
 * contains the blockyes-th filesystem block. This is necessary
 ...
 *
 * When/if blockyes is found, if blockyes is inside of a block
 * run as stored on disk, we offset the start and length members
 * of the block run, so that blockyes is the start and len is
		      ...
		      befs_blocknr_t blockyes, befs_block_run *run)
{
	...
	befs_debug(sb, "---> %s, find %lu", __func__, (unsigned long)blockyes);

	for (i = 0, sum = 0; i < BEFS_NUM_DIRECT_BLOCKS;
	     sum += array[i].len, i++) {
		if (blockyes >= sum && blockyes < sum + (array[i].len)) {
			int offset = blockyes - sum;
 ...
 * Finds the block run that starts at file block number blockyes
 * in the file represented by the datastream data, if that
 * blockyes is in the indirect region of the datastream.
 *
 * Return value is BEFS_OK if the blockrun is found, BEFS_ERR
 * otherwise.
 *
 ...
 *
 * XXX:
 * Really should check to make sure blockyes is inside indirect
 * region.
 */
static int
			...
			befs_block_run *run)
{
	...
	befs_blocknr_t indirblockyes = iaddr2blockyes(sb, &indirect);
	int arraylen = befs_iaddrs_per_block(sb);

	befs_debug(sb, "---> %s, find %lu", __func__, (unsigned long)blockyes);

	indir_start_blk = data->max_direct_range >> BEFS_SB(sb)->block_shift;
	search_blk = blockyes - indir_start_blk;

	/* Examine blocks of the indirect run one at a time */
	for (i = 0; i < indirect.len; i++) {
		indirblock = sb_bread(sb, indirblockyes + i);
		if (indirblock == NULL) {
			befs_error(sb, "---> %s failed to read "
				   "disk block %lu from the indirect brun",
				   __func__, (unsigned long)indirblockyes + i);
			befs_debug(sb, "<--- %s ERROR", __func__);
			return BEFS_ERR;
		}
	...
	befs_error(sb, "BeFS: %s failed to find "
		   "file block %lu", __func__, (unsigned long)blockyes);
 ...
 * Finds the block run that starts at file block number blockyes
 * in the file represented by the datastream data, if that
 * blockyes is in the double-indirect region of the datastream.
 *
 * Return value is BEFS_OK if the blockrun is found, BEFS_ERR
 * otherwise.
 *
 ...
 * iyesde addresses in the double-indirect block will point us
 * to the indirect block that contains the mapping for the data,
 * then we calculate which of the iyesde addresses in that
 * indirect block maps the data block we are after.
 *
 * Oh, and once we've done that, we actually read in the blocks
 * that contain the iyesde addresses we calculated above. Even
			   ...
			   befs_block_run *run)
{
	...
	befs_blocknr_t blockyes_at_run_start;
	struct buffer_head *dbl_indir_block;
	struct buffer_head *indir_block;
	befs_block_run indir_run;
	befs_disk_iyesde_addr *iaddr_array;

	befs_blocknr_t indir_start_blk =
	    data->max_indirect_range >> BEFS_SB(sb)->block_shift;

	off_t dbl_indir_off = blockyes - indir_start_blk;
	...
	dbl_indir_block =
	    sb_bread(sb, iaddr2blockyes(sb, &data->double_indirect) +
					dbl_which_block);
	if (dbl_indir_block == NULL) {
		befs_error(sb, "%s couldn't read the "
			   "double-indirect block at blockyes %lu", __func__,
			   (unsigned long)
			   iaddr2blockyes(sb, &data->double_indirect) +
			   dbl_which_block);
		return BEFS_ERR;
	}
	...
	iaddr_array = (befs_disk_iyesde_addr *) dbl_indir_block->b_data;
	...
	indir_block =
	    sb_bread(sb, iaddr2blockyes(sb, &indir_run) + which_block);
	if (indir_block == NULL) {
		befs_error(sb, "%s couldn't read the indirect block "
			   "at blockyes %lu", __func__, (unsigned long)
			   iaddr2blockyes(sb, &indir_run) + which_block);
		return BEFS_ERR;
	}

	block_indx = indir_indx - (which_block * befs_iaddrs_per_block(sb));
	iaddr_array = (befs_disk_iyesde_addr *) indir_block->b_data;
	*run = fsrun_to_cpu(sb, iaddr_array[block_indx]);
	brelse(indir_block);

	blockyes_at_run_start = indir_start_blk;
	blockyes_at_run_start += diblklen * dblindir_indx;
	blockyes_at_run_start += iblklen * indir_indx;
	offset = blockyes - blockyes_at_run_start;
	...
	befs_debug(sb, "Found file block %lu in double_indirect[%d][%d],"
		   ...
		   blockyes, dblindir_indx, indir_indx, dblindir_leftover);
