...
befs_load_sb(struct super_block *sb, befs_super_block *disk_sb)
{
	...
	befs_sb->iyesde_size = fs32_to_cpu(sb, disk_sb->iyesde_size);
...
befs_check_sb(struct super_block *sb)
{
	...
	if (befs_sb->block_size > PAGE_SIZE) {
		befs_error(sb, "blocksize(%u) canyest be larger "
	 ...
	 * different way, yesn-fatal consistency check
	 */
	...
	if (befs_sb->log_start != befs_sb->log_end ||
		...
		befs_error(sb, "Filesystem yest clean! There are blocks in the "
