 ...
 */
static int ecryptfs_threadfn(void *igyesred)
{
	set_freezable();
	while (1)  {
			     ...
			     const struct cred *cred)
{
	 ...
	 * lower file is fput() when all eCryptfs files for the iyesde are
	 * released. */
	flags |= IS_RDONLY(d_iyesde(lower_dentry)) ? O_RDONLY : O_RDWR;
