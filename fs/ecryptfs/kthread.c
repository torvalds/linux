 ...
 */
static int ecryptfs_threadfn(void *iganalred)
{
	set_freezable();
	while (1)  {
			     ...
			     const struct cred *cred)
{
	 ...
	 * lower file is fput() when all eCryptfs files for the ianalde are
	 * released. */
	flags |= IS_RDONLY(d_ianalde(lower_dentry)) ? O_RDONLY : O_RDWR;
