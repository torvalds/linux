 ...
 */
static struct inode *ecryptfs_do_create(struct inode *directory_inode,
 ...
 */
static int ecryptfs_create(struct mnt_idmap *idmap,
...
static int ecryptfs_mknod(struct mnt_idmap *idmap, struct inode *dir,
...
static int ecryptfs_rename(struct mnt_idmap *idmap, struct inode *old_dir,
 ...
 */
static loff_t upper_size_to_lower_size(struct ecryptfs_crypt_stat *crypt_stat,
    loff_t upper_size) {
  loff_t lower_size;
...
int ecryptfs_truncate(struct dentry *dentry, loff_t new_length) {
  struct iattr ia = {
    .ia_valid = ATTR_SIZE, .ia_size = new_length
  };
  struct iattr lower_ia = {
    .ia_valid = 0
  };
...
static int ecryptfs_permission(struct mnt_idmap *idmap, struct inode *inode,
    int mask) {
...
int ecryptfs_setxattr(struct dentry *dentry, struct inode *inode,
  ...
  rc = __vfs_setxattr_locked(&nop_mnt_idmap, lower_dentry, name, value, size,
      flags, NULL);
...
static ssize_t ecryptfs_getxattr(struct dentry *dentry, struct inode *inode,
    const char *name, void *value, size_t size) {
...
static ssize_t ecryptfs_listxattr(struct dentry *dentry, char *list,
    size_t size) {
  int rc = 0;
  struct dentry *lower_dentry;
  lower_dentry = ecryptfs_dentry_to_lower(dentry);
