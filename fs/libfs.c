...
static struct dentry *find_next_child(struct dentry *parent,
    struct dentry *prev) {
...
static ssize_t simple_attr_write_xsigned(struct file *file,
 ...
 */
int simple_nosetlease(struct file *filp, int arg, struct file_lease **flp,
    void **priv) {
  return -EINVAL;
}
