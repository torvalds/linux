...
struct vfsmount *vfs_submount(const struct dentry *mountpoint,
...
static struct mount *clone_mnt(struct mount *old, struct dentry *root,
  ...
  mnt->mnt.mnt_flags
    &= ~(MNT_WRITE_HOLD | MNT_MARKED | MNT_INTERNAL | MNT_ONRB);
...
static void mnt_warn_timestamp_expiry(struct path *mountpoint,
  ...
  if (!__mnt_is_readonly(mnt)
    ...
    pr_warn(
        "%s filesystem being %s at %s supports timestamps until %ptTd (0x%llx)\n",
        sb->s_type->name,
...
static void statmount_sb_basic(struct kstatmount *s) {
  ...
  s->sm.sb_flags = sb->s_flags
      & (SB_RDONLY | SB_SYNCHRONOUS | SB_DIRSYNC | SB_LAZYTIME);
}
