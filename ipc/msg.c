...
static inline struct msg_queue *msq_obtain_object_check(
...
static inline unsigned long copy_msqid_to_user(void __user *buf,
...
static inline unsigned long copy_msqid_from_user(struct msqid64_ds *out,
    void __user *buf,
    int version) {
  switch (version) {
    case IPC_64:
...
static long ksys_msgctl(int msqid, int cmd, struct msqid_ds __user *buf,
    int version) {
  struct ipc_namespace *ns;
...
static long compat_ksys_msgctl(int msqid, int cmd, void __user *uptr,
    int version) {
  struct ipc_namespace *ns;
  int err;
  struct msqid64_ds msqid64;
...
static long do_msgrcv(int msqid, void __user *buf, size_t bufsz, long msgtyp,
    int msgflg,
...
#ifdef CONFIG_COMPAT
static long compat_do_msg_fill(void __user *dest, struct msg_msg *msg,
    size_t bufsz) {
